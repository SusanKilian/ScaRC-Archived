...
CHARACTER(255), PARAMETER :: scrchid='$Id: scrc.f90 10041 2012-02-10 13:43:13Z mcgratta $'
CHARACTER(255), PARAMETER :: scrcrev='$Revision: 10041 $'
CHARACTER(255), PARAMETER :: scrcdate='$Date: 2012-02-10 14:43:13 +0100 (Fr, 10 Feb 2012) $'
...
INTEGER :: TYPE_LATEX      = NSCARC_LATEX_TABLE
...
!WRITE(SCARC_LU,*) NM, ': ALLOCATING COMPACT(',NLEVEL_MIN,':',NLEVEL_MAX
...
!WRITE(SCARC_LU,*) 'NOM=',NOM,': NIC_S=',OS%NIC_S
!WRITE(SCARC_LU,*) 'NOM=',NOM,': NIC_R=',OS%NIC_r
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'ALLOCATING OSCARC for ',NOM
...
!WRITE(SCARC_LU,*) 'NOM=',NOM,': IJKW=',SIZE(M%OMESH(NOM)%IJKW)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(i3,a,i3,a,i3,a,i3,a,i3)') NM,': ALLOCATING OSCARC for ',NOM, ': NLEVEL_MIN=',NLEVEL_MIN,': NLEVEL_MAX=',NLEVEL_MAX
         ...
         DO IWF = 1, SBF%NW
            ...
            IF (NOM /= 0) THEN
               ...
               SELECT CASE (TYPE_MULTIGRID)
                  ...
                  CASE (NSCARC_MULTIGRID_ALGEBRAIC)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'HUI: ALLOCATING WALL(',IWF,')%ICN(',NCPL,')', MESHES(NOM)%IBAR, MESHES(NOM)%JBAR
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'A: NCE=',SCF%NCE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i4,a,i4,a,i4,a,i4,a,i4,i4,i4)') &
  '1: NM=',NM, ': NOM=',NOM,': OSCF%NG=',OSCF%NG, ': IWF=',IWF,': ICE=',SCF%NCE, IC, ICPL
                     ENDDO
                  ENDDO
               ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i4,a,i4,a,i4,a,i4,a,i4,i4,i4)') &
  '2: NM=',NM, ': NOM=',NOM,': OSCF%NG=',OSCF%NG, ': IWF=',IWF,': ICE=',SCF%NCE, IC, ICPL
                        ENDDO
                     ENDDO
                  ENDDO
         ...
         DO IWF = 1, SCF%NW
            ...
            IF (NOM /= 0) THEN
               ...
               IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
                  WRITE(SCARC_LU,*) '1:  SCF%WALL_PTR(',ICE,')=',SCF%WALL_PTR(ICE)
                  WRITE(SCARC_LU,*) '1: OSCF%WALL_PTR(',OSCF%IG,')=',OSCF%WALL_PTR(OSCF%IG)
                  WRITE(SCARC_LU,*) '1: OSCF%GHOST_PTR(',OSCF%IG,')=',OSCF%GHOST_PTR(OSCF%IG)
               ENDIF
               ...
               IF (TYPE_LAYER == NSCARC_LAYER_TWO .AND. TYPE_MULTIGRID == NSCARC_MULTIGRID_ALGEBRAIC) THEN
                  ...
                  OSCF%WALL_PTR(OSCF%IG) = -IWF
                  IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
                     WRITE(SCARC_LU,*) '2: WALL_PTR(',ICE2,')=',SCF%WALL_PTR(ICE2)
                     WRITE(SCARC_LU,*) '2: OSCF%WALL_PTR(',OSCF%IG,')=',OSCF%WALL_PTR(OSCF%IG)
                     WRITE(SCARC_LU,*) '2: OSCF%GHOST_PTR(',OSCF%IG,')=',OSCF%GHOST_PTR(OSCF%IG)
                  ENDIF
               ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'HUI2:  NCE=',SCF%NCE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'NL=',NL,': CALLING SETUP_COMPACT_FACE ', SCC%NW, IREFINE
               !!! wall cells along IOR=1
               IOFFSET = 0
               DO IZC=1,SCC%NZ
                  DO IYC=1,SCC%NY
      ...
      IF (NOMF(1) > 0) THEN   
         ...
         SELECT CASE (ABS(IOR0))
            CASE (1)
               JDIFF = WALLF(IWF(2))%IYN1 - WALLF(IWF(1))%IYN1
               KDIFF = WALLF(IWF(3))%IZN1 - WALLF(IWF(1))%IZN1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'JDIFF=',JDIFF,': KDIFF=',KDIFF
               IF (JDIFF==1 .AND. KDIFF==1) THEN
                  IY1 = WALLF(IWF(2))%IYN2/2
                  IY2 = IY1
                  IZ1 = WALLF(IWF(3))%IZN2/2
                  IZ2 = IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IY1=',IY1,': IY2=',IY2,': IZ1=',IZ1,': IZ2=',IZ2
               ELSE IF (JDIFF==2 .AND. KDIFF==2) THEN
                  IY1 = WALLF(IWF(1))%IYN2/2
                  IY2 = WALLF(IWF(2))%IYN2/2
                  IZ1 = WALLF(IWF(1))%IZN2/2
               ...
               ELSE IF (JDIFF==0 .AND. KDIFF==0) THEN
                  ...
                  IZ1 = WALLF(IWF(1))%IZN1/2
                  IZ2 = IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IY1=',IY1,': IY2=',IY2,': IZ1=',IZ1,': IZ2=',IZ2, WALLF(IWF(1))%IYN1,WALLF(IWF(1))%IZN1
               ELSE
                  WRITE(*,*) 'WRONG resolutions in SCARC_SETUP_COMPACT_FACE, IOR0=',IOR0
                  STOP
               ENDIF
            CASE (2)
               IDIFF = WALLF(IWF(2))%IXN1 - WALLF(IWF(1))%IXN1
               KDIFF = WALLF(IWF(3))%IZN1 - WALLF(IWF(1))%IZN1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IDIFF=',IDIFF,': KDIFF=',KDIFF
               IF (IDIFF==1 .AND. KDIFF==1) THEN
                  IX1 = WALLF(IWF(2))%IXN2/2
                  IX2 = IX1
                  IZ1 = WALLF(IWF(3))%IZN2/2
                  IZ2 = IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IX1=',IX1,': IX2=',IX2,': IZ1=',IZ1,': IZ2=',IZ2
               ELSE IF (IDIFF==2 .AND. KDIFF==2) THEN
                  IX1 = WALLF(IWF(1))%IXN2/2
                  IX2 = WALLF(IWF(2))%IXN2/2
                  IZ1 = WALLF(IWF(1))%IZN2/2
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IX1=',IX1,': IX2=',IX2,': IZ1=',IZ1,': IZ2=',IZ2
            ...
            CASE (3)
               IDIFF = WALLF(IWF(2))%IXN1 - WALLF(IWF(1))%IXN1
               JDIFF = WALLF(IWF(3))%IYN1 - WALLF(IWF(1))%IYN1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IDIFF=',IDIFF,': JDIFF=',JDIFF
               IF (IDIFF==1 .AND. JDIFF==1) THEN
                  IX1 = WALLF(IWF(2))%IXN2/2
                  IX2 = IX1
                  IY1 = WALLF(IWF(3))%IYN2/2
                  IY2 = IY1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IX1=',IX1,': IX2=',IX2,': IY1=',IY1,': IY2=',IY2
               ELSE IF (IDIFF==2 .AND. JDIFF==2) THEN
                  IX1 = WALLF(IWF(1))%IXN2/2
                  IX2 = WALLF(IWF(2))%IXN2/2
                  IY1 = WALLF(IWF(1))%IYN2/2
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IX1=',IX1,': IX2=',IX2,': IY1=',IY1,': IY2=',IY2
         ...
         SELECT CASE (IOR0)
            CASE (1)
               IX1 = MESHES(NOMF(1))%IBAR/IREFINE
               IX2 = IX1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'A 1: IX1=',IX1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'A 1: IX2=',IX2
            CASE (-1)
               IX1 = 1
               IX2 = IX1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'A-1: IX1=',IX1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'A-1: IX2=',IX2
            CASE (2)
               IY1 = MESHES(NOMF(1))%JBAR/IREFINE
               IY2 = IY1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'B 2: IY1=',IY1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'B 2: IY2=',IY2
            CASE (-2)
               IY1 = 1
               IY2 = IY1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'B-2: IY1=',IY1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'B-2: IY2=',IY2
            CASE (3)
               IZ1 = MESHES(NOMF(1))%KBAR/IREFINE
               IZ2 = IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'C 3: IZ1=',IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'C 3: IY2=',IZ2
            CASE (-3)
               IZ1 = 1
               IZ2 = IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'C-3: IZ1=',IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'C-3: IZ2=',IZ2
         END SELECT
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IX1=',IX1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IX2=',IX2
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IY1=',IY1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IY2=',IY2
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IZ1=',IZ1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IZ2=',IZ2
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IREFINE=',IREFINE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'MESHES(',NOMF(1),')%IBAR=',MESHES(NOMF(1))%IBAR/IREFINE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'MESHES(',NOMF(1),')%JBAR=',MESHES(NOMF(1))%JBAR/IREFINE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'MESHES(',NOMF(1),')%KBAR=',MESHES(NOMF(1))%KBAR/IREFINE
         !!!
         !!! Set WALLC(10:15, IWC)
         !!!
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,10i3)') 'NM=',NM,': NL=',NL,': WALLC(',IWC,')%ICN(',ICOUNT,')=',WALLC(IWC)%ICN(ICOUNT), NXC, NYC
                  WALLC(IWC)%ICE(ICOUNT) = ICE
                  ICOUNT = ICOUNT + 1
               ENDDO
            ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'NCE=',SC%NCE
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'NAE=',SC%NAE
            ...
            DO K = 1, SC%NZ
               DO I = 1, SC%NX
                  ...
                  ELSE IF (SC%SUBDIVISION(3,-1) > 0) THEN      !!! neighbor in upper x direction ?
                     IW0(-1) = SC%SUBDIVISION(1,-1)
                     IL0(-1) = SC%SUBDIVISION(2,-1)
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'IW0(-1)=',IW0(-1),': IL0(-1)=',IL0(-1), ': IC=', IC
                     IF (COMPACT_CELL_WITH_NEIGHBOR(SC%WALL, IC,IW,IW0(-1),IL0(-1))) THEN
!WRITE(SCARC_LU,*) ' --- yes', IW
                        SC%A_COL(IP) = -IW
                        IP = IP + 1
                     ENDIF
                  ENDIF
...
SELECT_TYPE: SELECT CASE (NTYPE)
   ...
   CASE (NSCARC_MATRIX_STENCIL)
      ...
      STENCIL_MESHES_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '=========== SETUP_OVERLAP FOR MATRIX_STENCIL NM=',NM
         ...
         STENCIL_EXTERNAL_CELL_LOOP: DO ICE = SCF%NC+1, SCF%NCE
           ...
           IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SCF%A_ROW(',ICE+1,')=',SCF%A_ROW(ICE+1)
   ...
   CASE (NSCARC_MATRIX_SYSTEM)
      ...
      SYSTEM_MESHES_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         DO IC = SCF%NC+1, SCF%NCE
            ...
            OSCF%NOM_PTR(JC) = ICE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') &
    'OVERLAPS: IC=',IC,': IW=',IW,': ICN=',ICN,': JC=',JC,': NOM=',NOM,': NOM_PTR(',JC,')=',OSCF%NOM_PTR(JC)

         ENDDO

         DO IC = SCF%NC+1, SCF%NCE

IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') '============================== NM=',NM,': IC=',IC
            IW = SCF%WALL_PTR(IC)
            IF (IW > 0) THEN
              ICN = SCF%WALL(IW)%ICN(1)
            ELSE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') &
          'NM=',NM,': NOM=',NOM,': JC=',JC,': JCA=',JCA,'  A: A_COL(',ICOL,')=',SCF%A_COL(ICOL),': IW=',IW,': ICN=',ICN
               ELSE
                  SCF%A_COL(ICOL) = JC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') &
          'NM=',NM,': NOM=',NOM,': JC=',JC,': JCA=',JCA,'  B: A_COL(',ICOL,')=',SCF%A_COL(ICOL),': IW=',IW,': ICN=',ICN
               ENDIF
            ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '=============================== IW = ', IW
            NOM  = SCF%WALL(IW)%NOM
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) THEN
   WRITE(SCARC_LU,*) 'ICW =',ICW,': ICC  =',ICC
   WRITE(SCARC_LU,*) 'ICE =',ICE,': ICE2 =',ICE2
   WRITE(SCARC_LU,*) 'ICG =',ICG,': ICG2 =',ICG2
   WRITE(SCARC_LU,*) 'NCOL=',NCOL,': NCOL2=',NCOL2
   WRITE(SCARC_LU,*) 'OSCF%P_ROW(ICG )=',OSCF%P_ROW(ICG),': OSCF%P_ROW(ICG +1)=',OSCF%P_ROW(ICG +1)
   WRITE(SCARC_LU,*) 'OSCF%P_ROW(ICG2)=',OSCF%P_ROW(ICG2),': OSCF%P_ROW(ICG2+1)=',OSCF%P_ROW(ICG2+1)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'A: NM=',NM,': SETTING SCC%WALL(',IWC,')%NOM=',NOM,': ICW=',ICW
            ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') 'CELLTYPE(',ICE,')=',SCF%CELLTYPE(ICE)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') 'EXT_PTR(',ICCE,')=',SCC%EXT_PTR(ICCE)
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'ICG=',ICG, OSCF%P_ROW(ICG), OSCF%P_ROW(ICG+1), NCOL
               ICOL = OSCF%P_ROW(ICG)
               IG  = IG + 1
               OSCF%IG0 = OSCF%IG0 + 1
               OSCC%NOM_PTR(OSCF%P_COL(ICOL)) = ICCE
               !OSCC%GHOST_PTR(IG) = ICCE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'TRARA: IG=',IG,': OSCC%GHOST_PTR(',IG,')=',ICCE
WRITE(*,*) 'SIZE(WALL_PTR)=',SIZE(SCC%WALL_PTR), ICCE, SCC%NCCE, NM
               !SCC%WALL_PTR(ICCE) = ICG
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') '1: NM=',NM,': NOM=',NOM,': ICG =',ICG,': NCOL=',NCOL,': OSCC%NOM_PTR(',OSCF%P_COL(ICOL),')=',OSCC%NOM_PTR(OSCF%P_COL(ICOL))
            ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') 'CELLTYPE(',ICE2,')=',SCF%CELLTYPE(ICE2)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') 'EXT_PTR(',ICCE,')=',SCC%EXT_PTR(ICCE)

IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'ICG2=',ICG2, OSCF%P_ROW(ICG2), OSCF%P_ROW(ICG2+1), NCOL2
               ICOL = OSCF%P_ROW(ICG2)
               IG  = IG + 1
               OSCF%IG0 = OSCF%IG0 + 1
               OSCC%NOM_PTR(OSCF%P_COL(ICOL)) = ICCE
               !OSCC%GHOST_PTR(IG) = ICCE
WRITE(SCARC_LU,*) 'TRARA: ICG=',IG,': OSCC%GHOST_PTR(',IG,')=',ICCE
               !SCC%WALL_PTR(ICCE) = ICG
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') '2:NM=',NM,': NOM=',NOM,': ICG2=',ICG2,': NCOL2=',NCOL2,': OSCC%NOM_PTR(',OSCF%P_COL(ICOL),')=',OSCC%NOM_PTR(OSCF%P_COL(ICOL))
            ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'NM=',NM,'================= IW=',IW,': NOM=',NOM,SCF%P_ROW(13),SCF%P_COL(27:30)
            ...
            IF (ICC < NSCARC_CELLTYPE_COARSE) THEN
               ...
               DO ICOL = SCF%P_ROW(ICW), SCF%P_ROW(ICW+1)-1
                  JC   = SCF%P_COL(ICOL)
WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') 'NM=',NM,':IW=',IW,': ICW=',ICW,': ICC=',ICC,': ICOL=',ICOL,': JC=',JC
                  IF (JC < 0) THEN
                     IC = OSCF%NOM_PTR(ABS(JC))
                     SCF%P_COL(ICOL) = SCF%CELLTYPE(IC)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,2i3)') &
    'ICW : OVERLAPS: IC=',IC,': IW=',IW,': ICW =',ICW,': JC=',JC,': NOM=',NOM,': NOM_PTR(',JC,')=',OSCF%NOM_PTR(ABS(JC)),SCF%P_COL(ICOL)
                  ...
                  ELSE IF (JC <= SCF%NCC) THEN
                     IF (SCC%INTERNAL_BDRY_CELL(JC) /= NSCARC_LAYER_ONE) THEN
                        ...
                        SCC%WALL(IWC)%ICW = JC
                        SCC%WALL(IWC)%IOR = SCF%WALL(IW)%IOR
                        WRITE(SCARC_LU,*) 'UPS %INTERNAL_BDRY_CELL(',JC,')=',SCC%INTERNAL_BDRY_CELL(JC),': IWC=',IWC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'A: NM=',NM,': SETTING SCC%WALL(',IWC,')%NOM=',NOM,': ICW=',JC
                     ENDIF
                  ENDIF
               ENDDO
            ENDIF
         ...
         !IF (IWC /= SCC%NW) THEN
         !   WRITE(SCARC_LU,*) 'Wrong number of wall cells ',IWC,' for level ',NL+1,' on mesh ',NM,' ... should be ',SCC%NW,'. Stopping program!'
         !   STOP
         !ENDIF

         IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) THEN
            WRITE(SCARC_LU,*) 'IWC=',IWC,': SCC%NW=',SCC%NW
            DO IC = 1, SCC%NC
               WRITE(SCARC_LU,*) 'FINAL INTERNAL_BDRY_CELL(',IC,')=',SCC%INTERNAL_BDRY_CELL(IC)
            ENDDO
            DO IW = 1, SCC%NW
               WRITE(SCARC_LU,*) 'FINAL WALL(',IW,')%NOM=',SCC%WALL(IW)%NOM
            ENDDO
            DO IW = 1, SCC%NW
               WRITE(SCARC_LU,*) 'FINAL WALL(',IW,')%IOR=',SCC%WALL(IW)%IOR
            ENDDO
            DO IW = 1, SCC%NW
               WRITE(SCARC_LU,*) 'FINAL WALL(',IW,')%ICW=',SCC%WALL(IW)%ICW
            ENDDO
         ENDIF
            ...
            IF (IW > 0) THEN
               ...
               DO ICOLG = OSCF%P_ROW(ICG), OSCF%P_ROW(ICG+1)-1
                  ...
                  IF (JC > 0 .AND. JC <= OSCF%NCC) THEN
                     ...
                     OSCF%P_COL(ICOLG) = IC
                     SCF%P_COL(ICOLE)  = IC
   IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,2i3,a,i3)') &
       'ICG1: OVERLAPS: IC=',IC,': IW=',IW,': ICG =',ICG,': JC=',JC,': NOM=',NOM,': NOM_PTR(',JC,')=',OSCC%NOM_PTR(ABS(JC)),OSCF%P_COL(ICOLG),' ICOLE=',ICOLE
                  ELSE
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') 'P_ROW(',ICE+1,')=',SCF%P_ROW(ICE+1)    
            ...
            ELSE
               ...
               IF (NL==NLEVEL_MIN.AND.TYPE_LAYER == NSCARC_LAYER_TWO) THEN
                  ...
                  DO ICOLG = OSCF%P_ROW(ICG2), OSCF%P_ROW(ICG2+1)-1
                     ...
                     IF (JC > 0 .AND. JC <= OSCF%NCC) THEN
                        IC = OSCC%NOM_PTR(JC)
   !WRITE(SCARC_LU,*) 'ICG2=',ICG2,': JC=',JC,': IC=',IC
                        IF (IC > 0) THEN
                           OSCF%P_COL(ICOLG) = IC
                           SCF%P_COL(ICOLE) = IC
                        ELSE
                        ...
                        ENDIF
   IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,2i3,a,i3)') &
       'ICG2: OVERLAPS: IC=',IC,': IW=',IW,': ICG2=',ICG2,': JC=',JC,': NOM=',NOM,': NOM_PTR(',JC,')=',OSCC%NOM_PTR(ABS(JC)),OSCF%P_COL(ICOLG), ': ICOLE=',ICOLE
                     ELSE
                        OSCF%P_COL(ICOLG) = SCF%CELLTYPE(ABS(JC))
                        SCF%P_COL(ICOLE) = SCF%CELLTYPE(ABS(JC))
                     ENDIF
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') 'P_ROW(',ICE2+1,')=',SCF%P_ROW(ICE2+1)    
               ENDIF
            ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'ALLOCATING FOR MULTIGRID NCE=',SC%NCE
                  ALLOCATE (SC%X(SC%NCE), STAT=IERR)
                  CALL CHKMEMERR ('SCARC', 'X', IERR)
                  SC%X = 0.0_EB
...
SELECT CASE (NTYPE)
   ...
   CASE (NSCARC_SIZE_MATRIXB)
      ...
      BANDED_SYSTEM_MESHES_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         DO IW = 1, SBF%NW
            ...
            IF (NOM /= 0) THEN
               ...
               IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3)') 'MATRIX SIZE: NM=',NM,'  NOM=',NOM,'  IC=',IC,'  NA0=',OSBF%NA0
      ...
      BANDED_SYSTEM_MESHES_LOOP2: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') 'SIZE MATRIX: NM=',NM,': NAE=',SBF%NAE,': NA=',SBF%NA
      ...
      COMPACT_SYSTEM_MESHES_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         DO IW = 1, SCF%NW
            ...
            IF (NOM /= 0) THEN
               ...
               IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3)') 'MATRIX SIZE: NM=',NM,'  NOM=',NOM,'  IC=',IC,'  NA0=',OSCF%NA0
...
WRITE(SCARC_LU,*) 'Allocating SCARC(',NM,')%OSCARC(',NOM,')%A_SIZE in length ', OSCF%NG
            ALLOCATE (OSCF%A_SIZE(1:OSCF%NG), STAT=IERR)
            CALL CHKMEMERR ('SCARC', 'OSCF%A_SIZE', IERR)
            OSCF%A_SIZE = 0

         ENDDO COMPACT_OTHER_MESHES_LOOP2
         WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') 'SIZE MATRIX: NM=',NM,': NAE=',SCF%NAE,': NA=',SCF%NA
   ...
   CASE (NSCARC_SIZE_TRANSFERC)
         ...
         DO IC = 1, SCF%NC
            SELECT CASE (SCF%CELLTYPE(IC))
               CASE (NSCARC_CELLTYPE_COARSE)
                  ...
                  SCF%NP  = SCF%NP  + 1
                  SCF%NR  = SCF%NP
      WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3)') 'COARSE: NM=',NM,'  IC=',IC,'  NCC=',SCF%NCC,': NP=',SCF%NP
               CASE (NSCARC_CELLTYPE_SFINE, NSCARC_CELLTYPE_WFINE)
                  SCF%NCF = SCF%NCF + 1
                  SCF%NP  = SCF%NP  + SCF%A_ROW(IC+1)-SCF%A_ROW(IC) - 1
                  SCF%NR  = SCF%NP
      WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3)') 'FINE  : NM=',NM,'  IC=',IC,'  NCC=',SCF%NCC,': NP=',SCF%NP
            END SELECT
         ENDDO
         SCF%NCCI = SCF%NCC
      
      WRITE(SCARC_LU,*) 'HALLOSUSI: NCCI=',SCF%NCCI
      WRITE(SCARC_LU,*) 'HALLOSUSI: NCF =',SCF%NCF
      WRITE(SCARC_LU,*) 'HALLOSUSI: NCC =',SCF%NCC
      WRITE(SCARC_LU,*) 'HALLOSUSI: NP  =',SCF%NP 
      WRITE(SCARC_LU,*) 'HALLOSUSI: NR  =',SCF%NR 
      WRITE(SCARC_LU,*) 'HALLOSUSI: NCCE=',SCF%NCCE
      ...
      WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,2i3)') 'COARSE: NM=',NM,'  NOM=',NOM,'  IC=',IC,'  NCC0=',OSCF%NCC0,': NP=',OSCF%NP0, SCF%CELLTYPE(IC)
                  CASE (NSCARC_CELLTYPE_SFINE, NSCARC_CELLTYPE_WFINE)
                     OSCF%NP0  = OSCF%NP0  + SCF%A_ROW(IC+1)-SCF%A_ROW(IC) - 1
                     OSCF%NR0  = OSCF%NP0  
                     OSCF%NCF0 = OSCF%NCF0 + 1
      WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3)') 'FINE  : NM=',NM,'  NOM=',NOM,'  IC=',IC,'  NCC0=',OSCF%NCC0,': NP=',OSCF%NP0
               END SELECT
      ...
      TRANSFER_MESHES_LOOP2: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         SCF%NCCE = SCF%NCC
         SCF%NCFE = SCF%NCF
      WRITE(SCARC_LU,*) 'HALLOSUSI: NCW =',SCF%NCW
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) NM, NOM, 'UPS: ALLOCATING OSCC%NOM_PTR in LENGTH ',OSCF%NCCI
   ...
   WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'POINTER: NM=',NM,': NOM=',NOM,': OSCC%GHOST_PTR(',IG,')=',OSCC%GHOST_PTR(IG)
            ENDDO
         ...
         WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') &
            'NM=',NM,': NP=',SCF%NP,': NPE=',SCF%NPE,': NRE=',SCF%NRE,': NCCE=',SCF%NCCE,': NCFE=',SCF%NCFE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3)') 'OLALA: SCC%NC=',SCC%NC,': SCC%NCE=',SCC%NCE,': SCC%NW=',SCC%NW
...
IF (TYPE_DEBUG >= NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,*) NM, 'ALLOCATING CELLTYPE IN LENGTH ', SCF%NC
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'ALLOCATING OSCARC: CELLTYPE in length ',OSCF%NG
...
DO NL = NLEVEL_MIN, NLEVEL_MAX
   DO NM = NMESHES_MIN, NMESHES_MAX
      SCF => SCARC(NM)%COMPACT(NL)  
      !CALL SCARC_LATEX_MATRIX(SCF%A, SCF%A_ROW, SCF%A_COL, SCF%NC, SCF%NC, NM, NL, 'A')
...
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_CELLTYPE, NL, 'SETUP_CELLTYPES', 'CELLTYPE AFTER EXCHANGE')
CALL SCARC_DEBUG_QUANTITY (NSCARC_DEBUG_WALLINFO , 2, 'SETUP_SYSTEM', 'WALLINFO')
...
MESHES_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
   ...
   SCC => SCARC(NM)%COMPACT(NL+1)
IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') '========================= NM=',NM
IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) (SCC%WALL(IW)%NOM,IW=1,SCC%NW)


   DO IW = 1, SCC%NW
   
      NOM = SCC%WALL(IW)%NOM
IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'NOM=',NOM,': NL=',NL,': IW=',IW
      OSCC => SCARC(NM)%OSCARC(NOM)%COMPACT(NL+1)
      ...
      DO ICOL = SCC%A_ROW(ICW)+1, SCC%A_ROW(ICW+1)-1
         ...
         IF (JC > SCC%NC) THEN
            ...
            IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') &
                'SCC%WALL(',IW,'): ICW=',SCC%WALL(IW)%ICW, &
                                 ': ICE(',NCPL,')=',SCC%WALL(IW)%ICE(NCPL),&
                                 ': ICN(',NCPL,')=',SCC%WALL(IW)%ICN(NCPL)
         ENDIF
      ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'INTERNAL_BDRY_CELL:'
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) (SC%INTERNAL_BDRY_CELL(IC),IC=1,SC%NC)
...
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_MEASURE, NL, 'SETUP_MEASURES', 'MEASURE AFTER INT RS3 ')
               ...
               DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1
                  ...
                  IF (JC<=SC%NC.AND.STRONGLY_COUPLED(SC%A, SC%A_ROW, IC, ICOL)) THEN
                     IF (SC%INTERNAL_BDRY_CELL(JC)/=0) THEN
                        ...
                        WRITE(SCARC_LU,*) 'DECREASING SC%MEASURE(',JC,')=',SC%MEASURE(JC)
                     ENDIF
                  ENDIF
               ENDDO
            ENDIF
...
!WRITE(SCARC_LU,*) 'NM=',NM,': MEASURE(',IC,')=',SC%MEASURE(IC)
...
WRITE(SCARC_LU,*) 'IN NMESHES=1 LOOP'
...
SELECT CASE (NTYPE)
   ...
   WRITE(SCARC_LU,*) '========================= COARSENING_BDRY'
...
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_MEASURE, NL, 'SETUP_MEASURES', 'MEASURE at beginning of CELLTYPE_RS3 ')
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_CELLTYPE, NL, 'SETUP_CELLTYPE', 'CELLTYPE at beginning of CELLTYPE_RS3 ')
ENDIF
      RS3_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
...
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_MEASURE, NL, 'SETUP_MEASURES', 'MEASURE new cycle  ')
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_CELLTYPE, NL, 'SETUP_CELLTYPE', 'CELLTYPE new cycle  ')
            !!! get maximum (remaining) measure for all cells
...
IF (TYPE_DEBUG>NSCARC_DEBUG_LESS) THEN
   WRITE(SCARC_LU,*) '========================= SETUP_CELLTYPE for IC=',IC, ' NM=',NM
...
ENDIF
                        ...
                        DO JCOL = SC%A_ROW(JC)+1, SC%A_ROW(JC+1)-1
                           IF (STRONGLY_COUPLED(SC%A, SC%A_ROW, JC, JCOL)) THEN
                              KC = SC%A_COL(JCOL)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'KC=',KC, SIZE(SC%CELLTYPE)
                              IF (KC /= 0) THEN
...
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_MEASURE, NL, 'SETUP_MEASURES', 'MEASURE after RS3 ')
CALL SCARC_DEBUG_QUANTITY(NSCARC_DEBUG_CELLTYPE, NL, 'SETUP_CELLTYPE', 'CELLTYPE after RS3  ')
...
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) THEN
   WRITE(SCARC_LU,*) 'NM=',NM,' IW=',IW,': IC=',IC, SC%CELLTYPE(IC)
ENDIF
                  ...
                  DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1       
                     ...
                     IF (STRONGLY_COUPLED(SC%A, SC%A_ROW, IC, ICOL)) THEN
                        JC = SC%A_COL(ICOL)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'MEASURE(',JC,')=',SC%MEASURE(JC),': MEASURE(',IC,')=',SC%MEASURE(IC)
         ...
         PMIS_LOOP2: DO NM = NMESHES_MIN, NMESHES_MAX
            ...
            INDEPENDENT_SET_LOOP_PMIS: DO IC = 1, SC%NCE
               IF (SC%CELLTYPE(IC) == NSCARC_CELLTYPE_COARSE) THEN
                  ...
                  DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1       
                     JC = SC%A_COL(ICOL)
WRITE(SCARC_LU,'(a,i3,a,i3,a,f12.6,a,i3,a,i3,a,f12.6)') 'NM=',NM,': MEASURE(',IC,')=',SC%MEASURE(IC),'-----> ICOL=',ICOL,': JC=',JC
                     IF (JC < 0) CYCLE
                     IF (SC%CELLTYPE(JC) == NSCARC_CELLTYPE_NONE) THEN
                        SC%CELLTYPE(JC) = NSCARC_CELLTYPE_SFINE
                        SC%MEASURE(JC) = 0.0_EB
WRITE(SCARC_LU,'(a,i3,a,f12.6)') '               MEASURE(',JC,')=',SC%MEASURE(JC)
                     ENDIF
                  ENDDO 
               ENDIF
...
WRITE(SCARC_LU,'(a,i3,a,f12.6)') '               MEASURE(',JC,')=',SC%MEASURE(JC)
                        DO JCOL = SC%A_ROW(JC)+1, SC%A_ROW(JC+1)-1
                           ...
                           IF (STRONGLY_COUPLED(SC%A, SC%A_ROW, JC, JCOL)) WRITE(SCARC_LU,*) 'INCREASING MEASURE OF ',KC,' to ', SC%MEASURE(KC), JCOL
                        ENDDO
                     ENDIF
                  ENDDO 
               ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IC=',IC,': COARSE CELL WEIGHTS=',WEIGHTS(1)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '=================== COARSE =============!!!!! IC=',IC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,9i8)') 'IC=',IC,': SFINE CELL has coarse NEIGHBOR=',NEIGHBOR(1:9)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,9f8.2)') 'IC=',IC,': SFINE CELL has coarse WEIGHTS= ',WEIGHTS(1:9)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '=================== FINE   =============!!!!! IC=',IC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '        ICOL=',ICOL,': JC=',JC
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '        JCOL=',JCOL,': KC=',KC
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '        --->  COARSE_CELL (',IW2,')=',COARSE_CELL(IW2)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              COARSE_INDEX(',IW2,')=',COARSE_INDEX(IW2)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              SUM_COARSE=',SUM_COARSE
ENDIF
                                 IW2 = IW2 + 1
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '        --->  JCOL=',JCOL,': IW=',IW
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              WEIGHTS(',IW,')=',WEIGHTS(IW)
ENDIF
                        ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '        --->  final data:'
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              NEIGHBOR(',IW,')=',NEIGHBOR(IW)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              WEIGHTS (',IW,')=',WEIGHTS(IW)
ENDIF
               ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'HIER NOCH MAL CHECKEN, NOCH ALTE VERSION, AN NEUE KOMMUNIKATION ANPASSEN !!!'
            SC%P_ROW(IC) = IP
            DO IW = 1, NWEIGHTS
               IF  (NEIGHBOR(IW) /= -1) THEN
                  ...
                  SC%P(IP)     = WEIGHTS(IW)
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '        --->  prolongation data:'
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              P_COL(',IP,')=',SC%P_COL(ip)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '              P    (',IP,')=',SC%P(ip)
ENDIF
                  IP = IP +1
               ENDIF
            ENDDO
   ...
   CASE (NSCARC_INTERPOL_DIRECT)
      ...
      MESHES_LOOP_DIRECT: DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         IP = 1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '------------------ COARSE', SC%NC
         INTERNAL_CELL_LOOP: DO IC = 1, SC%NC
         
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '------------------ INTERNAL', IC
            VALUES   = 0.0_EB
            WEIGHTS  = 0.0_EB
            NEIGHBOR = 0
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '------------------ COARSE'
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IC=',IC,': WEIGHTS=',WEIGHTS(1)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IDIAG=',IDIAG,' A(IDIAG)=', SC%A(IDIAG)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '-------------> SFINE:'
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SC%A_ROW(',IC,')+1=',SC%A_ROW(IC)+1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SC%A_ROW(',IC+1,')-1=',SC%A_ROW(IC+1)-1
ENDIF
                     DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1
                        JC = SC%A_COL(ICOL)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,f12.6,a,f12.6)') ' --- JC = ',JC,' SUM_COUPLED=',SUM_COUPLED,' SUM_CPOINTS=',SUM_CPOINTS
ENDIF
                     ENDDO
         
                     SCAL = - SUM_COUPLED/SUM_CPOINTS
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,f12.6,2f12.6)') 'IC=',IC,': SCAL=',SCAL, SUM_COUPLED, SUM_CPOINTS
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,i3,i3,3f12.6)') 'IC=',IC,': JC=',JC,': ',SC%CELLTYPE(JC), IW, NEIGHBOR(IW), WEIGHTS(IW), SC%A(ICOL), SC%A(IDIAG)
ENDIF
                           IW = IW +1
                        ENDIF
                     ENDDO
                     NWEIGHTS = IW - 1
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,8i7)') 'IC=',IC,': NEIGHBORS==',NEIGHBOR(1:IW-1)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,8f7.3)') 'IC=',IC,': WEIGHTS   ==',WEIGHTS(1:IW-1)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '------------------ WFINE'
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'IC=',IC,': JC=',JC,': SCAL=',SCAL
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,i3,i3,3f12.6)') 'JC=',JC,': KC=',KC,': ',SC%CELLTYPE(KC), IW, NEIGHBOR(IW), WEIGHTS(IW), SC%A(JCOL), SC%A(JDIAG)
ENDIF
                             IW = IW +1
                           ENDIF
                        ENDDO
                     ...
                     NWEIGHTS = IW - 1
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,10i8)') '1: IC=',IC,': NEIGHBORS==',NEIGHBOR(1:NWEIGHTS)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,10f8.2)') '1: IC=',IC,': WEIGHTS   ==',WEIGHTS(1:NWEIGHTS)
ENDIF
                     ...
                     ENDDO
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,10i8)') '2: IC=',IC,': NEIGHBORS==',NEIGHBOR(1:NWEIGHTS)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,10f8.2)') '2: IC=',IC,': WEIGHTS   ==',WEIGHTS(1:NWEIGHTS)
ENDIF
            ...
            !!!
            SC%P_ROW(IC) = IP
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'NM=',NM,': P_ROW(',IC,')=',SC%P_ROW(IC)
            DO IW = 1, NWEIGHTS
               IF  (NEIGHBOR(IW) /= -1) THEN
                  SC%P_COL(IP) = NEIGHBOR(IW)
                  SC%P(IP)     = WEIGHTS(IW)
         ...
         INTERNAL_CELL_LOOP2: DO IC = 1, SC%NC
            DO ICOL = SC%P_ROW(IC), SC%P_ROW(IC+1)-1
              ...
              ENDIF
IF (TYPE_DEBUG > NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3)') 'A:PROL2:IC=',IC,': P_COL(',ICOL,')=',SC%P_COL(ICOL)
            ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,*) 'GHOSTCELL_LOOP:------------> IW=',IW
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '---> IW=',IW,ICG, ICA
               DO ICOL = SC%P_ROW(ICA), SC%P_ROW(ICA+1)-1
                 JC = SC%P_COL(ICOL)
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '------------> ICOL=',ICOL, ': JC=',JC
                 IF (JC > SC%NC) THEN
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '------------------ READy '
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '---------------------------------------> INTERNAL_BDRY_CELL(',IC,')=',SC%INTERNAL_BDRY_CELL(IC)
            DIRECT_BDRY_IF: IF (SC%INTERNAL_BDRY_CELL(IC)/=0) THEN

IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '------------------ BOUNDARY CELL:', IC
               VALUES   = 0.0_EB
               WEIGHTS  = 0.0_EB
               NEIGHBOR = 0
...
ENDIF
                  DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1
                     JC = SC%A_COL(ICOL)
                     SUM_COUPLED = SUM_COUPLED + SC%A(ICOL)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SUMMING ',JC, SUM_COUPLED
                     IF (JC > SC%NC) CYCLE
...
ENDIF
                  ...
                  DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1
                     JC = SC%A_COL(ICOL)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'TRYING ',JC
                     IF (JC > SC%NC) CYCLE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'P_ROW(',SC%NC+1,')=',SC%P_ROW(SC%NC+1)
         SC%P_ROW(SC%NC+1) = IP
         SC%NROW0 = IP
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '================================ >IC=',IC
            DO ICOL = SC%P_ROW(IC), SC%P_ROW(IC+1)-1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'ICOL=',ICOL, SC%P_COL(ICOL)
              JC = SC%P_COL(ICOL)
              IF (JC <= SC%NC) THEN                 
                 SC%P_COL(ICOL) = SC%CELLTYPE(JC)         
              ENDIF
...
MESHES_LOOP: DO NM = NMESHES_MIN, NMESHES_MAX
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '==================== STARTING SETUP_RESTRICTION:', NM
      ...
      DO IC = 1, SC%NCE
         ...
         ROW_LOOP: DO ICP2 = SC%P_ROW(IC),SC%P_ROW(IC+1)-1
            IF (SC%P_COL(ICP2) == ICP) THEN
               ...
               IF (BFIRST) THEN
                  SC%R_ROW(ICP) = IC2
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'HALLO: IC=',IC,': ICP2=',ICP2,': R_ROW(',ICP,')=',SC%R_ROW(ICP)
               ENDIF
               SC%R_COL(IC2) = IC
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) ' ----------->  R_COL(',ICP,')=',SC%R_COL(IC2)
               IC2 = IC2 + 1
               BFIRST = .FALSE.
               EXIT ROW_LOOP
            ENDIF
   ...
   OTHER_MESHES_LOOP: DO NOM = NMESHES_MIN, NMESHES_MAX
      ...
      IF (NOM == NM) CYCLE OTHER_MESHES_LOOP
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3)') '====================    STARTING RESTRICTION FOR NM=',NM, ': NOM=',NOM
      OS  => SCARC(NM)%OSCARC(NOM)      
      IF (OS%NICMAX_S==0 .AND. OS%NICMAX_R==0) CYCLE OTHER_MESHES_LOOP
      OSC  => SCARC(NM)%OSCARC(NOM)%COMPACT(NL)      
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,i3,a,i3,a,i3)') 'RESTRICT: NM=',NM,': NOM=',NOM,': IG0:',IG0,': ICP=',ICP,': OSC%NG=',OSC%NG, ': OSC%NCC=',OSC%NCC
         BFIRST = .TRUE.
         DO IG = 1, OSC%NG
            ...
            OTHER_ROW_LOOP: DO ICP2 = OSC%P_ROW(IG),OSC%P_ROW(IG+1)-1
               IF (OSC%P_COL(ICP2) == ICC) THEN
                  ...
                  IF (BFIRST) THEN
                     OSC%R_ROW(ICC) = IC2
                  ENDIF
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'R_ROW(',ICC,')=',OSC%R_ROW(ICC)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'R(',IC2,')=',OSC%R(IC2)
                  OSC%R_COL(IC2) = IG
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'R_COL(',IC2,')=',OSC%R_COL(IC2)
                  IC2 = IC2 + 1
                  BFIRST = .FALSE.
                  EXIT OTHER_ROW_LOOP
               ENDIF
      ...
      OSC%R_ROW(ICC+1)=IC2
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) ' ----------->  R_ROW(',ICC+1,')=',OSC%R_ROW(ICC+1)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'TRANSFER_MATRIX: SCF%NCCE=',SCF%NCCE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'HALLO7: NCCE=',SCF%NCCE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SC%NC  =',SCF%NC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SC%NCC =',SCF%NCC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SC%NCCE=',SCF%NCCE
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '=================================== ICP1=',ICP1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) '========================================================================='
...
BTWO = (TYPE_DEBUG>NSCARC_DEBUG_LESS).AND.NMESHES==2.AND.(IC>0)
BDEBUG = BTWO
BDEBUG = .FALSE.
BDEBUG2= .TRUE.
IF (BDEBUG) WRITE(SCARC_LU,*) '------------------------------> '
IF (BDEBUG) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3)') 'A: ------> ICP1=',ICP1,': ICP2=',ICP2,': IC  =',IC  

            IDIAG = SCF%A_ROW(IC)
            PW    = P_WEIGHT(IC, ICP2, NM, NL)
            AUX1  = SCF%A(IDIAG) * PW
IF (BDEBUG) WRITE(SCARC_LU,'(a,f12.6,a,f12.6,a,f12.6,a,f12.6)') 'A: AUX1 = ', SCF%A(IDIAG),'*',PW,' = ', AUX1
...
IF (BDEBUG) WRITE(SCARC_LU,'(a,f12.6,a,f12.6,a,f12.6)') 'A: AUX1 = AUX1 + ', SCF%A(ICOL),'*',PW,' = ', AUX1
...
IF (BDEBUG) WRITE(SCARC_LU,'(a,f12.6,a,f12.6,a,f12.6,2i3)') 'A: ---------------> AUX2 = AUX2 + ',P_WEIGHT(IC,ICP1,NM,NL) ,'*',AUX1,' = ', AUX2, IC, ICP1

         ENDDO
         VAL(ICP2) = AUX2
  IF (BDEBUG) WRITE(SCARC_LU,'(a,i3,a,i3,a,f12.6)') '================================================'
  IF (BDEBUG2) WRITE(SCARC_LU,'(a,i3,a,i3,a,f12.6)') '---------->  ICP1=',ICP1,': VAL(',ICP2,')=',VAL(ICP2)
  IF (BDEBUG) WRITE(SCARC_LU,'(a,i3,a,i3,a,f12.6)') '================================================'
     ...
     ! WRITE(SCARC_LU,*) '==============================================='
     ! WRITE(SCARC_LU,*) 'MAUX1:'
     ! WRITE(SCARC_LU,'(i4,24i7)') (ICP2, ICP2=0,18)
     ! IF (NMESHES == 2) THEN
     ! WRITE(SCARC_LU,'(i4,14f7.2)') (IC, (MAUX1(IC,ICP2),ICP2=1,14),IC=1,SCF%NCE)
     ! ELSE
     ! WRITE(SCARC_LU,'(i4,18f7.2)') (IC, (MAUX1(IC,ICP2),ICP2=1,18),IC=1,SCF%NCE)
     ! ENDIF
     ! WRITE(SCARC_LU,*) '==============================================='
      ...
      SCC%A_ROW(ICP1) = IP
      SCC%A_COL(IP)   = ICP1
  WRITE(SCARC_LU,'(a,i3,a,f12.6,a,i3,a,i3,a,i3,a,i3)') &
          '----------> HALLO: A(',IP,')=',SCC%A(IP),&
      ...
      !DO ICP2 = 1, SCF%NCC
         IF (ICP2 /= ICP1 .AND. ABS(VAL(ICP2)) >= 1.0E-12_EB) THEN
            SCC%A(IP)     = VAL(ICP2)
            SCC%A_COL(IP) = ICP2
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'ICP1=',ICP1,': ICP2=',ICP2,': A_COL(',IP,')=',SCC%A_COL(IP)
            IP  = IP + 1
         ENDIF
      ENDDO ICP2_LOOP3
   
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'MATRIX VALUES IN LINE ICP1:', ICP1
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(20f9.3)') (VAL(ICP2),ICP2=1,SCF%NCCE)
      VAL = 0.0_EB
   ENDDO ICP1_LOOP
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'MATVEC_PRODUCT: EXCHANGE ', NSCARC_EXCHANGE_VECTOR, TYPE_VECTOR
...
IF (TYPE_DEBUG > NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,*) 'MATVEC: VC2(',IC,')=',VC2(IC)
            ENDDO
...
REAL(EB) :: AUX, OMEGA=1.5_EB, VBO, VCO
TYPE (MESH_TYPE), POINTER :: M
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'JACOBI: VB2(',I,',',J,',',K,')=',VB2(I,J, K),AB(IC,ID)
                     ENDDO
                  ENDDO
               ENDDO
...
!IF (NMESHES == 1) THEN
!   WRITE(SCARC_LU,'(10f12.6)') ((VB2(I,1,K), I=0,9), K=5,0,-1)
!ELSE
!   WRITE(SCARC_LU,'(6f12.6)') ((VB2(I,1,K), I=0,5), K=5,0,-1)
!ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,5f12.6)') 'A: SSOR: VB2(',I,',',1,',',K,')=',VB2(I,1, K),AB(IC,ID), VB2(I,1,K-1), VB2(I-1,1,K), VBO
                        ENDDO
                     ENDDO
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '--------------------------------------------------'
                     DO K = NZ, 1, - 1
                        DO I = NX, 1, - 1
                           IC = (K-1) * NX + I
                           IF (IC==NC) CYCLE
                           ...
                           VBO = VB2(I,1,K)
                           VB2 (I, 1, K) = VB2 (I, 1, K) - AUX * OMEGA / AB(IC,ID)
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,5f12.6)') 'B: SSOR: VB2(',I,',',1,',',K,')=',VB2(I,1, K),AB(IC,ID), VB2(I,1,K+1), VB2(I+1,1,K), VBO
                        ENDDO
                     ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i4,a,f12.6,a,i4,a,f12.6)') 'JACOBI: VC2(',IC,')=',VC2(IC),' : A(',IC,')=',AC(AC_ROW(IC))
               ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'A: =========================== IC=',IC
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,f20.12,a,i3,a,f20.12,a,i3,a,2f20.12)') 'A: SSOR: AUX=',AUX,': AC(',ICOL,')=',AC(ICOL), ': VC2(',AC_COL(ICOL),')=', VC2(AC_COL(ICOL)), VCO
                  ENDDO LOWER_DIAG_LOOP
                  VC2(IC) = (VC2(IC) - AUX * OMEGA) / AC(AC_ROW(IC))
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i4,a,f20.12,a,i4,a,f20.12)') 'A: SSOR: VC2(',IC,')=',VC2(IC),' : A(',IC,')=',AC(AC_ROW(IC))
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'B:=========================== IC=',IC
                  AUX = 0.0_EB
                  UPPER_DIAG_LOOP: DO ICOL = AC_ROW(IC)+1, AC_ROW(IC+1)-1
                     IF (AC_COL(ICOL) <= IC) CYCLE UPPER_DIAG_LOOP
   VCO = VC2(AC_COL(ICOL))
                     AUX = AUX + AC(ICOL) * VC2(AC_COL(ICOL))
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,f20.12,a,i3,a,f20.12,a,i3,a,2f20.12)') 'B: SSOR: AUX=',AUX,': AC(',ICOL,')=',AC(ICOL), ': VC2(',AC_COL(ICOL),')=', VC2(AC_COL(ICOL)), VCO
                  ENDDO UPPER_DIAG_LOOP
                  VC2(IC) = VC2(IC) - AUX * OMEGA / AC(AC_ROW(IC))
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i4,a,f20.12,a,i4,a,f20.12)') 'B: SSOR: VC2(',IC,')=',VC2(IC),' : A(',IC,')=',AC(AC_ROW(IC))
...
CALL SCARC_DEBUG_LEVEL (VEC_G, 'SCARC_METHOD_CG', 'G INIT', NL)
...
CALL SCARC_SETUP_ENVIRONMENT(CROUTINE, NSCOPE, NVECTOR, NL)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'BEVORE SETUP_SOLVER'
CALL SCARC_SETUP_SOLVER(NL)

!CALL SCARC_VECTOR_DEFINE(VEC_F, 1.0_EB, NL)
!CALL SCARC_VECTOR_DEFINE(VEC_X, 0.0_EB, NL)

IF (NMESHES == 1) THEN
   WRITE(*,*) '===================================='
   WRITE(*,'(a,f25.16)') 'F(4)=',SCARC(1)%COMPACT(1)%F(4)
   WRITE(*,'(a,f25.16)') 'F(5)=',SCARC(1)%COMPACT(1)%F(5)
   WRITE(*,'(a,f25.16)') 'F(6)=',SCARC(1)%COMPACT(1)%F(6)
ELSE
   WRITE(*,*) '===================================='
   WRITE(*,'(a,f25.16)') 'F(4)=',SCARC(1)%COMPACT(1)%F(4)
   WRITE(*,'(a,f25.16)') 'F(5)=',SCARC(1)%COMPACT(1)%F(5)
   WRITE(*,'(a,f25.16)') 'F(6)=',SCARC(2)%COMPACT(1)%F(1)
ENDIF
...
IF (NMESHES == 1) THEN
   WRITE(*,*) '===================================='
   WRITE(*,'(a,f25.16)') 'X(4)=',SCARC(1)%COMPACT(1)%X(4)
   WRITE(*,'(a,f25.16)') 'X(5)=',SCARC(1)%COMPACT(1)%X(5)
   WRITE(*,'(a,f25.16)') 'X(6)=',SCARC(1)%COMPACT(1)%X(6)
ELSE
   WRITE(*,*) '===================================='
   WRITE(*,'(a,f25.16)') 'X(4)=',SCARC(1)%COMPACT(1)%X(4)
   WRITE(*,'(a,f25.16)') 'X(5)=',SCARC(1)%COMPACT(1)%X(5)
   WRITE(*,'(a,f25.16)') 'X(6)=',SCARC(2)%COMPACT(1)%X(1)
ENDIF
...
!DO K=M%KBAR,1,-1
!WRITE(SCARC_LU,'(8i4)') (M%WALL(IW)%PRESSURE_BC_INDEX,IW=1,M%N_EXTERNAL_WALL_CELLS)
!ENDDO
!WRITE(SCARC_LU,*) 'PRHS'
!DO K=M%KBAR,1,-1
!WRITE(SCARC_LU,'(4f25.16)') (M%PRHS(I,1,K),I=1,M%IBAR)
!ENDDO
!WRITE(SCARC_LU,*) 'F'
!DO K=M%KBAR,1,-1
!WRITE(SCARC_LU,'(4f25.16)') (SB%F(I,1,K),I=1,M%IBAR)
!ENDDO
...
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'A 1: F(',I,',',J,',',K,')=',SB%F(I,J,K)
                           ELSE IF (SB%WALL(IW)%BTYPE==NEUMANN) THEN
                              SB%F(I,J,K) = SB%F(I,J,K) + SB%DXI * M%BXS(1,K)                   ! Neumann
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'B 1: F(',I,',',J,',',K,')=',SB%F(I,J,K)
                           ENDIF
                        CASE (-1)
                           IF (SB%WALL(IW)%BTYPE==DIRICHLET) THEN
                              SB%F(I,J,K) = SB%F(I,J,K) - 2.0_EB * SB%DXI2 *M%BXF(1,K)          ! Dirichlet
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'A-1: F(',I,',',J,',',K,')=',SB%F(I,J,K), M%PRHS(I,J,K)
                           ELSE IF (SB%WALL(IW)%BTYPE==NEUMANN) THEN
                              SB%F(I,J,K) = SB%F(I,J,K) - SB%DXI *M%BXF(1,K)                    ! Neumann
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'B-1: F(',I,',',J,',',K,')=',SB%F(I,J,K), M%PRHS(I,J,K)
                           ENDIF
                        CASE (3)
                           IF (SB%WALL(IW)%BTYPE==DIRICHLET) THEN
                              SB%F(I,J,K) = SB%F(I,J,K) - 2.0_EB * SB%DZI2 * M%BZS(I,1)         ! Dirichlet
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'A 3: F(',I,',',J,',',K,')=',SB%F(I,J,K), M%PRHS(I,J,K)
                           ELSE IF (SB%WALL(IW)%BTYPE==NEUMANN) THEN
                              SB%F(I,J,K) = SB%F(I,J,K) + SB%DZI * M%BZS(I,1)                   ! Neumann
!WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,2f25.16)') 'B 3: F(',I,',',J,',',K,')=',SB%F(I,J,K), M%PRHS(I,J,K)
                           ENDIF
                        CASE (-3)
                           IF (SB%WALL(IW)%BTYPE==DIRICHLET) THEN
...
!WRITE(SCARC_LU,'(a,i3,a,2f25.16)') 'A 1: F(',IC,')=',SC%F(IC)
                           ELSE IF (SC%WALL(IW)%BTYPE==NEUMANN) THEN
                              SC%F(IC) = SC%F(IC) + SC%DXI * M%BXS(1,K)                   ! Neumann
!WRITE(SCARC_LU,'(a,i3,a,2f25.16)') 'B 1: F(',IC,')=',SC%F(IC)
                           ENDIF
                        CASE (-1)
                           IF (SC%WALL(IW)%BTYPE==DIRICHLET) THEN
...
!WRITE(SCARC_LU,'(a,i3,a,2f25.16)') 'A 3: F(',IC,')=',SC%F(IC)
                           ELSE IF (SC%WALL(IW)%BTYPE==NEUMANN) THEN
                              SC%F(IC) = SC%F(IC) + SC%DZI * M%BZS(I,1)                   ! Neumann
!WRITE(SCARC_LU,'(a,i3,a,2f25.16)') 'B 3: F(',IC,')=',SC%F(IC)
                           ENDIF
                        CASE (-3)
                           IF (SC%WALL(IW)%BTYPE==DIRICHLET) THEN
...
!WRITE(SCARC_LU,'(a,i3,a,2f25.16)') 'A 2: F(',IC,')=',SC%F(IC)
                           ELSE IF (SC%WALL(IW)%BTYPE==NEUMANN) THEN
                              SC%F(IC) = SC%F(IC) + SC%DYI * M%BYS(I,K)                   ! Neumann
!WRITE(SCARC_LU,'(a,i3,a,2f25.16)') 'B 2: F(',IC,')=',SC%F(IC)
                           ENDIF
                        CASE (-2)
                           IF (SC%WALL(IW)%BTYPE==DIRICHLET) THEN
...
!WRITE(SCARC_LU,'(12i4)') IX_FI, IY_FI, IZ_FI, IC_CO, IC_FI(1:8)
         ...
         CASE (NSCARC_MULTIGRID_ALGEBRAIC)
            ...
            DO NM = NMESHES_MIN, NMESHES_MAX
               ...
               DO IC_CO = 1, NC_CO
IF (TYPE_DEBUG >= NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,*) '============================', IC_CO
                  AUX = 0.0_EB
                  DO ICOL = R_ROW(IC_CO), R_ROW(IC_CO+1)-1
                     IC = R_COL(ICOL)
                     AUX0 = AUX
                     AUX = AUX + DC_FI(IC) * R(ICOL)
IF (TYPE_DEBUG >= NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,'(3i6,4f24.16)') IC_CO, IC, ICOL, AUX, DC_FI(IC), R(ICOL), AUX0
                  ENDDO
                  FC_CO(IC_CO) = AUX
IF (TYPE_DEBUG >= NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,'(a,i3,a,f24.16)') 'FC_CO(',IC_CO,')=',AUX
               ENDDO
         ...
         CASE (NSCARC_MULTIGRID_ALGEBRAIC)
            ...
            DO NM = NMESHES_MIN, NMESHES_MAX
               ...
               DO IC = 1, NC_FI
                  ...
                  ENDDO
                  DC_FI(IC) = AUX
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,f24.16)') 'DC_FI(',IC,')=',DC_FI(IC)
               ENDDO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! Get fine grid ghost number related to a neighboring coarse cell
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
INTEGER FUNCTION GHOST_NUMBER(IC, NC, NCE, P_PTR)
INTEGER, POINTER, DIMENSION(:,:), INTENT(IN) :: P_PTR
INTEGER, INTENT(IN) :: IC, NC, NCE
INTEGER :: JC, IC0

IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'GHOST_NUMBER searching for IC:', IC, NC, NCE
IC0 = 0
DO JC = NC+1, NC+ 2*(NCE-NC)
   WRITE(SCARC_LU,*) 'P_PTR(',JC,',1)=', P_PTR(JC,1)
   IF (P_PTR(JC,1) == IC) THEN
      IC0 = JC
      EXIT
   ENDIF
ENDDO
...
IF (IC0 == 0) THEN
   WRITE(*,*) 'GHOST_NUMBER: NEIGHBORING CELL NUMBER NOT FOUND, STOPPING !'
   STOP
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'OS%NIC_R=',OS%NIC_R
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'OS%NIC_S=',OS%NIC_S
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'TYPE_LAYER=',TYPE_LAYER
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'NSCARC_COUPLING_MAX=',NSCARC_COUPLING_MAX
                     NLEN = MAX(OS%NIC_R, OS%NIC_S)*TYPE_LAYER*NSCARC_COUPLING_MAX*2+10
               END SELECT
               ALLOCATE (OS%RECV_BUF(NLEN))
               OS%RECV_BUF = 0.0_EB
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SUSI: ALLOCATING RECV_BUF in LENGTH ', NLEN
            ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NA =',OSC%NA
            ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NC =',OSC%NC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCCI=',OSC%NCCI
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCE=',OSC%NCE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NW =',OSC%NW
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NP =',OSC%NP
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NR =',OSC%NR
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCC=',OSC%NCC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'R: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCF=',OSC%NCF
            ENDIF
         ...
         SELECT_SYSTEM: SELECT CASE (TYPE_SYSTEM)
            ...
            CASE (NSCARC_SYSTEM_COMPACT)
               ...
               ELSE
                  ...
                  OSC%WALL(1:OSC%NW)%NCPL = SOC%WALL(1:SOC%NW)%NCPL
IF (TYPE_DEBUG > NSCARC_DEBUG_MEDIUM) THEN
   WRITE(SCARC_LU,*) '============ EXCHANGE_WALL: NM=', NM,': NOM=',NOM
   WRITE(SCARC_LU,*) 'IXG=',OSC%WALL(1:OSC%NW)%IXG
   WRITE(SCARC_LU,*) 'IYG=',OSC%WALL(1:OSC%NW)%IYG
   WRITE(SCARC_LU,*) 'IZG=',OSC%WALL(1:OSC%NW)%IZG
   WRITE(SCARC_LU,*) 'IXW=',OSC%WALL(1:OSC%NW)%IXW
   WRITE(SCARC_LU,*) 'IYW=',OSC%WALL(1:OSC%NW)%IYW
   WRITE(SCARC_LU,*) 'IZW=',OSC%WALL(1:OSC%NW)%IZW
   WRITE(SCARC_LU,*) 'IXN1=',OSC%WALL(1:OSC%NW)%IXN1
   WRITE(SCARC_LU,*) 'IXN2=',OSC%WALL(1:OSC%NW)%IXN2
   WRITE(SCARC_LU,*) 'IYN1=',OSC%WALL(1:OSC%NW)%IYN1
   WRITE(SCARC_LU,*) 'IYN2=',OSC%WALL(1:OSC%NW)%IYN2
   WRITE(SCARC_LU,*) 'IZN1=',OSC%WALL(1:OSC%NW)%IZN1
   WRITE(SCARC_LU,*) 'IZN2=',OSC%WALL(1:OSC%NW)%IZN2
   WRITE(SCARC_LU,*) 'IOR=',OSC%WALL(1:OSC%NW)%IOR
   WRITE(SCARC_LU,*) 'NOM=',OSC%WALL(1:OSC%NW)%NOM
   WRITE(SCARC_LU,*) 'ICW=',OSC%WALL(1:OSC%NW)%ICW
   WRITE(SCARC_LU,*) 'NCPL=',OSC%WALL(1:OSC%NW)%NCPL
ENDIF
                  DO IW=1,OSC%NW   
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) THEN
   WRITE(SCARC_LU,*) (OSC%WALL(IW)%ICN(IERR), IERR=1,OSC%WALL(IW)%NCPL)
ENDIF
                     ENDIF
                  ENDDO
                  IF (NL==NLEVEL_MIN.AND.TYPE_LAYER == NSCARC_LAYER_TWO .AND. TYPE_MULTIGRID == NSCARC_MULTIGRID_ALGEBRAIC) THEN
      ...
      CASE (NSCARC_EXCHANGE_GRID) 
         ...
         ELSE
            ...
            OSC%NC =  SOC%NC
            OSC%NW =  SOC%NW
WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'EXCHANGE_GRID:SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NC=',OSC%NC
WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'EXCHANGE_GRID:SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NW=',OSC%NW
         ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%BANDED(',NL,')%NA =',OSB%NA
         ENDIF
      ...
      CASE (NSCARC_EXCHANGE_SIZE_MATRIXC) 
         ...
         ELSE
            SOC     => SCARC(NOM)%COMPACT(NL)
            OSC%NA  =  SCARC(NOM)%OSCARC(NM)%COMPACT(NL)%NA0
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NA =',OSC%NA
         ENDIF
      ...
      CASE (NSCARC_EXCHANGE_SIZE_TRANSFERC) 
         ...
         ELSE
            ...
            OSC%NCC =  SCARC(NOM)%OSCARC(NM)%COMPACT(NL)%NCC0
            OSC%NCF =  SCARC(NOM)%OSCARC(NM)%COMPACT(NL)%NCF0
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3)') 'SENDING FOR LEVEL ', NL
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NC =',OSC%NC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCE=',OSC%NCE
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCCI =',OSC%NCCI
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NW =',OSC%NW
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NP =',OSC%NP
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NR =',OSC%NR
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCC=',OSC%NCC
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'S: SCARC(,',NM,')%OSCARC(,',NOM,')%COMPACT(',NL,')%NCF=',OSC%NCF
         ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'CALLING PACK_BMATRIX FOR ',NM, NL, TYPE_MATRIX
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'PACKING: ', TYPE_EXCHANGE, NW, NX, NY, NC, NM, NL

               SELECT_EXCHANGE_COMPACT: SELECT CASE (TYPE_EXCHANGE)

                  CASE (NSCARC_EXCHANGE_VECTOR) 
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'BEFORE PACK_CVECTOR_REAL:  NM=',NM,': NOM=',NOM
                  ...
                  CASE (NSCARC_EXCHANGE_MATRIX) 
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'BEFORE PACK_MATRIX:  NM=',NM,': NOM=',NOM,': NW=',NW, 'TYPE_MATRIX=',TYPE_MATRIX
...
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'BEFORE PACK_CVECTOR for CELLTYPES NM=',NM,': NOM=',NOM
                     CALL SCARC_PACK_CVECTOR_INT2 (BUFFER, WALL, NSCARC_VECTOR_CELLTYPE, NLEN, NW, NM, NL)
      
                  CASE (NSCARC_EXCHANGE_WEIGHTS) 
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'BEFORE PACK_CMATRIX NM=',NM,': NOM=',NOM
                     CALL SCARC_PACK_CMATRIX(BUFFER, WALL, NSCARC_MATRIX_PROLONGATION, NLEN, NW, NM, NOM, NL)
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'AFTER PACK_CMATRIX'
         ...
         SELECT CASE(TYPE_SYSTEM)
            ...
            CASE (NSCARC_SYSTEM_COMPACT)
               ...
               SELECT_EXCHANGE_COMPACT2: SELECT CASE (TYPE_EXCHANGE)
                  ...
                  CASE (NSCARC_EXCHANGE_MATRIX)
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'NM=',NM,': NOM=',NOM,': UNPACK CMATRIX : NL=',NL
                  ...
                  CASE (NSCARC_EXCHANGE_CELLTYPE)
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'BEFORE UNPACK_CVECTOR for CELLTYPES NM=',NM,': NOM=',NOM
                     CALL SCARC_UNPACK_CVECTOR_INT2 (BUFFER, WALL, NSCARC_VECTOR_CELLTYPE, NM, NOM, NL)
      
                  CASE (NSCARC_EXCHANGE_WEIGHTS)
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'BEFORE UNPACK_CMATRIX'
                     CALL SCARC_UNPACK_CMATRIX(BUFFER, WALL, NSCARC_MATRIX_PROLONGATION, NOM, NM, NL)
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'AFTER UNPACK_CMATRIX'
...
IF (TYPE_DEBUG >= NSCARC_DEBUG_MEDIUM) WRITE(SCARC_LU,*) '================================= NM=', NM
...
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'IW=',IW,': SEND_BUF(',LL+2,')=',SEND_BUF(LL+2)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,f25.16,i3)') 'UNPACK_BVECTOR: X(',I,',',J,',',K,')=',BVECTOR(I,J,K), ISUM
ENDDO UNPACK_RECV
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) THEN
   WRITE(SCARC_LU,*) '================================= NM=', NM, ' NTYPE=',NTYPE
   WRITE(SCARC_LU,*) ' NLEN=',NLEN
   WRITE(SCARC_LU,*) ' NW=',NW
   WRITE(SCARC_LU,*) ' NM',NM
   WRITE(SCARC_LU,*) ' NL',NL
ENDIF
...
PACK_SEND2: DO IW=1,NW
   ...
   NCPL = WALL(IW)%NCPL
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,5i3)') 'IW=',IW,': NCPL=',NCPL,': ICN:',(WALL(IW)%ICN(1:NCPL))
   DO ICPL = 1, NCPL
      IC = WALL(IW)%ICN(ICPL)
      IWW = IWW + 1
      SEND_BUF(LL+1) = REAL(IW,EB)
      SEND_BUF(LL+2) = CVECTOR(IC)
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'SUSI:IW=',IW,': IC=',IC,': SEND_BUF(',LL+2,')=',SEND_BUF(LL+2), WALL(IW)%NCPL
      LL = LL+2
   ENDDO
ENDDO PACK_SEND2
NLEN=2*IWW+1
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '================================= NM=', NM, ' NTYPE=',NTYPE
...
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,f20.12,i3)') 'SUSI:IW=',IW,': IC=',-1,': SEND_BUF(',LL,')=',SEND_BUF(LL)
   LL = LL+1

   IC = WALL(IW)%ICN(1)
   SEND_BUF(LL) = CVEC(IC)
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,f20.12,i3)') 'SUSI:IW=',IW,': IC=',IC,': SEND_BUF(',LL,')=',SEND_BUF(LL)
   LL = LL+1
...
SEND_BUF(NLEN) = -999.0_EB
IF (TYPE_DEBUG> NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,f20.12,i3)') 'SUSI:IW=',-1,': IC=',-1,': SEND_BUF(',NLEN,')=',SEND_BUF(NLEN)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '=================== CALLING UNPACK_CVECTOR_REAL'
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'UNPACK_CVECTOR: X(',IC,')=',CVECTOR(IC)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '=================== CALLING UNPACK_CVECTOR_REAL2'
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '==========================', NM, NOM, NL
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(i3,a,i3,a,f25.16,a,i3,a,f25.16,a,i3)') &
  IW,',UNPACK_OVECTOR: OVEC(',ICG,')=',OVEC(ICG),': VEC(',ICE,')=',VEC(ICE), ': LL=',LL
   LL = LL + 1
...
PACK_SEND: DO IW=1,NW
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(i3,a,i3,a,f12.6,i3)') NM,': PACK_CVECTOR : SEND_BUF(',LL+2,')=',SEND_BUF(LL+2), IC
      LL = LL+2
   ENDDO
ENDDO PACK_SEND
NLEN=2*IWW+1
...
PACK_SEND: DO IW=1,NW
   ...
   DO ICPL = 1, NCPL
      ...
      SEND_BUF(LL+1) = REAL(IW,EB)
      SEND_BUF(LL+2) = REAL(CVECTOR(IC),EB)
   IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(i3,a,i3,a,f12.6,i3)') NM,': PACK_CVECTOR2: SEND_BUF(',LL+2,')=',SEND_BUF(LL+2), IC
      LL = LL+2
   ENDDO
   IF (NL==NLEVEL_MIN.AND.TYPE_LAYER == NSCARC_LAYER_TWO) THEN
      DO ICPL = 1, NCPL
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '=================== CALLING UNPACK_CVECTOR_INT'
CVECTOR => POINT_TO_CVECTOR_INT (NTYPE, NM, NL)
...
UNPACK_RECV: DO
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) NM,': UNPACK_CVECTOR : CVECTOR(',IC,')=',CVECTOR(IC)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '=================== CALLING UNPACK_CVECTOR_INT2'
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) '==========================', NM, NOM, NL
...
UNPACK_RECV: DO
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) NM,': UNPACK2_OVECTOR: OVEC(',IC1,')=',OVEC(IC1)

   IC1 = SC%WALL(IW)%ICE(1)
   VEC(IC1) = NINT(ZSUM1/REAL(WALL(IW)%NCPL,EB))
   IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) NM,': UNPACK2_VECTOR: VEC(',IC1,')=',VEC(IC1)
   ...
   IF (NL==NLEVEL_MIN.AND.TYPE_LAYER == NSCARC_LAYER_TWO) THEN
      ...
      IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) NM,': UNPACK2_OVECTOR: OVEC(',IC2,')=',OVEC(IC2)

      IC2 = SC%WALL(IW)%ICE2(1)
      VEC(IC2) = NINT(ZSUM2/REAL(WALL(IW)%NCPL,EB))
      IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) NM,': UNPACK2_VECTOR: VEC(',IC2,')=',VEC(IC2)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'NTYPE=',NTYPE, NSCARC_MATRIX_SUBDIAG, NM, NL
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS)   WRITE(SCARC_LU,*) '========================PACK_CMATRIX================= NM=',NM,': NTYPE=',NTYPE,': NW=',NW
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i3,a,2i3,a,i3,a,f12.6)') &
       'PACK_CMATRIX: IW=',IW,': NCPL=',NCPL, WALL(IW)%ICN(1), ': SEND_BUF(',LL,')=', SEND_BUF(LL)
            LL = LL+1
         ENDDO
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,*) 'NLEN=',NLEN
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_LESS)  WRITE(SCARC_LU,'(a,i3,a,i3)') 'PACKING FOR NSCARC_STENCIL_SIZE NM=',NM,': NW=',NW
      ...
      IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i4,a,i4,a,i4,a,i4)') 'A: PACKING STENCIL: IW=', IW,': NCOL=',SC%A_ROW(IC+1)-SC%A_ROW(IC),': IC=',IC,': LL=',LL
      ...
      IF (TYPE_DEBUG > NSCARC_DEBUG_LESS) WRITE(SCARC_LU,'(a,i4,a,i4,a,i4,a,i4)') 'B: PACKING STENCIL: IW=', IW,': NCOL=',SC%A_ROW(IC+1)-SC%A_ROW(IC),': IC=',IC,': LL=',LL
   ...
   IF (TYPE_DEBUG > NSCARC_DEBUG_NONE)  WRITE(SCARC_LU,'(a)') 'PACKING FOR NSCARC_MATRIX_SYSTEM'
      ...
      IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) &
         WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,f12.6,a,i3,a,2f12.6)') 'NM=',NM,': IW=',IW,': IC=',IC,': PACK_SYSTEM: SENDING A(',ICOL,')=',SC%A(ICOL),': JC=',JC,' SENDING ', SEND_BUF(LL), SEND_BUF(LL+1)
               LL = LL + 2
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) THEN
   WRITE(SCARC_LU,*) 'PACKED VECTOR FROM NM=',NM,': NLEN=',NLEN
   WRITE(SCARC_LU,'(8f12.6)') (SEND_BUF(LL), LL=1,NLEN)
ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'FL =========== IW=',IW,': NOM=',WALL(IW)%NOM,': NCPL=',NCPL,': IC=',IC
         ...
         IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) THEN
            ...
            WRITE(SCARC_LU,'(2i3,a,i3,a,f12.6,2i3)') NM,iW,': PACK_WEIGHTS: SEND_BUF(',LL+1,')=',SEND_BUF(LL+1)
            WRITE(SCARC_LU,'(2i3,a,i3,a,f12.6,2i3)') NM,iW,': PACK_WEIGHTS: SEND_BUF(',LL+2,')=',SEND_BUF(LL+2)
         ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3)') 'SL =========== IW=',IW,': NOM=',WALL(IW)%NOM,': NCPL=',NCPL,': IC=',IC
               SEND_BUF(LL)   = REAL(SC%CELLTYPE(IC),EB)
               SEND_BUF(LL+1) = REAL(SC%P_ROW(IC+1)-SC%P_ROW(IC),EB)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,*) 'SEND_BUF(',NLEN,')=',SEND_BUF(NLEN)
      
END SUBROUTINE SCARC_PACK_CMATRIX
 


INTEGER FUNCTION SECOND_LINE_NBR1(IW, WALL, NX, NY)
INTEGER, INTENT(IN) :: IW, NX, NY
TYPE (SCARC_WALL_TYPE), DIMENSION (:), INTENT(IN)  :: WALL
INTEGER:: IC

IF (TYPE_DIMENSION == NSCARC_DIMENSION_TWO) THEN
   SELECT CASE(WALL(IW)%IOR)
      CASE ( 1)
         IC = (WALL(IW)%IZW-1)*NX + WALL(IW)%IXW+1
      CASE (-1)
         IC = (WALL(IW)%IZW-1)*NX + WALL(IW)%IXW-1
      CASE ( 3)
         IC = (WALL(IW)%IZW  )*NX + WALL(IW)%IXW
      CASE (-3)
         IC = (WALL(IW)%IZW-2)*NX + WALL(IW)%IXW
   END SELECT
ELSE
   SELECT CASE(WALL(IW)%IOR)
      CASE ( 1)
         IC = (WALL(IW)%IZW-1)*NX*NY + (WALL(IW)%IYW-1)*NX + WALL(IW)%IXW+1
      CASE (-1)
         IC = (WALL(IW)%IZW-1)*NX*NY + (WALL(IW)%IYW-1)*NX + WALL(IW)%IXW-1
      CASE ( 2)
         IC = (WALL(IW)%IZW-1)*NX*NY + (WALL(IW)%IYW  )*NX + WALL(IW)%IXW
      CASE (-2)
         IC = (WALL(IW)%IZW-1)*NX*NY + (WALL(IW)%IYW-2)*NX + WALL(IW)%IXW
      CASE ( 3)
         IC = (WALL(IW)%IZW  )*NX*NY + (WALL(IW)%IYW-1)*NX + WALL(IW)%IXW
      CASE (-3)
         IC = (WALL(IW)%IZW-2)*NX*NY + (WALL(IW)%IYW-1)*NX + WALL(IW)%IXW
   END SELECT
ENDIF
...
INTEGER FUNCTION SECOND_LINE_NBR2(IW, WALL, NX, NY)
INTEGER, INTENT(IN) :: IW, NX, NY
TYPE (SCARC_WALL_TYPE), DIMENSION (:), INTENT(IN)  :: WALL
INTEGER:: IC
...
IF (TYPE_DIMENSION == NSCARC_DIMENSION_TWO) THEN
   SELECT CASE(WALL(IW)%IOR)
      CASE ( 1)
         IC = (WALL(IW)%IZN1-1)*NX + WALL(IW)%IXN1-1
      CASE (-1)
         IC = (WALL(IW)%IZN1-1)*NX + WALL(IW)%IXN1+1
      CASE ( 3)
         IC = (WALL(IW)%IZN1-2)*NX + WALL(IW)%IXN1
      CASE (-3)
         IC = (WALL(IW)%IZN1  )*NX + WALL(IW)%IXN1
   END SELECT
ELSE
   SELECT CASE(WALL(IW)%IOR)
      CASE ( 1)
         IC = (WALL(IW)%IZN1-1)*NX*NY + (WALL(IW)%IYN1-1)*NX + WALL(IW)%IXN1-1
      CASE (-1)
         IC = (WALL(IW)%IZN1-1)*NX*NY + (WALL(IW)%IYN1-1)*NX + WALL(IW)%IXN1+1
      CASE ( 2)
         IC = (WALL(IW)%IZN1-1)*NX*NY + (WALL(IW)%IYN1-2)*NX + WALL(IW)%IXN1
      CASE (-2)
         IC = (WALL(IW)%IZN1-1)*NX*NY + (WALL(IW)%IYN1  )*NX + WALL(IW)%IXN1
      CASE ( 3)
         IC = (WALL(IW)%IZN1-2)*NX*NY + (WALL(IW)%IYN1-1)*NX + WALL(IW)%IXN1
      CASE (-3)
         IC = (WALL(IW)%IZN1  )*NX*NY + (WALL(IW)%IYN1-1)*NX + WALL(IW)%IXN1
   END SELECT
ENDIF
...
INTEGER  , INTENT(IN)    :: NTYPE, NM, NOM, NL
REAL (EB):: ZSUM
INTEGER IW, LL, I, IC, JC, JC0, IROW, ICOL, NCOL, NROW, NCPL, ICELLTYPE, ICE, ICG, ICN, ICC
TYPE (SCARC_COMPACT_TYPE), POINTER:: SC
TYPE (OSCARC_COMPACT_TYPE), POINTER:: OSC
...
IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
   ...
   WRITE(SCARC_LU,*) 'STARTING SCARC_UNPACK_CMATRIX: RECV_BUF='
   WRITE(SCARC_LU,'(8f12.6)') (RECV_BUF(I), I=1, 90)
ENDIF
...
SELECT CASE (NTYPE)
      ...
      UNPACK_RECV_SUBDIAG: DO
         ...
         DO ICOL = SC%A_ROW(IC)+1, SC%A_ROW(IC+1)-1
            IF (IW == -SC%A_COL(ICOL)) THEN
               ...
               !SC%WALL(IW)%ICW = IC
   IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
      WRITE(SCARC_LU,'(a,i3,a,i3,a,i6)') '==================UNPACK_CMATRIX FOR NM=',NM,': NL=',NL,':  IC=',IC
      WRITE(SCARC_LU,*) 'SC%A(',ICOL,')=',SC%A(ICOL)
      WRITE(SCARC_LU,*) 'SC%A_ICOL(',ICOL,')=',SC%A_COL(ICOL)
      WRITE(SCARC_LU,*) 'SC%WALL(',IW,')%ICW=',SC%WALL(IW)%ICW
   ENDIF
            ENDIF
         ENDDO
   ...
   CASE (NSCARC_MATRIX_STENCIL)
      ...
      IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
        write(scarc_lu,'(a,5i4)') '===================================== NM=',NM, SC%NC, NL
        WRITE(SCARC_LU,*) 'UNPACK_MATRIX_STENCIL: '
        WRITE(SCARC_LU,'(5f12.6)') RECV_BUF(1:30)
      ENDIF
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS)  WRITE(SCARC_LU,*) 'OSC%A_SIZE(',ICG,')=',OSC%A_SIZE(ICG), LL
         LL = LL + 2
...
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS)  WRITE(SCARC_LU,*) 'OSC%A_SIZE(',ICG,')=',OSC%A_SIZE(ICG), LL
   ...
   CASE (NSCARC_MATRIX_SYSTEM)
      ...
      IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
        write(scarc_lu,'(a,5i4)') '===================================== NM=',NM, SC%NC, NL
        WRITE(SCARC_LU,*) 'UNPACK_MATRIX_SYSTEM: '
      ENDIF
      ...
      UNPACK_RECV_SYSTEM: DO
         ...
         IW = NINT(RECV_BUF(LL))
IF (TYPE_DEBUG > NSCARC_DEBUG_LESS)  WRITE(SCARC_LU,'(a,i3,a,i3,a,f12.6)') '1: ============ IW=',IW, ' READING RECV_BUF(',LL,')=',RECV_BUF(LL)
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE)  WRITE(SCARC_LU,'(i3,a,i6,a,f12.6,a,i6,a,5i5,f12.6)') &
   NM,'A : UNPACK_MATRIX: A(',ICOL,')=',SC%A(ICOL), ' A_COL(',ICOL,')=', SC%A_COL(ICOL),ICOL, ICE, LL+1, JC, RECV_BUF(LL)
            LL = LL + 2
         ENDDO 
         ...
         IF (NL==NLEVEL_MIN.AND.TYPE_LAYER == NSCARC_LAYER_TWO) THEN
            ...
            DO ICOL = SC%A_ROW(ICE), SC%A_ROW(ICE+1)-1
               ...
               SC%A(ICOL)    = RECV_BUF(LL+1)
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE)  WRITE(SCARC_LU,'(i3,a,i6,a,f12.6,a,i6,a,5i5,f12.6)') &
   NM,'B : UNPACK_MATRIX: A(',ICOL,')=',SC%A(ICOL), ' A_COL(',ICOL,')=', SC%A_COL(ICOL),ICOL, ICE, LL+1, JC, RECV_BUF(LL)
               LL = LL + 2
            ENDDO 
          ENDIF
   ...
   CASE (NSCARC_MATRIX_PROLONGATION)
      ...
      IF (TYPE_DEBUG >= NSCARC_DEBUG_LESS) THEN
        write(scarc_lu,'(a,5i4)') '===================================== NM=',NM, SC%NC, NL, NOM
        WRITE(SCARC_LU,*) 'UNPACK_MATRIX_PROLONGATION: '
      ENDIF
      ...
      UNPACK_RECV_PROL: DO
         ...
         IW = NINT(RECV_BUF(LL))
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i4,a,i4)') 'FL: =============== IW=',IW,': LL=',LL
         LL = LL + 1
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) WRITE(SCARC_LU,'(a,i3,a,5i3)') 'NROW=',NROW, ' OSC%NC=',OSC%NC, ICG, ICE, ICN, IW
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE)  WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,2i3,a,i3,a,i3)') 'FL ==== IW=',IW,':NM=',NM,': NCOL=',NCOL,': NROW=',NROW, ': ICG=',ICG , LL, ': OSC%CELLTYPE(',ICG,')=',OSC%CELLTYPE(ICG)
         LL = LL + 2
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) &
   WRITE(SCARC_LU,'(i3,a,i3,a,f12.6,a,i3,a,3i5)') NM,': UNPACK_WEIGHTS: P(',ICOL,')=',OSC%P(ICOL), ' P_COL(',ICOL,')=',&
                                           OSC%P_COL(ICOL),ICOL, ICG
            LL = LL + 2
         ENDDO 
         NROW = NROW + NCOL
         OSC%P_ROW(ICG+1) = NROW
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) &
   WRITE(SCARC_LU,'(a,i3,a,3i3)') ' P_ROW(',ICG+1,')=',OSC%P_ROW(ICG+1), NROW, NCOL
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE)  WRITE(SCARC_LU,'(a,i3,a,i3,a,i3,a,i3,a,2i3,a,i3,a,i3)') 'SL ==== IW=',IW,':NM=',NM,': NCOL=',NCOL,': NROW=',NROW, ': ICG=',ICG , LL, ': OSC%CELLTYPE(',ICG,')=',OSC%CELLTYPE(ICG)
            LL = LL + 2
...
IF (TYPE_DEBUG > NSCARC_DEBUG_NONE) &
   WRITE(SCARC_LU,*) '---------------------------------> AFTER SECOND LAYER'
      ...
      DO NM = NMESHES_MIN, NMESHES_MAX
         ...
         IF (NL == NLEVEL_MIN) THEN
               ...
               WRITE(SCARC_LU,*)  '----------------------------',&
                                  '-------------------------------'
IF (NMESHES == 1) THEN
WRITE(SCARC_LU,'(2f25.16)') VC(4),VC(5)
ELSE IF (NM==1) THEN
WRITE(SCARC_LU,'(2f25.16)') VC(4),VC(5)
               WRITE(SCARC_LU,*)  '----------------------------',&
                                  '-------------------------------'
