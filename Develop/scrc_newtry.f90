...
INTEGER, PARAMETER :: NSCARC_AGGREGATE_INITIAL       =  1, &    ! basic initial setting
                      NSCARC_AGGREGATE_FILLUP        =  2, &    ! only filling up
                      NSCARC_AGGREGATE_COMPLETE      =  3       ! complete setting 
...
INTEGER, PARAMETER :: NSCARC_COARSENING_A1           =  1, &    ! aggressive 1 (path =1, length =2)
                      NSCARC_COARSENING_A2           =  2, &    ! aggressive 2 (path =2, length =2)
                      NSCARC_COARSENING_BASIC        =  3, &    ! basic coarsening
                      NSCARC_COARSENING_BDRY         =  4, &    ! boundary oriented coarsening
                      NSCARC_COARSENING_FALGOUT      =  5, &    ! parallel Falgout
                      NSCARC_COARSENING_PMIS         =  6, &    ! PMIS coarsening
                      NSCARC_COARSENING_PMISG        =  7, &    ! PMIS-variant coarsening
                      NSCARC_COARSENING_RS3          =  8       ! parallel RS3
...
INTEGER, PARAMETER :: NSCARC_DEBUG_BDRY              =  1, &    ! show pressure_bc_index
                      ...
                      NSCARC_DEBUG_MATRIX            =  5, &    ! show matrix
...
INTEGER, PARAMETER :: NSCARC_EXCHANGE_AGGREGATES     =  1, &    ! exchange aggregates - preprocessing
                      NSCARC_EXCHANGE_CELL_NUMBER    =  2, &    ! neighboring cell numbers on overlap
                      NSCARC_EXCHANGE_CELL_TYPE      =  3, &    ! cell type for coarsening process
                      NSCARC_EXCHANGE_CELL_WIDTH     =  4, &    ! neighboring grid widths on overlap
                      NSCARC_EXCHANGE_MATRIX_COLS    =  5, &    ! columns of neighboring matrix on overlap
                      NSCARC_EXCHANGE_MATRIX_DIAG    =  6, &    ! size of neighboring matrix 
                      NSCARC_EXCHANGE_MATRIX_SIZE    =  7, &    ! size of neighboring matrix 
                      NSCARC_EXCHANGE_MATRIX_VALS    =  8, &    ! values of neighboring matrix on overlap
                      NSCARC_EXCHANGE_NULLSPACE      =  9, &    ! exchange sum of nullspace entries
                      NSCARC_EXCHANGE_PRESSURE       = 10, &    ! vector values along internal boundaries
                      NSCARC_EXCHANGE_SETUP          = 11, &    ! initialize wall information
                      NSCARC_EXCHANGE_TRANSFER       = 12, &    ! AMG information for transfer matrix
                      NSCARC_EXCHANGE_VECTOR         = 13, &    ! exchange vector for mat-vec multiplication
                      NSCARC_EXCHANGE_VECTOR_MEAN    = 14, &    ! exchange vector for mat-vec multiplication and build sum
                      NSCARC_EXCHANGE_WEIGHTS        = 15       ! weights for prolongation
...
INTEGER, PARAMETER :: NSCARC_MATRIX_BANDWISE         =  1, &    ! matrix in bandwise storage 
                      ...
                      NSCARC_MATRIX_POISSON          =  6, &    ! Poisson matrix in compact storage (default)
                      NSCARC_MATRIX_POISSON_SYM      =  7, &    ! symmetric Poisson matrix in compact storage (default)
                      NSCARC_MATRIX_PROLONGATION     =  8, &    ! Prolongation matrix (SAMG only)
                      NSCARC_MATRIX_RESTRICTION      =  9, &    ! Restriction matrix (SAMG only)
                      NSCARC_MATRIX_SAMG             = 10       ! matrix for algebraic multigrid (SAMG only)
...
INTEGER, PARAMETER :: NSCARC_ORDER_AGGREGATING       =  1, &    ! be next in line for aggregation
                      NSCARC_ORDER_LOCKED            =  2, &    ! locked for aggregation
                      NSCARC_ORDER_UNDEFINED         =  3       ! still not defined for aggregation
...
INTEGER :: TYPE_AGGREGATE          = NSCARC_AGGREGATE_INITIAL    ! default aggregation type for exchange
...
TYPE SCARC_MATRIX_COMPACT_TYPE
   ...
   INTEGER,  ALLOCATABLE, DIMENSION (:) :: COL_MKL            ! column pointer for MKL numbering
...
TYPE SCARC_GRID_TYPE
   ...
   TYPE (SCARC_MATRIX_COMPACT_TYPE) :: POISSON             ! Poisson matrix in compact storage technique (default)
   TYPE (SCARC_MATRIX_BANDWISE_TYPE):: POISSON_BW          ! Poisson matrix in bandwise storage technique
#ifdef WITH_MKL
   TYPE (SCARC_MATRIX_COMPACT_TYPE) :: POISSON_SYM         ! symmetric part of compact Poisson matrix (only for MKL)
#endif
   ...
   TYPE (SCARC_MATRIX_COMPACT_TYPE) :: SAMG                ! Matrix for Smoothed Aggregation Multigrid
   ...
   REAL(EB), ALLOCATABLE, DIMENSION (:) :: ADIAG_INVERSE   ! vector of inverse diagonal entries and temporary vector
   REAL(EB), ALLOCATABLE, DIMENSION (:) :: DIAG            ! matrix diagonal, possible inverted depending on method
   ...
   INTEGER,  ALLOCATABLE, DIMENSION (:) :: AGG_LOCAL       ! Mapping to local aggregate numbers (AMG only)
   ...
   INTEGER,  ALLOCATABLE, DIMENSION (:) :: LAGG_TO_GAGG    ! Mapping from local to global aggregate numbers (AMG only)

   ! Pointer arrays for data exchange with neighbors
   INTEGER, ALLOCATABLE, DIMENSION (:)   :: IC_TO_ICGLOB    ! mapping from local IC to global IC 
   ...
   INTEGER, ALLOCATABLE, DIMENSION (:)   :: ICG_TO_GAGG        ! mapping from ICG to global aggregate
   INTEGER, ALLOCATABLE, DIMENSION (:)   :: ICG_TO_LAGG        ! mapping from ICG to local aggregate
   ...
   INTEGER :: N_AGGREGATES = NSCARC_ONE_INT                    ! Number of aggregates (SAMG only)
...
TYPE SCARC_OSCARC_TYPE
   ...
   INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT2, RECV_INT2         ! Integer exchange buffers (double length)
   INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT7, RECV_INT7         ! Integer exchange buffers (sevenfold length)
...
TYPE (SCARC_MATRIX_COMPACT_TYPE),  POINTER :: A =>NULL(), OA =>NULL()
TYPE (SCARC_MATRIX_COMPACT_TYPE),  POINTER :: AC=>NULL(), OAC=>NULL()
TYPE (SCARC_MATRIX_COMPACT_TYPE),  POINTER :: AF=>NULL(), OAF=>NULL()
...
TYPE (SCARC_MULTIGRID_TYPE), POINTER :: MG =>NULL(), MGF =>NULL(), MGC =>NULL()
TYPE (SCARC_MULTIGRID_TYPE), POINTER :: OMG=>NULL(), OMGF=>NULL(), OMGC=>NULL()

TYPE (SCARC_MATRIX_COMPACT_TYPE), POINTER :: PROL=>NULL(), OPROL=>NULL()
TYPE (SCARC_MATRIX_COMPACT_TYPE), POINTER :: REST=>NULL(), OREST=>NULL()
TYPE (SCARC_MATRIX_COMPACT_TYPE), POINTER :: CONN=>NULL(), OCONN=>NULL()
TYPE (SCARC_MATRIX_COMPACT_TYPE), POINTER :: SAMG=>NULL(), OSAMG=>NULL()
...
INTEGER,  POINTER, DIMENSION(:) ::  RECV_INT0, RECV_INT1, RECV_INT2, RECV_INT7 
...
!
! Setup information for data exchanges, matrix systems, aggregates, used methods and vectors
!
CALL SCARC_SETUP_EXCHANGES                  ; IF (STOP_STATUS==SETUP_STOP) RETURN
CALL SCARC_SETUP_SYSTEMS                    ; IF (STOP_STATUS==SETUP_STOP) RETURN

IF (IS_AMG) CALL SCARC_SETUP_SAMG          ; IF (STOP_STATUS==SETUP_STOP) RETURN
...
MESHES_LOOP1: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_ALLOCATE_LOG3(LC%IS_SOLID, 0, LC%NX+1, 0, LC%NY+1, 0, LC%NZ+1, NSCARC_INIT_FALSE, 'IS_SOLID')
...
WRITE(MSG%LU_DEBUG,*) '1:F%NOP=',F%NOP, NL
WRITE(MSG%LU_DEBUG,*) '1:F%DH=',F%DH, NL
...
WRITE(MSG%LU_DEBUG,*) '3:F%NOP=',F%NOP, NL
WRITE(MSG%LU_DEBUG,*) '3:F%DH=',F%DH, NL
         F%NX  = L%NX
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM, NOM, IWG, IOR0, NCG:', NM, NOM, IWG, IOR0, OG%NCG
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'ALLOCATING ICE_TO_ICN IN LENGTH ', G%NC+1, G%NCE
#endif
   ENDIF
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'ALLOCATING ICE_TO_ICN ON COARSE LEVEL IN LENGTH ', GC%NC+1, GC%NCE
#endif
         ENDIF
...
ENDIF
CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_SETUP, NSCARC_NONE, NLEVEL_MIN)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'ALLOCATING SEND_INT1 in length ', SIZE(OS%SEND_INT1), NM, NOM
WRITE(MSG%LU_DEBUG,*) 'ALLOCATING RECV_INT1 in length ', SIZE(OS%RECV_INT1), NM, NOM
#endif
      NLEN = 2 * OG%NCG 
      CALL SCARC_ALLOCATE_INT1 (OS%SEND_INT2 , 1, NLEN, NSCARC_INIT_ZERO, 'SEND_INT2')
      CALL SCARC_ALLOCATE_INT1 (OS%RECV_INT2 , 1, NLEN, NSCARC_INIT_ZERO, 'RECV_INT2')
      ...
      NLEN = 2 * NSCARC_MAX_STENCIL * OG%NCG
      CALL SCARC_ALLOCATE_INT1 (OS%SEND_INT7 , 1, NLEN, NSCARC_INIT_ZERO, 'SEND_INT7')
      CALL SCARC_ALLOCATE_INT1 (OS%RECV_INT7 , 1, NLEN, NSCARC_INIT_ZERO, 'RECV_INT7')
...
IF (NMESHES > 1) THEN
   DO NL = NLEVEL_MIN, NLEVEL_MAX
      CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_CELL_NUMBER, NSCARC_NONE, NL)
      CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_CELL_WIDTH,  NSCARC_NONE, NL)
   ENDDO
ENDIF
...
!
! If there is more than mesh, exchange matrix values in overlapping parts
! This must be done for all multilevel methods at least at the finest grid level, furthermore
! also at all higher levels except for the AMG method (in this case it will be done later)
!
IF (NMESHES > 1) THEN
...
CALL SCARC_DEBUG_QUANTITY (NSCARC_DEBUG_MATRIX, NLEVEL_MIN, 'SYSTEM-MATRIX')
   ...
   CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_MATRIX_COLS, NSCARC_MATRIX_POISSON, NLEVEL_MIN)
   CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_MATRIX_VALS, NSCARC_MATRIX_POISSON, NLEVEL_MIN)

   IF (TYPE_MATRIX == NSCARC_MATRIX_COMPACT) &
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_ALLOCATE_INT1 (G%IC_TO_ICGLOB, 1, G%NCE, NSCARC_INIT_ZERO, 'IC_TO_ICGLOB')
   DO IC = 1, G%NC
      G%IC_TO_ICGLOB(IC) = IC + G%NC_OFFSET(NM)
   ENDDO
   ...
   DO IW = 1, G%NW
      ...
      G%IC_TO_ICGLOB(ICE) = ICN + G%NC_OFFSET(NOM)
   ENDDO

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_GLOBAL_NUMBERING: IC_TO_ICGLOB'
WRITE(MSG%LU_DEBUG,'(8I6)') G%IC_TO_ICGLOB
#endif
ENDDO
...
! Extract overlapping matrix part and add to main matrix
! -------------------------------------------------------------------------------------------
SUBROUTINE SCARC_EXTRACT_MATRIX_OVERLAPS (NTYPE, NL)
USE SCARC_POINTERS, ONLY : L, G, F, OL, OG, A
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   SELECT CASE (TYPE_GRID)
      ...
      CASE (NSCARC_GRID_STRUCTURED)
         ...
         A => SCARC_POINT_TO_MATRIX_COMPACT(NTYPE)
 
         IP = A%ROW(G%NC+1)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'EXTRACT_MATRIX_OVERLAPS: NM=', NM,': IP=',IP, G%POISSON%N_ROW
#endif
         DO IFACE = 1, 6                                         ! check if this face has connection to last cell
            ...
            DO INBR = 1, F%N_NEIGHBORS
               ...
               CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
               OA => SCARC_POINT_TO_OTHER_MATRIX_COMPACT(NSCARC_MATRIX_POISSON)
   
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5I6)') 'IFACE, IOR0, INBR, NOM:', IFACE, IOR0, INBR, NOM, G%POISSON%N_ROW
WRITE(MSG%LU_DEBUG,*) 'G%POISSON%ROW:'
WRITE(MSG%LU_DEBUG,'( 8I6)') G%POISSON%ROW(1:G%POISSON%N_ROW)
#endif
               ICOL = 1
               DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
                  ...
                  DO ICOL = OA%ROW(ICG), OA%ROW(ICG+1)-1
                     A%COL(IP) = ABS(OA%COL(ICOL))            ! TODO CAUTION COL_MKL or COL?
                     A%VAL(IP) = OA%VAL(ICOL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,6I6,E12.4)') '       ----> B: ICG, ICE, ICOL, IP, ROW, COL, VAL :', &
                                       ICG, ICE, ICOL, IP, A%ROW(ICE), A%COL(IP), A%VAL(IP)
#endif
                     IP = IP + 1
                  ENDDO
      ...
      ! ---------- Version for structured grids
      CASE (NSCARC_GRID_UNSTRUCTURED)
         WRITE(*,*) 'SCARC-ERROR: No bandwise version of EXTRACT_MATRIX_OVERLAPS available yet'
...
SELECT_STORAGE_TYPE: SELECT CASE (SCARC_MATRIX_LEVEL(NL))
   ...
   CASE (NSCARC_MATRIX_COMPACT)
      ...
      CALL SCARC_POINT_TO_GRID(NM, NL)
      A => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_POISSON)
      CALL SCARC_ALLOCATE_MATRIX_COMPACT (A, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'G%POISSON')
      ...
      DO INBR = 1, SCARC(NM)%N_NEIGHBORS
         ...
         CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
         OA => SCARC_POINT_TO_OTHER_MATRIX_COMPACT (NSCARC_MATRIX_POISSON)
         CALL SCARC_ALLOCATE_MATRIX_COMPACT(OA, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'OG%POISSON')
      ENDDO
      ...
      DO IZ = 1, L%NZ
         DO IY = 1, L%NY
            DO IX = 1, L%NX
               ...
               CALL SCARC_SETUP_MATRIX_MAINDIAG_COMPACT (IC, IX, IY, IZ, IP)
   
               ! Lower subdiagonals
               IF (VALID_SUBDIAG(IX, IY, IZ,  3)) &
                  CALL SCARC_SETUP_MATRIX_SUBDIAG_COMPACT(IC, IX, IY, IZ, IX, IY, IZ-1, IP,  3)
               IF (VALID_SUBDIAG(IX, IY, IZ,  2)) &
                  CALL SCARC_SETUP_MATRIX_SUBDIAG_COMPACT(IC, IX, IY, IZ, IX, IY-1, IZ, IP,  2)
               IF (VALID_SUBDIAG(IX, IY, IZ,  1)) &
                  CALL SCARC_SETUP_MATRIX_SUBDIAG_COMPACT(IC, IX, IY, IZ, IX-1, IY, IZ, IP,  1)
      ...
      G%POISSON%ROW(G%NC+1) = IP
      G%POISSON%N_VAL       = IP-1                         ! set correct number of matrix entries
   
      CALL SCARC_RESIZE_MATRIX_COMPACT(G%POISSON, G%POISSON%N_VAL, G%POISSON%N_ROW, 'AC')
   ...
   CASE (NSCARC_MATRIX_BANDWISE)
      ...
      ! Allocate main matrix on non-overlapping part of mesh
      CALL SCARC_POINT_TO_GRID(NM, NL)
      AB => SCARC_POINT_TO_MATRIX_BANDWISE (NSCARC_MATRIX_POISSON)
      CALL SCARC_ALLOCATE_MATRIX_BANDWISE(AB, NL, NSCARC_INIT_ZERO, 'G%POISSON_BW')
   
      ! For every neighbor allocate little matrix on overlapping part of mesh
      DO INBR = 1, SCARC(NM)%N_NEIGHBORS
         NOM = S%NEIGHBORS(INBR)
         CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
         OAB => SCARC_POINT_TO_MATRIX_BANDWISE (NSCARC_MATRIX_POISSON)
         CALL SCARC_ALLOCATE_MATRIX_BANDWISE(OAB, NL, NSCARC_INIT_ZERO, 'OG%POISSON_BW')
      ENDDO
      ...
      DO IZ = 1, L%NZ
         DO IY = 1, L%NY
            DO IX = 1, L%NX
               ...
               IF (VALID_SUBDIAG(IX, IY, IZ, 3)) &
                  CALL SCARC_SETUP_MATRIX_SUBDIAG_BANDWISE(IC, IX, IY, IZ, IX  , IY  , IZ-1, 3)
               IF (VALID_SUBDIAG(IX, IY, IZ, 2)) &
                  CALL SCARC_SETUP_MATRIX_SUBDIAG_BANDWISE(IC, IX, IY, IZ, IX  , IY-1, IZ  , 2)
               IF (VALID_SUBDIAG(IX, IY, IZ, 1)) &
                  CALL SCARC_SETUP_MATRIX_SUBDIAG_BANDWISE(IC, IX, IY, IZ, IX-1, IY  , IZ  , 1)
   
               ! Main diagonal entry
               CALL SCARC_SETUP_MATRIX_MAINDIAG_BANDWISE (IC, IX, IY, IZ)
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_MATRIX_MAINDIAG_COMPACT (IC, IX, IY, IZ, IP)
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_MATRIX_SUBDIAG_COMPACT (IC, IX1, IY1, IZ1, IX2, IY2, IZ2, IP, IOR0)
...
! If two meshes with different step sizes meet, we get a weighted stencil along internal wall cells
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_MATRIX_MAINDIAG_BANDWISE (IC, IX, IY, IZ)
...
END SUBROUTINE SCARC_SETUP_MATRIX_MAINDIAG_BANDWISE


! ------------------------------------------------------------------------------------------------
! Compute subdiagonal contribution in direction IOR0 in case of a COMPACT matrix
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_MATRIX_SUBDIAG_BANDWISE (IC, IX1, IY1, IZ1, IX2, IY2, IZ2, IOR0)
...
END SUBROUTINE SCARC_SETUP_MATRIX_SUBDIAG_BANDWISE
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_MATRIX_MKL_DOUBLE:', NL, G%NC
WRITE(MSG%LU_DEBUG,*) 'A%ROW:', A%N_ROW, A%N_VAL
WRITE(MSG%LU_DEBUG,'(8I8)') A%ROW
WRITE(MSG%LU_DEBUG,*) 'A%COL:'
DO IC = 1, A%N_ROW-1
   WRITE(MSG%LU_DEBUG,'(16I8)') (A%COL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
ENDDO
WRITE(MSG%LU_DEBUG,*) 'A%COL_MKL:'
DO IC = 1, A%N_ROW-1
   WRITE(MSG%LU_DEBUG,'(16I8)') (A%COL_MKL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
ENDDO
WRITE(MSG%LU_DEBUG,*) 'A%VAL:'
DO IC = 1, A%N_ROW-1
   WRITE(MSG%LU_DEBUG,'(16E12.4)') (A%VAL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
ENDDO
#endif
! 
   ...
   DO IC = 1, G%NC
      DO ICOL = A%ROW(IC), A%ROW(IC+1)-1
         ...
         ELSE IF (TYPE_MKL(NL) == NSCARC_MKL_GLOBAL) THEN
            JC = A%COL_MKL(ICOL)
            IF (JC >= IC + G%NC_OFFSET(NM)) AS%N_VAL = AS%N_VAL+1
         ELSE
...
IF (IS_MKL_LEVEL(NL)) THEN
   !CALL SCARC_ALLOCATE_INT1(KCOL_AUX, 1, A%N_STENCIL, NSCARC_INIT_NONE, 'KCOL_AUX')
   !CALL SCARC_ALLOCATE_INT1(KC_AUX  , 1, A%N_STENCIL, NSCARC_INIT_NONE, 'KC_AUX')
   !CALL SCARC_ALLOCATE_INT1(KCOL_AUX, 1, 50, NSCARC_INIT_NONE, 'KCOL_AUX')
   !CALL SCARC_ALLOCATE_INT1(KC_AUX  , 1, 50, NSCARC_INIT_NONE, 'KC_AUX')
   CALL SCARC_ALLOCATE_INT1(KCOL_AUX, 1, 50, NSCARC_HUGE_INT, 'KCOL_AUX')
   CALL SCARC_ALLOCATE_INT1(KC_AUX  , 1, 50, NSCARC_HUGE_INT, 'KC_AUX')
ENDIF
...
DO IC = 1, AS%N_ROW - 1
   ...
   ELSE IF (TYPE_MKL(NL) == NSCARC_MKL_GLOBAL) THEN
      ...
      ISYM = 1
      JC0 = A%COL_MKL(A%ROW(IC))
      DO ICOL = A%ROW(IC), A%ROW(IC+1)-1
         !JC = A%COL(ICOL)
         JC = A%COL_MKL(ICOL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'IC, ICOL, JC: ', IC, ICOL, JC
#endif
         IF (SCARC_MKL_MTYPE == 'SYMMETRIC') THEN
            IF (JC >= JC0) THEN
               KCOL_AUX(ISYM) = ICOL
...
WRITE(MSG%LU_DEBUG,*) 'KCOL_AUX:', KCOL_AUX(1:10)
WRITE(MSG%LU_DEBUG,*) 'KC_AUX  :', KC_AUX(1:10)
#endif
      ...
      SORT_LOOP: DO WHILE (JSYM <= NSYM)
         DO ISYM = 1, NSYM
            JC = KC_AUX(ISYM)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '---------> IC, JSYM, ISYM, JC:', IC, JSYM, ISYM, JC
#endif
            IF (JC == NSCARC_HUGE_INT) CYCLE
            IF (JC <= MINVAL(ABS(KC_AUX))) THEN
               ICOL = KCOL_AUX(ISYM)
               AS%VAL(IAS) = A%VAL(ICOL)
               !AS%COL(IAS) = A%COL(ICOL)
               AS%COL(IAS) = A%COL_MKL(ICOL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '             ICOL, IAS, VAL, COL:', IC, ISYM, JC, ICOL, IAS, AS%VAL(IAS), AS%COL(IAS)
#endif
...
IF (IS_MKL_LEVEL(NL)) THEN
   ...
   !DEALLOCATE (A%COL_MKL, STAT=IERROR)
ENDIF

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_MATRIX_MKL_DOUBLE:', NL, G%NC
WRITE(MSG%LU_DEBUG,*) 'AS%ROW:', AS%N_ROW, AS%N_VAL
WRITE(MSG%LU_DEBUG,*) AS%ROW
WRITE(MSG%LU_DEBUG,*) 'AS%COL:'
DO IC = 1, AS%N_ROW-1
   WRITE(MSG%LU_DEBUG,'(16I8)') (AS%COL(ICOL), ICOL=AS%ROW(IC), AS%ROW(IC+1)-1)
ENDDO
WRITE(MSG%LU_DEBUG,*) 'AS%VAL:'
DO IC = 1, AS%N_ROW-1
   WRITE(MSG%LU_DEBUG,'(16E12.4)') (AS%VAL(ICOL), ICOL=AS%ROW(IC), AS%ROW(IC+1)-1)
ENDDO
#endif
...
SELECT CASE (SCARC_MATRIX_LEVEL(NL))
   ...
   CASE (NSCARC_MATRIX_BANDWISE)
      ...
      AB => G%POISSON_BW
      IF (IS_PURE_NEUMANN) CALL SCARC_SETUP_MATRIX_BANDWISE_CONDENSED(NM)
...
! Define switch entries for toggle between original and condensed values
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_MATRIX_BANDWISE_CONDENSED (NM)
...
END SUBROUTINE SCARC_SETUP_MATRIX_BANDWISE_CONDENSED
...
WRITE(MSG%LU_DEBUG,*) 'AFTER SETUP_CLUSTER'
      ...
      ELSE
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_COARSE_SOLVER: PARDISO'
#endif
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE(TYPE_MATRIX)
         CASE (NSCARC_MATRIX_COMPACT)
            A => G%POISSON
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, G%NC, NSCARC_INIT_ZERO, 'MJAC')
            DO IC = 1, G%NC
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE (SCARC_MATRIX_LEVEL(NL))
         ...
         CASE (NSCARC_MATRIX_COMPACT)
            ...
            A => G%POISSON
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, A%N_VAL, NSCARC_INIT_ZERO, 'MGS')
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE(TYPE_MATRIX)
         ...
         CASE (NSCARC_MATRIX_COMPACT)
            ...
            A => G%POISSON
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, A%N_VAL, NSCARC_INIT_ZERO, 'MSGS')
            A%RELAX = A%VAL
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE(TYPE_MATRIX)
         ...
         CASE (NSCARC_MATRIX_COMPACT)
            ...
            A => G%POISSON
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, A%N_VAL, NSCARC_INIT_ZERO, 'MSOR')
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE(TYPE_MATRIX)
         ...
         CASE (NSCARC_MATRIX_COMPACT)
            ...
            A => G%POISSON
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, A%N_VAL, NSCARC_INIT_ZERO, 'MSSOR')
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE(TYPE_MATRIX)
         ...
         CASE (NSCARC_MATRIX_COMPACT)
            ...
            A => G%POISSON
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, A%N_VAL, NSCARC_INIT_ZERO, 'ILU')
            A%RELAX = A%VAL
...
SELECT_MATRIX_SIZE: SELECT CASE (NTYPE)
   ...
   CASE (NSCARC_SIZE_MATRIX)
         ...
         SELECT_MATRIX_TYPE: SELECT CASE (SCARC_MATRIX_LEVEL(NL))
            ...
            CASE (NSCARC_MATRIX_BANDWISE)
               ...
               DO INBR = 1, SCARC(NM)%N_NEIGHBORS
                  NOM = S%NEIGHBORS(INBR)
                  CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
                  OAB => SCARC_POINT_TO_OTHER_MATRIX_BANDWISE(NSCARC_MATRIX_POISSON)
               ...
               !   IF (NOM /= 0) THEN
               !      CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
               !      OAB => CALL SCARC_POINT_TO_OTHER_MATRIX_BANDWISE(NSCARC_MATRIX_POISSON)
               !      OAB%N_VAL     = OG%POISSON_BW%N_VAL + AB%N_STENCIL
               !      OAB%N_DIAG    = OG%POISSON_BW%N_DIAG + 1
               !      OAB%N_STENCIL = AB%N_STENCIL
               !   ENDIF
               !ENDDO
      ...
      !
      IF (NMESHES > 1) CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_MATRIX_SIZE, NSCARC_MATRIX_POISSON, NL)
...
SELECT CASE (ITYPE)
   ...
   CASE (NSCARC_RELAX_FFTO)
      ...
      CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_VECTOR, NV1, NL)
   ...
   CASE (NSCARC_RELAX_MKL)
      ...
      MKL_SCOPE_IF: IF (STACK(NS)%SOLVER%TYPE_SCOPE(0) == NSCARC_SCOPE_GLOBAL) THEN
         ...
         MKL_SCOPE_GLOBAL_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
            ...
            CALL SCARC_POINT_TO_GRID(NM, NL)
            MKL => L%MKL
            AS => SCARC_POINT_TO_MATRIX_COMPACT(NSCARC_MATRIX_POISSON_SYM)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'CLUSTER, PRE, V1:', V1
WRITE(MSG%LU_DEBUG,*) 'CLUSTER, PRE, V2:', V2
WRITE(MSG%LU_DEBUG,*) 'CLUSTER, PRE, AS%VAL:', AS%VAL
#endif
   IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'CLUSTER, POST, V1:', V1
WRITE(MSG%LU_DEBUG,*) 'CLUSTER, POST, V2:', V2
#endif
ENDDO MESHES_LOOP
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'PARDISO, PRE, NC:', G%NC
WRITE(MSG%LU_DEBUG,*) 'PARDISO, PRE, V1:', V1
WRITE(MSG%LU_DEBUG,*) 'PARDISO, PRE, V2:', V2
WRITE(MSG%LU_DEBUG,*) 'PARDISO, PRE, AS%VAL:', AS%VAL
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'PARDISO, POST, V1:', V1
WRITE(MSG%LU_DEBUG,*) 'PARDISO, POST, V2:', V2
#endif
ENDDO MESHES_LOOP
...
USE SCARC_POINTERS, ONLY: LF, LC, GF, GC, VF, VC, REST
...
WRITE(MSG%LU_DEBUG,'(8E12.4)') VC
      ...
      DO IC = 1, GF%N_COARSE
         DSUM = 0.0_EB
         DO ICOL = REST%ROW(IC), REST%ROW(IC+1)-1                            
            DSUM =  DSUM + REST%VAL(ICOL) * VF(REST%COL(ICOL))
         ENDDO
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'IC=',IC,': DSUM=',DSUM, REST%ROW(IC), REST%ROW(IC+1)-1
#endif
         VC(IC) = DSUM
      ENDDO
...
USE SCARC_POINTERS, ONLY: LF, LC, GF, GC, VF, VC, PROL
...
ELSE
   ...
   DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
      ...
      DO IC = 1, GF%N_FINE
         DSUM = 0.0_EB
         DO ICOL = PROL%ROW(IC), PROL%ROW(IC+1)-1                            
            DSUM = DSUM + PROL%VAL(ICOL) * VC(PROL%COL(ICOL))
         ENDDO
         VF(IC) = DSUM
      ENDDO
...
! 
! NSCARC_EXCHANGE_AGGREGATES:      aggregate numbers during SAMG setup
...
! NSCARC_EXCHANGE_MATRIX_DIAG:     Diagonal matrix elements of neighboring matrix
! NSCARC_EXCHANGE_MATRIX_SUBDIAG:  Overlapping diagonal 'leg' of neighboring matrix
! NSCARC_EXCHANGE_MATRIX_VALS:     Overlapping stencils of neighboring matrix
! 
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_EXCHANGE (NTYPE, NPARAM, NL)
USE SCARC_POINTERS, ONLY: OS, G, OG, VC
...
RECEIVE_MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   RECEIVE_OMESHES_LOOP: DO NOM = 1, NMESHES
      ...
      SELECT_EXCHANGE_TYPE: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_AGGREGATES)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: CELL_AGGREGATES'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_INT2)

         CASE (NSCARC_EXCHANGE_CELL_NUMBER)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: CELL_NUMBER'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_INT1)

         CASE (NSCARC_EXCHANGE_CELL_WIDTH)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: CELL_WIDTH'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL0)

         CASE (NSCARC_EXCHANGE_MATRIX_DIAG)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_DIAG'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_MATRIX_COLS)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_COLS'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_INT7)

         CASE (NSCARC_EXCHANGE_MATRIX_SIZE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_SIZE'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_INT0)

         CASE (NSCARC_EXCHANGE_MATRIX_VALS)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_VALS'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL7)

         CASE (NSCARC_EXCHANGE_NULLSPACE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_NULLSPACE'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_PRESSURE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_PRESSURE'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_SETUP)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_SETUP'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_INT0)

         CASE (NSCARC_EXCHANGE_VECTOR)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_VECTOR'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_VECTOR_MEAN)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: RECV: MATRIX_VECTOR_MEAN'
            CALL SCARC_RECV_MESSAGE(NSCARC_BUFFER_REAL2)
...
SEND_PACK_MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   SEND_PACK_OMESHES_LOOP: DO NOM = 1, NMESHES
      ...
      SEND_PACK_OMESHES_SELECT: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_AGGREGATES)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: AGGREGATES'
            CALL SCARC_PACK_AGGREGATES(NPARAM)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_INT2)

         CASE (NSCARC_EXCHANGE_CELL_NUMBER)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: CELL_NUMBER'
            CALL SCARC_PACK_CELL_NUMBER
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_INT1)

         CASE (NSCARC_EXCHANGE_CELL_WIDTH)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: CELL_WIDTH'
            CALL SCARC_PACK_CELL_WIDTH
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL0)

         CASE (NSCARC_EXCHANGE_MATRIX_SIZE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: MATRIX_SIZE'
            CALL SCARC_PACK_MATRIX_SIZE(NL)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_INT0)

         CASE (NSCARC_EXCHANGE_MATRIX_DIAG)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: MATRIX_DIAG'
            CALL SCARC_PACK_MATRIX_DIAG(NSCARC_MATRIX_POISSON)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_MATRIX_COLS)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: MATRIX_COLS'
            CALL SCARC_PACK_MATRIX_COLS(NPARAM)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_INT7)

         CASE (NSCARC_EXCHANGE_MATRIX_VALS)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: MATRIX_VALS'
            CALL SCARC_PACK_MATRIX_VALS(NPARAM, NL)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL7)

         CASE (NSCARC_EXCHANGE_NULLSPACE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: NULLSPACE'
            VC => G%AUX1
            CALL SCARC_PACK_VECTOR_SINGLE(VC)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_PRESSURE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: PRESSURE'
            IF (PREDICTOR) THEN
               HVC => POINT_TO_HVECTOR (NM, NSCARC_VECTOR_H)
            ELSE
               HVC => POINT_TO_HVECTOR (NM, NSCARC_VECTOR_HS)
            ENDIF
            CALL SCARC_PACK_PRESSURE(HVC)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_SETUP)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: SETUP'
            OS%SEND_INT0(1)=OG%NCG
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_INT0)

         CASE (NSCARC_EXCHANGE_VECTOR)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: VECTOR'
            VC => SCARC_POINT_TO_VECTOR(NM, NL, NPARAM)
            CALL SCARC_PACK_VECTOR_SINGLE(VC)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL1)

         CASE (NSCARC_EXCHANGE_VECTOR_MEAN)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: PACK: VECTOR_MEAN'
            VC => SCARC_POINT_TO_VECTOR(NM, NL, NPARAM)
            CALL SCARC_PACK_VECTOR_DOUBLE(VC)
            CALL SCARC_SEND_MESSAGE(NSCARC_BUFFER_REAL2)
...
SEND_UNPACK_MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   SEND_UNPACK_OMESHES_LOOP: DO NOM=1,NMESHES
      ...
      SEND_UNPACK_OMESHES_SELECT: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_AGGREGATES)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: AGGREGATES'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_INT2)
            CALL SCARC_UNPACK_AGGREGATES(NPARAM)

         CASE (NSCARC_EXCHANGE_CELL_WIDTH)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: CELL_WIDTH'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL0)
            CALL SCARC_UNPACK_CELL_WIDTH

         CASE (NSCARC_EXCHANGE_CELL_NUMBER)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: CELL_NUMBER'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_INT1)
            CALL SCARC_UNPACK_CELL_NUMBER

         CASE (NSCARC_EXCHANGE_MATRIX_SIZE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: MATRIX_SIZE'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_INT0)
            CALL SCARC_UNPACK_MATRIX_SIZE(NL)

         CASE (NSCARC_EXCHANGE_MATRIX_DIAG)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: MATRIX_DIAG'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL1)
            CALL SCARC_UNPACK_MATRIX_DIAG

         CASE (NSCARC_EXCHANGE_MATRIX_COLS)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: MATRIX_COLS'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_INT7)
            CALL SCARC_UNPACK_MATRIX_COLS(NPARAM)

         CASE (NSCARC_EXCHANGE_MATRIX_VALS)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: MATRIX_VALS'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL7)
            CALL SCARC_UNPACK_MATRIX_VALS(NPARAM, NL)

         CASE (NSCARC_EXCHANGE_NULLSPACE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: NULLSPACE'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL1)
            VC => G%AUX1
            CALL SCARC_UNPACK_VECTOR_SINGLE(VC)

         CASE (NSCARC_EXCHANGE_PRESSURE)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: PRESSURE'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL1)
            IF (PREDICTOR) THEN
               HVC => POINT_TO_HVECTOR (NM, NSCARC_VECTOR_H)
            ELSE
               HVC => POINT_TO_HVECTOR (NM, NSCARC_VECTOR_HS)
            ENDIF
            CALL SCARC_UNPACK_PRESSURE(HVC)

         CASE (NSCARC_EXCHANGE_SETUP)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: SETUP'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_INT0)
            OG%NCG = RECV_INT0(1)

         CASE (NSCARC_EXCHANGE_VECTOR)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: VECTOR'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL1)
            VC => SCARC_POINT_TO_VECTOR (NM, NL, NPARAM)
            CALL SCARC_UNPACK_VECTOR_SINGLE(VC)

         CASE (NSCARC_EXCHANGE_VECTOR_MEAN)
WRITE(MSG%LU_DEBUG,*) 'SCARC_EXCHANGE: UNPACK: VECTOR_MEAN'
            CALL SCARC_POINT_TO_BUFFER (NM, NOM, NSCARC_BUFFER_REAL2)
            VC => SCARC_POINT_TO_VECTOR (NM, NL, NPARAM)
            CALL SCARC_UNPACK_VECTOR_DOUBLE(VC)
...
! ------------------------------------------------------------------------------------------------
! Perform requested receive of data
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_RECV_MESSAGE(NTYPE)
INTEGER, INTENT(IN) :: NTYPE
...
SELECT CASE(NTYPE)
   CASE (NSCARC_BUFFER_INT0)
      OS%RECV_INT0 = 0
      CALL MPI_IRECV(OS%RECV_INT0(1), SIZE(OS%RECV_INT0), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_INT1)
      OS%RECV_INT1 = 0
      CALL MPI_IRECV(OS%RECV_INT1(1), SIZE(OS%RECV_INT1), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_INT2)
      OS%RECV_INT2 = 0
      CALL MPI_IRECV(OS%RECV_INT2(1), SIZE(OS%RECV_INT2), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_INT7)
      OS%RECV_INT7 = 0
      CALL MPI_IRECV(OS%RECV_INT7(1), SIZE(OS%RECV_INT7), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL0)
      OS%RECV_REAL0 = 0.0_EB
      CALL MPI_IRECV(OS%RECV_REAL0(1), SIZE(OS%RECV_REAL0), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL1)
      OS%RECV_REAL1 = 0.0_EB
      CALL MPI_IRECV(OS%RECV_REAL1(1), SIZE(OS%RECV_REAL1), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL2)
      OS%RECV_REAL2 = 0.0_EB
      CALL MPI_IRECV(OS%RECV_REAL2(1), SIZE(OS%RECV_REAL2), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL7)
      OS%RECV_REAL7 = 0.0_EB
      CALL MPI_IRECV(OS%RECV_REAL7(1), SIZE(OS%RECV_REAL7), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
END SELECT

END SUBROUTINE SCARC_RECV_MESSAGE


! ------------------------------------------------------------------------------------------------
! Perform requested send of data
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SEND_MESSAGE(NTYPE)
INTEGER, INTENT(IN) :: NTYPE
...
SELECT CASE(NTYPE)
   CASE (NSCARC_BUFFER_INT0)
      CALL MPI_ISEND(OS%SEND_INT0(1), SIZE(OS%SEND_INT0), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_INT1)
      CALL MPI_ISEND(OS%SEND_INT1(1), SIZE(OS%SEND_INT1), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD,&
                     REQ(N_REQ),IERROR)
   CASE (NSCARC_BUFFER_INT2)
      CALL MPI_ISEND(OS%SEND_INT2(1), SIZE(OS%SEND_INT2), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD,&
                     REQ(N_REQ),IERROR)
   CASE (NSCARC_BUFFER_INT7)
      CALL MPI_ISEND(OS%SEND_INT7(1), SIZE(OS%SEND_INT7), MPI_INTEGER, SNODE, TAG, MPI_COMM_WORLD,&
                     REQ(N_REQ),IERROR)
   CASE (NSCARC_BUFFER_REAL0)
      CALL MPI_ISEND(OS%SEND_REAL0(1), SIZE(OS%SEND_REAL0), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL1)
      CALL MPI_ISEND(OS%SEND_REAL1(1), SIZE(OS%SEND_REAL1), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL2)
      CALL MPI_ISEND(OS%SEND_REAL2(1), SIZE(OS%SEND_REAL2), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
   CASE (NSCARC_BUFFER_REAL7)
      CALL MPI_ISEND(OS%SEND_REAL7(1), SIZE(OS%SEND_REAL7), MPI_DOUBLE_PRECISION, SNODE, TAG, MPI_COMM_WORLD, &
                     REQ(N_REQ), IERROR)
END SELECT

END SUBROUTINE SCARC_SEND_MESSAGE


! ------------------------------------------------------------------------------------------------
! Pack and unpack overlapping parts of cell numbers
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_CELL_NUMBER
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
      ...
      ENDIF
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,8I6)') 'PACK_CELL_NUMBER: IOR0, ICG, IWG, IXW, IYW, IZW, SEND_INT1', &
                               IOR0, ICG, IWG, IXW, IYW, IZW, OS%SEND_INT1(ICG), SIZE(OS%SEND_INT1)
#endif
   ENDDO
ENDDO
END SUBROUTINE SCARC_PACK_CELL_NUMBER

! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_CELL_NUMBER
USE SCARC_POINTERS, ONLY: L, G, OL, OG
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
      ...
      IZG = G%WALL(IWG)%IZG
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,9I6)') 'UNPACK_CELL_NUMBER: IOR0, ICG, ICE, IWG, IXG, IYG, IZG, RECV_INT1', &
                               IOR0, ICG, ICE, IWG, IXG, IYG, IZG, RECV_INT1(LL), SIZE(RECV_INT1)
#endif
   ...
   ENDDO
ENDDO
END SUBROUTINE SCARC_UNPACK_CELL_NUMBER


! ------------------------------------------------------------------------------------------------
! Pack and unpack cell width information 
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_CELL_WIDTH
...
END SUBROUTINE SCARC_PACK_CELL_WIDTH

! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_CELL_WIDTH
USE SCARC_POINTERS, ONLY: L, OL
...
END SUBROUTINE SCARC_UNPACK_CELL_WIDTH
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_PRESSURE(VC)
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
      IWG = OG%ICG_TO_IWG(ICG)
      OS%SEND_REAL1(ICG) = VC(G%WALL(IWG)%IXW, G%WALL(IWG)%IYW, G%WALL(IWG)%IZW)
   ENDDO
ENDDO
END SUBROUTINE SCARC_PACK_PRESSURE

! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_PRESSURE(VC)
USE SCARC_POINTERS, ONLY: OL, OG
REAL(EB), DIMENSION(:,:,:), POINTER :: VC
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_VECTOR_SINGLE(VC)
USE SCARC_POINTERS, ONLY: OL, OG, OS
REAL(EB), DIMENSION(:), POINTER :: VC
...
ELSE
   ...
   ENDDO
ENDIF
END SUBROUTINE SCARC_PACK_VECTOR_SINGLE

! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_VECTOR_SINGLE(VC)
USE SCARC_POINTERS, ONLY: OL, OG
REAL(EB), DIMENSION(:), POINTER :: VC
INTEGER :: IOR0, ICG, LL
LL = 1
DO IOR0 = -3, 3
   ...
   ENDDO
ENDDO
END SUBROUTINE SCARC_UNPACK_VECTOR_SINGLE


! ------------------------------------------------------------------------------------------------
! Pack and unpack overlapping and internal parts of specified vector
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_VECTOR_DOUBLE(VC)
USE SCARC_POINTERS, ONLY: OL, OG, OS
REAL(EB), DIMENSION(:), POINTER :: VC
...
END SUBROUTINE SCARC_PACK_VECTOR_DOUBLE

! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_VECTOR_DOUBLE(VC)
USE SCARC_POINTERS, ONLY: OL, OG
REAL(EB), DIMENSION(:), POINTER :: VC
INTEGER :: IOR0, ICG, LL
LL = 1
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '=============== EXCHANGE_COLS: TYPE_EXCHANGE=', TYPE_EXCHANGE
...
LL = 1
OS%SEND_INT7 = -1                        ! preset with -1 to be able to identify end of exchange
...
SELECT CASE (TYPE_GRID) 
   ...
   CASE (NSCARC_GRID_STRUCTURED)
      ...
      DO IOR0 = -3, 3
         IF (OL%NCG(IOR0) == 0) CYCLE
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '============= EXCHANGE_MATRIX_COLS: SEND: NCG0, NCG:', OL%NCG0(IOR0), OL%NCG(IOR0)
#endif
         DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
            ICW = OG%ICG_TO_ICW(ICG)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5I5)') 'EXCHANGE_MATRIX_COLS: SEND: IOR0, ICG, ICW, ICOL, VAL:', &
...
WRITE(MSG%LU_DEBUG,'(A,5I5)') 'EXCHANGE_MATRIX_COLS: SEND: IOR0, ICG, ICW, ICOL, VAL:', &
               ...
               ENDDO
            ELSE
               OS%SEND_INT7(LL) = NSCARC_HUGE_INT
               LL = LL + 1
            ENDIF
         ENDDO
      ENDDO
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_MATRIX_COLS(NTYPE)
USE SCARC_POINTERS, ONLY: OL, OG, OA
INTEGER, INTENT(IN) :: NTYPE
INTEGER :: IOR0, ICG, LL, ICE, ICP

OA => SCARC_POINT_TO_OTHER_MATRIX_COMPACT(NTYPE)
...
DO IOR0 = -3, 3
   IF (OL%NCG(IOR0) == 0) CYCLE
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 12I4)') 'UNPACK_MATRIX_COLS: NCG0, NCG:', OL%NCG0(IOR0), OL%NCG(IOR0), RECV_INT7(1:10)
#endif
   OA%ROW(ICP) = 1
   DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
      ICE = OG%ICG_TO_ICE(ICG)
      OA%COL(LL) = ABS(RECV_INT7(LL))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 4I4)') 'UNPACK_MATRIX_COLS: A: RECV: ICG, LL, RECV_INT7, OA%ROW(ICG), OA%COL(LL):', &
                                ICG, LL, RECV_INT7(LL), OA%COL(LL)
#endif
      DO WHILE (RECV_INT7(LL+1) > 0)
         LL = LL + 1
         OA%COL(LL) = ABS(RECV_INT7(LL))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 4I4)') 'UNPACK_MATRIX_COLS: B: RECV: ICG, LL, RECV_INT7, OA%ROW(ICG), OA%COL(LL):', &
                                ICG, LL, RECV_INT7(LL), OA%COL(LL)
#endif
      ENDDO
      LL = LL + 1
      ...
      OA%ROW(ICP) = LL
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '----------> UNPACK_MATRIX_COLS: OA%ROW(',ICP,')=',OA%ROW(ICP)
#endif
   ENDDO
   OA%N_ROW = ICP  
   OA%N_VAL = LL - 1
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 4I4)') 'UNPACK_MATRIX_COLS: OA%N_ROW=',OA%N_ROW
WRITE(MSG%LU_DEBUG,'(A, 4I4)') 'UNPACK_MATRIX_COLS: OA%N_VAL=',OA%N_VAL
#endif
ENDDO
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_MATRIX_VALS(NTYPE, NL)
      ...
      SELECT CASE (TYPE_GRID)
         ...
         CASE (NSCARC_GRID_STRUCTURED)
            ...
            DO IOR0 = -3, 3
               IF (OL%NCG(IOR0) == 0) CYCLE
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '============= EXCHANGE_MATRIX_VALS: NCG0, NCG:', OL%NCG0(IOR0), OL%NCG(IOR0)
#endif
               DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
                  ICW = OG%ICG_TO_ICW(ICG)
                  DO ICOL = A%ROW(ICW), A%ROW(ICW+1)-1
                     OS%SEND_REAL7(LL) = A%VAL(ICOL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 4I5,E12.4)') 'EXCHANGE_MATRIX_VALS: SEND: IOR0, ICG, ICW, ICOL, VAL:', IOR0, ICG, ICW, ICOL, A%VAL(ICOL)
#endif
                     LL = LL + 1
                  ENDDO
               ENDDO
         ...
         CASE (NSCARC_GRID_UNSTRUCTURED)
            ...
            DO IOR0 = -3, 3
               ...
               DO ICG= OL%NCG0(IOR0), OL%NCG(IOR0)
                  ...
                  IF (ICW > 0) THEN
                     DO ICOL = A%ROW(ICW), A%ROW(ICW+1)-1
                        OS%SEND_REAL7(LL) = A%VAL(A%COL(ICOL))
                        LL = LL + 1
                     ENDDO
                  ELSE
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_MATRIX_VALS(NTYPE, NL)
USE SCARC_POINTERS, ONLY: OL, OA, OAB
INTEGER, INTENT(IN) :: NTYPE, NL
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'UNPACK_MATRIX_VALS: A%ROW=',A%ROW
#endif
      DO IOR0 = -3, 3
         IF (OL%NCG(IOR0) == 0) CYCLE
         DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'UNPACK_MATRIX_VALS: ICG, OA%ROW(ICG)', ICG, OA%ROW(ICG)
#endif
            DO LL = OA%ROW(ICG), OA%ROW(ICG+1)-1
               OA%VAL(LL) = RECV_REAL7(LL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3I4,E12.4)') 'UNPACK_MATRIX_VALS, RECV: IOR0, ICG, LL, OA%VAL', &
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_MATRIX_SIZE(NL)
USE SCARC_POINTERS, ONLY: OS, OG
INTEGER, INTENT(IN) :: NL
SELECT CASE (SCARC_MATRIX_LEVEL(NL))
   CASE (NSCARC_MATRIX_BANDWISE)
      OS%SEND_INT0(1) = OG%POISSON_BW%N_VAL
      OS%SEND_INT0(2) = OG%POISSON_BW%N_DIAG
      OS%SEND_INT0(3) = OG%POISSON_BW%N_STENCIL
   CASE (NSCARC_MATRIX_COMPACT)
      OS%SEND_INT0(1) = OG%POISSON%N_VAL
      OS%SEND_INT0(2) = OG%POISSON%N_ROW
      OS%SEND_INT0(3) = OG%POISSON%N_STENCIL
END SELECT
END SUBROUTINE SCARC_PACK_MATRIX_SIZE
   
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_MATRIX_SIZE(NL)
USE SCARC_POINTERS, ONLY: OG
INTEGER, INTENT(IN) :: NL
SELECT CASE (SCARC_MATRIX_LEVEL(NL))
   CASE (NSCARC_MATRIX_BANDWISE)
      OG%POISSON_BW%N_VAL     = RECV_INT0(1)
      OG%POISSON_BW%N_DIAG    = RECV_INT0(2)
      OG%POISSON_BW%N_STENCIL = RECV_INT0(3)
   CASE (NSCARC_MATRIX_COMPACT)
      OG%POISSON%N_VAL     = RECV_INT0(1)
      OG%POISSON%N_ROW     = RECV_INT0(2)
      OG%POISSON%N_STENCIL = RECV_INT0(3)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'OG%POISSON%N_VAL=',OG%POISSON%N_VAL
WRITE(MSG%LU_DEBUG,*) 'OG%POISSON%N_ROW=',OG%POISSON%N_ROW
WRITE(MSG%LU_DEBUG,*) 'OG%POISSON%N_STENCIL=',OG%POISSON%N_STENCIL
#endif
END SELECT
END SUBROUTINE SCARC_UNPACK_MATRIX_SIZE


! ------------------------------------------------------------------------------------------------
! Pack and unpack overlapping information about matrix diagonals (compact storage technique only)
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_MATRIX_DIAG(NTYPE)
...
IF (IS_STRUCTURED) THEN
   DO IOR0 = -3, 3
      ...
      DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
         ...
         ICE = OG%ICG_TO_ICE(ICG)
         ICOL = A%ROW(ICW)
         OS%SEND_REAL1(ICG) = A%VAL(ICOL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 5I4,E12.4)') 'PACK_MATRIX_DIAG: IOR0, ICG, ICW, ICE, ICOL, VAL:', &
...
ELSE
   ...
   ENDDO
ENDIF
END SUBROUTINE SCARC_PACK_MATRIX_DIAG
      
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_MATRIX_DIAG
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
      ICE = OG%ICG_TO_ICE(ICG)
      G%DIAG(ICE) = RECV_REAL1(LL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 3I4,E12.4)') 'UNPACK_MATRIX_DIAG: NOM, IOR0, ICG, ICE, DIAG:', &
                                      IOR0, ICG, ICE, G%DIAG(ICE)
#endif
      LL = LL + 1
   ENDDO
ENDDO
END SUBROUTINE SCARC_UNPACK_MATRIX_DIAG


! ------------------------------------------------------------------------------------------------
! Pack aggregates information into send vector
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_AGGREGATES(NTYPE)
...
DO IOR0 = -3, 3
   ...
   IF (NTYPE /= NSCARC_AGGREGATE_COMPLETE) THEN
      DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
         ICW = OG%ICG_TO_ICW(ICG)
         ICE = OG%ICG_TO_ICE(ICG)
         ...
         ENDIF
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 6I8)') 'PACK_AGGREGATES: IOR0, ICG, ICW, SEND(LL:LL+1):', &
                                IOR0, ICG, ICW, ICE, OS%SEND_INT2(LL), OS%SEND_INT2(LL+1)
#endif
         LL = LL + 2
      ENDDO
   ELSE
      OS%SEND_INT2 = 0           ! mesh with biggest number fills in dummy data
   ENDIF
ENDDO
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '=========> SENDING '
WRITE(MSG%LU_DEBUG,'(7I8)') OS%SEND_INT2
#endif
END SUBROUTINE SCARC_PACK_AGGREGATES


! ------------------------------------------------------------------------------------------------
! Unpack aggregates information freom receive vector
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_AGGREGATES(NTYPE)
USE SCARC_POINTERS, ONLY: G, OS, OL, OG
INTEGER, INTENT(IN) :: NTYPE
INTEGER :: IOR0, ICG, ICW, ICE, LL

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '=========> RECEIVING '
WRITE(MSG%LU_DEBUG,'(7I8)') OS%RECV_INT2
#endif
LL = 1
DO IOR0 = -3, 3
   IF (OL%NCG(IOR0) == 0) CYCLE
   SELECT CASE (NTYPE)
      CASE (NSCARC_AGGREGATE_INITIAL, NSCARC_AGGREGATE_FILLUP)
         DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
            ICE = OG%ICG_TO_ICE(ICG)
            IF (G%AGGREGATES(ICE) == 0) G%AGGREGATES(ICE) = RECV_INT2(LL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 7I8)') 'UNPACK_AGGREGATES:A: IOR0, ICG, ICE, RECV, NAGG, AGG(ICE):', &
              IOR0, ICG, ICE, RECV_INT2(LL:LL+1), G%N_AGGREGATES, G%AGGREGATES(ICE)
#endif
            LL = LL + 2
         ENDDO
      CASE (NSCARC_AGGREGATE_COMPLETE) 
         DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
            ICW = OG%ICG_TO_ICW(ICG)
            ICE = OG%ICG_TO_ICE(ICG)
            G%AGGREGATES(ICE) = RECV_INT2(LL)
            G%AGGREGATES(ICW) = RECV_INT2(LL+1)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 8I8)') 'UNPACK_AGGREGATES:B: IOR0, ICG, ICW, ICE, NAGG, AGG(ICW), AGG(ICE):', &
              IOR0, ICG, ICW, ICE, G%N_AGGREGATES, G%AGGREGATES(ICW), G%AGGREGATES(ICE)
#endif
            LL = LL + 2
         ENDDO
   END SELECT
ENDDO
END SUBROUTINE SCARC_UNPACK_AGGREGATES
...
! ----------------------------------------------------------------------------------------------------
FUNCTION SCARC_POINT_TO_MATRIX_COMPACT(NTYPE)
USE SCARC_POINTERS, ONLY: G
TYPE(SCARC_MATRIX_COMPACT_TYPE), POINTER :: SCARC_POINT_TO_MATRIX_COMPACT
INTEGER, INTENT(IN) :: NTYPE

SELECT CASE(NTYPE)
   CASE (NSCARC_MATRIX_POISSON)
      SCARC_POINT_TO_MATRIX_COMPACT => G%POISSON
#ifdef WITH_MKL
   CASE (NSCARC_MATRIX_POISSON_SYM)
      SCARC_POINT_TO_MATRIX_COMPACT => G%POISSON_SYM
#endif
   CASE (NSCARC_MATRIX_CONNECTION)
      SCARC_POINT_TO_MATRIX_COMPACT => G%CONNECTION
   CASE (NSCARC_MATRIX_PROLONGATION)
      SCARC_POINT_TO_MATRIX_COMPACT => G%PROLONGATION
   CASE (NSCARC_MATRIX_RESTRICTION)
      SCARC_POINT_TO_MATRIX_COMPACT => G%RESTRICTION
END SELECT

END FUNCTION SCARC_POINT_TO_MATRIX_COMPACT


! ----------------------------------------------------------------------------------------------------
! This routine sets a pointer to the requested matrix of compact storage type
! ----------------------------------------------------------------------------------------------------
FUNCTION SCARC_POINT_TO_MATRIX_BANDWISE(NTYPE)
USE SCARC_POINTERS, ONLY: G
TYPE(SCARC_MATRIX_BANDWISE_TYPE), POINTER :: SCARC_POINT_TO_MATRIX_BANDWISE
INTEGER, INTENT(IN) :: NTYPE

SELECT CASE(NTYPE)
   CASE (NSCARC_MATRIX_POISSON)
      SCARC_POINT_TO_MATRIX_BANDWISE => G%POISSON_BW
   CASE DEFAULT
      WRITE(*,*) 'No other bandwise matrix available yet except of POISSON_BW'
END SELECT

END FUNCTION SCARC_POINT_TO_MATRIX_BANDWISE


! ----------------------------------------------------------------------------------------------------
! This routine sets a pointer to the requested matrix of compact storage type
! ----------------------------------------------------------------------------------------------------
FUNCTION SCARC_POINT_TO_OTHER_MATRIX_COMPACT(NTYPE)
USE SCARC_POINTERS, ONLY: OG
TYPE(SCARC_MATRIX_COMPACT_TYPE), POINTER :: SCARC_POINT_TO_OTHER_MATRIX_COMPACT
INTEGER, INTENT(IN) :: NTYPE

SELECT CASE(NTYPE)
   CASE (NSCARC_MATRIX_POISSON)
      SCARC_POINT_TO_OTHER_MATRIX_COMPACT => OG%POISSON
#ifdef WITH_MKL
   CASE (NSCARC_MATRIX_POISSON_SYM)
      SCARC_POINT_TO_OTHER_MATRIX_COMPACT => OG%POISSON_SYM
#endif
   CASE (NSCARC_MATRIX_CONNECTION)
      SCARC_POINT_TO_OTHER_MATRIX_COMPACT => OG%CONNECTION
   CASE (NSCARC_MATRIX_PROLONGATION)
      SCARC_POINT_TO_OTHER_MATRIX_COMPACT => OG%PROLONGATION
   CASE (NSCARC_MATRIX_RESTRICTION)
      SCARC_POINT_TO_OTHER_MATRIX_COMPACT => OG%RESTRICTION
END SELECT

END FUNCTION SCARC_POINT_TO_OTHER_MATRIX_COMPACT


! ----------------------------------------------------------------------------------------------------
! This routine sets a pointer to the requested matrix of compact storage type
! ----------------------------------------------------------------------------------------------------
FUNCTION SCARC_POINT_TO_OTHER_MATRIX_BANDWISE(NTYPE)
USE SCARC_POINTERS, ONLY: OG
TYPE(SCARC_MATRIX_BANDWISE_TYPE), POINTER :: SCARC_POINT_TO_OTHER_MATRIX_BANDWISE
INTEGER, INTENT(IN) :: NTYPE

SELECT CASE(NTYPE)
   CASE (NSCARC_MATRIX_POISSON)
      SCARC_POINT_TO_OTHER_MATRIX_BANDWISE => OG%POISSON_BW
   CASE DEFAULT
      WRITE(*,*) 'No other bandwise matrix available yet except of POISSON_BW'
END SELECT

END FUNCTION SCARC_POINT_TO_OTHER_MATRIX_BANDWISE


! -----------------------------------------------------------------------------
! Point to requested receive buffer
! -----------------------------------------------------------------------------
SUBROUTINE SCARC_POINT_TO_BUFFER(NM, NOM, NTYPE)
USE SCARC_POINTERS, ONLY: RECV_INT0, RECV_INT1, RECV_INT7, RECV_REAL0, RECV_REAL1, RECV_REAL2, RECV_REAL7
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SIZE(RECV_INT1):A:', SIZE(SCARC(NM)%OSCARC(NOM)%RECV_INT1)
#endif
      ELSE
         RECV_INT1  => SCARC(NOM)%OSCARC(NM)%SEND_INT1
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SIZE(RECV_INT1):B:', SIZE(SCARC(NOM)%OSCARC(NM)%SEND_INT1)
#endif
      ENDIF
   CASE (NSCARC_BUFFER_INT2)
      IF (RNODE/=SNODE) THEN
         RECV_INT2  => SCARC(NM)%OSCARC(NOM)%RECV_INT2
      ELSE
         RECV_INT2  => SCARC(NOM)%OSCARC(NM)%SEND_INT2
      ENDIF
   CASE (NSCARC_BUFFER_INT7)
      IF (RNODE/=SNODE) THEN
         RECV_INT7  => SCARC(NM)%OSCARC(NOM)%RECV_INT7
      ELSE
         RECV_INT7  => SCARC(NOM)%OSCARC(NM)%SEND_INT7
      ENDIF
   CASE (NSCARC_BUFFER_REAL0)
      IF (RNODE/=SNODE) THEN
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_RESIZE_INT1(WORKSPACE, NSIZE, CNAME)
...
! Reduce size of integer vector
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_RESIZE_REAL1(WORKSPACE, NSIZE, CNAME)
...
1000 FORMAT('Resizing   integer vector ',A20,' to size ',I8)
#endif
END SUBROUTINE SCARC_RESIZE_REAL1
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_RESIZE_MATRIX_COMPACT(A, NVAL, NROW, CNAME)
TYPE (SCARC_MATRIX_COMPACT_TYPE), INTENT(INOUT) :: A
INTEGER, INTENT(IN) :: NVAL, NROW
CHARACTER(*), INTENT(IN) :: CNAME
CHARACTER(40) :: CINFO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'RESIZING MATRIX NVAL, NROW, A%N_VAL, A%N_ROW:', CNAME, NVAL, NROW, A%N_VAL, A%N_ROW
#endif

! If the matrix already has the desired size or specified values are to small, return
IF (A%N_VAL == NVAL .AND. A%N_ROW == NROW) THEN
   RETURN
ELSE IF (A%N_VAL < NVAL) THEN
   CALL SCARC_SHUTDOWN(NSCARC_ERROR_MATRIX_SIZE, SCARC_NONE, NVAL)
ELSE IF (A%N_ROW < NROW) THEN
   CALL SCARC_SHUTDOWN(NSCARC_ERROR_MATRIX_SIZE, SCARC_NONE, NROW)
ENDIF

! If the size of the matrix values workspace is too large, reduce it to the desired size
IF (A%N_VAL > NVAL) THEN

   A%N_VAL = NVAL
   WRITE(CINFO,'(A,A)') TRIM(CNAME),'.VAL'

   CALL SCARC_ALLOCATE_REAL1(VAL, 1, NVAL, NSCARC_INIT_NONE, TRIM(CINFO))
   VAL(1:NVAL) = A%VAL(1:NVAL)
   DEALLOCATE(A%VAL)

   CALL SCARC_ALLOCATE_REAL1(A%VAL, 1, NVAL, NSCARC_INIT_NONE, TRIM(CINFO))
   A%VAL(1:NVAL) = VAL(1:NVAL)
   DEALLOCATE(VAL)

   WRITE(CINFO,'(A,A)') TRIM(CNAME),'.COL'

   CALL SCARC_ALLOCATE_INT1(COL, 1, NVAL, NSCARC_INIT_NONE, TRIM(CINFO))
   COL(1:NVAL) = A%COL(1:NVAL)
   DEALLOCATE(A%COL)

   CALL SCARC_ALLOCATE_INT1(A%COL, 1, NVAL, NSCARC_INIT_NONE, TRIM(CINFO))
   A%COL(1:NVAL) = COL(1:NVAL)
   DEALLOCATE(COL)

ENDIF

! If the size of the matrix column workspace is too large, reduce it to the desired size
IF (A%N_ROW > NROW) THEN

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'OLD ROW SIZE ', SIZE(A%ROW)
#endif
   A%N_ROW = NROW
   WRITE(CINFO,'(A,A)') TRIM(CNAME),'.ROW'

   CALL SCARC_ALLOCATE_INT1(ROW, 1, NROW, NSCARC_INIT_NONE, TRIM(CINFO))
   ROW(1:NROW) = A%ROW(1:NROW)
   DEALLOCATE(A%ROW)

   CALL SCARC_ALLOCATE_INT1(A%ROW, 1, NROW, NSCARC_INIT_NONE, TRIM(CINFO))
   A%ROW(1:NROW) = ROW(1:NROW)
   DEALLOCATE(ROW)

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NEW SIZE ', SIZE(A%ROW)
#endif
ENDIF

#ifdef WITH_SCARC_VERBOSE
WRITE(MSG%LU_VERBOSE,1000) A%CNAME
1000 FORMAT('Resizing   COMPACT matrix ',A20)
#endif
...
! Allocate matrix with corresponding pointer and length structures
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_ALLOCATE_MATRIX_BANDWISE(A, NL, NINIT, CNAME)
...
#endif
END SUBROUTINE SCARC_ALLOCATE_MATRIX_BANDWISE
...
! Deallocate matrix with corresponding pointer and length structures
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_DEALLOCATE_MATRIX_BANDWISE(A)
TYPE (SCARC_MATRIX_BANDWISE_TYPE), INTENT(INOUT) :: A
...
1000 FORMAT('Removing   COMPACT matrix ')
#endif
END SUBROUTINE SCARC_DEALLOCATE_MATRIX_BANDWISE
...
! Allocate needed workspace for definition of hierarchy of system matrices, prolongation, restriction, etc.
...
WRITE(MSG%LU_DEBUG,*) '========================================================'
   ...
   CALL SCARC_SETUP_AGGREGATION_ORDER(NL)           ! Determine aggregates and make them consistent along interfaces
   CALL SCARC_EXTRACT_MATRIX_DIAGONAL(NL)           ! Separately store Poisson matrix diagonal
   CALL SCARC_SETUP_CONNECTIOIN_STRENGTH(NL)        ! Determine strength of connection in Poisson matrix
   CALL SCARC_INVERT_MATRIX_DIAGONAL(NL)            ! Invert matrix diagonal
   CALL SCARC_SETUP_AGGREGATES(NL)                  ! Apply smoothed aggregation heuristic
   CALL SCARC_RELAX_NULLSPACE(NL)                   ! Improve near null space by Jacobi relaxation step
   CALL SCARC_SETUP_AGGREGATE_OPERATOR(NL)          ! Setup final aggregation operator
   ...
   CALL SCARC_SETUP_GALERKIN_OPERATOR(NL)           ! Setup Poisson matrix on next coarser level
...
! ------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_AGGREGATION_ORDER(NL)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SUB%N_NEIGHBORS:', NL
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   DO INBR = 1, SCARC(NM)%N_NEIGHBORS
      INT_BUFFER(DISPLS_NBR(PROCESS(NM)) + IP) = SCARC(NM)%NEIGHBORS(INBR)
      IP = IP + 1
   ENDDO
ENDDO
...
DO NM = 1, NMESHES
   DO INBR = 1, SUB%N_NEIGHBORS(NM)
      SUB%NEIGHBORS(INBR, NM) = INT_BUFFER(DISPLS_NBR(PROCESS(NM)) + INBR)
   ENDDO
#ifdef WITH_SCARC_DEBUG
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_POINT_TO_GRID (NM, NL)
   A => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_COMPACT)
...
USE SCARC_POINTERS, ONLY: G
INTEGER, INTENT(IN) :: NL
!REAL(EB) :: APPROX_SPECTRAL_RADIUS, OMEGA, SCAL
INTEGER :: NM, IC
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   ENDDO
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'SETUP_DIAG:'
   WRITE(MSG%LU_DEBUG,*) 'DIAG:'
   WRITE(MSG%LU_DEBUG,'(8E12.4)') G%DIAG
#endif
ENDDO MESHES_LOOP
...
INTEGER, INTENT(IN) :: NL
REAL(EB):: VAL, EPS, THETA = 0.19_EB, SCAL, CVAL_MAX
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_ALLOCATE_INT1 (G%AGGREGATES, 1, G%NCE, NSCARC_INIT_ZERO, 'G%AGGREGATES')

   A    => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_POISSON)
   CONN => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_CONNECTION)
   C%N_VAL = A%N_VAL           
   C%N_ROW = A%N_ROW
   CALL SCARC_ALLOCATE_MATRIX_COMPACT(C, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'G%CONNECTION')
   ...
   DO INBR = 1, SCARC(NM)%N_NEIGHBORS
      ...
      CALL SCARC_ALLOCATE_INT1 (OG%CPOINTS   , 1, OG%NCG, NSCARC_INIT_ZERO, 'OG%CPOINTS')
      CALL SCARC_ALLOCATE_INT1 (OG%AGGREGATES, 1, OG%NCG, NSCARC_INIT_ZERO, 'OG%AGGREGATES')
      
      OA    => SCARC_POINT_TO_OTHER_MATRIX_COMPACT(NSCARC_MATRIX_POISSON)
      OCONN => SCARC_POINT_TO_OTHER_MATRIX_COMPACT(NSCARC_MATRIX_CONNECTION)

      OCONN%N_VAL = OA%N_VAL           
      OCONN%N_ROW = OA%N_ROW           
      CALL SCARC_ALLOCATE_MATRIX_COMPACT(OCONN, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'OG%CONNECTION')
   ...
   WRITE(MSG%LU_DEBUG,*) 'G%DIAG:'
   WRITE(MSG%LU_DEBUG,'(8E12.4)') G%DIAG
   CALL SCARC_SHOW_MATRIX(A, 'CONNECTION', 'SETUP_STRENGTH0')
#endif
   ...
   DO IC = 1, G%NC
      ...
      DO ICOL = AF%ROW(IC), AF%ROW(IC+1) - 1
         JC  = AF%COL(ICOL)
         VAL = AF%VAL(ICOL)                              ! VAL = A_ij
   
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A,4I4,4E12.4)') 'IC, ICOL, JC, IAGG, VAL2, EPS, DIAG(JC), EPS*DIAG(JC):', &
                                 IC, ICOL, JC, IAGG, VAL**2, EPS, ABS(G%DIAG(JC)), EPS*ABS(G%DIAG(JC))
#endif
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) ' ------------> subdiag'
#endif
            IAGG = IAGG + 1
   
         ELSE
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) ' ------------> nothing'
#endif
         ENDIF
      ENDDO
      CONN%ROW(IC+1) = IAGG
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) '================== SETUP_STRENGTH1:'
   WRITE(MSG%LU_DEBUG,*) 'CONN%ROW:'
   WRITE(MSG%LU_DEBUG,'(8I8)') CONN%ROW
   WRITE(MSG%LU_DEBUG,*) 'CONN%COL:'
   DO IC = 1, G%NC
      WRITE(MSG%LU_DEBUG,'(16I8)') (CONN%COL(ICOL), ICOL=CONN%ROW(IC), CONN%ROW(IC+1)-1)
   ENDDO
   WRITE(MSG%LU_DEBUG,*) 'CONN%VAL:'
   DO IC = 1, G%NC
      WRITE(MSG%LU_DEBUG,'(16E12.4)') (CONN%VAL(ICOL), ICOL=CONN%ROW(IC), CONN%ROW(IC+1)-1)
   ENDDO
#endif
   
   DO IC = 1, G%NC
      CVAL_MAX = 0.0_EB
      DO ICOL = CONN%ROW(IC), CONN%ROW(IC+1) - 1
         CVAL_MAX = MAX(ABS(CONN%VAL(ICOL)), CVAL_MAX)
      ENDDO
      SCAL = 1.0_EB/CVAL_MAX
      DO ICOL = CONN%ROW(IC), CONN%ROW(IC+1) - 1
         CONN%VAL(ICOL) = ABS(CONN%VAL(ICOL))*SCAL
      ENDDO
   ENDDO
   
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) '================== SETUP_STRENGTH2:'
   WRITE(MSG%LU_DEBUG,*) 'CONN%ROW:'
   WRITE(MSG%LU_DEBUG,'(8I8)') CONN%ROW
   WRITE(MSG%LU_DEBUG,*) 'CONN%COL:'
   DO IC = 1, G%NC
...
USE SCARC_POINTERS, ONLY: G, CONN, SUB
INTEGER, INTENT(IN) :: NL
INTEGER :: NM, NM2, IC, JC, ICOL, IAGG, JAGG, NAGG, CURRENT_AGGREGATE, NAGG_LOCAL, N_AGGREGATES_PREV = 0
INTEGER, ALLOCATABLE, DIMENSION(:) :: AGGREGATES_GLOBAL

SUB => SUBDIVISION
MESH_INT = 0

CALL SCARC_ALLOCATE_INT1 (AGGREGATES_GLOBAL, 1, NMESHES, NSCARC_INIT_ZERO, 'AGGREGATES_GLOBAL')
CURRENT_AGGREGATE = 1

MESHES_LOOP1: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   AGGREGATION_LOOP: DO NAGG = 1, NMESHES

#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,*) '================================================================='
      WRITE(MSG%LU_DEBUG,*) '=============== AGGREGATING ON MESH ',NM,' FOR NAGG ', NAGG
      WRITE(MSG%LU_DEBUG,*) '================================================================='
#endif
      CURRENT_MESH_IF: IF (NM /= CURRENT_AGGREGATE) THEN

         AGGREGATES_GLOBAL = 0
         IF (NMESHES > 1) THEN
            IF (NM > CURRENT_AGGREGATE) THEN
               CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_AGGREGATES, NSCARC_AGGREGATE_COMPLETE, NL)
            ELSE IF (NM < CURRENT_AGGREGATE) THEN
               CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_AGGREGATES, NSCARC_AGGREGATE_FILLUP, NL)
            ENDIF
         ENDIF
         IF (N_MPI_PROCESSES>1) &
            CALL MPI_ALLGATHERV(MPI_IN_PLACE,1,MPI_INTEGER,AGGREGATES_GLOBAL,COUNTS,DISPLS, MPI_INTEGER,MPI_COMM_WORLD,IERROR)
         CURRENT_AGGREGATE = CURRENT_AGGREGATE + 1

#ifdef WITH_SCARC_DEBUG
         WRITE(MSG%LU_DEBUG,*) ' NOT MY TURN  =============  '
         WRITE(MSG%LU_DEBUG,CFORM_INT) G%AGGREGATES(1:G%NC)
         WRITE(MSG%LU_DEBUG,*) '-------------- overlap:'
         WRITE(MSG%LU_DEBUG,'(10I4)') G%AGGREGATES(G%NC+1: G%NCE)
#endif
      ...
      ELSE 
         ...
         G%N_FINE = G%NCE
         IF (NM > 1) THEN
            G%N_AGGREGATES = AGGREGATES_GLOBAL(NM-1) + 1       ! has already been initialized to 1 on mesh 1 
            N_AGGREGATES_PREV = G%N_AGGREGATES
         ENDIF

         ! Perform aggregation on internal part of mesh
         PASS1_LOOP: DO IC = 1, G%NC
            IF (G%AGGREGATES(IC) /= 0) CYCLE                        
         ...
         PASS2_LOOP: DO IC = 1, G%NC
            ...
            DO ICOL = CONN%ROW(IC), CONN%ROW(IC+1)-1
               JC = CONN%COL(ICOL)
               JAGG = G%AGGREGATES(JC)
               IF (JAGG > 0) THEN
                  IF (JC >= G%NC .OR. G%CPOINTS(JAGG)>0) THEN
                     G%AGGREGATES(IC) = -JAGG
                     EXIT
                  ENDIF
               ENDIF
            ENDDO
...
#ifdef WITH_SCARC_DEBUG
         WRITE(MSG%LU_DEBUG,*) 'SETUP_AGGREGATES_PASS2: =============  ', NAGG_LOCAL
         WRITE(MSG%LU_DEBUG,CFORM_INT) G%AGGREGATES(1:G%NC)
         WRITE(MSG%LU_DEBUG,*) '-------------- overlap:'
         WRITE(MSG%LU_DEBUG,'(10I4)') G%AGGREGATES(G%NC+1: G%NCE)
#endif
   
         !G%AGGREGATES = ABS(G%AGGREGATES)

         ! Process remaining nodes which have not been aggregated yet
         PASS3_LOOP: DO IC = 1, G%NC
   
            IAGG = G%AGGREGATES(IC)
   
            ! cell IC has not been aggregated
            IF (IAGG /= 0) THEN
               IF (IAGG > 0) THEN
                  G%AGGREGATES(IC) = IAGG 
            ...
            ! cell IC has not been aggregated
            G%AGGREGATES(IC) = G%N_AGGREGATES
            G%CPOINTS(G%N_AGGREGATES) = IC
   
            DO ICOL = CONN%ROW(IC), CONN%ROW(IC+1)-1
               JC = CONN%COL(ICOL)
               IF (JC <= G%NC .AND. G%AGGREGATES(JC) == 0) G%AGGREGATES(JC) = G%N_AGGREGATES
            ENDDO
            G%N_AGGREGATES = G%N_AGGREGATES + 1
...
#ifdef WITH_SCARC_DEBUG
         WRITE(MSG%LU_DEBUG,*) 'SETUP_AGGREGATES_PASS3: =============  ', NAGG_LOCAL
         WRITE(MSG%LU_DEBUG,CFORM_INT) G%AGGREGATES(1:G%NC)
         WRITE(MSG%LU_DEBUG,*) '-------------- overlap:'
         WRITE(MSG%LU_DEBUG,'(10I4)') G%AGGREGATES(G%NC+1: G%NCE)
#endif
   
         ! Set number of coarse and fine grid cells on current AMG level 
         G%N_COARSE = G%N_AGGREGATES
         GC%N_FINE   = G%N_AGGREGATES
         AGGREGATES_GLOBAL(CURRENT_AGGREGATE) = G%N_AGGREGATES
      
         IF (NM == 1) THEN
            GC%NC_LOCAL(NM) = G%N_AGGREGATES - N_AGGREGATES_PREV 
         ELSE
            GC%NC_LOCAL(NM) = G%N_AGGREGATES - N_AGGREGATES_PREV + 1
         ENDIF
 
         ! make aggregates consistent along interfaces and broadcast global aggregate numbers
         IF (NMESHES > 1) CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_AGGREGATES, NSCARC_AGGREGATE_INITIAL, NL)
         IF (N_MPI_PROCESSES>1) &
            CALL MPI_ALLGATHERV(MPI_IN_PLACE,1,MPI_INTEGER,AGGREGATES_GLOBAL,COUNTS,DISPLS, MPI_INTEGER,MPI_COMM_WORLD,IERROR)
...
#ifdef WITH_SCARC_DEBUG
         WRITE(MSG%LU_DEBUG,*) 'FINAL: G%N_AGGREGATES  =',G%N_AGGREGATES
         WRITE(MSG%LU_DEBUG,*) 'AGGREGATES_GLOBAL - END  = ', AGGREGATES_GLOBAL
         WRITE(MSG%LU_DEBUG,*) 'FINAL: N_AGGREGATES_PREV  =',N_AGGREGATES_PREV
         WRITE(MSG%LU_DEBUG,*) 'FINAL: G%N_FINE  =',G%N_FINE
         WRITE(MSG%LU_DEBUG,*) 'FINAL: G%N_COARSE=',G%N_COARSE
         WRITE(MSG%LU_DEBUG,*) 'FINAL: GC%N_FINE  =',GC%N_FINE
         WRITE(MSG%LU_DEBUG,*) 'FINAL: GC%NC_LOCAL(NM)  =',GC%NC_LOCAL(NM)
         WRITE(MSG%LU_DEBUG,*) 'C:----------> NM=',NM,': NAGG:', NAGG,': CURRENT_AGGREGATER=',CURRENT_AGGREGATE,&
                               ': AGGREGATES_GLOBAL:',AGGREGATES_GLOBAL
         WRITE(MSG%LU_DEBUG,*) 'CURRENT_AGGREGATE = ', CURRENT_AGGREGATE
         WRITE(MSG%LU_DEBUG,CFORM_INT) G%AGGREGATES(1:G%NC)
         WRITE(MSG%LU_DEBUG,*) '-------------- overlap:'
         WRITE(MSG%LU_DEBUG,'(10I4)') G%AGGREGATES(G%NC+1: G%NCE)
         ...
         WRITE(MSG%LU_DEBUG,*) 'CURRENT_AGGREGATE = ', CURRENT_AGGREGATE
         WRITE(MSG%LU_DEBUG,*) 'AGGREGATES_GLOBAL - END  = ', AGGREGATES_GLOBAL
         WRITE(MSG%LU_DEBUG,*) ' END END END END END END END END END END END END END END END END END END'
#endif
   
      ENDIF CURRENT_MESH_IF
   ENDDO AGGREGATION_LOOP
ENDDO MESHES_LOOP1

MESHES_LOOP2: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
      CALL SCARC_POINT_TO_GRID_DOUBLE(NM, NL+1)
!!!!!!!!!!!!! ONLY TEMPORARILY FOR DEBUGGING
WRITE(*,*) TRIM(CHID)
IF (TRIM(CHID) == 'b1') THEN
   WRITE(MSG%LU_DEBUG,*) 'CAUTION: PRESETTING AGG'
   G%N_COARSE = 6
   GC%N_FINE = 6
   G%AGGREGATES = 6
   GC%NC_LOCAL(1) = 6
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'AGGREGATES:SZSU:'
   WRITE(MSG%LU_DEBUG,CFORM_INT) G%AGGREGATES(1:G%NC)
   WRITE(MSG%LU_DEBUG,*) 'CPOINTS:'
   WRITE(MSG%LU_DEBUG,CFORM_INT) G%CPOINTS
   WRITE(MSG%LU_DEBUG,*) 'FINAL0: G%N_AGGREGATES  =',G%N_AGGREGATES
   WRITE(MSG%LU_DEBUG,*) 'FINAL0: G%N_FINE  =',G%N_FINE
   WRITE(MSG%LU_DEBUG,*) 'FINAL0: G%N_COARSE=',G%N_COARSE
   WRITE(MSG%LU_DEBUG,*) 'FINAL0: GC%N_FINE  =',GC%N_FINE
#endif
ENDIF
!!!!!!!!!!!!! ONLY TEMPORARILY FOR DEBUGGING

   ! reduce size of CPOINTS-array to real number of aggretates
   CALL SCARC_RESIZE_INT1(G%CPOINTS, G%N_AGGREGATES, 'G%CPOINTS')     
...
! Broadcast number of local mesh cells on level NL to all and build global sum
IF (N_MPI_PROCESSES > 1) &
   CALL MPI_ALLGATHERV(MPI_IN_PLACE,1,MPI_INTEGER,GC%NC_LOCAL,COUNTS,DISPLS, MPI_INTEGER,MPI_COMM_WORLD,IERROR)

GC%NC_GLOBAL = SUM(GC%NC_LOCAL(1:NMESHES))
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'G%NC_LOCAL=', GC%NC_LOCAL
   WRITE(MSG%LU_DEBUG,*) 'G%NC_GLOBAL=', GC%NC_GLOBAL
#endif

! Store information on local and global cells numbers on data structure of corresponding discretization type
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX

   CALL SCARC_POINT_TO_GRID_DOUBLE(NM, NL, NL+1)

   DO IC = G%NC+1, G%NCE
      G%AGGREGATES(IC) = ABS(G%AGGREGATES(IC))
   ENDDO
   !IF (MINVAL(G%AGGREGATES) <= 0) CALL SCARC_SHUTDOWN(NSCARC_ERROR_AGGREGATION, SCARC_NONE, -1)
...
#ifdef WITH_SCARC_DEBUG
   CALL SCARC_BLENDER_AGGREGATES0(NM, NL, 'Agg')
   CALL SCARC_BLENDER_AGGREGATES(NM, NL, 'Aggregates')
#endif
ENDDO
...
! ------------------------------------------------------------------------------------------------------
! Aggregation of internal cells
! ------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_AGGREGATES_INTERNAL(IC)
USE SCARC_POINTERS, ONLY: CONN
...
DO ICOL = CONN%ROW(IC), CONN%ROW(IC+1)-1 
   JC = CONN%COL(ICOL)
   IF (IC /= JC .AND. JC <= G%NC) THEN
      HAS_NEIGHBORS = .TRUE.
      IF (G%AGGREGATES(JC) /= 0) THEN
...
ELSE IF (.NOT. HAS_AGGREGATED_NEIGHBORS) THEN
   ...
   DO ICOL = CONN%ROW(IC), CONN%ROW(IC+1)-1 
      JC = CONN%COL(ICOL)
      IF (JC <= G%NC) G%AGGREGATES(CONN%COL(ICOL)) = G%N_AGGREGATES
   ENDDO
   G%N_AGGREGATES = G%N_AGGREGATES + 1
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   WRITE(MSG%LU_DEBUG,*) 'AF%ROW:'
   WRITE(MSG%LU_DEBUG,'(8I8)') AF%ROW
   WRITE(MSG%LU_DEBUG,*) 'AF%COL:'
   DO IC = 1, G%NC
      WRITE(MSG%LU_DEBUG,'(16I8)') (AF%COL(ICOL), ICOL=AF%ROW(IC), AF%ROW(IC+1)-1)
   ENDDO
   WRITE(MSG%LU_DEBUG,*) 'AF%VAL:'
   DO IC = 1, G%NC
      WRITE(MSG%LU_DEBUG,'(16E12.4)') (AF%VAL(ICOL), ICOL=AF%ROW(IC), AF%ROW(IC+1)-1)
   ENDDO
#endif

   CALL SCARC_ALLOCATE_REAL1 (G%NULLSPACE, 1, G%NCE, NSCARC_INIT_ONE, 'G%NULLSPACE')
   CALL SCARC_ALLOCATE_REAL1 (G%AUX1, 1, G%NCE+1, NSCARC_INIT_ZERO, 'G%AUX1')
   CALL SCARC_ALLOCATE_REAL1 (G%AUX2, 1, G%NCE+1, NSCARC_INIT_ZERO, 'G%AUX2')

   L%MG%OMEGA = OMEGA/APPROX_SPECTRAL_RADIUS
   G%AUX1 = G%NULLSPACE
   
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'G%AUX1:'
   WRITE(MSG%LU_DEBUG,'(7E12.4)') G%AUX1
#endif
   ...
   DO IC = 1, G%NC
      ...
      IDIAG = AF%ROW(IC)                                                
      JC = AF%COL(IDIAG)
      G%AUX2(IC) = AF%VAL(IDIAG)* G%AUX1(JC)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A, 3I4,3E12.4)') 'NULLSPACE: IC; IDIAG, JC, AUX1=', &
                                          IC, IDIAG, JC, AF%VAL(IDIAG), G%AUX1(JC), G%AUX2(IC)
#endif
   
      DO ICOL = AF%ROW(IC)+1, AF%ROW(IC+1)-1                          
         JC = AF%COL(ICOL)
         G%AUX2(IC) =  G%AUX2(IC) + AF%VAL(ICOL) * G%AUX1(JC)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A, 3I4,3E12.4)') 'NULLSPACE: IC; IDIAG, JC, AUX1=', &
                                          IC, IDIAG, JC, AF%VAL(IDIAG), G%AUX1(JC), G%AUX2(IC)
#endif
   ...
   IF (NMESHES > 1 .AND. NM == 1) THEN
      WRITE(MSG%LU_DEBUG,'(7E12.4)') G%AUX2
   ELSE IF (NMESHES > 1 .AND. NM == 2) THEN
      WRITE(MSG%LU_DEBUG,'(6E12.4)') G%AUX2
   ELSE
      WRITE(MSG%LU_DEBUG,'(13E12.4)') G%AUX2
   ENDIF
#endif
   
   ! scale it by omega and inverse of diagonal:    d = omega D^{-1} d
   DO IC = 1, G%NC
      G%AUX2(IC) = L%MG%OMEGA * G%DIAG(IC) * G%AUX2(IC)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A, I4,3E12.4)') 'DIAG: IC =', IC, L%MG%OMEGA, G%DIAG(IC), G%AUX2(IC)
#endif
   ENDDO
   
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'RELAX_NULLSPACE:B: AUX2:'
   WRITE(MSG%LU_DEBUG,CFORM_REAL) G%AUX2
#endif
   ...
   WRITE(MSG%LU_DEBUG,CFORM_REAL) G%AUX1(1: G%NCE)
   WRITE(MSG%LU_DEBUG,*) 'RELAX_NULLSPACE: AUX2:'
   WRITE(MSG%LU_DEBUG,CFORM_REAL) G%AUX2(1: G%NCE)
   WRITE(MSG%LU_DEBUG,*) 'RELAX_NULLSPACE: DIAG:'
   WRITE(MSG%LU_DEBUG,CFORM_REAL) G%DIAG(1: G%NCE)
   WRITE(MSG%LU_DEBUG,*) 'RELAX_NULLSPACE: NULLSPACE:'
   WRITE(MSG%LU_DEBUG,CFORM_REAL) G%NULLSPACE(1: G%NCE)
#endif
...
USE SCARC_POINTERS, ONLY: L, OL, G, OG, F, A, SAMG
INTEGER, INTENT(IN) :: NL
INTEGER :: ICC_GLOBAL, ICC, IC, IP, NM, N_ROW, N_VAL, ICC1, ICC2, IFACE, INBR, IOR0, NOM, ICG, ICE
INTEGER :: IAGG_LOCAL, IAGG_GLOBAL, IAGG_MAX
LOGICAL :: IS_INCLUDED 
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_POINT_TO_GRID (NM, NL)
   A    => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_POISSON)
   SAMG => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_SAMG)

   ! first use very conservative bounds for the size of the aggretate operator matrix 
   ! reduce it later once the real size is known
   SAMG%N_VAL = A%N_VAL                  
   SAMG%N_ROW = A%N_VAL                  
   CALL SCARC_ALLOCATE_MATRIX_COMPACT(SAMG, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'G%SAMG')
  
   ! Again conservative upper bound for length - to be reduced later
   CALL SCARC_ALLOCATE_INT1(G%SAMG_LOCAL, 1,  G%NCE, NSCARC_INIT_ZERO, 'G%SAMG_LOCAL')
   CALL SCARC_ALLOCATE_INT1(GC%LAGG_TO_GAGG, 1, G%NCE, NSCARC_INIT_ZERO, 'GC%LAGG_TO_GAGG')

#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) '=================== SETUP_AGGREGATE_OPERATOR:'
   WRITE(MSG%LU_DEBUG,*) 'ORIG: SAMG%ROW:', SAMG%N_ROW, SAMG%N_VAL, SIZE(SAMG%ROW), SIZE(SAMG%VAL)
#endif

   IF (NM == 1) THEN
      ICC1 = 1 
   ELSE
      ICC1 = GC%NC_LOCAL(NM-1) + 1 
   ENDIF
   ICC2 = ICC1 + GC%NC_LOCAL(NM) - 1

   ICC1 = 1
   ICC2 = GC%NC_GLOBAL

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_AGGREGATE_OPERATOR: GC%NC_LOCAL:', GC%NC_LOCAL
WRITE(MSG%LU_DEBUG,*) 'SETUP_AGGREGATE_OPERATOR: ICC1, ICC2:', ICC1, ICC2
WRITE(MSG%LU_DEBUG,*) 'SETUP_AGGREGATE_OPERATOR: N_FINE, NC, NCE:', G%N_FINE, G%NC, G%NCE
#endif

   IP = 1
   ICC = 1
   SAMG%ROW(ICC) = 1
   DO ICC_GLOBAL = ICC1, ICC2

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3I4)') 'SETUP_AGGREGATE_OPERATOR: ICC, N_COARSE, NC_OFFSET(NM)=',ICC, G%N_COARSE, GC%NC_OFFSET(NM)
#endif
      IS_INCLUDED = .FALSE.
      DO IC = 1, G%N_FINE
         IF (G%AGGREGATES(IC) /= ICC_GLOBAL) CYCLE
         SAMG_LOCAL(IC) = ICC
         IS_INCLUDED = .TRUE.
         SAMG%COL(IP) = IC
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5I4)') '         bingo, IC, IP, ICC_GLOBAL, ICC, IS_INCLUDED:', IC, IP, ICC_GLOBAL, ICC, SAMG_LOCAL(IC)
#endif
         IP = IP + 1
      ENDDO
      IF (IS_INCLUDED) THEN
   ...
   ! TODO resize aggregation matrices !!!
   SAMG%N_ROW = N_ROW
   SAMG%N_VAL = N_VAL

   ! Get mapping from global to local aggregate numbers
   IAGG_MAX = 0
   DO IC = 1, G%NCE
      IAGG_GLOBAL = G%AGGREGATES(IC)              ! This is the global aggregate number for cell IC
      IAGG_LOCAL  = SAMG_LOCAL(IC)               ! This is the local  aggregate number for cell IC
      IAGG_MAX = MAX(IAGG_LOCAL, IAGG_MAX)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'MAPPING: IC, IAGG_GLOBAL, IAGG_LOCAL, IAGG_MAX:', IC, IAGG_GLOBAL, IAGG_LOCAL, IAGG_MAX
#endif
      IF (GC%LAGG_TO_GAGG(IAGG_LOCAL) /= 0) THEN
         IF (GC%LAGG_TO_GAGG(IAGG_LOCAL) /= IAGG_GLOBAL) WRITE(*,*) 'ERROR IN MAPPING OF GAGG TO IAGG FOR CELL ', IC
      ELSE
         GC%LAGG_TO_GAGG(IAGG_LOCAL) = IAGG_GLOBAL
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) ' ---------> LAGG_TO_GAGG(',IAGG_LOCAL,')=', IAGG_GLOBAL
#endif
      ENDIF
   ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SAMG%N_ROW=',SAMG%N_ROW
WRITE(MSG%LU_DEBUG,*) 'SAMG%N_VAL=',SAMG%N_VAL
CALL SCARC_DEBUG_MATRIX (SAMG, 'AGG','AFTER SETUP AGGREGATE OPERATOR')
WRITE(MSG%LU_DEBUG,*) 'AGGREGATES:'
WRITE(MSG%LU_DEBUG,'(8I6)') G%AGGREGATES(1:G%NCE)
WRITE(MSG%LU_DEBUG,*) 'AGG_LOCAL:'
WRITE(MSG%LU_DEBUG,'(8I6)') G%SAMG_LOCAL(1:G%NCE)
WRITE(MSG%LU_DEBUG,*) 'LAGG_TO_GAGG:'
WRITE(MSG%LU_DEBUG,'(8I6)') GC%LAGG_TO_GAGG(1:G%NCE)
#endif
   ...
   DO IFACE = 1, 6                         
      ...
      DO INBR = 1, FF%N_NEIGHBORS
         ...
         IF (NOM /= NMESHES) CYCLE         
         CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)

         OGF => SCARC(NM)%OSCARC(NOM)%LEVEL(NL)%STRUCTURED

         CALL SCARC_ALLOCATE_INT1(OG%ICG_TO_GAGG,  OL%NCG0(IOR0), OL%NCG(IOR0), NSCARC_INIT_ZERO, 'OG%ICG_TO_GAGG')
         CALL SCARC_ALLOCATE_INT1(OG%ICG_TO_LAGG,  OL%NCG0(IOR0), OL%NCG(IOR0), NSCARC_INIT_ZERO, 'OG%ICG_TO_LAGG')

         DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
            ICE = OG%ICG_TO_ICE(ICG)
            OG%ICG_TO_GAGG(ICG) = G%AGGREGATES(ICE)
            OG%ICG_TO_LAGG(ICG) = G%SAMG_LOCAL(ICE)
         ENDDO
CALL SCARC_DEBUG_INT1(OG%ICG_TO_GAGG, OL%NCG0(IOR0), OL%NCG(IOR0), 'ICG_TO_GAGG', 'SETUP_AGGREGATE_OPERATOR')
CALL SCARC_DEBUG_INT1(OG%ICG_TO_LAGG, OL%NCG0(IOR0), OL%NCG(IOR0), 'ICG_TO_LAGG', 'SETUP_AGGREGATE_OPERATOR')
      ENDDO
   ENDDO

ENDDO

END SUBROUTINE SCARC_SETUP_AGGREGATE_OPERATOR
...
USE SCARC_POINTERS, ONLY:  G, A, OA, PROL, OPROL
INTEGER, INTENT(IN) :: NL
REAL(EB):: DSUM, SCAL, APPROX_SPECTRAL_RADIUS, PSAVE !, TOL = 1.0E-12_EB
INTEGER :: IC, JC, ICC, ICOL, ICCOL, JCCOL, IP, JJC, IQ, NM, INBR, NOM, IFACE, ICE, ICG, IOR0

!APPROX_SPECTRAL_RADIUS = 1.9501727966593339_EB          ! 4x4 case
!APPROX_SPECTRAL_RADIUS = 1.9885080677405649_EB          ! 8x8 case
!APPROX_SPECTRAL_RADIUS = 1.9889170247464196_EB          ! 8x8
APPROX_SPECTRAL_RADIUS = 2.0_EB                          ! current default value

DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX

   CALL SCARC_POINT_TO_GRID_DOUBLE (NM, NL)
   A    => SCARC_POINT_TO_MATRIX_COMPACT(NSCARC_MATRIX_POISSON)
   PROL => SCARC_POINT_TO_MATRIX_COMPACT(NSCARC_MATRIX_PROLONGATION)

   ! Allocate prolongation matrix - again a conservative bound for the size (according to poisson matrix) is used
   ! This will be reduced to real size once this is known
   PROL%N_VAL = A%N_VAL        
   PROL%N_ROW = G%NCE + 1                  
   CALL SCARC_ALLOCATE_MATRIX_COMPACT(PROL, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'G%PROLONGATION')

   ! For every neighbor allocate a small prolongation matrix on the corresponding overlapping part 
   DO INBR = 1, SCARC(NM)%N_NEIGHBORS
      NOM = S%NEIGHBORS(INBR)
      CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
      OA    => SCARC_POINT_TO_MATRIX_COMPACT(NSCARC_MATRIX_POISSON)
      OPROL => SCARC_POINT_TO_MATRIX_COMPACT(NSCARC_MATRIX_PROLONGATION)
      OPROL%N_VAL = A%N_VAL              ! TODO : CHECK : TOO BIG !!!
      OPROL%N_ROW = G%NCE + 1            ! TODO : CHECK : TOO BIG !!!
      CALL SCARC_ALLOCATE_MATRIX_COMPACT(OPROL, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'OG%PROLONGATION')
   ENDDO

   ! Allocate auxiliary workspace for QR-decomposition of aggregate operator
   CALL SCARC_ALLOCATE_REAL1(G%Q, 1, G%N_FINE+1, NSCARC_INIT_ZERO, 'G%Q')
   CALL SCARC_ALLOCATE_REAL1(G%R, 1, G%N_FINE+1, NSCARC_INIT_ZERO, 'G%R')     ! TODO check length!

#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'SETUP_PROLONGATION: G%N_COARSE = ', G%N_COARSE, G%SAMG%N_ROW-1
#endif

   SAMG => SCARC_POINT_TO_MATRIX_COMPACT(NSCARC_MATRIX_SAMG)

   ! First step: Copy blocks into Q according to aggregates and compute norms for single aggregates
   IQ = 1
   G%AUX1 = 0.0_EB
   DO ICC = 1, SAMG%N_ROW-1
   
      DSUM = 0.0_EB
      DO ICOL = SAMG%ROW(ICC), SAMG%ROW(ICC+1)-1
         IC = SAMG%COL(ICOL)
         G%Q(IQ) = G%NULLSPACE(IC)
         DSUM = DSUM + G%Q(IQ)**2
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A,4I5,E12.4)') 'SETUP_PROL: --- ICC, ICOL, IC, IQ, G%Q(IC):', ICC, ICOL, IC, IQ, G%Q(IQ)
#endif
         IQ = IQ + 1
      ENDDO
      DO ICOL = SAMG%ROW(ICC), SAMG%ROW(ICC+1)-1
         G%AUX1(SAMG%COL(ICOL)) = DSUM
      ENDDO
      G%R(ICC) = DSUM
   
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'A:   G%R(',ICC,')=',G%R(ICC)
#endif 
   
   ENDDO
ENDDO

! Exchange sums of nullspace entries within single aggregates
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'G%AUX1:'
WRITE(MSG%LU_DEBUG,CFORM_REAL) G%AUX1
#endif

   DO ICC = 1, SAMG%N_ROW-1
      ICOL = SAMG%ROW(ICC)
      IC = SAMG%COL(ICOL)
      !G%R(ICC) = SQRT(G%R(ICC))
      G%R(ICC) = SQRT(G%AUX1(IC))
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'B:   G%R(',ICC,')=',G%R(ICC)
#endif 
   ENDDO
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_POINT_TO_GRID_DOUBLE (NM, NL, NL+1)
   ! Scale entries by norms 
   IQ = 1
   DO ICC = 1, SAMG%N_ROW-1
      DO ICOL = SAMG%ROW(ICC), SAMG%ROW(ICC+1)-1
         IC = SAMG%COL(ICOL)
         G%Q(IQ) = G%Q(IQ)/G%R(ICC)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A,3I5,E12.4)') '--- ICC, ICOL, IC, G%Q(IC):', ICC, ICOL, IC, G%Q(IQ)
#endif
         IQ = IQ + 1
      ENDDO
   ENDDO
   ...
   IF (NL < NLEVEL_MAX) THEN
      GC%N_FINE = G%N_COARSE
      GC%NC = GC%N_FINE
      GC%NCE = SAMG%N_ROW-1
      CALL SCARC_ALLOCATE_REAL1(GC%NULLSPACE, 1, GC%NCE, NSCARC_INIT_ZERO, 'GC%NULLSPACE')
      GC%NULLSPACE(1:GC%NCE) = G%R(1:GC%NCE)
      CALL SCARC_RESIZE_INT1(GC%LAGG_TO_GAGG, GC%NCE, 'GC%LAGG_TO_GAGG')
   ENDIF

#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'G%NULLSPACE:'
   WRITE(MSG%LU_DEBUG,'(8E12.4)') G%NULLSPACE
   WRITE(MSG%LU_DEBUG,*) 'G%R'
   WRITE(MSG%LU_DEBUG,'(8E12.4)') G%R
   WRITE(MSG%LU_DEBUG,*) 'G%Q:'
   WRITE(MSG%LU_DEBUG,'(8E12.4)') G%Q
   ...
   ! for testing purposes:   rho ~2.0,   


   LF%MG%OMEGA = 4.0_EB/3.0_EB
   SCAL = LF%MG%OMEGA/APPROX_SPECTRAL_RADIUS
   ...
   WRITE(MSG%LU_DEBUG,'(8E12.4)') G%Q
   WRITE(MSG%LU_DEBUG,*) 'G%N_FINE  =',G%N_FINE
   WRITE(MSG%LU_DEBUG,*) 'G%N_COARSE=',G%N_COARSE
   WRITE(MSG%LU_DEBUG,*) 'GC%LAGG_TO_GAGG=',GC%LAGG_TO_GAGG
#endif
CALL SCARC_DEBUG_MATRIX(SAMG, 'AGG','BEFORE FIRST RELAX STEP')
   ...
   DO IC = 1, G%NC
      DO ICC = 1, SAMG%N_ROW-1
         ...
         DSUM = 0.0_EB
         DO ICCOL = SAMG%ROW(ICC), SAMG%ROW(ICC+1)-1
            JC = SAMG%COL(ICCOL)
            ICOL = SCARC_MATCH_MATRIX_COLUMN(IC, JC)
            IF (ICOL /= -1) DSUM = DSUM - SCAL * G%DIAG(IC) * AF%VAL(ICOL) * G%Q(ICCOL)
         ENDDO
   
         IF (ABS(DSUM) /= 0.0_EB) THEN
            PROL%VAL(IP) = DSUM
            PROL%COL(IP) = ICC
            PROL%COL_MKL(IP) = GC%LAGG_TO_GAGG(ICC)
            IP = IP + 1
         ENDIF
...
CALL SCARC_DEBUG_MATRIX(SAMG, 'AGG','AFTER FIRST RELAX STEP')
CALL SCARC_DEBUG_MATRIX(G%PROLONGATION, 'PROLONGATION','AFTER FIRST RELAX STEP')
...
CALL SCARC_DEBUG_MATRIX(SAMG, 'AGG','AFTER EXCHANGE_PROL')
CALL SCARC_DEBUG_MATRIX(G%PROLONGATION, 'PROLONGATION','AFTER EXCHANGE_PROL')
   
   ! Note: IP from above is used 
   DO IFACE = 1, 6                                         ! check if this face has connection to last cell
      ...
      DO INBR = 1, FF%N_NEIGHBORS
         ...
         IF (NOM /= NMESHES) CYCLE                      ! only check for common matrix entries with last mesh
         CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
         OPROL => SCARC_POINT_TO_OTHER_MATRIX_COMPACT (NSCARC_MATRIX_PROLONGATION)

#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A,5I6)') 'IFACE, IOR0, INBR, NOM:', IFACE, IOR0, INBR, NOM, OPROL%N_ROW
WRITE(MSG%LU_DEBUG,*) 'OPROL%ROW:'
WRITE(MSG%LU_DEBUG,'( 8I6)') OPROL%ROW(1:OPROL%N_ROW)
#endif
         ICOL = 1
         DO ICG = OLF%NCG0(IOR0), OLF%NCG(IOR0)
            ...
            ICE = OG%ICG_TO_ICE(ICG)
            PROL%ROW(ICE) = IP 
            !IF (OPROL%COL(ICOL) > 0) THEN
            !   WRITE(*,*) 'CAUTION: ERROR IN RESORTING PROL'
            !   !STOP
            !ENDIF

            DO ICOL = OPROL%ROW(ICG), OPROL%ROW(ICG+1)-1
               PROL%COL_MKL(IP) = ABS(OPROL%COL(ICOL))
               PROL%VAL(IP) = OPROL%VAL(ICOL)
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A,6I6,E12.4)') '       ----> B: ICG, ICE, ICOL, IP, ROW, COL, VAL :', &
                                       ICG, ICE, ICOL, IP, PROL%ROW(ICE), PROL%COL(IP), PROL%VAL(IP)
#endif
               IP = IP + 1
            ENDDO
   ...
   !CALL SCARC_RESIZE_MATRIX_COMPACT(G%PROLONGATION, PROL%N_VAL, PROL%N_ROW, 'PROLONGATION')

CALL SCARC_DEBUG_MATRIX(SAMG, 'AGG','AFTER RESORT PROL ')
CALL SCARC_DEBUG_MATRIX(PROL, 'PROLONGATION','AFTER RESORT PROL ')
...
#endif
         IF (IC > G%NC) CYCLE
         DO JCCOL = PROL%ROW(IC), PROL%ROW(IC+1) - 1
            JJC = PROL%COL(JCCOL)
#ifdef WITH_SCARC_DEBUG2
   WRITE(MSG%LU_DEBUG,'(A, 5I5)') 'Subtracting: ICC, ICCOL, IC, JCCOL, JJC:', ICC, ICCOL, IC, JCCOL, JJC
#endif
            IF (JJC == ICC) THEN
               PSAVE = PROL%VAL(JCCOL)
               PROL%VAL(JCCOL) = PROL%VAL(JCCOL) + G%Q(ICCOL)
#ifdef WITH_SCARC_DEBUG2
   WRITE(MSG%LU_DEBUG,'(A, 3E12.4)') '       PSAVE,Q,PF  ---->', PSAVE, G%Q(ICCOL), PROL%VAL(JCCOL)
#endif
               CYCLE
            ENDIF
         ENDDO
   ...
   DEALLOCATE(G%R, STAT=IERROR)
   DEALLOCATE(G%Q, STAT=IERROR)

   DEALLOCATE (G%AUX1, STAT=IERROR)
   DEALLOCATE (G%AUX2, STAT=IERROR)

CALL SCARC_DEBUG_MATRIX(G%PROLONGATION, 'G%PROLONGATION','End of SETUP_PROLONGATION')
ENDDO
...
USE SCARC_POINTERS, ONLY: REST, PROL, G
INTEGER, INTENT(IN) :: NL
INTEGER :: NM, IC, IRROW, IRCOL, IPCOL, ICC_GLOBAL
LOGICAL :: IS_INCLUDED
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   PROL => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_PROLONGATION)
   REST => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_RESTRICTION)

   REST%N_VAL = PROL%N_VAL + 100
   REST%N_ROW = PROL%N_ROW + 100

#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'RESTRICTION:  N_VAL, N_ROW=', REST%N_VAL, REST%N_ROW 
   WRITE(MSG%LU_DEBUG,*) 'PROLONGATION: N_VAL, N_ROW=', PROL%N_VAL, PROL%N_ROW 
#endif
   CALL SCARC_ALLOCATE_MATRIX_COMPACT(REST, NL, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'G%RESTRICTION')
   ...
   REST%ROW(IRROW) = IRCOL

   DO WHILE (IRROW <= GC%NCE) 

      ICC_GLOBAL = GC%LAGG_TO_GAGG(IRROW)
      IS_INCLUDED = .FALSE.

      DO IC = 1, G%NCE                  ! counter of fine cell (including overlaps)

#ifdef WITH_SCARC_DEBUG2
   WRITE(MSG%LU_DEBUG,*) '================= ICC_GLOBAL, IC = ', ICC_GLOBAL, IC
#endif
         ROW_LOOP: DO IPCOL = PROL%ROW(IC), PROL%ROW(IC+1)-1
            IF (PROL%COL_MKL(IPCOL) == ICC_GLOBAL) THEN
               IS_INCLUDED = .TRUE.
               REST%VAL(IRCOL) = PROL%VAL(IPCOL)
               REST%COL(IRCOL) = IC

#ifdef WITH_SCARC_DEBUG2
   WRITE(MSG%LU_DEBUG,'(A,5I6,E12.4)') '     IC, IRCOL, IPCOL, IRROW, REST%COL, REST%VAL:', &
                                        IC, IRCOL, IPCOL, IRROW, REST%COL(IRCOL), REST%VAL(IRCOL)
#endif
               IRCOL = IRCOL + 1
               EXIT ROW_LOOP
            ENDIF
...
CALL SCARC_DEBUG_MATRIX(REST, 'G%RESTRICTION','AFTER SETUP_RESTRICTION')
...
! ------------------------------------------------------------------------------------------------------
DOUBLE PRECISION FUNCTION SCARC_VALUE_AP(IC, ICC)
USE SCARC_POINTERS, ONLY : GC
INTEGER, INTENT(IN) :: IC, ICC
INTEGER :: JC, IA, IP !, IWG, NOM
REAL(EB) :: DSUM
...
!      NOM = G%WALL(IWG)%NOM 
!   ENDIF
   DO IP = PROL%ROW(JC), PROL%ROW(JC+1) -1
      IF (PROL%COL_MKL(IP) == GC%LAGG_TO_GAGG(ICC)) THEN
         DSUM = DSUM + AF%VAL(IA)*PROL%VAL(IP)
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A,6I4,3E12.4)') 'VALUE_AP: IC, ICC, IA, JC, IP, PROL%COL(IP), DSUM, AF%VAL(IA), PROL%VAL(IP):',&
                       IC, ICC, IA, JC, IP, PROL%COL(IP), DSUM, AF%VAL(IA), PROL%VAL(IP)
#endif
         CYCLE
      ENDIF
   ENDDO
...
USE SCARC_POINTERS, ONLY: SUB, LF, LC, GF, GC, AF, AC, RF, RC, PF, PC
INTEGER, INTENT(IN) :: NL
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   AF   => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_POISSON)
   PROL => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_PROLONGATION)
   REST => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_RESTRICTION)
   LF = L;  L => NULL()
   GF = G;  G => NULL()
   ...
   AC => SCARC_POINT_TO_MATRIX_COMPACT (NSCARC_MATRIX_POISSON)
   LC = L;  L => NULL()
   GC = G;  G => NULL()

   AC%N_ROW = G%N_COARSE+1
   AC%N_VAL = A%N_ROW**2             ! only temporarily TODO TOO BIG
   CALL SCARC_ALLOCATE_MATRIX_COMPACT(AC, NL+1, NSCARC_PRECISION_DOUBLE, NSCARC_INIT_ZERO, 'ACP')
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '------------------------ IPC = ', IPC,' ----------------------------'
WRITE(MSG%LU_DEBUG,*) '    REST%ROW(IRW)-REST%ROW(IRC+1)-1: ', (REST%COL(IR), IR=REST%ROW(IRC),REST%ROW(IRC+1)-1)
WRITE(MSG%LU_DEBUG,*) '--------------------------------------------------------------------'
#endif
      ! Compute entry AC(IRC,IPC) = SUM( R(IRC,IC)*A(IC,:)*P(:,IPC))
      DRSUM = 0.0_EB
      DO IR = REST%ROW(IRC), REST%ROW(IRC+1) - 1
         IC = REST%COL(IR)
         DAP = SCARC_VALUE_AP(IC, IPC)
         DRSUM = DRSUM + DAP*REST%VAL(IR)
#ifdef WITH_SCARC_DEBUG
         WRITE(MSG%LU_DEBUG,'(A,4I4,3E12.4)') 'GAL: IRC, IPC, IC, IR, VAL_AP, REST%VAL, DRSUM:', &
                                               IRC, IPC, IC, IR, DAP, REST%VAL(IR), DRSUM
#endif
      ENDDO
      IF (ABS(DRSUM) > TOL) THEN
         ...
         A%COL(IA) = IPC
#ifdef WITH_MKL
IF (IS_MKL_LEVEL(NL+1)) A%COL_MKL(IA) = A%COL(IA) + GC%NC_OFFSET(NM)
#endif
         IA = IA + 1
#ifdef WITH_SCARC_DEBUG
         WRITE(MSG%LU_DEBUG,'(A,6I5,E12.4)') 'GALERKIN: IPC, IRC, IR, IC, IA, DRSUM:',&
                                              IPC,IRC,IR,IC,IA,A%COL(IA),A%VAL(IA)
#endif
      ENDIF

      ! Then process remaining entries in row
      DO IPC = 1, G%N_COARSE                   ! columns of prolongation matrix
...
WRITE(MSG%LU_DEBUG,*) '======================== IPC = ', IPC,' ============================'
         ...
         DO IR = REST%ROW(IRC), REST%ROW(IRC+1) - 1
            ...
            DRSUM = DRSUM + DAP*REST%VAL(IR)
#ifdef WITH_SCARC_DEBUG
            WRITE(MSG%LU_DEBUG,'(A,4I4,3E12.4)') 'GAL: IRC, IPC, IR, IC, VAL_AP, REST%VAL, DRSUM:', &
                                                  IRC, IPC, IR, IC, DAP, REST%VAL(IR), DRSUM
#endif
         ENDDO
         IF (ABS(DRSUM) > TOL) THEN
            A%VAL(IA) = DRSUM
      ...
      A%ROW(IRC+1) = IA
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'A%ROW(',IRC+1,')=', A%ROW(IRC+1)
#endif
   ENDDO

   ! temporarily
   A%COL_MKL = A%COL
   A%N_STENCIL = AF%N_STENCIL

   GC%NC = A%N_ROW - 1

#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'GALERKIN: A%ROW'
   WRITE(MSG%LU_DEBUG,*) 'A%ROW:', A%N_ROW, A%N_VAL
   WRITE(MSG%LU_DEBUG,'(8I8)') A%ROW
   WRITE(MSG%LU_DEBUG,*) 'A%COL:'
   DO IC = 1, A%N_ROW-1
...
! ------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_DEBUG_MATRIX(A, CNAME, CTEXT)
CHARACTER(*), INTENT(IN) :: CNAME, CTEXT
TYPE (SCARC_MATRIX_COMPACT_TYPE), INTENT(INOUT) :: A      
INTEGER :: IC, ICOL
...
IF (A%N_ROW /= SIZE(A%ROW)) WRITE(MSG%LU_DEBUG,*) &
      '!!!!!!!!!!!!!!!! C-A-U-T-I-O-N : different sizes for ',TRIM(CNAME),'%N_ROW !!!!!!!!!!!!!!!! '
...
DO IC = 1, A%N_ROW-1
   ...
   !WRITE(MSG%LU_DEBUG,*) IC,':', (A%COL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
ENDDO
IF (ALLOCATED(A%COL_MKL)) THEN
   WRITE(MSG%LU_DEBUG,*)
   WRITE(MSG%LU_DEBUG,*) "------------->", TRIM(CNAME),'%COL_MKL:'
   DO IC = 1, A%N_ROW-1
      IF (A%ROW(IC) == 0) CYCLE
      WRITE(MSG%LU_DEBUG,'(I4,A,16I6)') IC,':', (A%COL_MKL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
      !WRITE(MSG%LU_DEBUG,*) IC,':', (A%COL_MKL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
   ENDDO
ENDIF
WRITE(MSG%LU_DEBUG,*)
...
DO IC = 1, A%N_ROW-1
   IF (A%ROW(IC) == 0) CYCLE
   WRITE(MSG%LU_DEBUG,'(I4,A,16E14.4)') IC,':', (A%VAL(ICOL), ICOL=A%ROW(IC), A%ROW(IC+1)-1)
...
END SUBROUTINE SCARC_DEBUG_MATRIX
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   IF (IS_AMG .AND. NL > NLEVEL_MIN) THEN
      WRITE(MSG%LU_DEBUG, '(4E14.6)') (VC(IC), IC = 1, G%N_FINE)
...
SELECT CASE (NTYPE)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         DO IC=1,G%NC
            DO IP=AS%ROW(IC),AS%ROW(IC+1)-1
               WRITE(MSG%LU_DEBUG,*) IC,AS%COL(IP),AS%VAL(IP)
            ENDDO
            WRITE(MSG%LU_DEBUG,*)
         ENDDO
   ...
   CASE (NSCARC_DEBUG_CONNECTION)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         WRITE(MSG%LU_DEBUG,*) 'G%NC=', G%NC
         WRITE(MSG%LU_DEBUG,*) 'CONN%ROW(G%NC)=', CONN%ROW(G%NC)
         WRITE(MSG%LU_DEBUG,*) 'CONN%ROW(G%NC+1)=', CONN%ROW(G%NC+1)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(SVAL) =',SIZE(CONN%VAL)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(SCOL) =',SIZE(CONN%COL)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(SROW) =',SIZE(CONN%ROW)
         WRITE(MSG%LU_DEBUG,*) '---------------------- SROW:', G%NC
         WRITE(MSG%LU_DEBUG,*) (CONN%ROW(IC), IC=1,G%NC+1)
         WRITE(MSG%LU_DEBUG,*) '---------------------- SCOL:'
         DO IC = 1, G%NC
            WRITE(MSG%LU_DEBUG,*) IC,':',(CONN%COL(IP),IP=CONN%ROW(IC),CONN%ROW(IC+1)-1)
         ENDDO
         DO IC = 1, G%NC
            WRITE(MSG%LU_DEBUG,*) IC,':',(CONN%VAL(IP),IP=CONN%ROW(IC),CONN%ROW(IC+1)-1)
         ENDDO
   ...
   CASE (NSCARC_DEBUG_PROLONGATION)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         WRITE(MSG%LU_DEBUG,*) 'G%NC=', G%NC
         WRITE(MSG%LU_DEBUG,*) 'PROL%ROW(G%NC)=', PROL%ROW(G%NC)
         WRITE(MSG%LU_DEBUG,*) 'PROL%ROW(G%NC+1)=', PROL%ROW(G%NC+1)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(PVAL) =',SIZE(PROL%VAL)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(PCOL) =',SIZE(PROL%COL)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(PROW) =',SIZE(PROL%ROW)
         WRITE(MSG%LU_DEBUG,*) '---------------------- PROW:', G%NC
         WRITE(MSG%LU_DEBUG,*) (PROL%ROW(IC), IC=1,G%NC+1)
         WRITE(MSG%LU_DEBUG,*) '---------------------- PCOL:'
         DO IC = 1, G%NC
            WRITE(MSG%LU_DEBUG,*) IC,':',(PROL%COL(IP),IP=PROL%ROW(IC),PROL%ROW(IC+1)-1)
         ENDDO
         DO IC = 1, G%NC
            WRITE(MSG%LU_DEBUG,*) IC,':',(PROL%VAL(IP),IP=PROL%ROW(IC),PROL%ROW(IC+1)-1)
         ENDDO
   ...
   CASE (NSCARC_DEBUG_RESTRICTION)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         WRITE(MSG%LU_DEBUG,*) 'G%NC=', G%NC
         WRITE(MSG%LU_DEBUG,*) 'REST%ROW(G%NC)=', REST%ROW(G%NC)
         WRITE(MSG%LU_DEBUG,*) 'REST%ROW(G%NC+1)=', REST%ROW(G%NC+1)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(REST%VAL) =',SIZE(REST%VAL)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(REST%COL) =',SIZE(REST%COL)
         WRITE(MSG%LU_DEBUG,*) 'SIZE(REST%ROW) =',SIZE(REST%ROW)
         WRITE(MSG%LU_DEBUG,*) '---------------------- REST%ROW:', G%NC
         WRITE(MSG%LU_DEBUG,*) (REST%ROW(IC), IC=1,G%NC+1)
         WRITE(MSG%LU_DEBUG,*) '---------------------- REST%COL:'
         DO IC = 1, G%NC
            WRITE(MSG%LU_DEBUG,*) IC,':',(REST%COL(IP),IP=REST%ROW(IC),REST%ROW(IC+1)-1)
         ENDDO
         DO IC = 1, G%NC
            WRITE(MSG%LU_DEBUG,*) IC,':',(REST%VAL(IP),IP=REST%ROW(IC),REST%ROW(IC+1)-1)
         ENDDO
...
WRITE(MSG%LU_DEBUG,*) '          COLUMNS2, STENCIL2:', IC, NLEN, COLUMNS(1:NLEN), STENCIL(1:NLEN)
      ...
      DO I = 1, NLEN
         ICOL = COLUMNS(I)
         WRITE(MCOL,1001, ADVANCE="NO") AF%COL(ICOL)-1
         WRITE(MVAL,1002, ADVANCE="NO") AF%VAL(ICOL)
      ENDDO
      WRITE(MROW,1001, ADVANCE="NO") AF%ROW(IC)-1
...
! Print out matrix information on level NL for matlab
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PYTHON_AGGREGATES(NM, NL, CNAME)
INTEGER, INTENT(IN) :: NM, NL
CHARACTER(*), INTENT(IN) :: CNAME
INTEGER :: IC, MAGG
CHARACTER(60) :: CAGG
...
!WRITE(MAGG, *) '['
DO IC = 1, G%N_FINE-1
   WRITE(MAGG,1001, ADVANCE="NO") G%AGGREGATES(IC)
ENDDO
WRITE(MAGG,1002, ADVANCE="NO") G%AGGREGATES(G%N_FINE)
!WRITE(MAGG, *) ' ]'
CLOSE(MAGG)

1001 FORMAT(I4,',')
1002 FORMAT(I4)
END SUBROUTINE SCARC_PYTHON_AGGREGATES


! ------------------------------------------------------------------------------------------------
! Print out matrix information on level NL for matlab
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_BLENDER_AGGREGATES(NM, NL, CNAME)
USE SCARC_POINTERS, ONLY: M, L, OL, G, OG, F, XCOR, YCOR, ZCOR
INTEGER, INTENT(IN) :: NM, NL
CHARACTER(*), INTENT(IN) :: CNAME
REAL(EB) :: INCX, INCY, INCZ, XCOR0, YCOR0, ZCOR0
INTEGER :: IC, IAGG, ICPT, MAGG, II, JJ, KK, IFACE, IOR0, NOM, ICG, INBR, ICW, ICE
CHARACTER(60) :: CAGG
   ...
   DO JJ = 1, L%NY
      DO II=1, L%NX
         ...
         !ELSE
            ...
            ENDIF
            IF (IC == ICPT) THEN
               WRITE(MAGG,1000) IC, -IAGG, XCOR0, YCOR0, ZCOR0
            ELSE
               WRITE(MAGG,1000) IC,  IAGG, XCOR0, YCOR0, ZCOR0
            ENDIF
         !ENDIF
...
DO IFACE = 1, 6                                  
   ...
   DO INBR = 1, F%N_NEIGHBORS
      ...
      DO ICG = OL%NCG0(IOR0), OL%NCG(IOR0)
         ...
         KK = G%ICZ(ICW) 
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'PY: ICG, ICW, ICE, II, JJ, KK:', ICG, ICW, ICE, II, JJ, KK
#endif
         ...
         ENDIF
         IAGG = ABS(G%AGGREGATES(ICE))
         WRITE(MAGG,1000) ICE, IAGG, XCOR0, YCOR0, ZCOR0
      ENDDO
...
END SUBROUTINE SCARC_BLENDER_AGGREGATES
