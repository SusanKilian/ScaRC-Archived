...
INTEGER, PARAMETER :: NSCARC_ERROR_AMG_DISABLED      =  1, &    !> AMG-method is currently missing
                      ...
                      NSCARC_ERROR_LEVEL_MAX         = 15, &    !> error with grid resolution
                      NSCARC_ERROR_NEIGHBOR_NUMBER   = 16, &    !> wrong neighbor number
                      NSCARC_ERROR_NEIGHBOR_TYPE     = 17, &    !> wrong neighbor type
                      NSCARC_ERROR_MATRIX_ALLOCATION = 18, &    !> error in matrix allocation
                      NSCARC_ERROR_MATRIX_COPY       = 19, &    !> subdiagonal missing
                      NSCARC_ERROR_MATRIX_SETUP      = 20, &    !> error in matrix setup
                      NSCARC_ERROR_MATRIX_SIZE       = 21, &    !> error in matrix size
                      NSCARC_ERROR_MATRIX_SUBDIAG    = 22, &    !> subdiagonal missing
                      NSCARC_ERROR_MATRIX_SYMMETRY   = 23, &    !> matrix not symmetric
                      NSCARC_ERROR_MKL_CLUSTER       = 24, &    !> cluster_sparse_solver not available
                      NSCARC_ERROR_MKL_INTERNAL      = 25, &    !> pardiso solver not available
                      NSCARC_ERROR_MKL_PARDISO       = 26, &    !> pardiso solver not available
                      NSCARC_ERROR_MKL_STORAGE       = 27, &    !> wrong storage scheme for MKL-solvers
                      NSCARC_ERROR_MULTIGRID_LEVEL   = 28, &    !> wrong multigrid level
                      NSCARC_ERROR_PARSE_INPUT       = 29, &    !> wrong input parameter
                      NSCARC_ERROR_POC_STOP          = 30, &    !> temporary stop, only proof of concept
                      NSCARC_ERROR_STACK_MESSAGE     = 31, &    !> error with stack message
                      NSCARC_ERROR_STACK_SOLVER      = 32, &    !> error in solver stack
                      NSCARC_ERROR_STENCIL           = 33, &    !> error in matrix stencil
                      NSCARC_ERROR_RHS_SETUP         = 34, &    !> error with rhs setup
                      NSCARC_ERROR_VECTOR_LENGTH     = 35       !> error in vector length
...
INTEGER, PARAMETER :: NSCARC_EXCHANGE_FDS            =  1, &    !> use exchange information from surrounding FDS code
                      ...
                      NSCARC_EXCHANGE_MATRIX_MKL_SIZE= 12, &    !> size of MKL matrix 
                      NSCARC_EXCHANGE_MATRIX_MKL_VAL = 13       !> values of MKL matrix
...
INTEGER, PARAMETER :: NSCARC_MODE_MASTER             =  1, &    !> working mode is master
                      NSCARC_MODE_SLAVE              =  2       !> working mode is slave
...
LOGICAL :: SCARC_MASTER_SLAVE = .TRUE.           
...
!> 
INTEGER, ALLOCATABLE, DIMENSION (:) :: NC_LOCAL, NA_LOCAL   !> Local number of cells and matrix entries on finest level
INTEGER :: NC_GLOBAL(NSCARC_LEVEL_MAX) = 0                  !> number of global cells
INTEGER :: NA_GLOBAL(NSCARC_LEVEL_MAX) = 0                  !> number of matrix entries 
...
INTEGER :: N_REQ = 0, N_EXCHANGES = 0, TAG = 99             !> Information for data exchange (use same TAG for all exchanges)
...
TYPE SCARC_GRID_TYPE
   ...
   INTEGER, ALLOCATABLE, DIMENSION (:) :: NA_LOCAL             !> number of matrix entries in local meshes
   ...
   INTEGER, ALLOCATABLE, DIMENSION (:) :: NA_OFFSET            !> offset in matrix entries numbering between meshes
   INTEGER :: NA_GLOBAL = NSCARC_ZERO_INT                      !> global number of matrix entries in all meshes
...
TYPE SCARC_NEIGHBOR_TYPE
   ...
   INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT1, RECV_INT1            !> Integer exchange buffers (usual length)
...
!> ---------- Public variables
!> 
PUBLIC :: SCARC_METHOD, SCARC_SETUP, SCARC_SOLVER, SCARC_GRID, SCARC_MASTER, SCARC_MASTER_SLAVE
...
!> ------------------------------------------------------------------------------------------------
!> Master coarse grid solver
!> ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_MASTER
USE SCARC_POINTERS, ONLY : G, AC_SYM, MKL
INTEGER :: NM, KSIZE(3) = 0, MYSELF, TAG0
INTEGER :: NC_MAX, NA_MAX, NC_SUM, NA_SUM, REQ0(100), N_REQ0, ICYCLE
INTEGER :: ICL, ICG, IAL, IAG, IC_OFFSET, IA_OFFSET, IP, IC
INTEGER,  ALLOCATABLE, DIMENSION (:) :: NA_SLAVES, NC_SLAVES, NL_SLAVES
INTEGER :: STATUS(MPI_STATUS_SIZE)
REAL(EB), ALLOCATABLE, DIMENSION (:) :: SEND_REAL_NC, RECV_REAL_NC   
REAL(EB), ALLOCATABLE, DIMENSION (:) :: SEND_REAL_NA, RECV_REAL_NA   
INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT_NC,  RECV_INT_NC    
INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT_NA,  RECV_INT_NA    
REAL(EB), ALLOCATABLE, DIMENSION (:) :: X, B

#ifdef WITH_SCARC_DEBUG
WRITE (MSG%FILE_DEBUG, '(A,A,i3.3)') TRIM(CHID),'.debug',NMESHES+1
MSG%LU_DEBUG = 99
OPEN (MSG%LU_DEBUG, FILE=MSG%FILE_DEBUG, ACTION = 'readwrite')
#endif

CALL MPI_COMM_RANK(MPI_COMM_WORLD, MYSELF, IERROR)
WRITE(*,*) MYID, ': HELLO I AM THE MASTER, NMESHES, MPI_COMM_WORLD, MYSELF =', NMESHES, MPI_COMM_WORLD, MYSELF

REQ0 = MPI_REQUEST_NULL
TAG0 = MPI_ANY_TAG
N_REQ0 = 0
!>
!> Receive number of cells and number of matrix entries from single meshes
!>
ALLOCATE (NC_SLAVES(NMESHES), STAT=IERROR)           
ALLOCATE (NA_SLAVES(NMESHES), STAT=IERROR)          
ALLOCATE (NL_SLAVES(NMESHES), STAT=IERROR)          

!CALL MPI_BARRIER(MPI_COMM_WORLD, IERROR)
DO NM = 1, NMESHES

   !N_REQ0 = N_REQ0 + 1
   !CALL MPI_IRECV(KSIZE, 2, MPI_INTEGER, NM-1, TAG0, MPI_COMM_WORLD, REQ0(N_REQ0), IERROR)

   CALL MPI_RECV(KSIZE, 3, MPI_INTEGER, NM-1, NM-1, MPI_COMM_WORLD, STATUS, IERROR)
   NC_SLAVES(NM) = KSIZE(1)
   NA_SLAVES(NM) = KSIZE(2)
   NL_SLAVES(NM) = KSIZE(3)

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) MYID, 'MASTER: received from ', NM-1,' this ', KSIZE(1:3), N_REQ0, REQ0(1:NMESHES)
#endif

ENDDO

!CALL MPI_WAITALL(N_REQ0,REQ0(1:N_REQ0),MPI_STATUSES_IGNORE,IERROR)
!CALL MPI_BARRIER(MPI_COMM_WORLD, IERROR)

!>
!> Allocate corresponding send and receive vectors for exchange of local matrices
!>
NC_MAX = MAXVAL(NC_SLAVES)
NA_MAX = MAXVAL(NA_SLAVES)

NLEVEL_MAX = MAXVAL(NL_SLAVES)

NC_SUM = SUM(NC_SLAVES)
NA_SUM = SUM(NA_SLAVES)

ALLOCATE (SEND_INT_NC(NC_MAX) , STAT=IERROR)            !> for exchange of A%ROW and solution vectors
ALLOCATE (RECV_INT_NC(NC_MAX) , STAT=IERROR)

ALLOCATE (SEND_INT_NA(NA_MAX) , STAT=IERROR)            !> for exchange of A%COL
ALLOCATE (RECV_INT_NA(NA_MAX) , STAT=IERROR)

ALLOCATE (SEND_REAL_NC(NC_MAX), STAT=IERROR)            !> for exchange of B and X
ALLOCATE (RECV_REAL_NC(NC_MAX), STAT=IERROR)

ALLOCATE (SEND_REAL_NA(NA_MAX), STAT=IERROR)            !> for exchange of A%VAL
ALLOCATE (RECV_REAL_NA(NA_MAX), STAT=IERROR)

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'MASTER: NC_SLAVES =', NC_SLAVES
WRITE(*,*) 'MASTER: NA_SLAVES =', NA_SLAVES
WRITE(*,*) 'MASTER: NL_SLAVES =', NL_SLAVES
WRITE(*,*) 'MASTER: NL_SLAVES(1) =', NL_SLAVES(1)
WRITE(*,*) 'MASTER: NA_MAX, NC_MAX =', NA_MAX, NC_MAX
WRITE(*,*) 'MASTER: NA_SUM, NC_SUM =', NA_SUM, NC_SUM
#endif

ALLOCATE (SCARC(NMESHES+1:NMESHES+1), STAT=IERROR)
CALL CHKMEMERR ('SCARC_SETUP', 'MASTER-SCARC', IERROR)

ALLOCATE (SCARC(NMESHES+1)%LEVEL(NLEVEL_MAX:NLEVEL_MAX), STAT=IERROR)
CALL CHKMEMERR ('SCARC_SETUP_TYPES', 'MASTER-LEVEL', IERROR)

CALL SCARC_POINT_TO_MASTER(NMESHES+1, NLEVEL_MAX)

G%NC = NC_SUM

AC_SYM => G%AC_SYM
AC_SYM%N_VAL = NA_SUM
AC_SYM%N_ROW = NC_SUM + 1 

IS_MKL_LEVEL(NLEVEL_MAX) = .TRUE.
CALL SCARC_ALLOCATE_MATRIX_COMPACT (AC_SYM, NLEVEL_MAX, NSCARC_INIT_ZERO, 'MASTER%AC_SYM')

N_REQ0 = 0
DO NM = 1, NMESHES

   !N_REQ0 = N_REQ0 + 1
   !CALL MPI_IRECV(RECV_INT_NC , NC_SLAVES(NM), MPI_INTEGER,          NM-1, TAG0, MPI_COMM_WORLD, REQ0(N_REQ0), IERROR)
   !N_REQ0 = N_REQ0 + 1
   !CALL MPI_IRECV(RECV_INT_NA , NA_SLAVES(NM), MPI_INTEGER,          NM-1, TAG0, MPI_COMM_WORLD, REQ0(N_REQ0), IERROR)
   !N_REQ0 = N_REQ0 + 1
   !CALL MPI_IRECV(RECV_REAL_NA, NA_SLAVES(NM), MPI_DOUBLE_PRECISION, NM-1, TAG0, MPI_COMM_WORLD, REQ0(N_REQ0), IERROR)

   CALL MPI_RECV(RECV_INT_NC, NC_SLAVES(NM), MPI_INTEGER, NM-1, NM-1, MPI_COMM_WORLD, STATUS, IERROR)
   CALL MPI_RECV(RECV_INT_NA, NA_SLAVES(NM), MPI_INTEGER, NM-1, NM-1, MPI_COMM_WORLD, STATUS, IERROR)
   CALL MPI_RECV(RECV_REAL_NA, NA_SLAVES(NM), MPI_DOUBLE_PRECISION, NM-1, NM-1, MPI_COMM_WORLD, STATUS, IERROR)

   IC_OFFSET = SUM(NC_SLAVES(1:NM-1))
   IA_OFFSET = SUM(NA_SLAVES(1:NM-1))

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) '---------------- NM = ', NM
WRITE(*,*) 'IC_OFFSET =', IC_OFFSET
WRITE(*,*) 'IA_OFFSET =', IA_OFFSET
#endif
   DO ICL = 1, NC_SLAVES(NM)
      ICG = ICL + IC_OFFSET
      AC_SYM%ROW(ICG) = RECV_INT_NC (ICL) + IA_OFFSET
#ifdef WITH_SCARC_DEBUG
WRITE(*,'(A,I4,A,I6)') 'AC_SYM%ROW(',ICG,')=',AC_SYM%ROW(ICG)
#endif
   ENDDO

   DO IAL = 1, NA_SLAVES(NM)
      IAG = IAL + IA_OFFSET
      AC_SYM%COL(IAG) = RECV_INT_NA (IAL)  
      AC_SYM%VAL(IAG) = RECV_REAL_NA (IAL) 
#ifdef WITH_SCARC_DEBUG
WRITE(*,'(A,I4,A,I6, A,I4,A,E12.4)') 'AC_SYM%COL(',IAG,')=',AC_SYM%COL(IAG), &
                                                ':     AC_SYM%VAL(',IAG,')=',AC_SYM%VAL(IAG)
#endif
   ENDDO

ENDDO
AC_SYM%ROW(ICG+1) = IAG +1

!IF (N_MPI_PROCESSES>1.AND.N_REQ0/=0) CALL MPI_WAITALL(N_REQ0,REQ0(1:N_REQ0),MPI_STATUSES_IGNORE,IERROR)
!N_REQ0 = 0

#ifdef WITH_SCARC_DEBUG
         WRITE(*,*) '----------- SHOWING SYMMETRIC MATRIX ENTRIES'
         WRITE(*,*) 'G%NC=', G%NC
         WRITE(*,*) 'AC_SYM%N_ROW = ', AC_SYM%N_ROW
         WRITE(*,*) 'AC_SYM%N_VAL = ', AC_SYM%N_VAL
         WRITE(*,*) 'AC_SYM%ROW(G%NC)=', AC_SYM%ROW(G%NC)
         WRITE(*,*) 'AC_SYM%ROW(G%NC+1)=', AC_SYM%ROW(G%NC+1)
         WRITE(*,*) 'SIZE(ASC) =',    SIZE(AC_SYM%VAL)
         WRITE(*,*) 'SIZE(AC_SYM%COL) =',SIZE(AC_SYM%COL)
         WRITE(*,*) 'SIZE(AC_SYM%ROW) =',SIZE(AC_SYM%ROW)
         WRITE(*,*) '---------------------- AC_SYM%ROW:', G%NC
         WRITE(*,'(8I6)') (AC_SYM%ROW(IC), IC=1,G%NC+1)
         WRITE(*,*) '---------------------- AC_SYM%COL:'
         DO IC = 1, G%NC
            WRITE(*,*) IC,':',(AC_SYM%COL(IP),IP=AC_SYM%ROW(IC),AC_SYM%ROW(IC+1)-1)
         ENDDO
         DO IC = 1, G%NC
            WRITE(*,*) IC,':',(AC_SYM%VAL(IP),IP=AC_SYM%ROW(IC),AC_SYM%ROW(IC+1)-1)
         ENDDO
         DO IC=1,G%NC
            DO IP=AC_SYM%ROW(IC),AC_SYM%ROW(IC+1)-1
               WRITE(*,*) IC,AC_SYM%COL(IP),AC_SYM%VAL(IP)
            ENDDO
            WRITE(*,*)
         ENDDO
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'SETTING UP PARDISO'
#endif
CALL SCARC_SETUP_PARDISO(NMESHES+1, NLEVEL_MAX, NSCARC_MODE_MASTER)

CALL SCARC_ALLOCATE_REAL1(X, 1, G%NC, NSCARC_INIT_NONE, 'X')
CALL SCARC_ALLOCATE_REAL1(B, 1, G%NC, NSCARC_INIT_NONE, 'B')

DO ICYCLE = 1, 10000
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'RECEIVING VECTOR B '
#endif
ICG = 1
DO NM = 1, NMESHES
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'Trying to receive B from process ', NM, ' in length ', NC_SLAVES(NM)
#endif
   CALL MPI_RECV(RECV_REAL_NC, NC_SLAVES(NM), MPI_DOUBLE_PRECISION, NM-1, NM-1, MPI_COMM_WORLD, STATUS, IERROR)
   DO ICL = 1, NC_SLAVES(NM)
      B(ICG) = RECV_REAL_NC(ICL)
      ICG = ICG + 1
   ENDDO
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'B(',NM,'):'
WRITE(*,'(8E14.4)') RECV_REAL_NC
#endif
ENDDO

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'BEFORE PARDISO: RIGHT HAND SIDE B:'
WRITE(*,'(8E14.4)') B(1:G%NC)
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'AFTER PARDISO: SOLUTION X:'
WRITE(*,'(8E14.4)') X(1:G%NC)
#endif
...
DO NM = 1, NMESHES
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'Trying to send X to process ', NM, ' in length ', NC_SLAVES(NM)
#endif
   DO ICL = 1, NC_SLAVES(NM)
      SEND_REAL_NC(ICL) = X(ICG)
      ICG = ICG + 1
   ENDDO
   CALL MPI_SEND(SEND_REAL_NC, NC_SLAVES(NM), MPI_DOUBLE_PRECISION, NM-1, NM-1, MPI_COMM_WORLD, IERROR)
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'X(',NM,'):'
WRITE(*,'(8E14.4)') SEND_REAL_NC
#endif
ENDDO
ENDDO


END SUBROUTINE SCARC_MASTER


!> ------------------------------------------------------------------------------------------------
!> Master coarse grid solver
!> ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_EXCHANGE_MASTER_MATRIX(NL)
USE SCARC_POINTERS, ONLY: G, AC
INTEGER, INTENT(IN) :: NL
INTEGER :: NM, KSIZE(3), MYSELF
!REAL(EB), ALLOCATABLE, DIMENSION (:) :: SEND_REAL_NC, RECV_REAL_NC   
REAL(EB), ALLOCATABLE, DIMENSION (:) :: SEND_REAL_NA, RECV_REAL_NA   
INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT_NC,  RECV_INT_NC    
INTEGER,  ALLOCATABLE, DIMENSION (:) :: SEND_INT_NA,  RECV_INT_NA    
...
CALL MPI_COMM_RANK(MPI_COMM_WORLD, MYSELF, IERROR)
#ifdef WITH_SCARC_DEBUG
WRITE(*,*) MYID, ': HELLO I AM A SLAVE, NMESHES, MPI_COMM_WORLD, MYSELF =', NMESHES, MPI_COMM_WORLD, MYSELF
#endif
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   ALLOCATE (SEND_INT_NC(G%NC) , STAT=IERROR)            !> for exchange of A%ROW and solution vectors
   ALLOCATE (RECV_INT_NC(G%NC) , STAT=IERROR)
   
   ALLOCATE (SEND_INT_NA(G%AC_SYM%N_VAL) , STAT=IERROR)            !> for exchange of A%COL
   ALLOCATE (RECV_INT_NA(G%AC_SYM%N_VAL) , STAT=IERROR)

   ALLOCATE (SEND_REAL_NA(G%AC_SYM%N_VAL), STAT=IERROR)            !> for exchange of A%VAL
   ALLOCATE (RECV_REAL_NA(G%AC_SYM%N_VAL), STAT=IERROR)

   KSIZE(1) = G%NC
   KSIZE(2) = G%AC_SYM%N_VAL
   KSIZE(3) = NL

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'SLAVE: TRYING TO SEND KSIZE:', KSIZE(1:2),': to process ', NMESHES
#endif
   !N_REQ = N_REQ + 1
   !CALL MPI_ISEND(KSIZE, 2, MPI_INTEGER, NMESHES, NM-1, MPI_COMM_WORLD, REQ(N_REQ), IERROR)
   CALL MPI_SEND (KSIZE, 3, MPI_INTEGER, NMESHES, NM-1, MPI_COMM_WORLD, IERROR)

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'SLAVE: HAVING SEND KSIZE:', KSIZE(1:2)
#endif
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   !N_REQ = N_REQ + 1
   !CALL MPI_ISEND(AC%ROW(1:AC%N_ROW-1), AC%N_ROW-1, MPI_INTEGER, NMESHES, NM-1, MPI_COMM_WORLD, REQ(N_REQ), IERROR)
   CALL MPI_SEND (AC%ROW(1:AC%N_ROW-1), AC%N_ROW-1, MPI_INTEGER, NMESHES, NM-1, MPI_COMM_WORLD, IERROR)

   !N_REQ = N_REQ + 1
   !CALL MPI_ISEND(AC%COL(1:AC%N_VAL), AC%N_VAL, MPI_INTEGER, NMESHES, NM-1, MPI_COMM_WORLD, REQ(N_REQ), IERROR)
   CALL MPI_SEND (AC%COL(1:AC%N_VAL), AC%N_VAL, MPI_INTEGER, NMESHES, NM-1, MPI_COMM_WORLD, IERROR)

   !N_REQ = N_REQ + 1
   !CALL MPI_ISEND(AC%VAL(1:AC%N_VAL), AC%N_VAL, MPI_DOUBLE_PRECISION, NMESHES, NM-1, MPI_COMM_WORLD, REQ(N_REQ), IERROR)
   CALL MPI_SEND (AC%VAL(1:AC%N_VAL), AC%N_VAL, MPI_DOUBLE_PRECISION, NMESHES, NM-1, MPI_COMM_WORLD, IERROR)

#ifdef WITH_SCARC_DEBUG
WRITE(*,*) 'SLAVE: SENDING AC%ROW'
WRITE(*,'(8I6)') AC%ROW(1:AC%N_ROW-1)
WRITE(*,*) 'SLAVE: SENDING AC%COL'
WRITE(*,'(8I6)') AC%COL(1:AC%N_VAL)
WRITE(*,*) 'SLAVE: SENDING AC%VAL'
WRITE(*,'(8E12.4)') AC%VAL(1:AC%N_VAL)
#endif
...
!> ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP(COMM_SLAVE)
REAL(EB) :: TNOW
integer, intent(IN) :: COMM_SLAVE

MPI_SLAVE_WORLD = COMM_SLAVE
...
SELECT CASE (NERROR)
   ...
   CASE (NSCARC_ERROR_LEVEL_MAX)
      CERROR = 'Too many multigrid levels '
...
IF (NLEVEL > 10) CALL SCARC_SHUTDOWN(NSCARC_ERROR_LEVEL_MAX, SCARC_NONE, NLEVEL)
...
IF (N_MPI_PROCESSES>1) &
   CALL MPI_ALLREDUCE(MPI_IN_PLACE, RANK_INT, 1, MPI_INTEGER, MPI_SUM, MPI_SLAVE_WORLD, IERROR)
...
!> -----------------------------------------------------------------------------
!> Get information about global numbers of unknowns 
!> -----------------------------------------------------------------------------
...
IF (NL == NLEVEL_MIN) CALL SCARC_ALLOCATE_INT1 (NC_LOCAL, 1, NMESHES, NSCARC_INIT_ZERO, 'NC_LOCAL')
...
MESHES_LOOP1: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   ENDDO
   IF (NL == NLEVEL_MAX) NC_LOCAL (NM) = G%NC_LOCAL(NM)
...
!> -----------------------------------------------------------------------------
!> Get information about global numbers of matrix entries 
!> -----------------------------------------------------------------------------
...
IF (NL == NLEVEL_MIN) CALL SCARC_ALLOCATE_INT1 (NA_LOCAL, 1, NMESHES, NSCARC_INIT_ZERO, 'NA_LOCAL')
...
MESHES_LOOP1: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   DO NM2 = LOWER_MESH_INDEX, UPPER_MESH_INDEX
      MESH_INT(NM2) = G%NA_LOCAL(NM2)
   ENDDO
   IF (NL == NLEVEL_MAX) NA_LOCAL (NM) = G%NA_LOCAL(NM)
...
#ifdef WITH_MKL
IF (IS_MKL_LEVEL(NL)) G%AC%COL_GLOBAL(IP) = G%AC%COL(IP) + G%NC_OFFSET(NM)
#else
...
!> Decide whether cell is interior or exterior cell
SELECT CASE (IOR0)
   CASE ( 1)
...
#ifdef WITH_MKL
      IF (IS_MKL_LEVEL(NL)) AC%COL_GLOBAL(IP)= AC%COL(IP) + G%NC_OFFSET(NM)
#endif
      IP = IP + 1
   ELSE
...
ELSE IF (L%FACE(IOR0)%N_NEIGHBORS /= 0) THEN
   ...
   IF (IW > 0) then                                    !> if available, build corresponding subdiagonal entry
      ...
      IF (IS_MKL_LEVEL(NL)) THEN                       !> if MKL method used, also store its global number
         ...
         AC%COL_GLOBAL(IP) = ABS(G%ICE_TO_ICN(ICE)) + G%NC_OFFSET(G%WALL(IW)%NOM)
      ENDIF
#else
      NDUMMY = NL                                      !> set these variables for simplicity, so that they can be used 
      NDUMMY = NM                                      !> as an input parameter even for the non-MKL case
...
SEARCH_WALL_CELLS_LOOP: DO IW = F%NCW0, F%NCW0 + F%NCW - 1
   GWC => G%WALL(IW)
   IF (GWC%NOM == 0) CYCLE
   IF (G%CELL_NUMBER(GWC%IXW, GWC%IYW, GWC%IZW) /= IC) CYCLE
   IF (IS_UNSTRUCTURED.AND.L%IS_SOLID(GWC%IXG, GWC%IYG, GWC%IZG)) RETURN
      ...
      SELECT_KRYLOV_PRECON: SELECT CASE (TYPE_PRECON)
         ...
         CASE (NSCARC_RELAX_MKL)
            ...
            SELECT CASE(TYPE_SCOPE(1))
               ...
               CASE (NSCARC_SCOPE_LOCAL)
                  CALL SCARC_SETUP_PRECON(NSTACK, NSCARC_SCOPE_LOCAL)
                  CALL SCARC_SETUP_PARDISO(NLEVEL_MIN, NLEVEL_MIN, NSCARC_MODE_SLAVE)
...
SELECT_COARSE: SELECT CASE (TYPE_COARSE)
   ...
   CASE (NSCARC_COARSE_DIRECT)
      ...
      IF (SCARC_MASTER_SLAVE) THEN
         ...
         !> Send local matrix parts to master solver where a global matrix is
         !> composed for a separate coarse grid solution with PARDISO solver
         CALL SCARC_EXCHANGE_MASTER_MATRIX(NLMAX)
      ...
      ELSE
         ...
         ELSE
            ...
            CALL SCARC_SETUP_MKL(NSCARC_SOLVER_COARSE, NSCOPE, NSTAGE, NSTACK, NLMIN, NLMAX)
            CALL SCARC_SETUP_PARDISO(NLMIN, NLMAX, NSCARC_MODE_SLAVE)
         ENDIF
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      CALL CLUSTER_SPARSE_SOLVER_S(MKL%CT, MKL%MAXFCT, MKL%MNUM, MKL%MTYPE, MKL%PHASE, G%NC_GLOBAL, &
                                   ...
                                   MKL%MSGLVL, DUMMY, DUMMY, MPI_SLAVE_WORLD, MKL%ERROR)
...
!> Initialize Pardiso solver
!> ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_PARDISO(NLMIN, NLMAX, NMODE)
USE SCARC_POINTERS, ONLY: L, G, MKL, AC_SYM
INTEGER, INTENT(IN) :: NLMIN, NLMAX, NMODE
INTEGER :: NM, NL, I, IDUMMY(1)=0
INTEGER :: ILOWER = 0, IUPPER = 0, ILMIN = 0, ILMAX = 0
REAL (EB) :: TNOW
#ifdef WITH_MKL_FB
REAL (FB) :: DUMMY(1)=0.0_FB
#else
...
IF (NMODE == NSCARC_MODE_SLAVE) THEN
   ILOWER = LOWER_MESH_INDEX
   IUPPER = UPPER_MESH_INDEX
   ILMIN  = NLMIN
   ILMAX  = NLMAX
ELSE
   ILOWER = NMESHES+1
...
MESHES_LOOP: DO NM = ILOWER, IUPPER
   LEVEL_LOOP: DO NL = ILMIN, ILMAX

      IF (NMODE == NSCARC_MODE_SLAVE) THEN
         CALL SCARC_POINT_TO_GRID(NM, NL)
      ELSE
         CALL SCARC_POINT_TO_MASTER(NM, NL)
      ENDIF
      MKL    => L%MKL
      AC_SYM => G%AC_SYM
...
WRITE(MSG%LU_DEBUG,*) 'SETTING UP PARDISO: AC_SYM%ROW='
WRITE(MSG%LU_DEBUG,'(8I6)') AC_SYM%ROW
WRITE(MSG%LU_DEBUG,*) 'SETTING UP PARDISO: AC_SYM%COL='
WRITE(MSG%LU_DEBUG,'(8I6)') AC_SYM%COL
WRITE(MSG%LU_DEBUG,*) 'SETTING UP PARDISO: AC_SYM%VAL='
WRITE(MSG%LU_DEBUG,'(8E10.2)') AC_SYM%VAL
#endif
      !>
      !> perform only reordering and symbolic factorization
      !>
...
!>
IF (N_MPI_PROCESSES>1) & 
   CALL MPI_ALLREDUCE(MPI_IN_PLACE, RANK_REAL, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_SLAVE_WORLD, IERROR)
   ...
   CASE (NSCARC_RELAX_MKL)
      ...
      MKL_SCOPE_IF: IF (STACK(NS)%SOLVER%TYPE_SCOPE(0) == NSCARC_SCOPE_GLOBAL) THEN
         ...
         MKL_SCOPE_GLOBAL_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
            ...
            CALL CLUSTER_SPARSE_SOLVER_S(MKL%CT, MKL%MAXFCT, MKL%MNUM, MKL%MTYPE, MKL%PHASE, G%NC_GLOBAL, &
                                         AC_SYM%VAL_FB, AC_SYM%ROW, AC_SYM%COL, MKL%PERM, MKL%NRHS, MKL%IPARM, &
                                         MKL%MSGLVL, V1_FB, V2_FB, MPI_SLAVE_WORLD, MKL%ERROR)
            ...
            CALL CLUSTER_SPARSE_SOLVER_D(MKL%CT, MKL%MAXFCT, MKL%MNUM, MKL%MTYPE, MKL%PHASE, G%NC_GLOBAL, &
                                         AC_SYM%VAL, AC_SYM%ROW, AC_SYM%COL, MKL%PERM, MKL%NRHS, MKL%IPARM, &
                                         MKL%MSGLVL, V1, V2, MPI_SLAVE_WORLD, MKL%ERROR)
#endif
            IF (MKL%ERROR /= 0) CALL SCARC_SHUTDOWN(NSCARC_ERROR_MKL_INTERNAL, SCARC_NONE, MKL%ERROR)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'Slave ', NM,': Trying to send B to process ', NMESHES
#endif
      CALL MPI_SEND (V1, G%NC, MPI_DOUBLE_PRECISION, NMESHES, NM-1, MPI_COMM_WORLD, IERROR)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'Slave ', NM,': Send Ready'
#endif
   ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'Slave ', NM,': Trying to receive X from process ', NMESHES
#endif
      CALL MPI_RECV (V2, G%NC, MPI_DOUBLE_PRECISION, NMESHES, NM-1, MPI_COMM_WORLD, STATUS, IERROR)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'Slave ', NM,': Receive Ready'
#endif
   ENDDO
      ...
      SELECT_EXCHANGE_TYPE: SELECT CASE (TYPE_EXCHANGE)
         ...
         CASE (NSCARC_EXCHANGE_SETUP)
            ...
            CALL MPI_IRECV(OS%RECV_INT0(1), 1, MPI_INTEGER, SNODE, &
                           TAG, MPI_SLAVE_WORLD, REQ(N_REQ), IERROR)
...
IF (N_MPI_PROCESSES > 1) &
   CALL MPI_ALLGATHERV(MPI_IN_PLACE,1,MPI_INTEGER,MESH_REAL,COUNTS,DISPLS,&
                       MPI_DOUBLE_PRECISION,MPI_SLAVE_WORLD,IERROR)
...
#ifdef WITH_MKL
WRITE(*,*) MYID, ': IS_MKL_LEVEL =', IS_MKL_LEVEL
IF (IS_MKL_LEVEL(NL)) THEN
...
IF (MYID>0) THEN
   CALL MPI_SEND(LINE,LINE_LENGTH,MPI_CHARACTER,0,MYID,MPI_SLAVE_WORLD,IERROR)
ELSE
   LINE_ARRAY(0) = LINE
   DO N=1,N_MPI_PROCESSES-1
      CALL MPI_RECV(LINE_ARRAY(N),LINE_LENGTH,MPI_CHARACTER,N,N,MPI_SLAVE_WORLD,STATUS,IERROR)
   ENDDO
...
SELECT CASE (NTYPE)
   ...
   CASE (NSCARC_DEBUG_MATRIX_SYM)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         WRITE(MSG%LU_DEBUG,*) 'G%NC=', G%NC
         WRITE(MSG%LU_DEBUG,*) 'AC_SYM%N_VAL=',AC_SYM%N_VAL
         WRITE(MSG%LU_DEBUG,*) 'AC_SYM%N_ROW=',AC_SYM%N_ROW
