   ...
   CASE (NSCARC_PRECON_SSOR)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         FORWARD_CELL_LOOP: DO IC = 1, NC
            ...
            V2(IC) = (V2(IC) - AUX * OMEGA_SSOR) / AC(AC_ROW(IC))
...
call scarc_debug_level (NSOL, 'BLOCK_SOLVER', 'NSOL CLUSTER', NL)
call scarc_debug_level (NRHS, 'BLOCK_SOLVER', 'NRHS CLUSTER', NL)
...
call scarc_debug_level (NSOL, 'BLOCK_SOLVER', 'NSOL CLUSTER2', NL)
call scarc_debug_level (NRHS, 'BLOCK_SOLVER', 'NRHS CLUSTER2', NL)
...
!SCARC(1)%LEVEL(1)%F_MAIN = 0.0_EB
!SCARC(1)%LEVEL(1)%F_MAIN(25) = 1000.0_EB
!SCARC(1)%LEVEL(1)%F_MAIN(33) = 1000.0_EB
...
call scarc_debug_level (SC%G, 'PRECON', 'ADD: G SUM', NL)
   ...
   CASE (NSCARC_TWOLEVEL_MUL)
      ...
      CALL SCARC_RESTRICTION (SC%W, SC%F, NL, NL+1)             !>  F_coarse := rest(W_fine)
call scarc_debug_level (SC%W, 'PRECON', 'MUL: W BEFORE COARSE', NL)
...
call scarc_debug_level (SC%Y, 'PRECON', 'MUL: Y AFTER PROL', NL)
...
call scarc_debug_level (SC%G, 'PRECON', 'MUL: G AFTER ADD TO Y', NL)
