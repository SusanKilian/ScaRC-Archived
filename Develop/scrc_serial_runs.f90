...
INTEGER, INTENT(IN) :: NL
INTEGER :: ICCG, ICC, IC, IP, NM, N_ROW, N_VAL, ICC1, ICC2
...
WRITE(MSG%LU_DEBUG,'(A,3I4)') 'SETUP_ZONE_OPERATOR: ICC, N_COARSE, NC_OFFSET(NM)=',ICC, GF%N_COARSE, GC%NC_OFFSET(NM)
      ...
      DO IC = 1, GF%N_FINE
         ...
         ZF%COL(IP) = IC
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5I4)') '         bingo, IC, IP, ICCG, ICC, IS_INCLUDED:', &
                                               IC, IP, ICCG, ICC, GF%ZONES_LOCAL(IC)
#endif
         IP = IP + 1
      ENDDO
      IF (IS_INCLUDED) THEN
...
INTEGER, INTENT(IN) :: NL
INTEGER  :: NM, NOM, IC, JC, IACOL, IPCOL, ICOL, IP, ICCG, INBR, IFOUND
