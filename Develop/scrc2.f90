...
!> -----------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SOLVER(T_IN,SIZE_REQ7,N_REQ7_IN,REQ7_IN)

USE MESH_POINTERS

REAL(EB), INTENT(IN) :: T_IN
...
SUBROUTINE SCARC_MATVEC_PRODUCT_ON_GRID(X, W, NL)                         !>  W := A*X

! This routine take the 1D Array X (which represents data with ghost cells filled) and performs:
! A. Map 1D X to 3D X3(I,J,K).
! B. Compute Face fluxes FLX(I,J,K,AXIS) from X3(I,J,K).
! C. Match fluxes at all interfaces.
! D. Do final computation of divergence on grid to get W3(I,J,K).
! E. Map 3D W3(I,J,K) to 1D W.

! Both X and W are 1D.
INTEGER,  INTENT(IN) :: NL
REAL(EB), INTENT(IN) :: X
REAL(EB), INTENT(OUT):: W
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   ! Might need more variables being set.

   ALLOCATE(X3(1:IBAR,1:JBAR.1:KBAR),W3(1:IBAR,1:JBAR.1:KBAR),FLX(0:IBAR,0:JBAR,0:KBAR,IAXIS:KAXIS));
   X3=0._EB; W3=0._EB; FLX=0._EB
   ...
   DO K = 1, KBAR
      DO J = 1, JBAR
         DO I = 1, IBAR-1
            FLX(I,J,K,IAXIS) = RDXN(I)*(X3(I+1, J, K)-X3(I, J, K))
         ENDDO
      ENDDO
   ENDDO
   DO K = 1, KBAR
      DO J = 1, JBAR-1
         DO I = 1, IBAR
            FLX(I,J,K,JAXIS) = RDYN(J)*(X3(I,J+1, K)-X3(I, J, K))
         ENDDO
      ENDDO
   ENDDO
   DO K = 1, KBAR-1
      DO J = 1, JBAR
         DO I = 1, IBAR
            FLX(I,J,K,KAXIS) = RDZN(K)*(X3(I,J  ,K+1)-X3(I, J, K))
         ENDDO
      ENDDO
   ENDDO
   ! External Faces, apply some sort of flux matching:
   ! C. Match fluxes at all interfaces.
   DO IW=1,N_EXTERNAL_WALL_CELLS
      ! wall.f90 routine line 722, KP_WALL_LOOP: ...
      ! This fills FLX(0,J,K,IAXIS),FLX(IBAR,J,K,IAXIS),FLX(I,0,K,JAXIS),FLX(I,JBAR,K,JAXIS), etc.
   ENDDO
   ...
   DO K = 1, KBAR
      DO J = 1, JBAR
         DO I = 1, IBAR
            W3(I,J,K) = RDX(I)*(FLX(I,J,K,IAXIS)-FLX(I-1,J,K,IAXIS)) + &
                        RDY(J)*(FLX(I,J,K,JAXIS)-FLX(I,J-1,K,JAXIS)) + &
                        RDZ(K)*(FLX(I,J,K,KAXIS)-FLX(I,J,K-1,KAXIS))
         ENDDO
      ENDDO
   ENDDO
   ...
   DEALLOCATE(X3,W3,FLX)

ENDDO MESH_LOOP


RETURN
END SUBROUTINE SCARC_MATVEC_PRODUCT_ON_GRID


SOME LOOPS ON PRESSURE ZONES:


SCARC_INDEFINITE=0
SCARC_DEFINITE  =1

! Define locally if Matrix for Zone is Indefinite:
DEFINITE_MATRIX(1:N_ZONE)=SCARC_INDEFINITE
DO IPZ=1,N_ZONE
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            IF (IC/=0) THEN
               IW = WALL_INDEX(IC,-1)
               IF (WALL(IW)%PRESSURE_BC_INDEX==DIRICHLET) DEFINITE_MATRIX(IPZ) = SCARC_DEFINITE
               IW = WALL_INDEX(IC, 1)
               IF (WALL(IW)%PRESSURE_BC_INDEX==DIRICHLET) DEFINITE_MATRIX(IPZ) = SCARC_DEFINITE
               IW = WALL_INDEX(IC,-2)
               IF (WALL(IW)%PRESSURE_BC_INDEX==DIRICHLET) DEFINITE_MATRIX(IPZ) = SCARC_DEFINITE
...
! All reduce:
CALL MPI_ALLREDUCE(MPI_IN_PLACE,DEFINITE_MATRIX,N_ZONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)



! Once all processors know which zones are indefinite, condensation follows per zone.
...
IF (.NOT.PROFILING) THEN
   ...
   DO WHILE(.NOT.FLAG)
      ...
      IF (WAIT_TIME>MPI_TIMEOUT) THEN
         WRITE(LU_ERR,'(A,A,I6,A,A)') TRIM(RNAME),' timed out for MPI process ',MYID,' running on Scrc'
         FLAG = .TRUE.
         DO NNN=1,NR
...
SUBROUTINE MESH_FV_EXCHANGE

! Exchange Information between Meshes

INTEGER :: NM,LL,RNODE,SNODE,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE
...
END SUBROUTINE MESH_FV_EXCHANGE
