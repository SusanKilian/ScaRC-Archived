...
INTEGER, PARAMETER :: NSCARC_EXCHANGE_MGM1           = 19         !< Type of data exchange: pressure values
INTEGER, PARAMETER :: NSCARC_EXCHANGE_MGM2           = 20         !< Type of data exchange: pressure values
...
INTEGER, PARAMETER :: NSCARC_METHOD_MGM2             =  5         !< Global ScaRC method: McKeeney-Greengard-Mayo solver
...
INTEGER, PARAMETER :: NSCARC_MGM_ZERO                =  0         !< Type of internal MGM boundary: zero
...
INTEGER, PARAMETER :: NSCARC_MGM_SIMPLE              =  3         !< Type of internal MGM boundary: simple
INTEGER, PARAMETER :: NSCARC_MGM_EXTRAPOLATION       =  4         !< Type of internal MGM boundary: linear extrapolatioln
INTEGER, PARAMETER :: NSCARC_MGM_TAYLOR              =  5         !< Type of internal MGM boundary: Taylor expansion
INTEGER, PARAMETER :: NSCARC_MGM_MIXED               =  6         !< Type of external MGM boundary: mixed Neumann/Dirichlet
INTEGER, PARAMETER :: NSCARC_MGM_DIRICHLET           =  7         !< Type of external MGM boundary: Dirichlet only
INTEGER, PARAMETER :: NSCARC_MGM_LEVEL2              =  8         !< Type of MGM pass: First (inhomogeneous Poisson)
INTEGER, PARAMETER :: NSCARC_MGM_MERGE               =  9         !< Type of MGM pass: First (inhomogeneous Poisson)
INTEGER, PARAMETER :: NSCARC_MGM_MERGE2              = 19         !< Type of MGM pass: First (inhomogeneous Poisson)
...
INTEGER, PARAMETER :: NSCARC_MGM_PREDICTOR           = 10         !< Type of MGM pass: First (inhomogeneous Poisson)
INTEGER, PARAMETER :: NSCARC_MGM_CORRECTOR           = 11         !< Type of MGM pass: First (inhomogeneous Poisson)
...
CHARACTER(40) :: SCARC_MGM_INTERNAL         = 'SIMPLE' 
CHARACTER(40) :: SCARC_MGM_EXTERNAL         = 'MIXED' 
...
INTEGER :: TYPE_MGM_INTERNAL       = NSCARC_MGM_SIMPLE           !< Type of internal MGM boundary conditions
INTEGER :: TYPE_MGM_EXTERNAL       = NSCARC_MGM_MIXED            !< Type of external MGM boundary conditions
...
TYPE SCARC_MESSAGE_TYPE
   ...
   CHARACTER(20) :: CFORM1, CFORM2, CFORM3
#endif
...
TYPE SCARC_MGM_TYPE
   ...
   REAL(EB), ALLOCATABLE, DIMENSION (:,:,:) :: H1, H2, H3      !< Pressure vectors of different parts
   REAL(EB), ALLOCATABLE, DIMENSION (:,:,:) :: UP, VP, WP      !< Velocity vectors Poisson step
   REAL(EB), ALLOCATABLE, DIMENSION (:,:,:) :: UL, VL, WL      !< Velocity vectors Laplace step
   REAL(EB), ALLOCATABLE, DIMENSION (:,:,:) :: UU, VV, WW      !< Velocity vectors predictor
   REAL(EB), ALLOCATABLE, DIMENSION (:)     :: X1, X2          !< ScaRC solution vectors of different parts
   REAL(EB), ALLOCATABLE, DIMENSION (:)     :: UWALL, VWALL, WWALL      !< Velocity components along internal BC's
   REAL(EB), ALLOCATABLE, DIMENSION (:,:)   :: AAA, IA         !< Lower part of LU-decomposition for unstructured AC
   REAL(EB), ALLOCATABLE, DIMENSION (:,:)   :: LLL, IL         !< Lower part of LU-decomposition for unstructured AC
   REAL(EB), ALLOCATABLE, DIMENSION (:,:)   :: UUU, IU         !< Upper part of LU-decomposition for unstructured AC
   REAL(EB), ALLOCATABLE, DIMENSION (:)     :: B, X, Y         !< Right hand side, solution vectors
   REAL(EB)::  VELOCITY_ERROR, VELOCITY_ERROR_MAX = NSCARC_INIT_UNDEF
...
PUBLIC :: SCARC_MGM_INTERNAL               !< Boundary condition for MGM method
PUBLIC :: SCARC_MGM_EXTERNAL               !< Boundary condition for MGM method
...
SELECT CASE (TRIM(SCARC_METHOD))
   ...
   CASE ('MGM')
      ...
      ! set type of MGM boundary condition
      SELECT CASE (TRIM(SCARC_MGM_INTERNAL))
         CASE ('ZERO')
            TYPE_MGM_INTERNAL = NSCARC_MGM_ZERO
         CASE ('SIMPLE')
            TYPE_MGM_INTERNAL = NSCARC_MGM_SIMPLE
         CASE ('EXTRAPOLATION')
            TYPE_MGM_INTERNAL = NSCARC_MGM_EXTRAPOLATION
         CASE ('TAYLOR')
            TYPE_MGM_INTERNAL = NSCARC_MGM_TAYLOR
         !CASE DEFAULT
         !   CALL SCARC_SHUTDOWN(NSCARC_ERROR_PARSE_INPUT, SCARC_SMOOTH_SCOPE, NSCARC_NONE)
      END SELECT

      ! set type of MGM boundary condition
      SELECT CASE (TRIM(SCARC_MGM_EXTERNAL))
         CASE ('MIXED')
            TYPE_MGM_EXTERNAL = NSCARC_MGM_MIXED
         CASE ('DIRICHLET')
            TYPE_MGM_EXTERNAL = NSCARC_MGM_DIRICHLET
         !CASE DEFAULT
         !   CALL SCARC_SHUTDOWN(NSCARC_ERROR_PARSE_INPUT, SCARC_SMOOTH_SCOPE, NSCARC_NONE)
      END SELECT
...
#ifdef WITH_SCARC_DEBUG
         IF (L%NX < 10) THEN
            MSG%CFORM1 = "( E16.8)"  ; WRITE(MSG%CFORM1(2:2),'(I1.1)') L%NX
         ELSE
            MSG%CFORM1 = "(  E16.8)" ; WRITE(MSG%CFORM1(2:3),'(I2.2)') L%NX
         ENDIF
         IF (L%NX+1 < 10) THEN
            MSG%CFORM2 = "( E16.8)"  ; WRITE(MSG%CFORM2(2:2),'(I1.1)') L%NX+1
         ELSE
            MSG%CFORM2 = "(  E16.8)" ; WRITE(MSG%CFORM2(2:3),'(I2.2)') L%NX+1
         ENDIF
         IF (L%NX+2 < 10) THEN
            MSG%CFORM3 = "( E16.8)"  ; WRITE(MSG%CFORM3(2:2),'(I1.1)') L%NX+2
         ELSE
            MSG%CFORM3 = "(  E16.8)" ; WRITE(MSG%CFORM3(2:3),'(I2.2)') L%NX+2
         ENDIF
         WRITE(*,*) 'MSG%CFORM1=', MSG%CFORM1
         WRITE(*,*) 'MSG%CFORM2=', MSG%CFORM2
         WRITE(*,*) 'MSG%CFORM3=', MSG%CFORM3
#endif
...
IF (NMESHES > 1) THEN
   ...
   IF (IS_MGM) CALL SCARC_ASSIGN_GRID_TYPE(NSCARC_GRID_STRUCTURED)
#ifdef WITH_SCARC_DEBUG
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'KRYLOV A:', TYPE_SCOPE(0)
#endif
         CALL SCARC_SETUP_POISSON (NM, NLEVEL_MIN)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'KRYLOV B:', TYPE_SCOPE(0)
#endif
         CALL SCARC_SETUP_BOUNDARY(NM, NLEVEL_MIN)

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'KRYLOV C:', TYPE_SCOPE(0)
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'IS_CG_GMG:  ', NL, TYPE_SCOPE(0)
#endif
                  ENDDO
               ENDIF
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '========================= SUSI: STRUCTURED'
#endif
         CALL SCARC_ASSIGN_GRID_TYPE (NSCARC_GRID_STRUCTURED)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '========================= SUSI: UNSTRUCTURED'
#endif
         CALL SCARC_ASSIGN_GRID_TYPE (NSCARC_GRID_UNSTRUCTURED)
...
IF (IS_MGM) THEN
   ...
   CALL SCARC_ASSIGN_GRID_TYPE (TYPE_GRID_BACKUP)        
ENDIF
...
WRITE(MSG%LU_DEBUG,*)
   ...
   CASE DEFAULT
      ...
      SELECT CASE (TYPE_MGM_EXTERNAL)

         CASE (NSCARC_MGM_MIXED)
            ...
            DO IW = 1, G%NW
               ...
               IP = A%ROW(IC)
               IF (GWC%BTYPE == DIRICHLET .OR. GWC%BTYPE == INTERNAL) THEN ! Neumann & Dirichlet at external boundaries 
                  A%VAL(IP) = A%VAL(IP) - F%SCAL_BOUNDARY
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,7I6,2E16.8)') 'A :MGM-DIRICHLET: IW, I, J, K, NOM, IC, BTYPE, A%VAL:', &
                                    IW, I, J, K, NOM, IC, GWC%BTYPE, A%VAL(IP), -F%SCAL_BOUNDARY
#endif
               ELSE
                  A%VAL(IP) = A%VAL(IP) + F%SCAL_BOUNDARY
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,7I6,2E16.8)') 'B :MGM-NEUMANN  : IW, I, J, K, NOM, IC, BTYPE, A%VAL:', &
                                    IW, I, J, K, NOM, IC, GWC%BTYPE, A%VAL(IP), F%SCAL_BOUNDARY
#endif
               ENDIF
         ...
         CASE (NSCARC_MGM_DIRICHLET)
            ...
            DO IW = 1, G%NW
               ...
               IP = A%ROW(IC)
               IF (IW <= L%N_WALL_CELLS_EXT) THEN                           ! Purely Dirichlet 
                  A%VAL(IP) = A%VAL(IP) - F%SCAL_BOUNDARY
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,7I6,E12.4)') 'A :DIRICHLET: IW, I, J, K, NOM, IC, BTYPE, A%VAL:', IW, I, J, K, NOM, IC, GWC%BTYPE, A%VAL(IP)
               ...
               ELSE
                  A%VAL(IP) = A%VAL(IP) + F%SCAL_BOUNDARY
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,7I6,E12.4)') 'B :NEUMANN  : IW, I, J, K, NOM, IC, BTYPE, A%VAL:', IW, I, J, K, NOM, IC, GWC%BTYPE, A%VAL(IP)
#endif
               ENDIF
...
#ifdef WITH_SCARC_DEBUG
CALL SCARC_DEBUG_CMATRIX(A, 'POISSON', 'POISSON AFTER MGM_SETUP_BOUNDARY')
#endif
...
#ifdef WITH_MKL
WRITE(*,*) 'B: TYPE_PRECON = ', TYPE_PRECON
         STACK(NSTACK)%SOLVER => PRECON_MKL
         CALL SCARC_SETUP_PRECON(NSTACK, NSCARC_SCOPE_LOCAL)
...
IF (HAS_MULTIPLE_GRIDS) CALL SCARC_ASSIGN_GRID_TYPE(NSCARC_GRID_STRUCTURED)   
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '==================================================================================='
WRITE(MSG%LU_DEBUG,*) '================= SCARC_SOLVER : START: ICYC = ', ICYC
WRITE(MSG%LU_DEBUG,*) '==================================================================================='
WRITE(MSG%LU_DEBUG,*) ' ........... starting ScaRC ............', PREDICTOR
WRITE(MSG%LU_DEBUG,*) ' ----------------------- PART1 of MGM '
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: FVX'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%FVX(I,1,K), I=0, IBAR), K=KBAR,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: FVZ'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%FVZ(I,1,K), I=0, IBAR), K=KBAR,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: U'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%U(I,1,K), I=0, IBAR), K=KBAR,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: US'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%US(I,1,K), I=0, IBAR), K=KBAR,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: W'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%W(I,1,K), I=0, IBAR), K=KBAR,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: WS'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%WS(I,1,K), I=0, IBAR), K=KBAR,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: PRHS'
WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MYID+1)%PRHS(I,1,K), I=1, IBAR+1), K=KBAR+1,1,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: H'
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MESHES(MYID+1)%H(I,1,K), I=0, IBAR+1), K=KBAR+1,0,-1)
WRITE(MSG%LU_DEBUG,*) 'SCARC_SOLVER:1: HS'
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MESHES(MYID+1)%HS(I,1,K), I=0, IBAR+1), K=KBAR+1,0,-1)
#endif
...
SELECT_METHOD: SELECT CASE (TYPE_METHOD)
   ...
   CASE (NSCARC_METHOD_MGM)
      ...
      CALL SCARC_MGM_VELOCITY_SETUP(NLEVEL_MIN)

      CALL SCARC_DEBUG_MGM('PART1 of MGM: BEFORE POISSON SOLUTION',0)                    !!!! DEBUG

      CALL SCARC_ASSIGN_GRID_TYPE(NSCARC_GRID_STRUCTURED)
      CALL SCARC_METHOD_KRYLOV (NSCARC_MGM_STACK_POISSON, NSCARC_STACK_ZERO, NSCARC_RHS_INHOMOGENEOUS, NLEVEL_MIN)
      CALL SCARC_MGM_PARTIAL_SOLUTION (NLEVEL_MIN, NSCARC_MGM_POISSON)
      CALL SCARC_MGM_UPDATE_GHOSTCELLS(NLEVEL_MIN, NSCARC_MGM_POISSON)

      CALL SCARC_DEBUG_MGM('PART2 of MGM: AFTER POISSON SOLUTION',0)                     !!!! DEBUG
      CALL SCARC_MGM_VELOCITY_ERROR(NLEVEL_MIN, NSCARC_MGM_POISSON)


      MGM_CORRECTION_LOOP: DO ITE_MGM = 1, 10

         PRES_ON_WHOLE_DOMAIN = .FALSE.
         CALL SCARC_MGM_VELOCITY_ERROR(NLEVEL_MIN, NSCARC_MGM_LAPLACE)
         IF (VELOCITY_ERROR_MAX <= VELOCITY_TOLERANCE) EXIT MGM_CORRECTION_LOOP

         CALL SCARC_MGM_VELOCITY_PROJECTION(NLEVEL_MIN, NSCARC_MGM_POISSON)

         CALL SCARC_DEBUG_MGM('PART3 of MGM: AFTER VELOCITY_ERROR',0)                       !!!! DEBUG

         CALL SCARC_MGM_VELOCITY_OBSTRUCTIONS (NLEVEL_MIN)
         CALL SCARC_ASSIGN_GRID_TYPE (NSCARC_GRID_UNSTRUCTURED)
         CALL SCARC_METHOD_KRYLOV (NSCARC_MGM_STACK_LAPLACE, NSCARC_STACK_ZERO, NSCARC_RHS_HOMOGENEOUS, NLEVEL_MIN)

         CALL SCARC_DEBUG_MGM('PART4 of MGM: AFTER VELOCITY_ERROR',0)                       !!!! DEBUG

         CALL SCARC_MGM_PARTIAL_SOLUTION (NLEVEL_MIN, NSCARC_MGM_LAPLACE)
         CALL SCARC_MGM_UPDATE_GHOSTCELLS(NLEVEL_MIN, NSCARC_MGM_LAPLACE)

         CALL SCARC_DEBUG_MGM('PART5 of MGM: AFTER LAPLACE SOLUTION',0)                    !!!! DEBUG

         CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_MGM2, NSCARC_NONE, NLEVEL_MIN)
         !CALL SCARC_MGM_PARTIAL_SOLUTION (NLEVEL_MIN, NSCARC_MGM_MERGE)

         CALL SCARC_DEBUG_MGM('PART6 of MGM: AFTER MERGE ',0)                             !!!! DEBUG

         CALL SCARC_MGM_VELOCITY_PROJECTION(NLEVEL_MIN, NSCARC_MGM_LAPLACE)

         CALL SCARC_DEBUG_MGM('PART7 of MGM: AFTER LAPLACE VELOCITY ERROR ',0)                             !!!! DEBUG
         CALL SCARC_MGM_VELOCITY_PROJECTION(NLEVEL_MIN, NSCARC_MGM_MERGE)

      ENDDO MGM_CORRECTION_LOOP

      IF (ITE_MGM > 10) WRITE(*,*) 'MGM-convergence failed!'

      IF (PREDICTOR) THEN
         CALL SCARC_MGM_PARTIAL_SOLUTION (NLEVEL_MIN, NSCARC_MGM_PREDICTOR)
      ELSE
         CALL SCARC_MGM_PARTIAL_SOLUTION (NLEVEL_MIN, NSCARC_MGM_CORRECTOR)
      ENDIF

      CALL SCARC_DEBUG_MGM('PART8 of MGM: LEAVING SCARC ',1)                             !!!! DEBUG
...
SUBROUTINE SCARC_DEBUG_MGM(CTEXT, NTYPE)
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MGM%UL(I,1,K), I=0, M%IBAR), K=M%KBAR,0,-1)
...
END SUBROUTINE SCARC_DEBUG_MGM
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '======================>>> CG : START =', ITE
...
USE SCARC_POINTERS, ONLY: M, L, F, G, SV, ST, STP, GWC, PRHS, HP, UU, VV, WW
USE SCARC_ITERATION_ENVIRONMENT
...
INTEGER :: NM, IW, IW1, IW2, IOR0, I, J, K, IC, TYPE_MGM_INTERNAL0
REAL(EB) :: VAL
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 5I6,2E12.4)') 'SETUP_WORKSPACE: DIRICHLET: IW, I, J, K, IC, VAL, B(IC):', &
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 5I6,2E12.4)') 'SETUP_WORKSPACE: NEUMANN  : IW, I, J, K, IC, VAL, B(IC):', &
                                       IW, I, J, K, IC, VAL, ST%B(IC)
#endif
...
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%H3(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
               ...
               TYPE_MGM_INTERNAL0 = TYPE_MGM_INTERNAL
               IF (TYPE_MGM_INTERNAL == NSCARC_MGM_EXTRAPOLATION) THEN
                 IF (TOTAL_PRESSURE_ITERATIONS == 1) THEN
                    TYPE_MGM_INTERNAL0 = NSCARC_MGM_ZERO
                 ELSE IF (TOTAL_PRESSURE_ITERATIONS == 2) THEN
                    TYPE_MGM_INTERNAL0 = NSCARC_MGM_SIMPLE
                 ENDIF
               !ELSE IF (TYPE_MGM_INTERNAL == NSCARC_MGM_SIMPLE) THEN
               !  IF (TOTAL_PRESSURE_ITERATIONS == 1) TYPE_MGM_INTERNAL0 = NSCARC_MGM_ZERO
               ENDIF
               ...
               !
               SELECT CASE (TYPE_MGM_INTERNAL0)

                  !
                  ! --- ZERO boundary
                  !
                  CASE (NSCARC_MGM_ZERO)

#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) '%%%%%%%%%%%%%%%%% ZERO SETTING:', TOTAL_PRESSURE_ITERATIONS
#endif
                  !
                  ! --- Simple time delayed mean value
                  !
                  CASE (NSCARC_MGM_SIMPLE)
   ...
   WRITE(MSG%LU_DEBUG,*) '%%%%%%%%%%%%%%%%% SIMPLE SETTING:', TOTAL_PRESSURE_ITERATIONS
                     ...
                     DO IW = 1, L%N_WALL_CELLS_EXT
                        ...
                        SELECT CASE (IOR0)
                           CASE(1)
                              VAL=  0.5_EB * (MGM%H2(I-1, J, K) + MGM%H2(I, J, K)) 
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: SIMPLE: IOR =  1: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I-1,J,K), MGM%H2(I,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(-1)
                              VAL=  0.5_EB * (MGM%H2(I, J, K) + MGM%H2(I+1, J, K))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: SIMPLE: IOR = -1: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K), MGM%H2(I+1,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(2)
                              VAL=  0.5_EB * (MGM%H2(I, J-1, K) + MGM%H2(I, J, K))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: SIMPLE: IOR =  2: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J-1,K), MGM%H2(I,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(-2)
                              VAL=  0.5_EB * (MGM%H2(I, J, K) + MGM%H2(I, J+1, K))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: SIMPLE: IOR = -2: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K), MGM%H2(I,J+1,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(3)
                              VAL=  0.5_EB * (MGM%H2(I, J, K-1) + MGM%H2(I, J, K))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: SIMPLE: IOR =  3: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K-1), MGM%H2(I,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(-3)
                              VAL=  0.5_EB * (MGM%H2(I, J, K) + MGM%H2(I, J, K+1))
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: SIMPLE: IOR = -3: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
   ...
   WRITE(MSG%LU_DEBUG,*) '%%%%%%%%%%%%%%%%% EXPOL SETTING:', TOTAL_PRESSURE_ITERATIONS
                     ...
                     DO IW = 1, L%N_WALL_CELLS_EXT
                        ...
                        SELECT CASE (IOR0)
                           CASE(1)
                              VAL =  MGM%H2(I-1, J, K) + MGM%H2(I, J, K) - 0.5_EB*(MGM%H3(I-1, J, K) + MGM%H3(I, J, K))  
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: EXPOL: IOR =  1: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I-1,J,K), MGM%H2(I,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(-1)
                              VAL =  MGM%H2(I, J, K) + MGM%H2(I+1, J, K) - 0.5_EB*(MGM%H3(I, J, K) + MGM%H3(I+1, J, K))  
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: EXPOL: IOR = -1: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K), MGM%H2(I+1,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(2)
                              VAL =  MGM%H2(I, J-1, K) + MGM%H2(I, J, K) - 0.5_EB*(MGM%H3(I, J-1, K) + MGM%H3(I, J, K))  
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: EXPOL: IOR =  2: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J-1,K), MGM%H2(I,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(-2)
                              VAL =  MGM%H2(I, J, K) + MGM%H2(I, J+1, K) - 0.5_EB*(MGM%H3(I, J, K) + MGM%H3(I, J+1, K))  
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: EXPOL: IOR = -2: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K), MGM%H2(I,J+1,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(3)
                              VAL =  MGM%H2(I, J, K-1) + MGM%H2(I, J, K) - 0.5_EB*(MGM%H3(I, J, K-1) + MGM%H3(I, J, K))  
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: EXPOL: IOR =  3: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K-1), MGM%H2(I,J,K), VAL, F%SCAL_DIRICHLET*VAL
#endif
                           CASE(-3)
                              VAL =  MGM%H2(I, J, K) + MGM%H2(I, J, K+1) - 0.5_EB*(MGM%H3(I, J, K) + MGM%H3(I, J, K+1))  
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,4E14.6)') 'MGM-BC: EXPOL: IOR = -3: IW,I,J,K, HPM, HPP%, VAL, VAL2: ', &
                                    IW, I,J,K, MGM%H2(I,J,K), MGM%H2(I,J,K+1), VAL, F%SCAL_DIRICHLET*VAL
#endif
...
WRITE(MSG%LU_DEBUG,'(A,4i4,3E14.6)') 'MGM-BC: TAYLOR: IOR = -3: IW,I,J,K, HPM, HPP, VAL, VAL2: ', &
               ...
               DO IW = L%N_WALL_CELLS_EXT+1, L%N_WALL_CELLS_EXT + L%N_WALL_CELLS_INT
                  ...
                  SELECT CASE (IOR0)
                     CASE(1)
                        ST%B(IC) =  L%DXI * DTI * MGM%UWALL(IW)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5i4,2E14.6)') 'MGM-BC: INTERNAL: IOR =  1: IW,I,J,K,IC,UU(IW),B(IC):', IW,I,J,K, IC, MGM%UWALL(IW), ST%B(IC)
#endif
                     CASE(-1)
                        ST%B(IC) = -L%DXI * DTI * MGM%UWALL(IW)
#ifdef WITH_SCARC_DEBUG
...
#endif
                     CASE(2)
                        ST%B(IC) =  L%DYI * DTI * MGM%VWALL(IW)

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5i4,2E14.6)') 'MGM-BC: INTERNAL: IOR =  2: IW,I,J,K,IC,VV(IW),B(IC):', IW,I,J,K, IC, MGM%VWALL(IW), ST%B(IC)
#endif
                     CASE(-2)
                        ST%B(IC) = -L%DYI * DTI * MGM%VWALL(IW)
#ifdef WITH_SCARC_DEBUG
...
#endif
                     CASE(3)
                        ST%B(IC) =  L%DZI * DTI * MGM%WWALL(IW)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,5i4,2E14.6)') 'MGM-BC: INTERNAL: IOR =  3: IW,I,J,K,IC,WW(IW),B(IC):', IW,I,J,K, IC, MGM%WWALL(IW), ST%B(IC)
#endif
                     CASE(-3)
                        ST%B(IC) = -L%DZI * DTI * MGM%WWALL(IW)
#ifdef WITH_SCARC_DEBUG
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SET_WORKSPACE:3: NC, NCE:', G%NC, G%NCE
#endif
         DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
            L  => SCARC(NM)%LEVEL(NL)
            ST => SCARC(NM)%LEVEL(NL)%STAGE(SV%TYPE_STAGE)
...
2000 FORMAT (A30,': Iterations: ',i6,':   Convergence Rate =',E16.8,/)
#endif
...
#ifdef WITH_SCARC_DEBUG2
      WRITE(MSG%LU_DEBUG,'(A, 4I6, E12.4)') 'UPDATE_MAIN_CELLS: IC, IX, IY, IZ, HP(IC):', &
...
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((HP(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
      ...
      SELECT CASE (TYPE_MGM_EXTERNAL) 
         ...
         ! -------- Same mixture of Neumann and Dirichlet as global problem -----------------------
         CASE (NSCARC_MGM_MIXED)
      ...
                     IF (GWC%BTYPE==DIRICHLET) THEN
               ...
               END SELECT
#ifdef WITH_SCARC_DEBUG2
               WRITE(MSG%LU_DEBUG,'(A, 5I6, E12.4)') 'UPDATE_MGM_GHOST_CELLS:MIXED: IW, IOR0, IXW, IYW, IZG, HP:',&
                                                      IW, IOR0, IXW, IYW, IZG, HP(IXW, IYW, IZG)
#endif
         ...
         CASE (NSCARC_MGM_DIRICHLET)
            ...
            WALL_CELLS_LOOP3: DO IW = 1, L%N_WALL_CELLS_EXT
               ...
               END SELECT
#ifdef WITH_SCARC_DEBUG2
               WRITE(MSG%LU_DEBUG,'(A, 5I6, E12.4)') 'UPDATE_MGM_GHOST_CELLS:PURE-D: IW, IOR0, IXW, IYW, IZG, HP:',&
                                                      IW, IOR0, IXW, IYW, IZG, HP(IXW, IYW, IZG)
#endif
...
RECEIVE_MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   RECEIVE_OMESHES_LOOP: DO NOM = 1, NMESHES
      ...
      SELECT_EXCHANGE_TYPE: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_MGM1)
            CALL SCARC_RECV_MESSAGE_REAL (NM, NOM, NL, NSCARC_BUFFER_LAYER1, 'MGM1')

         CASE (NSCARC_EXCHANGE_MGM2)
   ...
   SEND_PACK_OMESHES_LOOP: DO NOM = 1, NMESHES
      ...
      SEND_PACK_OMESHES_SELECT: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_MGM1)
            ...
            CALL SCARC_SEND_MESSAGE_REAL (NM, NOM, NL, NSCARC_BUFFER_LAYER1, 'MGM1')

         CASE (NSCARC_EXCHANGE_MGM2)
            CALL SCARC_PACK_MGM2(NM, NL)
...
SEND_UNPACK_MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   SEND_UNPACK_OMESHES_LOOP: DO NOM = 1, NMESHES
      ...
      SEND_UNPACK_OMESHES_SELECT: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_MGM2)
            CALL SCARC_UNPACK_MGM2 (NM, NOM, NL)
...
VC => SCARC(NM)%LEVEL(NLEVEL_MIN)%MGM%H1
OS%SEND_BUFFER_REAL = NSCARC_HUGE_REAL_EB
DO IOR0 = -3, 3
   IF (OL%GHOST_LASTW(IOR0) == 0) CYCLE
   DO ICG = OL%GHOST_FIRSTW(IOR0), OL%GHOST_LASTW(IOR0)
...
! Note: Vector VC is numbered via I, J, K values
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_MGM20(NM)
USE SCARC_POINTERS, ONLY: OL, OG, OS
INTEGER, INTENT(IN) :: NM
REAL(EB), DIMENSION(:,:,:), POINTER :: VC
INTEGER :: IOR0, ICG, IWG

VC => SCARC(NM)%LEVEL(NLEVEL_MIN)%MGM%H2
OS%SEND_BUFFER_REAL = NSCARC_HUGE_REAL_EB
DO IOR0 = -3, 3
   IF (OL%GHOST_LASTW(IOR0) == 0) CYCLE
   DO ICG = OL%GHOST_FIRSTW(IOR0), OL%GHOST_LASTW(IOR0)
      ...
      OS%SEND_BUFFER_REAL(ICG) = VC(G%WALL(IWG)%IXW, G%WALL(IWG)%IYW, G%WALL(IWG)%IZW)
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'PACK_MGM2: IOR0, ICG, IWG, SEND:', IOR0, ICG, IWG, OS%SEND_BUFFER_REAL(ICG)
#endif
   ENDDO
ENDDO
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'PACK_MGM2: SEND_BUFFER_REAL:'
WRITE(MSG%LU_DEBUG,'(8E16.8)') OS%SEND_BUFFER_REAL(1:16)
#endif
END SUBROUTINE SCARC_PACK_MGM20
...
! Note: Vector VC is numbered via I, J, K values
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_MGM20(NM, NOM)
USE SCARC_POINTERS, ONLY: OL, OG, RECV_BUFFER_REAL
INTEGER, INTENT(IN) :: NM, NOM
REAL(EB), DIMENSION(:,:,:), POINTER :: VC
INTEGER :: LL, IOR0, IWG, IXG, IYG, IZG, ICG
...
VC => SCARC(NM)%LEVEL(NLEVEL_MIN)%MGM%H2
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'UNPACK_MGM2: RECV_BUFFER_REAL:'
WRITE(MSG%LU_DEBUG,'(8E16.8)') RECV_BUFFER_REAL(1:16)
WRITE(MSG%LU_DEBUG,*) 'UNPACK_MGM2: VC:'
WRITE(MSG%LU_DEBUG,'(8E16.8)') VC 
#endif
LL = 1
DO IOR0 = -3, 3
   IF (OL%GHOST_LASTW(IOR0) == 0) CYCLE
   UNPACK_MGM1: DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      ...
      VC(IXG, IYG, IZG) = RECV_BUFFER_REAL(LL)
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'UNPACK_MGM2: IOR0, IWG, IXG, IYG, IZG, VC:', IOR0, IWG, IXG, IYG, IZG, VC(IXG, IYG, IZG)
#endif
      LL = LL + 1
   ENDDO UNPACK_MGM1
ENDDO
END SUBROUTINE SCARC_UNPACK_MGM20

! ------------------------------------------------------------------------------------------------
!> \brief Pack overlapping parts of specified vector VC (numbered via IC values)
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_MGM2(NM, NL)
USE SCARC_POINTERS, ONLY: OL, OG, OS
INTEGER, INTENT(IN) :: NM, NL
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_MGM2(NM, NOM, NL)
USE SCARC_POINTERS, ONLY: OL, OG, RECV_BUFFER_REAL
...
WRITE(MSG%LU_DEBUG,'(8E16.8)') SCARC_POINT_TO_BUFFER_REAL(1:16)
#endif
      ELSE
...
DO I = 1, N
  DY(I) = DY(I) + DA(I) * DX(I)
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A, I6, 3E16.8)') 'I, DX, DA, DY:', I, DX(I), DA(I), DY(I)
#endif
ENDDO
!$OMP END PARALLEL DO 
...
#ifdef WITH_SCARC_DEBUG
                  IF (JC /= 0) WRITE(MSG%LU_DEBUG,'(A,3I6, E12.4, I6, 2E12.4)') &
...
! ----------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_MGM_PARTIAL_SOLUTION(NL, NTYPE)
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) '================= MGM_PARTIAL_SOLUTION:',NTYPE
WRITE(MSG%LU_DEBUG,*) 'MGM%H1'
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%H1(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
WRITE(MSG%LU_DEBUG,*) 'MGM%H2'
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%H2(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
...
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%H3(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
   ...
   SELECT CASE(NTYPE)
      ...
      CASE (NSCARC_MGM_PREDICTOR)
         MGM%H1 = 0.0_EB
         DO IZ = 0, L%NZ+1
            DO IY = 0, L%NY+1
               DO IX = 0, L%NX+1
                  MGM%H1(IX, IY, IZ) = M%H(IX, IY, IZ)

#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 3I6,1E16.8)') 'MGM:H1:PREDICTOR: IX, IY, IZ, H1:', IX, IY, IZ, MGM%H1(IX,IY,IZ)
#endif
               ENDDO
            ENDDO
         ENDDO
         DO IZ = 1, L%NZ
            DO IY = 1, L%NY
               DO IX = 1, L%NX
                  IF (L%IS_SOLID(IX, IY, IZ)) MGM%H1(IX,IY,IZ) = 0.0_EB
               ENDDO
            ENDDO
         ENDDO

      CASE (NSCARC_MGM_CORRECTOR)
         MGM%H1 = 0.0_EB
         DO IZ = 0, L%NZ+1
            DO IY = 0, L%NY+1
               DO IX = 0, L%NX+1
                  MGM%H1(IX, IY, IZ) = M%HS(IX, IY, IZ)

#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 3I6,1E16.8)') 'MGM:H1:CORRECTOR: IX, IY, IZ, H1:', IX, IY, IZ, MGM%H1(IX,IY,IZ)
#endif
               ENDDO
            ENDDO
         ENDDO
...
#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 4I6,1E16.8)') 'MGM:H1:A: IX, IY, IZ, ICS, H1:', IX, IY, IZ, ICS, MGM%H1(IX,IY,IZ)
#endif
               ENDDO
            ENDDO
         ENDDO
         ...
         DO IW = 1, L%N_WALL_CELLS_EXT
            ...
            IF (GWC%BTYPE /= INTERNAL) CYCLE
               ...
               MGM%H1(IX, IY, IZ) = ST%X(ICE) 
#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 5I6,1E16.8)') 'MGM:H1:B: IX, IY, IZ, IW, ICE, H1:', IX, IY, IZ, IW, ICE, MGM%H1(IX,IY,IZ)
#endif
         ENDDO
            ...
            DO IZ = 0, L%NZ+1
               DO IY = 0, L%NY+1
                  DO IX = 0, L%NX+1
                     ...
                     MGM%H3(IX, IY, IZ) = MGM%H2(IX, IY, IZ)  

#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 4I6,1E16.8)') 'MGM:H3: IX, IY, IZ, ICU, H3:', IX, IY, IZ, ICU, MGM%H3(IX,IY,IZ)
#endif
                  ENDDO
               ENDDO
            ENDDO
...
#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 4I6,1E16.8)') 'MGM:H2:A: IX, IY, IZ, ICU, H2:', IX, IY, IZ, ICU, MGM%H2(IX,IY,IZ)
#endif
               ENDDO
            ENDDO
         ENDDO
         ...
         DO IZ = 0, L%NZ+1
            DO IY = 0, L%NY+1
               DO IX = 0, L%NX+1
                  ...
                  !IF (L%IS_SOLID(IX, IY, IZ)) CYCLE
                  MGM%H1(IX, IY, IZ) = MGM%H1(IX, IY, IZ) + MGM%H2(IX, IY, IZ)

#ifdef WITH_SCARC_DEBUG2
IF (IY == 1) WRITE(MSG%LU_DEBUG,'(A, 3I6,2E16.8)') 'MGM:M: IX, IY, IZ, H1, H2, H:',  IX, IY, IZ, &
                                                    MGM%H1(IX,IY,IZ), MGM%H2(IX,IY,IZ)
#endif
               ENDDO
            ENDDO
         ENDDO
         ...
         DO IZ = 1, L%NZ
            DO IY = 1, L%NY
               DO IX = 1, L%NX
                  ...
                  IF (L%IS_SOLID(IX, IY, IZ)) CYCLE
                  !IF (MESHES(NM)%SOLID(MESHES(NM)%CELL_INDEX(IX,IY,IZ))) CYCLE
                  ICS = L%STRUCTURED%CELL_NUMBER(IX, IY, IZ)              ! structured cell number
                  ICU = L%UNSTRUCTURED%CELL_NUMBER(IX, IY, IZ)            ! corresponding unstructured cell number
                  !IF (ICS <= G%NC .AND. ICU /= -1) HP(IX, IY, IZ) = MGM%H1(IX, IY, IZ) + MGM%H2(IX, IY, IZ)
                  HP(IX, IY, IZ) = MGM%H1(IX, IY, IZ) + MGM%H2(IX, IY, IZ)

#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A, 7I6,3E16.8)') 'MGM:M: IX, IY, IZ, ICS, ICU, X1(ICS), X1(ICU), X(ICS):', &
                                              IX, IY, IZ, ICS, ICU, &
                      L%STRUCTURED%CELL_NUMBER(IX, IY, IZ), &
                      L%UNSTRUCTURED%CELL_NUMBER(IX, IY, IZ), &
                      HP(IX,IY,IZ), MGM%H1(IX,IY,IZ), MGM%H2(IX,IY,IZ)
#endif
               ENDDO
            ENDDO
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'END OF PARTIAL SOLUTION'
WRITE(MSG%LU_DEBUG,*) 'MGM%H1'
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%H1(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
WRITE(MSG%LU_DEBUG,*) 'MGM%H2'
WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%H2(I,1,K), I=0, L%NX+1), K=L%NZ+1,0,-1)
...
END SUBROUTINE SCARC_MGM_VELOCITY_SETUP

! ---------------------------------------------------------------------------------------------------------------
!> \brief Set internal boundary conditions for unstructured, homogeneous part of McKeeney-Greengard-Mayo method
! ---------------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_MGM_VELOCITY_PROJECTION(NL, NTYPE)
USE SCARC_ITERATION_ENVIRONMENT
USE SCARC_POINTERS, ONLY: M, L, MGM, UU, VV, WW
REAL(EB), DIMENSION(:,:,:), POINTER :: H1, H2
INTEGER, INTENT(IN) :: NL, NTYPE
INTEGER  :: NM, I, J, K
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   MGM_PART_SELECT: SELECT CASE (NTYPE)
      ...
      !
      ! ------------- Inhomogeneous structured Poisson step
      !
      CASE (NSCARC_MGM_MERGE)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '================================ POISSON-PREDICTOR: ', PREDICTOR, M%IBAR, M%JBAR, M%KBAR
#endif
      
            DO K=1,M%KBAR
               DO J=1,M%JBAR
                  DO I=0,M%IBAR
                     UU(I,J,K) = M%U(I,J,K) - DT*( M%FVX(I,J,K) + M%RDXN(I)*(H2(I+1,J,K)-H1(I,J,K) + H2(I+1,J,K)-H1(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,7E16.8)') 'VELO X:P: ',I,J,K,M%U(I,J,K), M%FVX(I,J,K), &
                                                  H1(I+1,J,K), H1(I,J,K), H2(I+1,J,K), H2(I,J,K), UU(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=1,M%KBAR
               DO J=0,M%JBAR
                  DO I=1,M%IBAR
                     VV(I,J,K) = M%V(I,J,K) - DT*( M%FVY(I,J,K) + M%RDYN(J)*(H1(I,J+1,K)-H1(I,J,K) + H2(I,J+1,K)-H2(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,7E16.8)') 'VELO Y:P: ',I,J,K,M%V(I,J,K), M%FVY(I,J,K), &
                                                  H1(I,J+1,K), H1(I,J,K), H2(I,J+1,K), H2(I,J,K), VV(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=0,M%KBAR
               DO J=1,M%JBAR
                  DO I=1,M%IBAR
                     WW(I,J,K) = M%W(I,J,K) - DT*( M%FVZ(I,J,K) + M%RDZN(K)*(H1(I,J,K+1)-H1(I,J,K) + H2(I,J,K+1)-H2(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,7E16.8)') 'VELO Z:P: ',I,J,K,M%W(I,J,K), M%FVZ(I,J,K), &
                                      H1(I,J,K+1), H1(I,J,K), H2(I,J,K+1), H2(I,J,K), WW(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
...
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,*) '================================ POISSON-CORRECTOR: ', PREDICTOR, M%IBAR, M%JBAR, M%KBAR
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,6E16.8)') 'VELO X:C: ', I,J,K,M%U(I,J,K),M%US(I,J,K),M%FVX(I,J,K),HP(I+1,J,K),HP(I,J,K),UU(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,6E16.8)') 'VELO Y:C: ',I,J,K,M%V(I,J,K),M%VS(I,J,K),M%FVY(I,J,K),HP(I,J+1,K),HP(I,J,K),VV(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,6E16.8)') 'VELO Z:C: ', I,J,K,M%W(I,J,K),M%WS(I,J,K),M%FVZ(I,J,K),HP(I,J,K+1),HP(I,J,K),WW(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
      ...
      CASE (NSCARC_MGM_POISSON)
         ...
         MGM_POISSON_PREDICTOR_IF: IF (PREDICTOR) THEN
...
WRITE(MSG%LU_DEBUG,*) '================================ POISSON-PREDICTOR: ', PREDICTOR, M%IBAR, M%JBAR, M%KBAR
            ...
            DO K=1,M%KBAR
               DO J=1,M%JBAR
                  DO I=0,M%IBAR
                     UU(I,J,K) = M%U(I,J,K) - DT*( M%FVX(I,J,K) + M%RDXN(I)*(HP(I+1,J,K)-HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,5E16.8)') 'VELO X:P: ',I,J,K,M%U(I,J,K), M%FVX(I,J,K), HP(I+1,J,K), HP(I,J,K), UU(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=1,M%KBAR
               DO J=0,M%JBAR
                  DO I=1,M%IBAR
                     VV(I,J,K) = M%V(I,J,K) - DT*( M%FVY(I,J,K) + M%RDYN(J)*(HP(I,J+1,K)-HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,5E16.8)') 'VELO Y:P: ',I,J,K,M%V(I,J,K), M%FVY(I,J,K), HP(I,J+1,K), HP(I,J,K), VV(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=0,M%KBAR
               DO J=1,M%JBAR
                  DO I=1,M%IBAR
                     WW(I,J,K) = M%W(I,J,K) - DT*( M%FVZ(I,J,K) + M%RDZN(K)*(HP(I,J,K+1)-HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3i4,5E16.8)') 'VELO Z:P: ',I,J,K,M%W(I,J,K), M%FVZ(I,J,K), HP(I,J,K+1), HP(I,J,K), WW(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
      
         ELSE MGM_POISSON_PREDICTOR_IF
...
WRITE(MSG%LU_DEBUG,'(A,3i4,6E16.8)') 'VELO Z:C: ', I,J,K,M%W(I,J,K),M%WS(I,J,K),M%FVZ(I,J,K),HP(I,J,K+1),HP(I,J,K),WW(I,J,K)
         ...
         ENDIF MGM_POISSON_PREDICTOR_IF

      !
      ! ------------- Homogeneous structured Laplace step
      !
      CASE (NSCARC_MGM_LAPLACE)
         ...
         MGM_LAPLACE_PREDICTOR_IF: IF (PREDICTOR) THEN
      
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,*) '================================ LAPLACE-PREDICTOR: ', PREDICTOR, M%IBAR, M%JBAR, M%KBAR
#endif
      
            DO K=1,M%KBAR
               DO J=1,M%JBAR
                  DO I=0,M%IBAR
                     UU(I,J,K) = MGM%UP(I,J,K) - DT*( M%RDXN(I)*(HP(I+1,J,K)-HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,'(A,3i4,4E16.8)') 'VELO X:P: ',I,J,K,MGM%UP(I,J,K), HP(I+1,J,K), HP(I,J,K), UU(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=1,M%KBAR
               DO J=0,M%JBAR
                  DO I=1,M%IBAR
                     VV(I,J,K) = MGM%VP(I,J,K) - DT*(M%RDYN(J)*(HP(I,J+1,K)-HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,'(A,3i4,4E16.8)') 'VELO Y:P: ',I,J,K,MGM%VP(I,J,K), HP(I,J+1,K), HP(I,J,K), VV(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=0,M%KBAR
               DO J=1,M%JBAR
                  DO I=1,M%IBAR
                     WW(I,J,K) = MGM%WP(I,J,K) - DT*(M%RDZN(K)*(HP(I,J,K+1)-HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,'(A,3i4,4E16.8)') 'VELO Z:P: ',I,J,K,MGM%WP(I,J,K), HP(I,J,K+1), HP(I,J,K), WW(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
      
         ELSE MGM_LAPLACE_PREDICTOR_IF
      
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,*) '================================ LAPLACE-CORRECTOR: ', PREDICTOR, M%IBAR, M%JBAR, M%KBAR
#endif
            ...
            DO K=1,M%KBAR
               DO J=1,M%JBAR
                  DO I=0,M%IBAR
                     UU(I,J,K) = 0.5_EB*( M%U(I,J,K) + MGM%UP(I,J,K) - DT*(M%RDXN(I)*(HP(I+1,J,K)-HP(I,J,K))) )
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,'(A,3i4,5E16.8)') 'VELO X:C: ', I,J,K,M%U(I,J,K), MGM%UP(I,J,K), HP(I+1,J,K), HP(I,J,K), UU(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=1,M%KBAR
               DO J=0,M%JBAR
                  DO I=1,M%IBAR
                     VV(I,J,K) = 0.5_EB*( M%V(I,J,K) + MGM%VP(I,J,K)- DT*(M%RDYN(J)*(HP(I,J+1,K)-HP(I,J,K))) )
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,'(A,3i4,5E16.8)') 'VELO Y:C: ', I,J,K,M%V(I,J,K), MGM%VP(I,J,K),HP(I,J+1,K), HP(I,J,K), VV(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO
            ...
            DO K=0,M%KBAR
               DO J=1,M%JBAR
                  DO I=1,M%IBAR
                     WW(I,J,K) = 0.5_EB*( M%W(I,J,K) + MGM%WP(I,J,K) - DT*(M%RDZN(K)*(HP(I,J,K+1)-HP(I,J,K))) )
#ifdef WITH_SCARC_DEBUG
      WRITE(MSG%LU_DEBUG,'(A,3i4,6E16.8)') 'VELO Z:C: ', I,J,K,M%W(I,J,K), MGM%WP(I,J,K), HP(I,J,K+1), HP(I,J,K), WW(I,J,K)
#endif
                  ENDDO
               ENDDO
            ENDDO

         ENDIF MGM_LAPLACE_PREDICTOR_IF

   END SELECT MGM_PART_SELECT
ENDDO

 
END SUBROUTINE SCARC_MGM_VELOCITY_PROJECTION


! ---------------------------------------------------------------------------------------------------------------
!> \brief Set internal boundary conditions for unstructured, homogeneous part of McKeeney-Greengard-Mayo method
! ---------------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_MGM_VELOCITY_ERROR(NL, NTYPE)
USE SCARC_ITERATION_ENVIRONMENT
USE SCARC_POINTERS, ONLY: M, L, MGM, GWC, HP, UU, VV, WW
INTEGER, INTENT(IN) :: NL, NTYPE
INTEGER :: NM, I, J, K, IW, IOR0, III, KKK
REAL(EB) :: UN_NEW_OTHER, UN_NEW, DHFCT
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'PRES-INFO:FVX:', M%N_EXTERNAL_WALL_CELLS, M%N_INTERNAL_WALL_CELLS
   WRITE(MSG%LU_DEBUG,'(5I5)') (M%WALL(IW)%BOUNDARY_TYPE, IW = 1, M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS)
   WRITE(MSG%LU_DEBUG,'(6E16.8)') ((M%FVX(III,1,KKK), III=0, M%IBAR), KKK=M%KBAR,0,-1)
   WRITE(MSG%LU_DEBUG,*) 'PRES-INFO:H'
   WRITE(MSG%LU_DEBUG,'(7E16.8)') ((HP(III,1,KKK), III=0, M%IBAR+1), KKK=M%KBAR+1,0,-1)
   WRITE(MSG%LU_DEBUG,*) 'PRES-INFO:U'
   WRITE(MSG%LU_DEBUG,'(7E16.8)') ((UU(III,1,KKK), III=0, M%IBAR), KKK=M%KBAR,0,-1)
#endif

   DHFCT = 1.0_EB
   IF (NTYPE == NSCARC_MGM_LAPLACE) DHFCT = 1.0_EB

   DO IW = 1, L%N_WALL_CELLS_EXT + L%N_WALL_CELLS_INT

      !IF (M%WALL(IW)%BOUNDARY_TYPE /= SOLID_BOUNDARY         .AND. &
      !    M%WALL(IW)%BOUNDARY_TYPE /= INTERPOLATED_BOUNDARY) CYCLE

      IF (M%WALL(IW)%BOUNDARY_TYPE /= SOLID_BOUNDARY) CYCLE
      ...
      !   IF (NTYPE == NSCARC_MGM_LAPLACE) DHFCT = 0.0_EB
      !ENDIF
      ...
      IF (PREDICTOR) THEN
         SELECT CASE(IOR0)
            CASE( 1)
               UN_NEW = UU(I,J,K)   - DT*(M%FVX(I,J,K)   + M%RDXN(I)  *(MGM%H1(I+1,J,K)-MGM%H1(I,J,K))*DHFCT)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,6E16.8)') 'P 1: ',IW,I,J,K,UU(I,J,K), M%FVX(I,J,K), HP(I+1,J,K), HP(I,J,K), DHFCT, UN_NEW
#endif
            CASE(-1)
               UN_NEW = UU(I-1,J,K) - DT*(M%FVX(I-1,J,K) + M%RDXN(I-1)*(HP(I,J,K)-HP(I-1,J,K))*DHFCT)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,6E16.8)') 'P-1: ',IW,I,J,K,UU(I-1,J,K), M%FVX(I-1,J,K), HP(I,J,K), HP(I-1,J,K), DHFCT, UN_NEW
#endif
            CASE( 2)
               UN_NEW = VV(I,J,K)   - DT*(M%FVY(I,J,K)   + M%RDYN(J)  *(HP(I,J+1,K)-HP(I,J,K))*DHFCT)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,6E16.8)') 'P 2: ',IW,I,J,K,VV(I,J,K), M%FVY(I,J,K), HP(I,J+1,K), HP(I,J,K), DHFCT, UN_NEW
#endif
            CASE(-2)
               UN_NEW = VV(I,J-1,K) - DT*(M%FVY(I,J-1,K) + M%RDYN(J-1)*(HP(I,J,K)-HP(I,J-1,K))*DHFCT)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,6E16.8)') 'P-2: ',IW,I,J,K,VV(I,J-1,K), M%FVY(I,J-1,K), HP(I,J,K), HP(I,J-1,K), DHFCT, UN_NEW
#endif
            CASE( 3)
               UN_NEW = WW(I,J,K)   - DT*(M%FVZ(I,J,K)   + M%RDZN(K)  *(HP(I,J,K+1)-HP(I,J,K))*DHFCT)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,6E16.8)') 'P 3: ',IW,I,J,K,WW(I,J,K), M%FVZ(I,J,K), HP(I,J,K+1), HP(I,J,K), DHFCT, UN_NEW
#endif
            CASE(-3)
               UN_NEW = WW(I,J,K-1) - DT*(M%FVZ(I,J,K-1) + M%RDZN(K-1)*(HP(I,J,K)-HP(I,J,K-1))*DHFCT)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4i4,6E16.8)') 'P-3: ',IW,I,J,K,WW(I,J,K-1), M%FVZ(I,J,K-1), HP(I,J,K), HP(I,J,K-1), DHFCT, UN_NEW
#endif
         END SELECT
      ELSE
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'HALLOHALLO SUSI: WELCHE VELOCITIES?'
#endif
         SELECT CASE(IOR0)
            CASE( 1)
               UN_NEW = 0.5_EB*(M%U(I,J,K)+MGM%UU(I,J,K)     - DT*(M%FVX(I,J,K)   + M%RDXN(I)  *(HP(I+1,J,K)-HP(I,J,K))*DHFCT))
            CASE(-1)
               UN_NEW = 0.5_EB*(M%U(I-1,J,K)+MGM%UU(I-1,J,K) - DT*(M%FVX(I-1,J,K) + M%RDXN(I-1)*(HP(I,J,K)-HP(I-1,J,K))*DHFCT))
            CASE( 2)
               UN_NEW = 0.5_EB*(M%V(I,J,K)+MGM%VV(I,J,K)     - DT*(M%FVY(I,J,K)   + M%RDYN(J)  *(HP(I,J+1,K)-HP(I,J,K))*DHFCT))
            CASE(-2)
               UN_NEW = 0.5_EB*(M%V(I,J-1,K)+MGM%VV(I,J-1,K) - DT*(M%FVY(I,J-1,K) + M%RDYN(J-1)*(HP(I,J,K)-HP(I,J-1,K))*DHFCT))
            CASE( 3)
               UN_NEW = 0.5_EB*(M%W(I,J,K)+MGM%WW(I,J,K)     - DT*(M%FVZ(I,J,K)   + M%RDZN(K)  *(HP(I,J,K+1)-HP(I,J,K))*DHFCT))
            CASE(-3)
               UN_NEW = 0.5_EB*(M%W(I,J,K-1)+MGM%WW(I,J,K-1) - DT*(M%FVZ(I,J,K-1) + M%RDZN(K-1)*(HP(I,J,K)-HP(I,J,K-1))*DHFCT))
         END SELECT
      ENDIF
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4I4, 3E16.8)') 'VELO-ERR:SP: ',  IW, I, J, K, UN_NEW, UN_NEW_OTHER, UN_NEW - UN_NEW_OTHER
#endif
         ELSE
            UN_NEW_OTHER = -SIGN(1._EB,REAL(IOR0,EB))*MESHES(NM)%WALL(IW)%ONE_D%U_NORMAL
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4I4, 3E16.8)') 'VELO-ERR:SC: ',  IW, I, J, K, UN_NEW, UN_NEW_OTHER, UN_NEW - UN_NEW_OTHER
#endif
         ENDIF
      !ENDIF
      ...
      MGM%VELOCITY_ERROR = UN_NEW - UN_NEW_OTHER
      IF (ABS(MGM%VELOCITY_ERROR) > MGM%VELOCITY_ERROR_MAX) MGM%VELOCITY_ERROR_MAX = MGM%VELOCITY_ERROR

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4I4, 3E16.8)') '--------------> FINAL : ',  IW, I, J, K, MGM%VELOCITY_ERROR, MGM%VELOCITY_ERROR_MAX
#endif

   ENDDO
   MESH_REAL(NM) = MGM%VELOCITY_ERROR_MAX
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'ALL MESHES: VELOCITY_ERROR_MAX =', VELOCITY_ERROR_MAX
#endif

END SUBROUTINE SCARC_MGM_VELOCITY_ERROR

! ---------------------------------------------------------------------------------------------------------------
!> \brief Set internal boundary conditions for unstructured, homogeneous part of McKeeney-Greengard-Mayo method
! ---------------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_MGM_VELOCITY_OBSTRUCTIONS(NL)
USE SCARC_ITERATION_ENVIRONMENT
...
WRITE(MSG%LU_DEBUG,*) '====================================================', PREDICTOR
   ...
   !CALL SCARC_DEBUG_VECTOR3_BIG(WW, NM, 'MGM: W  : SETUP_WORKSPACE')
#endif
      ...
      !DO IW = 1, L%N_WALL_CELLS_EXT + L%N_WALL_CELLS_INT
         ...
         SELECT CASE (IOR0)
            CASE(1)
               MGM%UWALL(IW) = M%U(I-1,J,K) - DT*( M%FVX(I-1,J,K) + M%RDXN(I-1)*(HP(I,J,K) - HP(I-1,J,K)) )
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,4E16.8,a,E16.8)') 'MGM:P:A : IW,IOR0 : I,J,K : U,FVX,HPP,HP : UU', &
  IW, IOR0,' : ', I-1, J, K, ' : ',M%U(I-1,J,K), M%FVX(I-1,J,K), HP(I,J,K), HP(I-1,J,K), ' : ',MGM%UWALL(IW)
#endif
            CASE(-1)
               MGM%UWALL(IW) = M%U(I,J,K) - DT*( M%FVX(I,J,K) + M%RDXN(I)*(HP(I+1,J,K) - HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,4E16.8,a,E16.8)') 'MGM:P:B : IW,IOR0 : I,J,K : U,FVX,HPP,HP : UU', &
  IW, IOR0,' : ', I, J, K, ' : ',M%U(I,J,K), M%FVX(I,J,K), HP(I+1,J,K), HP(I,J,K), ' : ',MGM%UWALL(IW)
#endif
            CASE(2)
               MGM%VWALL(IW) = M%V(I,J-1,K) - DT*( M%FVY(I,J-1,K) + M%RDYN(J-1)*(HP(I,J,K) - HP(I,J-1,K)) )
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,4E16.8,a,E16.8)') 'MGM:P:C : IW,IOR0 : I,J,K : V,FVY,HPP,HP : VV', &
  IW, IOR0,' : ', I, J-1, K, ' : ',M%V(I,J-1,K), M%FVY(I,J-1,K), HP(I,J,K), HP(I,J-1,K), ' : ',MGM%VWALL(IW)
#endif
            CASE(-2)
               MGM%VWALL(IW) = M%V(I,J,K) - DT*( M%FVY(I,J,K) + M%RDYN(J)*(HP(I,J+1,K) - HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,4E16.8,a,E16.8)') 'MGM:P:D : IW,IOR0 : I,J,K : V,FVY,HPP,HP : VV', &
  IW, IOR0,' : ', I, J, K, ' : ',M%V(I,J,K), M%FVY(I,J,K), HP(I,J+1,K), HP(I,J,K), ' : ',MGM%VWALL(IW)
#endif
            CASE(3)
               MGM%WWALL(IW) = M%W(I,J,K-1) - DT*( M%FVZ(I,J,K-1) + M%RDZN(K-1)*(HP(I,J,K) - HP(I,J,K-1)) )
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,4E16.8,a,E16.8)') 'MGM:P:E : IW,IOR0 : I,J,K : W,FVZ,HPP,HP : WW', &
  IW, IOR0,' : ', I, J, K-1, ' : ',M%W(I,J,K-1), M%FVZ(I,J,K-1), HP(I,J,K), HP(I,J,K-1), ' : ',MGM%WWALL(IW)
#endif
            CASE(-3)
               MGM%WWALL(IW) = M%W(I,J,K) - DT*( M%FVZ(I,J,K) + M%RDZN(K)*(HP(I,J,K+1) - HP(I,J,K)) )
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,4E16.8,a,E16.8)') 'MGM:P:F : IW,IOR0 : I,J,K : W,FVZ,HPP,HP : WW', &
  IW, IOR0,' : ', I, J, K, ' : ',M%W(I,J,K), M%FVZ(I,J,K), HP(I,J,K+1), HP(I,J,K), ' : ',MGM%WWALL(IW)
#endif
         END SELECT
...
WRITE(MSG%LU_DEBUG,*) '====================================================', CORRECTOR
      ...
      DO IW = L%N_WALL_CELLS_EXT+1, L%N_WALL_CELLS_EXT + L%N_WALL_CELLS_INT
         ...
         SELECT CASE (IOR0)
            CASE(1)
               MGM%UWALL(IW) = 0.5_EB * (M%U(I-1,J,K) + M%US(I-1,J,K) &
                                        - DT*( M%FVX(I-1,J,K) + M%RDXN(I-1)*(HP(I,J,K) - HP(I-1,J,K)) ))
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,5E16.8,a,E16.8)') 'MGM:C:A : IW,IOR0 : I,J,K : U,FVX,HPP,HP : UU', &
  IW, IOR0,' : ', I-1, J, K, ' : ',M%U(I-1,J,K), M%US(I-1,J,K),M%FVX(I-1,J,K), HP(I,J,K), HP(I-1,J,K), ' : ',MGM%UWALL(IW)
#endif
            CASE(-1)
               MGM%UWALL(IW) = 0.5_EB * (M%U(I,J,K)  + M%US(I,J,K) &
                                        - DT*( M%FVX(I,J,K) + M%RDXN(I)*(HP(I+1,J,K) - HP(I,J,K)) ))
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,5E16.8,a,E16.8)') 'MGM:C:B : IW,IOR0 : I,J,K : U,FVX,HPP,HP : UU', &
  IW, IOR0,' : ', I, J, K, ' : ',M%U(I,J,K), M%US(I,J,K),M%FVX(I,J,K), HP(I+1,J,K), HP(I,J,K), ' : ',MGM%UWALL(IW)
#endif
            CASE(2)
               MGM%VWALL(IW) = 0.5_EB * (M%V(I,J-1,K) + M%VS(I,J-1,K) &
                                        - DT*( M%FVY(I,J-1,K) + M%RDYN(J-1)*(HP(I,J,K) - HP(I,J-1,K)) ))
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,5E16.8,a,E16.8)') 'MGMC:C : IW,IOR0 : I,J,K : V,FVY,HPP,HP : VV', &
  IW, IOR0,' : ', I, J-1, K, ' : ',M%V(I,J-1,K), M%VS(I,J-1,K), M%FVY(I,J-1,K), HP(I,J,K), HP(I,J-1,K), ' : ',MGM%VWALL(IW)
#endif
            CASE(-2)
               MGM%VWALL(IW) = 0.5_EB * (M%V(I,J,K) + M%VS(I,J,K) &
                                        - DT*( M%FVY(I,J,K) + M%RDYN(J)*(HP(I,J+1,K) - HP(I,J,K)) ))
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,5E16.8,a,E16.8)') 'MGM:C:D : IW,IOR0 : I,J,K : V,FVY,HPP,HP : VV', &
  IW, IOR0,' : ', I, J, K, ' : ',M%V(I,J,K), M%VS(I,J,K), M%FVY(I,J,K), HP(I,J+1,K), HP(I,J,K), ' : ',MGM%VWALL(IW)
#endif
            CASE(3)
               MGM%WWALL(IW) = 0.5_EB * (M%W(I,J,K-1) + M%WS(I,J,K-1) &
                                        - DT*( M%FVZ(I,J,K-1) + M%RDZN(K-1)*(HP(I,J,K) - HP(I,J,K-1)) ))
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,5E16.8,a,E16.8)') 'MGM:C:E : IW,IOR0 : I,J,K : W,FVZ,HPP,HP : WW', &
  IW, IOR0,' : ', I, J, K-1, ' : ',M%W(I,J,K-1), M%WS(I,J,K-1), M%FVZ(I,J,K-1), HP(I,J,K), HP(I,J,K-1), ' : ',MGM%WWALL(IW)
#endif
            CASE(-3)
               MGM%WWALL(IW) = 0.5_EB * (M%W(I,J,K) + M%WS(I,J,K) &
                                        - DT*( M%FVZ(I,J,K) + M%RDZN(K)*(HP(I,J,K+1) - HP(I,J,K)) ))
#ifdef WITH_SCARC_DEBUG
  WRITE(MSG%LU_DEBUG,'(a,2I4,a,3i4,a,5E16.8,a,E16.8)') 'MGM:C:F : IW,IOR0 : I,J,K : W,FVZ,HPP,HP : WW', &
  IW, IOR0,' : ', I, J, K, ' : ',M%W(I,J,K), M%WS(I,J,K), M%FVZ(I,J,K), HP(I,J,K+1), HP(I,J,K), ' : ',MGM%WWALL(IW)
#endif
         END SELECT
...
END SUBROUTINE SCARC_MGM_VELOCITY_OBSTRUCTIONS
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   DO NL = NLMIN, NLMAX
      ...
      CALL SCARC_ALLOCATE_REAL3(MGM%UL, 0, L%NX, 0, L%NY, 0, L%NZ, NSCARC_INIT_ZERO, 'MGM%UL', CROUTINE)
      CALL SCARC_ALLOCATE_REAL3(MGM%VL, 0, L%NX, 0, L%NY, 0, L%NZ, NSCARC_INIT_ZERO, 'MGM%VL', CROUTINE)
      CALL SCARC_ALLOCATE_REAL3(MGM%WL, 0, L%NX, 0, L%NY, 0, L%NZ, NSCARC_INIT_ZERO, 'MGM%WL', CROUTINE)

      CALL SCARC_ALLOCATE_REAL3(MGM%UP, 0, L%NX, 0, L%NY, 0, L%NZ, NSCARC_INIT_ZERO, 'MGM%UP', CROUTINE)
      CALL SCARC_ALLOCATE_REAL3(MGM%VP, 0, L%NX, 0, L%NY, 0, L%NZ, NSCARC_INIT_ZERO, 'MGM%VP', CROUTINE)
      CALL SCARC_ALLOCATE_REAL3(MGM%WP, 0, L%NX, 0, L%NY, 0, L%NZ, NSCARC_INIT_ZERO, 'MGM%WP', CROUTINE)
      ...
      CALL SCARC_ALLOCATE_REAL1(MGM%UWALL, MGM%NW1, MGM%NW2, NSCARC_INIT_ZERO, 'MGM%UWALL', CROUTINE)
      CALL SCARC_ALLOCATE_REAL1(MGM%VWALL, MGM%NW1, MGM%NW2, NSCARC_INIT_ZERO, 'MGM%VWALL', CROUTINE)
      CALL SCARC_ALLOCATE_REAL1(MGM%WWALL, MGM%NW1, MGM%NW2, NSCARC_INIT_ZERO, 'MGM%WWALL', CROUTINE)
...
WRITE(MSG%LU_DEBUG,'(5E16.8)') (MGM%B(I), I=1, G%NC)
#endif
...
WRITE(MSG%LU_DEBUG,'(5E16.8)') (MGM%Y(I), I=1, G%NC)
WRITE(MSG%LU_DEBUG,*) '=============================== X'
WRITE(MSG%LU_DEBUG,'(5E16.8)') (MGM%X(I), I=1, G%NC)
#endif
...
DO IZ = L%NZ, 1, -1
   ...
   DO IY = L%NY, 1, -1
      DO IX = 1, L%NX
         ...
         ENDIF
      ENDDO
      WRITE(LU_DUMP,'(E12.4)') VALUES(1:L%NX)
   ENDDO
ENDDO
!WRITE(LU_DUMP,*) '============================================================='
CLOSE(LU_DUMP)
...
DO IZ = L%NZ, 1, -1
   ...
   DO IY = L%NY, 1, -1
      WRITE(MSG%LU_DEBUG,'(8E16.8)') (VC(IX, IY, IZ), IX = 1, L%NX)
   ENDDO
ENDDO
WRITE(MSG%LU_DEBUG,*) '============================================================='
...
DO IZ = MESHES(NM)%KBP1, 0, -1
   ...
   !DO IY = MESHES(NM)%JBP1, 0, -1
   DO IY = MESHES(NM)%JBAR, 1, -1
      WRITE(MSG%LU_DEBUG,'(10E16.8)') (HH(IX, IY, IZ), IX = 0, MESHES(NM)%IBP1)
   ENDDO
ENDDO
WRITE(MSG%LU_DEBUG,*) '============================================================='
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   WRITE(MSG%LU_DEBUG, '(4E16.8)') (VC(IC), IC = G%NC+1, G%NCE)
   !ENDIF
ENDDO
...
ENDDO
WRITE(MSG%LU_DEBUG, *) '---------------- Overlap ----------------'
WRITE(MSG%LU_DEBUG, '(4E16.8)') (X(IC), IC = GG%NC+1, GG%NCE)
   ...
   CASE (NSCARC_DEBUG_PRESSURE)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         IF (PREDICTOR) THEN
            ...
            DO KKK = NNZ+1, 0, -1
               WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%RHO(III, 1, KKK), III=0,NNX+1)
            ENDDO
            WRITE(MSG%LU_DEBUG,*) 'H'
            DO KKK = NNZ+1, 0, -1
               WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%H(III, 1, KKK), III=0,NNX+1)
            ENDDO
         ELSE
            WRITE(MSG%LU_DEBUG,*) 'RHOS'
            DO KKK = NNZ+1, 0, -1
               WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%RHOS(III, 1, KKK), III=0,NNX+1)
            ENDDO
            WRITE(MSG%LU_DEBUG,*) 'HS'
            DO KKK = NNZ+1, 0, -1
               WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%HS(III, 1, KKK), III=0,NNX+1)
            ENDDO
         ENDIF
         WRITE(MSG%LU_DEBUG,*) 'FVX'
         DO KKK = NNZ+1, 0, -1
            WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%FVX(III, 1, KKK), III=0,NNX+1)
         ENDDO
         WRITE(MSG%LU_DEBUG,*) 'FVY'
         DO KKK = NNZ+1, 0, -1
            WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%FVY(III, 1, KKK), III=0,NNX+1)
         ENDDO
         WRITE(MSG%LU_DEBUG,*) 'FVZ'
         DO KKK = NNZ+1, 0, -1
            WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%FVZ(III, 1, KKK), III=0,NNX+1)
         ENDDO
         WRITE(MSG%LU_DEBUG,*) 'KRES'
         DO KKK = NNZ+1, 0, -1
            WRITE(MSG%LU_DEBUG,'(I8,10E16.8)') KKK, (M%KRES(III, 1, KKK), III=0,NNX+1)
         ENDDO
         WRITE(MSG%LU_DEBUG,*) 'M%PRHS'
         DO KKK = NNZ+1, 1, -1
            WRITE(MSG%LU_DEBUG,'(I8,9E16.8)') KKK, (M%PRHS(III, 1, KKK), III=1,NNX+1)
         ENDDO
         !WRITE(MSG%LU_DEBUG,*) 'P%PRHS'
         !DO KKK = NNZ+1, 1, -1
         !WRITE(MSG%LU_DEBUG,'(I8,9E16.8)') KKK, (P%PRHS(III, 1, KKK), III=1,NNX+1)
         !ENDDO
