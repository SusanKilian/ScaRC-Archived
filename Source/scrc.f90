...
TYPE (SCARC_MATRIX_COMPACT_TYPE), POINTER :: ACC=>NULL()
...
LEVEL_LOOP: DO NL = NLEVEL_MIN, NLEVEL_MAX - 1
   ...
   MESHES_LOOP2: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
      ...
      SELECT CASE(TYPE_GRID)
         CASE (NSCARC_GRID_STRUCTURED)
            ACC => SCARC(NM)%LEVEL(NL+1)%STRUCTURED%AC
         CASE (NSCARC_GRID_UNSTRUCTURED)
            ACC => SCARC(NM)%LEVEL(NL+1)%UNSTRUCTURED%AC
      END SELECT
      ACC%N_ROW = MG%N_COARSE+1
      ACC%N_ROW = MG%N_VAL_COARSE
      CALL SCARC_ALLOCATE_MATRIX_COMPACT(ACC, NL+1, NSCARC_PRECISION_DOUBLE, &
                                         NSCARC_INIT_ZERO, 'MG(NL+1)%AC')

      CALL SCARC_SETUP_COARSE_MATRIX(ACC)
...
WRITE(MSG%LU_DEBUG,'(8I8)') ACC%COL_GLOBAL
#endif
