...
LEVEL_LOOP: DO NL = NLEVEL_MIN, NLEVEL_MAX - 1
   ...
   MESHES_LOOP2: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
      ...
      SELECT CASE(TYPE_GRID)
         CASE (NSCARC_GRID_STRUCTURED)
            SCARC(NM)%LEVEL(NL+1)%STRUCTURED%AC%N_ROW = MG%N_COARSE+1
            SCARC(NM)%LEVEL(NL+1)%STRUCTURED%AC%N_VAL = MG%N_VAL_COARSE
         CASE (NSCARC_GRID_UNSTRUCTURED)
            SCARC(NM)%LEVEL(NL+1)%UNSTRUCTURED%AC%N_ROW = MG%N_COARSE+1
            SCARC(NM)%LEVEL(NL+1)%UNSTRUCTURED%AC%N_VAL = MG%N_VAL_COARSE
      END SELECT
