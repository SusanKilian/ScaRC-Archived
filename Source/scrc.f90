...
!> 
INTEGER, ALLOCATABLE, DIMENSION (:) :: NC_LOCAL             !> Local number of cells entries on finest level
INTEGER, ALLOCATABLE, DIMENSION (:) :: NA_LOCAL             !> Local number of matrix entries (symmetric matrix) on finest level
INTEGER :: NC_GLOBAL(NSCARC_LEVEL_MAX) = 0                  !> number of global grid cells
INTEGER :: NA_GLOBAL(NSCARC_LEVEL_MAX) = 0                  !> number of global matrix entries (symmetric matrix)
...
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: COUNTS_SZ          !> Counter array for data exchange of size information
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: COUNTS_NC          !> Counter array for data exchange of grid vector
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: COUNTS_NA          !> Counter array for data exchange of matrix entries
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: DISPLS             !> Displacement array for data exchange
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: DISPLS_SZ          !> Displacement array for data exchange of size information
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: DISPLS_NC          !> Displacement array for data exchange of grid vector
INTEGER,  ALLOCATABLE, DIMENSION (:)  :: DISPLS_NA          !> Displacement array for data exchange of matrix entries
...
INTEGER,  ALLOCATABLE, DIMENSION (:) :: INT_BUFFER_RECV1, INT_BUFFER_SEND1
INTEGER,  ALLOCATABLE, DIMENSION (:) :: INT_BUFFER_RECV2, INT_BUFFER_SEND2
INTEGER,  ALLOCATABLE, DIMENSION (:) :: INT_BUFFER_RECV3, INT_BUFFER_SEND3
REAL(EB), ALLOCATABLE, DIMENSION (:) :: REAL_BUFFER_RECV1, REAL_BUFFER_SEND1
REAL(EB), ALLOCATABLE, DIMENSION (:) :: REAL_BUFFER_RECV2, REAL_BUFFER_SEND2
REAL(EB), ALLOCATABLE, DIMENSION (:) :: REAL_BUFFER_RECV3, REAL_BUFFER_SEND3
REAL(FB), ALLOCATABLE, DIMENSION (:) :: REAL_FB_BUFFER_RECV3, REAL_FB_BUFFER_SEND3
...
!> ---------- Public variables
!> 
PUBLIC :: SCARC_SETUP, SCARC_METHOD, SCARC_SOLVER, SCARC_GRID, SCARC_TWOLEVEL, SCARC_MASTER, SCARC_MASTER_SLAVE
...
#ifdef WITH_MKL
USE SCARC_POINTERS, ONLY : ACS, MKL
#endif
INTEGER :: NM, TAG0, MASTER_ID
INTEGER :: REQ0(100), N_REQ0, N, ICYCLE, IDUMMY = 0
INTEGER :: ICL, ICG, IAG, IAL, IC_OFFSET, IA_OFFSET !,IC
INTEGER,  ALLOCATABLE, DIMENSION (:) :: NA_SLAVES, NC_SLAVES, NL_SLAVES
REAL(EB), ALLOCATABLE, DIMENSION (:) :: X, B
REAL(FB), ALLOCATABLE, DIMENSION (:) :: X_FB, B_FB
REAL(EB) :: RDUMMY = 0.0_EB
...
!>
!> Initialize sizes and offset vectors for global communications of initial data, vectors and matrix entries
!>
CALL SCARC_ALLOCATE_INT1 (COUNTS,    0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'COUNTS')
CALL SCARC_ALLOCATE_INT1 (COUNTS_SZ, 0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'COUNTS_SZ')
CALL SCARC_ALLOCATE_INT1 (COUNTS_NC, 0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'COUNTS_NC')
CALL SCARC_ALLOCATE_INT1 (COUNTS_NA, 0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'COUNTS_NA')

CALL SCARC_ALLOCATE_INT1 (DISPLS,    0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'DISPLS')
CALL SCARC_ALLOCATE_INT1 (DISPLS_SZ, 0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'DISPLS_SZ')
CALL SCARC_ALLOCATE_INT1 (DISPLS_NC, 0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'DISPLS_NC')
CALL SCARC_ALLOCATE_INT1 (DISPLS_NA, 0, N_MPI_PROCESSES-1, NSCARC_INIT_ZERO, 'DISPLS_NA')

DO N = 0, N_MPI_PROCESSES - 1
   DO NM = 1, NMESHES
      IF (PROCESS(NM) == N) COUNTS(N)    = COUNTS(N)    + 1
      IF (PROCESS(NM) == N) COUNTS_SZ(N) = COUNTS_SZ(N) + 3
   ENDDO
ENDDO

DO N = 1, N_MPI_PROCESSES -1
   DISPLS(N)    = COUNTS(N-1)    + DISPLS(N-1)
   DISPLS_SZ(N) = COUNTS_SZ(N-1) + DISPLS_SZ(N-1)
ENDDO

#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'COUNTS'
WRITE(MSG%LU_DEBUG,'(8I6)') COUNTS
WRITE(MSG%LU_DEBUG,*) 'COUNTS_SZ'
WRITE(MSG%LU_DEBUG,'(8I6)') COUNTS_SZ
WRITE(MSG%LU_DEBUG,*) 'DISPLS'
WRITE(MSG%LU_DEBUG,'(8I6)') DISPLS
WRITE(MSG%LU_DEBUG,*) 'DISPLS_SZ'
WRITE(MSG%LU_DEBUG,'(8I6)') DISPLS_SZ
#endif

CALL SCARC_ALLOCATE_INT1 (NC_SLAVES, 1, NMESHES, NSCARC_INIT_ZERO, 'NC_SLAVES')
CALL SCARC_ALLOCATE_INT1 (NA_SLAVES, 1, NMESHES, NSCARC_INIT_ZERO, 'NA_SLAVES')
CALL SCARC_ALLOCATE_INT1 (NL_SLAVES, 1, NMESHES, NSCARC_INIT_ZERO, 'NL_SLAVES')

CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_RECV1, 1, 3*NMESHES, NSCARC_INIT_ZERO, 'INT_BUFFER_RECV1')

CALL MPI_GATHERV(IDUMMY, COUNTS_SZ(MYID), MPI_INTEGER, &
                 INT_BUFFER_RECV1, COUNTS_SZ, DISPLS_SZ, MPI_INTEGER, MASTER_ID, MPI_COMM_WORLD, IERROR)

DO NM = 1, NMESHES
   NC_SLAVES(NM) = INT_BUFFER_RECV1((NM-1)*3 + 1)
   NA_SLAVES(NM) = INT_BUFFER_RECV1((NM-1)*3 + 2)
   NL_SLAVES(NM) = INT_BUFFER_RECV1((NM-1)*3 + 3)
ENDDO
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'MASTER: NC_SLAVES =', NC_SLAVES
WRITE(MSG%LU_DEBUG,*) 'MASTER: NA_SLAVES =', NA_SLAVES
WRITE(MSG%LU_DEBUG,*) 'MASTER: NL_SLAVES =', NL_SLAVES
#endif

DO N = 0, N_MPI_PROCESSES - 1
   DO NM = 1, NMESHES
      IF (PROCESS(NM) == N) COUNTS_NC(N) = COUNTS_NC(N) + NC_SLAVES(NM)
      IF (PROCESS(NM) == N) COUNTS_NA(N) = COUNTS_NA(N) + NA_SLAVES(NM)
   ENDDO
ENDDO

DO N = 1, N_MPI_PROCESSES -1
   DISPLS_NC(N) = COUNTS_NC(N-1) + DISPLS_NC(N-1)
   DISPLS_NA(N) = COUNTS_NA(N-1) + DISPLS_NA(N-1)
ENDDO

#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'COUNTS_NC'
WRITE(MSG%LU_DEBUG,'(8I6)') COUNTS_NC
WRITE(MSG%LU_DEBUG,*) 'DISPLS_NC'
WRITE(MSG%LU_DEBUG,'(8I6)') DISPLS_NC
WRITE(MSG%LU_DEBUG,*) 'COUNTS_NA'
WRITE(MSG%LU_DEBUG,'(8I6)') COUNTS_NA
WRITE(MSG%LU_DEBUG,*) 'DISPLS_NA'
WRITE(MSG%LU_DEBUG,'(8I6)') DISPLS_NA
#endif
...
CALL SCARC_ALLOCATE_MATRIX_COMPACT (ACS, NLEVEL_MAX, TYPE_MKL_PRECISION, NSCARC_INIT_ZERO, 'MASTER%ACS')

CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_RECV2,  1, G%NC, NSCARC_INIT_ZERO, 'INT_BUFFER_RECV2')
CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_RECV3,  1, G%NA, NSCARC_INIT_ZERO, 'INT_BUFFER_RECV3')
CALL SCARC_ALLOCATE_REAL1(REAL_BUFFER_RECV2, 1, G%NC, NSCARC_INIT_ZERO, 'REAL_BUFFER_RECV2')
CALL SCARC_ALLOCATE_REAL1(REAL_BUFFER_RECV3, 1, G%NA, NSCARC_INIT_ZERO, 'REAL_BUFFER_RECV3')

!>
!> Gather local matrix row, column and value information from slaves
!>
CALL MPI_GATHERV(IDUMMY, COUNTS_NC(MYID), MPI_INTEGER, &
                 INT_BUFFER_RECV2, COUNTS_NC, DISPLS_NC, MPI_INTEGER, MASTER_ID, MPI_COMM_WORLD, IERROR)
CALL MPI_GATHERV(IDUMMY, COUNTS_NA(MYID), MPI_INTEGER, &
                 INT_BUFFER_RECV3, COUNTS_NA, DISPLS_NA, MPI_INTEGER, MASTER_ID, MPI_COMM_WORLD, IERROR)
CALL MPI_GATHERV(RDUMMY, COUNTS_NA(MYID), MPI_DOUBLE_PRECISION, &
                 REAL_BUFFER_RECV3, COUNTS_NA, DISPLS_NA, MPI_DOUBLE_PRECISION, MASTER_ID, MPI_COMM_WORLD, IERROR)


!>
!> Set them together to a global matrix
!>
IC_OFFSET = 0
IA_OFFSET = 0
...
DO NM = 1, NMESHES
   DO ICL = 1, NC_SLAVES(NM)
      ICG = ICL + IC_OFFSET
      ACS%ROW(ICG) = INT_BUFFER_RECV2(ICG) + IA_OFFSET
   ENDDO
   IC_OFFSET = IC_OFFSET + NC_SLAVES(NM)
ENDDO
...
IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
   DO NM = 1, NMESHES
      DO IAL = 1, NA_SLAVES(NM)
         IAG = IAL + IA_OFFSET
         ACS%COL(IAG) = INT_BUFFER_RECV3(IAG) 
         ACS%VAL_FB(IAG) = REAL(REAL_BUFFER_RECV3(IAG),FB)
      ENDDO
      IA_OFFSET = IA_OFFSET + NA_SLAVES(NM)
   ENDDO
ELSE
   DO NM = 1, NMESHES
      DO IAL = 1, NA_SLAVES(NM)
         IAG = IAL + IA_OFFSET
         ACS%COL(IAG) = INT_BUFFER_RECV3(IAG) 
         ACS%VAL(IAG) = REAL_BUFFER_RECV3(IAG) 
      ENDDO
      IA_OFFSET = IA_OFFSET + NA_SLAVES(NM)
   ENDDO
ENDIF
ACS%ROW(ICG+1) = IAG + 1
         ...
         WRITE(*,*) 'G%NC=', G%NC
         WRITE(*,*) 'ACS%N_ROW = ', ACS%N_ROW
         WRITE(*,*) 'ACS%N_VAL = ', ACS%N_VAL
         WRITE(*,*) 'ACS%ROW(G%NC)=', ACS%ROW(G%NC)
         WRITE(*,*) 'ACS%ROW(G%NC+1)=', ACS%ROW(G%NC+1)
         WRITE(*,*) 'SIZE(ASC) =',    SIZE(ACS%VAL)
         WRITE(*,*) 'SIZE(ACS%COL) =',SIZE(ACS%COL)
         WRITE(*,*) 'SIZE(ACS%ROW) =',SIZE(ACS%ROW)
         WRITE(*,*) '---------------------- ACS%ROW:', G%NC
         WRITE(*,'(8I6)') (ACS%ROW(IC), IC=1,G%NC+1)
         WRITE(*,*) '---------------------- ACS%COL:'
         DO IC = 1, G%NC
            WRITE(*,*) IC,':',(ACS%COL(IP),IP=ACS%ROW(IC),ACS%ROW(IC+1)-1)
         ENDDO
         IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
            DO IC = 1, G%NC
               WRITE(*,*) IC,':',(ACS%VAL_FB(IP),IP=ACS%ROW(IC),ACS%ROW(IC+1)-1)
            ENDDO
            DO IC=1,G%NC
               DO IP=ACS%ROW(IC),ACS%ROW(IC+1)-1
                  WRITE(*,*) IC,ACS%COL(IP),ACS%VAL_FB(IP)
               ENDDO
               WRITE(*,*)
            ENDDO
         ELSE
            DO IC = 1, G%NC
               WRITE(*,*) IC,':',(ACS%VAL(IP),IP=ACS%ROW(IC),ACS%ROW(IC+1)-1)
            ENDDO
            DO IC=1,G%NC
               DO IP=ACS%ROW(IC),ACS%ROW(IC+1)-1
                  WRITE(*,*) IC,ACS%COL(IP),ACS%VAL(IP)
               ENDDO
               WRITE(*,*)
            ENDDO
         ENDIF
...
IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
   CALL SCARC_ALLOCATE_REAL1_FB(X_FB, 1, G%NC, NSCARC_INIT_ZERO, 'X_FB')
   CALL SCARC_ALLOCATE_REAL1_FB(B_FB, 1, G%NC, NSCARC_INIT_ZERO, 'B_FB')
ELSE
   CALL SCARC_ALLOCATE_REAL1(X, 1, G%NC, NSCARC_INIT_ZERO, 'X')
   CALL SCARC_ALLOCATE_REAL1(B, 1, G%NC, NSCARC_INIT_ZERO, 'B')
ENDIF
...
DO ICYCLE = 1, 100000
   ...
   RDUMMY = 0.0_EB
   CALL MPI_GATHERV(RDUMMY, COUNTS_NC(MYID), MPI_DOUBLE_PRECISION, &
                    REAL_BUFFER_RECV2, COUNTS_NC, DISPLS_NC, MPI_DOUBLE_PRECISION, MASTER_ID, MPI_COMM_WORLD, IERROR)


#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'AFTER receive of  RIGHT HAND SIDE B:', COUNTS_NC, G%NC
WRITE(MSG%LU_DEBUG,'(8E14.6)') B(1:G%NC)
#endif

   IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
      X_FB = 0.0_FB
      B_FB = REAL(REAL_BUFFER_RECV2, FB)
      CALL PARDISO_S(MKL%PT, MKL%MAXFCT, MKL%MNUM, MKL%MTYPE, MKL%PHASE, G%NC, &
                     ACS%VAL_FB, ACS%ROW, ACS%COL, MKL%PERM, MKL%NRHS, MKL%IPARM, &
                     MKL%MSGLVL, B_FB, X_FB, MKL%ERROR)
      X = REAL(X_FB,EB)
   ELSE
      X = 0.0_EB
      B = REAL_BUFFER_RECV2
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'AFTER PARDISO: SOLUTION X:', MKL%ERROR
WRITE(MSG%LU_DEBUG,'(8E14.6)') X(1:G%NC)
#endif

   IF (MKL%ERROR /= 0) CALL SCARC_SHUTDOWN(NSCARC_ERROR_MKL_INTERNAL, SCARC_NONE, MKL%ERROR)

   CALL MPI_SCATTERV(X, COUNTS_NC, DISPLS_NC, MPI_DOUBLE_PRECISION, &
                     RDUMMY, COUNTS_NC(MYID), MPI_DOUBLE_PRECISION, MASTER_ID, MPI_COMM_WORLD, IERROR)
...
INTEGER :: NM, MASTER_ID, N, IP1, IP2, IC_OFFSET, IA_OFFSET
INTEGER ::  IDUMMY = 0
REAL(EB) :: RDUMMY = 0.0_EB

#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) MYID, ': HELLO I AM A SLAVE, NMESHES, MPI_COMM_WORLD =', NMESHES, MPI_COMM_WORLD, PROCESS
WRITE(MSG%LU_DEBUG,*) MYID, 'N_MPI_PROCESSES = ', N_MPI_PROCESSES
#endif
MASTER_ID = NMESHES 
...
DO N = 0, N_MPI_PROCESSES 
   DO NM = 1, NMESHES
      IF (PROCESS(NM) == N) THEN
         COUNTS_SZ(N) = COUNTS_SZ(N) + 3 
         COUNTS_NC(N) = COUNTS_NC(N) + NC_LOCAL(NM)
         COUNTS_NA(N) = COUNTS_NA(N) + NA_LOCAL(NM)
      ENDIF
   ENDDO
ENDDO

DO N = 1, N_MPI_PROCESSES
   DISPLS_SZ(N) = COUNTS_SZ(N-1) + DISPLS_SZ(N-1)
   DISPLS_NC(N) = COUNTS_NC(N-1) + DISPLS_NC(N-1)
   DISPLS_NA(N) = COUNTS_NA(N-1) + DISPLS_NA(N-1)
ENDDO
...
CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_SEND1, 1, COUNTS_SZ(MYID), NSCARC_INIT_ZERO, 'INT_BUFFER_SEND1')
CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_RECV1, 1, 3*NMESHES  , NSCARC_INIT_ZERO, 'INT_BUFFER_RECV1')

CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_SEND2, 1, COUNTS_NC(MYID), NSCARC_INIT_ZERO, 'INT_BUFFER_RECV2')
CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_RECV2, 1, NC_GLOBAL(NLEVEL_MAX), NSCARC_INIT_ZERO, 'INT_BUFFER_RECV2')

CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_SEND3, 1, COUNTS_NA(MYID), NSCARC_INIT_ZERO, 'INT_BUFFER_RECV3')
CALL SCARC_ALLOCATE_INT1 (INT_BUFFER_RECV3, 1, NA_GLOBAL(NLEVEL_MAX), NSCARC_INIT_ZERO, 'INT_BUFFER_RECV3')

CALL SCARC_ALLOCATE_REAL1 (REAL_BUFFER_SEND2, 1, COUNTS_NC(MYID), NSCARC_INIT_ZERO, 'REAL_BUFFER_RECV2')
CALL SCARC_ALLOCATE_REAL1 (REAL_BUFFER_RECV2, 1, NC_GLOBAL(NLEVEL_MAX), NSCARC_INIT_ZERO, 'REAL_BUFFER_RECV2')

CALL SCARC_ALLOCATE_REAL1 (REAL_BUFFER_SEND3, 1, COUNTS_NA(MYID), NSCARC_INIT_ZERO, 'REAL_BUFFER_RECV3')
CALL SCARC_ALLOCATE_REAL1 (REAL_BUFFER_RECV3, 1, NA_GLOBAL(NLEVEL_MAX), NSCARC_INIT_ZERO, 'REAL_BUFFER_RECV3')

IP1 = 0
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   INT_BUFFER_SEND1(IP1 + 1) = G%NC
   INT_BUFFER_SEND1(IP1 + 2) = G%ACS%N_VAL
   INT_BUFFER_SEND1(IP1 + 3) = NL
   IP1 = IP1 + 3
ENDDO
CALL MPI_GATHERV(INT_BUFFER_SEND1, COUNTS_SZ(MYID), MPI_INTEGER, &
                 INT_BUFFER_RECV1, COUNTS_SZ, DISPLS_SZ, MPI_INTEGER, MASTER_ID, MPI_COMM_WORLD, IERROR)


!>
!> Collect local matrix row, column and value information in send vectors in order to send them to master
!>
IC_OFFSET = 0
IA_OFFSET = 0
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   DO IP1 = 1, NC_LOCAL(NM)
      IP2 = IP1 + IC_OFFSET
      INT_BUFFER_SEND2(IP2) = AC%ROW(IP1)
   ENDDO
   IC_OFFSET = IC_OFFSET + NC_LOCAL(NM)

   IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
      DO IP1 = 1, NA_LOCAL(NM)
         IP2 = IP1 + IA_OFFSET
         INT_BUFFER_SEND3(IP2)  = AC%COL(IP1)
         REAL_BUFFER_SEND3(IP2) = REAL(AC%VAL_FB(IP1),EB)
      ENDDO
   ELSE
      DO IP1 = 1, NA_LOCAL(NM)
         IP2 = IP1 + IA_OFFSET
         INT_BUFFER_SEND3(IP2)  = AC%COL(IP1)
         REAL_BUFFER_SEND3(IP2) = AC%VAL(IP1)
      ENDDO
   ENDIF
   IA_OFFSET = IA_OFFSET + NA_LOCAL(NM)

#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,*) 'SLAVE: SENDING AC%ROW: ', NM
WRITE(MSG%LU_DEBUG,'(8I6)') AC%ROW(1:AC%N_ROW-1)
WRITE(MSG%LU_DEBUG,*) 'SLAVE: SENDING AC%COL: ', NM
WRITE(MSG%LU_DEBUG,'(8I6)') AC%COL(1:AC%N_VAL)
WRITE(MSG%LU_DEBUG,*) 'SLAVE: SENDING AC%VAL: ', NM
WRITE(MSG%LU_DEBUG,'(8E14.6)') AC%VAL(1:AC%N_VAL)
#endif

ENDDO

CALL MPI_GATHERV(INT_BUFFER_SEND2, COUNTS_NC(MYID), MPI_INTEGER, &
                 IDUMMY, COUNTS_NC, DISPLS_NC, MPI_INTEGER, MASTER_ID, MPI_COMM_WORLD, IERROR)
CALL MPI_GATHERV(INT_BUFFER_SEND3, COUNTS_NA(MYID), MPI_INTEGER, &
                 IDUMMY, COUNTS_NA, DISPLS_NA, MPI_INTEGER, MASTER_ID, MPI_COMM_WORLD, IERROR)
CALL MPI_GATHERV(REAL_BUFFER_SEND3, COUNTS_NA(MYID), MPI_DOUBLE_PRECISION, &
                 RDUMMY, COUNTS_NA, DISPLS_NA, MPI_DOUBLE_PRECISION, MASTER_ID, MPI_COMM_WORLD, IERROR)
...
IF (NL == NLEVEL_MAX) THEN
   CALL SCARC_ALLOCATE_INT1 (NC_LOCAL, 1, NMESHES, NSCARC_INIT_ZERO, 'NC_LOCAL')
   NC_LOCAL(1:NMESHES) = MESH_INT(1:NMESHES)
ENDIF
...
CALL SCARC_ALLOCATE_INT1(NA_LOCAL, 1, NMESHES, NSCARC_INIT_ZERO, 'NA_LOCAL')
NA_LOCAL(NM) = ACS%N_VAL
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      IF (TYPE_MKL_PRECISION == NSCARC_PRECISION_SINGLE) THEN
         ...
         CALL CLUSTER_SPARSE_SOLVER_S(MKL%CT, MKL%MAXFCT, MKL%MNUM, MKL%MTYPE, MKL%PHASE, G%NC_GLOBAL, &
                                      ACS%VAL_FB, ACS%ROW, ACS%COL, MKL%PERM, MKL%NRHS, MKL%IPARM, &
                                      MKL%MSGLVL, DUMMY_FB, DUMMY_FB, MPI_SLAVE_WORLD, MKL%ERROR)
         MKL%PHASE = 22
         CALL CLUSTER_SPARSE_SOLVER_S(MKL%CT, MKL%MAXFCT, MKL%MNUM, MKL%MTYPE, MKL%PHASE, G%NC_GLOBAL, &
                                      ACS%VAL_FB, ACS%ROW, ACS%COL, MKL%PERM, MKL%NRHS, MKL%IPARM, &
                                      MKL%MSGLVL, DUMMY_FB, DUMMY_FB, MPI_SLAVE_WORLD, MKL%ERROR)
