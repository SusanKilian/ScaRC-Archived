...
INTEGER, PARAMETER :: NSCARC_COARSENING_AGGREGATED   =  1    !< Type of grid coarsening: aggregation-based (AMG only)
INTEGER, PARAMETER :: NSCARC_COARSENING_AGGREGATEDS  =  2    !< Type of grid coarsening: aggregation-based, staggered (AMG only)
INTEGER, PARAMETER :: NSCARC_COARSENING_DOUBLED      =  3    !< Type of grid coarsening: default grid doubling (not AMG)
INTEGER, PARAMETER :: NSCARC_COARSENING_CUBIC        =  4    !< Type of grid coarsening: GMG-like cubic zones (AMG only)
...
INTEGER, PARAMETER :: NSCARC_ERROR_BICG_DISABLED     =  2    !< Type of error message: BICG temporarily disabled
...
INTEGER, PARAMETER :: NSCARC_EXCHANGE_NULLSPACE      =  6    !< Type of data exchange: nullspace entries (AMG only)
INTEGER, PARAMETER :: NSCARC_EXCHANGE_POISSON_COLS   =  7    !< Type of data exchange: (local) columns of Poisson matrix
INTEGER, PARAMETER :: NSCARC_EXCHANGE_POISSON_COLSG  =  8    !< Type of data exchange: global columns of Poisson matrix
INTEGER, PARAMETER :: NSCARC_EXCHANGE_POISSON_DIAGS  =  9    !< Type of data exchange: diagonal of Poisson matrix
INTEGER, PARAMETER :: NSCARC_EXCHANGE_POISSON_SIZES  = 10    !< Type of data exchange: size of Poisson matrix 
INTEGER, PARAMETER :: NSCARC_EXCHANGE_POISSON_VALS   = 11    !< Type of data exchange: values of Poisson matrix
INTEGER, PARAMETER :: NSCARC_EXCHANGE_PRESSURE       = 12    !< Type of data exchange: pressure values
INTEGER, PARAMETER :: NSCARC_EXCHANGE_VECTOR_MEAN    = 13    !< Type of data exchange: mean values of a vector
INTEGER, PARAMETER :: NSCARC_EXCHANGE_VECTOR_PLAIN   = 14    !< Type of data exchange: plain values of a vector
INTEGER, PARAMETER :: NSCARC_EXCHANGE_ZONE_NEIGHBORS = 15    !< Type of data exchange: aggregation zones (AMG only)
INTEGER, PARAMETER :: NSCARC_EXCHANGE_ZONE_TYPES     = 16    !< Type of data exchange: aggregation zones types (AMG only)
...
INTEGER, PARAMETER :: NSCARC_KRYLOV_CG               =  1    !< Type of Krylov solver: default CG-variant
INTEGER, PARAMETER :: NSCARC_KRYLOV_BICG             =  3    !< Type of Krylov solver: BICG-variant
...
REAL(EB), PARAMETER :: NSCARC_HUGE_REAL_EB           = -999999999_EB    !< Flag for an undefined double precision value
REAL(FB), PARAMETER :: NSCARC_HUGE_REAL_FB           = -999999999_FB    !< Flag for an undefined double precision value
...
REAL(EB), PARAMETER :: SCALR  = 0.015625_EB                             !< Scaling parameter for MG
REAL(EB), PARAMETER :: SCALP  = 0.0625_EB                               !< Scaling parameter for MG
REAL(EB), PARAMETER :: W1     =  1.0_EB                                 !< Weighting parameter for MG
REAL(EB), PARAMETER :: W3     =  3.0_EB                                 !< Weighting parameter for MG                     
REAL(EB), PARAMETER :: W4     =  4.0_EB                                 !< Weighting parameter for MG                    
REAL(EB), PARAMETER :: W9     =  9.0_EB                                 !< Weighting parameter for MG                   
REAL(EB), PARAMETER :: W12    = 12.0_EB                                 !< Weighting parameter for MG                  
REAL(EB), PARAMETER :: W16    = 16.0_EB                                 !< Weighting parameter for MG                 
...
INTEGER :: TYPE_KRYLOV             = NSCARC_KRYLOV_CG            !< Type of Krylov method 
...
USE SCARC_GLOBAL_CONSTANTS, ONLY : NSCARC_ZERO_INT
   ...
   INTEGER :: ITE_CG     = NSCARC_ZERO_INT         !< Statistical information for Krylov iterations
   INTEGER :: ITE_MG     = NSCARC_ZERO_INT         !< Statistical information for multigrid iterations
   INTEGER :: ITE_LU     = NSCARC_ZERO_INT         !< Statistical information for LU iterations
   INTEGER :: ITE_PRES   = NSCARC_ZERO_INT         !< Statistical information for pressure iterations
   INTEGER :: ITE_TOTAL  = NSCARC_ZERO_INT         !< Statistical information for total iterations
   INTEGER :: ITE_SMOOTH = NSCARC_ZERO_INT         !< Statistical information for smoothing iterations
   INTEGER :: ITE_COARSE = NSCARC_ZERO_INT         !< Statistical information for coarse grid iterations
   INTEGER :: ITE_GLOBAL = NSCARC_ZERO_INT         !< Statistical information for global iterations
...
#ifdef WITH_SCARC_STANDALONE
   INTEGER :: LU_SCARC                                            !< Logical unit for dump-out of complete ScaRC environment
   INTEGER :: LU_CNN, LU_CNN1, LU_CNN2, LU_CNN3                   !< Logical unit for dump-out of different data
   CHARACTER(120) :: FILE_SCARC                                   !< Output file name for dump-out complete ScaRC environment
   CHARACTER(60)  :: FILE_CNN, FILE_CNN1, FILE_CNN2, FILE_CNN3    !< Output file names for dump-out of different data
#endif

END TYPE SCARC_MESSAGE_TYPE

!> \brief Information about the mesh decomposition
...
!> \brief Workspace for FFT preconditioners
...
IF (MYID == 0) THEN
   ...
   WRITE(MSG%LU_MEM,1001) 'Number','Rank','Name','Calling routine','Type','State','Init-type','Dimension',&
                          'Lef1','Right1','Left2','Right2','Left3','Right3','Size','Sum(int)','Sum(EB)','Sum(FB)'
ENDIF
...
! If debug directive is set, open file for debug messages
...
SELECT CASE (TRIM(SCARC_COARSENING))
   ...
   CASE ('AGGREGATEDS')
      TYPE_COARSENING = NSCARC_COARSENING_AGGREGATEDS
   CASE ('DOUBLED')
...
IS_COARSE_AGGREGATED  = HAS_LEVELS_TWO .AND. (TYPE_COARSENING == NSCARC_COARSENING_AGGREGATEDS)
...
WRITE(MSG%LU_DEBUG,*) 'IS_COARSE_AGGREGATEDS=',IS_COARSE_AGGREGATEDS
...
USE SCARC_POINTERS, ONLY: M, L, LF, LC, FF, FC, WF, WC, OL, OLF, OLC, G, GC, GF, OGC, OGF, GWC, MWC, EWC
INTEGER :: NL, NM, NOM
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SIZES: NM, NOM, OG%NCG=',OG%NCG
WRITE(MSG%LU_DEBUG,*) 'SIZES: NM, NOM, NLEN_BUFFER_LAYER1 =', NM, NOM, OG%NLEN_BUFFER_LAYER1
WRITE(MSG%LU_DEBUG,*) 'SIZES: NM, NOM, NLEN_BUFFER_LAYER2 =', NM, NOM, OG%NLEN_BUFFER_LAYER2
WRITE(MSG%LU_DEBUG,*) 'SIZES: NM, NOM, NLEN_BUFFER_LAYER4 =', NM, NOM, OG%NLEN_BUFFER_LAYER4
WRITE(MSG%LU_DEBUG,*) 'SIZES: NM, NOM, NLEN_BUFFER_STENCIL=', NM, NOM, OG%NLEN_BUFFER_STENCIL
WRITE(MSG%LU_DEBUG,*) 'SIZES: NM, NOM, NLEN_BUFFER_FULL   =', NM, NOM, OG%NLEN_BUFFER_FULL
#endif
...
! -----------------------------------------------------------------------------------------
!> \brief Setup order in which ghost cells are processed during data exchanges
! -----------------------------------------------------------------------------------------
...
USE SCARC_POINTERS, ONLY: LF, LC, FF, FC, WF, WC, OLC, OGC 
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS:A: MESH_INT:', MESH_INT
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS:A: LOWER_MESH_INDEX=', LOWER_MESH_INDEX
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS:A: UPPER_MESH_INDEX=', UPPER_MESH_INDEX
#endif
...
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS:B: NM2=',NM,': MESH_INT:', MESH_INT
#endif
!   DO NM2 = LOWER_MESH_INDEX, UPPER_MESH_INDEX
!      MESH_INT(NM2) = G%NC_LOCAL(NM2)
!#ifdef WITH_SCARC_DEBUG
!WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS:B: NM2=',NM2,': MESH_INT:', MESH_INT
!#endif
!   ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS: NM = ', NM
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS: MESH_INT:', MESH_INT
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS: NC_LOCAL:', G%NC_LOCAL
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS: NC_GLOBAL(',NL,'):', NC_GLOBAL(NL)
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS: NC_GLOBAL:', NC_GLOBAL
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_DIMENSIONS: NC_OFFSET(',NM2,')=', G%NC_OFFSET
#endif
      ENDDO
   ENDIF
...
USE SCARC_POINTERS, ONLY: G, AC, OAC
   ...
   TYPE_MATRIX_IF: IF (TYPE_MATRIX == NSCARC_MATRIX_COMPACT) THEN
      ...
      MESHES_FINE_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         OMESHES_FINE_LOOP: DO INBR = 1, S%N_NEIGHBORS
            NOM = S%NEIGHBORS(INBR)
            CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NLEVEL_MIN)
            OAC => SCARC_POINT_TO_OTHER_CMATRIX (OG, NSCARC_MATRIX_POISSON)
            CALL SCARC_REDUCE_CMATRIX(OAC, 'OG%POISSONC', CROUTINE)
         ENDDO OMESHES_FINE_LOOP

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_SYSTEMS: NM =', NM
CALL SCARC_DEBUG_CMATRIX(AC, 'G%POISSONC', 'SETUP_SYSTEMS: AFTER EXTRAXT_MATRIX_OVERLAPS')
#endif
...
! -------------------------------------------------------------------------------------------
!> \brief Setup mapping vector from local to global numbering
! -------------------------------------------------------------------------------------------
...
CROUTINE = 'SCARC_SETUP_GLOBAL_CELL_NUMBERING'
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_GLOBAL_CELL_NUMBERING: NM =', NM
WRITE(MSG%LU_DEBUG,*) 'SETUP_GLOBAL_CELL_NUMBERING: LOCAL_TO_GLOBAL'
WRITE(MSG%LU_DEBUG,*) 'G%NC_OFFSET=', G%NC_OFFSET
...
! -------------------------------------------------------------------------------------------
!> \brief Extract overlapping matrix part and add to main matrix
! -------------------------------------------------------------------------------------------
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
WRITE(MSG%LU_DEBUG,*) 'GLOBAL_MATRIX_NUMBERING: NM =', NM
   CALL SCARC_POINT_TO_GRID (NM, NL)                                   ! Sets grid pointer G
   A => SCARC_POINT_TO_CMATRIX (G, NMATRIX)
   DO IC = 1, G%NC
...
SELECT_STORAGE_TYPE: SELECT CASE (SCARC_MATRIX_LEVEL(NL))
   ...
   CASE (NSCARC_MATRIX_COMPACT)
      ...
      CALL SCARC_ALLOCATE_CMATRIX (A, NL, NSCARC_PRECISION_DOUBLE, NSCARC_MATRIX_FULL, 'G%POISSONC', CROUTINE)
      ...
      DO INBR = 1, SCARC(NM)%N_NEIGHBORS
         ...
         CALL SCARC_ALLOCATE_CMATRIX(OA, NL, NSCARC_PRECISION_DOUBLE, NSCARC_MATRIX_FULL, 'OG%POISSONC', CROUTINE)
      ENDDO
   ...
   TYPE_MKL_SELECT: SELECT CASE (TYPE_MKL(NL)) 
      ...
      CASE(NSCARC_MKL_LOCAL) 
         ...
         DO ICOL = A%ROW(IC), A%ROW(IC+1)-1
            JC = A%COL(ICOL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '---------------- ICOL, JC =',ICOL, JC
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3I6,E12.4)') 'POISSON_MKL: IC, ICOL, JC, VAL:', IC, ICOL, JC, AS%VAL(IAS)
#endif
         ...
         AS%ROW(IC+1) = IAS
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,I6,A,I9)') '--------- AS%ROW(',IC+1,')=', IAS
#endif
...
#ifdef WITH_SCARC_DEBUG
CALL SCARC_DEBUG_CMATRIX(AS, 'POISSON_SYM', 'SETUP_MATRIX_MKL: BEFORE REDUCE')
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'EXTRACT_MATRIX_OVERLAPS: NM =', NM
WRITE(MSG%LU_DEBUG,*) 'CALLING EXTRACT_MATRIX_OVERLAPS, NMATRIX, NL:', NMATRIX, NL
               ...
               DO ICOL = OA%ROW(ICG)+1, OA%ROW(ICG+1)-1
                  ...
                  ELSE
                     ...
                     IF (ICG == OL%GHOST_FIRSTE(IOR0)) THEN
                        ...
                        IF (G%LOCAL_TO_GLOBAL(ICE2) == ICN) THEN
                           A%COL(IP) = ICE2
                        ENDIF
                     ELSE IF (ICG == OL%GHOST_LASTW(IOR0)) THEN
                        ICE1 = OG%ICG_TO_ICE(ICG-1, 1)
                        IF (G%LOCAL_TO_GLOBAL(ICE1) == ICN) THEN
                           A%COL(IP) = ICE1
                        ENDIF
                     ELSE
                        ICE1 = OG%ICG_TO_ICE(ICG-1, 1)
   ...
   ENDDO FACES_LOOP
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'AFTER EXTRACT_MATRIX_OVERLAPS: NM =', NM
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_ALLOCATE_REAL1 (G%DIAG, 1, G%NCE, NSCARC_INIT_ZERO, 'G%DIAG', CROUTINE)
   DO IC = 1, G%NC
      DO ICOL = A%ROW(IC), A%ROW(IC+1) - 1
         JC = A%COL(ICOL)
...
! -------------------------------------------------------------------------------------------
!> \brief Extract overlapping cell information (including second layers)
! -------------------------------------------------------------------------------------------
...
CROUTINE = 'SCARC_EXTRACT_CELL_OVERLAPS'

DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX

   CALL SCARC_POINT_TO_GRID (NM, NL)                                   ! Sets grid pointer G

   CALL SCARC_ALLOCATE_INT1 (G%ICE_TO_ECELL, G%NC+1, G%NCE2, NSCARC_INIT_ZERO, 'G%ICE_TO_ECELL', CROUTINE)
   CALL SCARC_ALLOCATE_INT1 (G%ICE_TO_ICELL, G%NC+1, G%NCE2, NSCARC_INIT_ZERO, 'G%ICE_TO_ICELL', CROUTINE)
   CALL SCARC_ALLOCATE_INT1 (G%ICE_TO_OCELL, G%NC+1, G%NCE2, NSCARC_INIT_ZERO, 'G%ICE_TO_OCELL', CROUTINE)
   CALL SCARC_ALLOCATE_INT1 (G%ICE_TO_GCELL, G%NC+1, G%NCE2, NSCARC_INIT_ZERO, 'G%ICE_TO_GCELL', CROUTINE)
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_ALLOCATE_INT1 (GF%ICE_TO_OZONE, GF%NC+1, GF%NCE2, NSCARC_INIT_ZERO, 'GF%ICE_TO_OZONE', CROUTINE)
   CALL SCARC_ALLOCATE_INT1 (GF%ICE_TO_GZONE, GF%NC+1, GF%NCE2, NSCARC_INIT_ZERO, 'GF%ICE_TO_GZONE', CROUTINE)
   CALL SCARC_ALLOCATE_INT1 (GF%ICE_TO_IZONE, GF%NC+1, GF%NCE2, NSCARC_INIT_ZERO, 'GF%ICE_TO_IZONE', CROUTINE)
   CALL SCARC_ALLOCATE_INT1 (GF%ICE_TO_EZONE, GF%NC+1, GF%NCE2, NSCARC_INIT_ZERO, 'GF%ICE_TO_EZONE', CROUTINE)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,7I6)') 'ZONE_OVERLAPS: NM, INBR, NOM, IOR0, ICG, ICE, ICE2:', &
                                              NM, INBR, NOM, IOR0, ICG, ICE, ICE2
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,8I6)') 'A: NM, ICG, IFOUND, N_ZONES IZL1, ICE, LOCAL(ICE), GLOBAL(ICE):', &
                       NM, ICG, IFOUND, GF%N_ZONES, IZL1, ICE, GF%ZONES_LOCAL(ICE), GF%ZONES_GLOBAL(ICE)
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,8I6)') 'B: NM, ICG, IFOUND, N_ZONES IZL2, ICE, LOCAL(ICE), GLOBAL(ICE):', &
                       NM, ICG, IFOUND, GF%N_ZONES, IZL2, ICE2, GF%ZONES_LOCAL(ICE2), GF%ZONES_GLOBAL(ICE2)
#endif
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A, 8I6)') 'NM, INBR, NOM, IOR0, ICG, ICW1, IZL1, IZ :', &
                                NM, INBR, NOM, IOR0, ICG, ICW1, IZL1, IZ
#endif
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A, 8I6)') 'NM, ,NBR, NOM, IOR0, ICG, ICW2, IZL2, IZ :', &
                                NM, INBR, NOM, IOR0, ICG, ICW2, IZL2, IZ
#endif
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A, 8I6)') 'NM, INBR, NOM, IOR0, ICG, ICE1, IZL1, IZ :', &
                                NM, INBR, NOM, IOR0, ICG, ICE1, IZL1, IZ
#endif
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A, 8I6)') 'NM, INBR, NOM, IOR0, ICG, ICE2, IZL2, IZ :', &
                                NM, INBR, NOM, IOR0, ICG, ICE2, IZL2, IZ
#endif

         ENDDO EXTERNAL_ZONES_LOOP
         OGF%NCGE = IZ
         CALL SCARC_REDUCE_INT1(OGF%ICG_TO_EZONE, OGF%NCGE, 'OGF%ICG_TO_IZONE', CROUTINE)
...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE(TYPE_MATRIX)
         CASE (NSCARC_MATRIX_COMPACT)
            A => G%POISSONC
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, G%NC, NSCARC_INIT_ZERO, 'G%POISSONC%RELAX', CROUTINE)
            DO IC = 1, G%NC
               A%RELAX(IC) = 1.0_EB/A%VAL(A%ROW(IC))
            ENDDO 
         CASE (NSCARC_MATRIX_BANDWISE)
            AB => G%POISSONB
            CALL SCARC_ALLOCATE_REAL1(AB%RELAXD, 1, AB%N_DIAG,  NSCARC_INIT_ZERO, 'G%POISSONC%RELAXD', CROUTINE)
            DO IC = 1, G%NC
   ...
   LEVEL_LOOP: DO NL = NLMIN, NLMAX
      ...
      SELECT CASE (SCARC_MATRIX_LEVEL(NL))
         ...
         CASE (NSCARC_MATRIX_COMPACT)
            ...
            A => G%POISSONC
            CALL SCARC_ALLOCATE_REAL1(A%RELAX, 1, A%N_VAL, NSCARC_INIT_ZERO, 'G%POISSONC%RELAX', CROUTINE)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'CALLING RELAX_MKL, TYPE:', STACK(NS)%SOLVER%TYPE_SCOPE(0)
#endif
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) '--------------> NM=',NM,': NOM=',NOM,': SCARC_EXCHANGE_PACK_LOOP'
#endif

      SEND_PACK_OMESHES_SELECT: SELECT CASE (NTYPE)

         CASE (NSCARC_EXCHANGE_AUXILIARY)
            CALL SCARC_PACK_AUXILIARY
...
SEND_UNPACK_MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   SEND_UNPACK_OMESHES_LOOP: DO NOM = 1, NMESHES
      ...
      CALL SCARC_POINT_TO_OTHER_GRID(NM, NOM, NL)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) '--------------> NM=',NM,': NOM=',NOM,': SCARC_EXCHANGE_UNPACK_LOOP'
#endif
      ...
      SEND_UNPACK_OMESHES_SELECT: SELECT CASE (NTYPE)
         ...
         CASE (NSCARC_EXCHANGE_AUXILIARY)
            CALL SCARC_UNPACK_AUXILIARY (NM, NOM)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': NOM=',NOM,': RECEIVING INT MESSAGE OF LENGTH ', NLEN, ' and TYPE ', NTYPE
#endif
#ifdef WITH_SCARC_VERBOSE2
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': NOM=',NOM,': SENDING INT MESSAGE OF LENGTH ', NLEN, ' and TYPE ', NTYPE
#endif
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      ...
      ENDIF
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,8I6, L8)') 'UNPACK_CELL_NUMBERS: IOR0, ICG, ICE, IWG, IXG, IYG, IZG, RECV_BUFFER_INT', &
                               IOR0, ICG, ICE, IWG, IXG, IYG, IZG, RECV_BUFFER_INT(LL), L%IS_SOLID(IXG, IYG, IZG)
#endif
      LL = LL + 1
   ENDDO
ENDDO
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_PACK_AUXILIARY
...
#ifdef WITH_SCARC_DEBUG
IF (ICW1 > 0) WRITE(MSG%LU_DEBUG,'(A, 6I6, E12.4)') 'PACK_AUXILIARY: IOR0, IWG, IXW, IYW, IZW, ICW1: ', &
                                                     IOR0, IWG, IXW, IYW, IZW, ICW1, G%AUX1(ICW1)
IF (ICW2 > 0) WRITE(MSG%LU_DEBUG,'(A, 6I6, E12.4)') 'PACK_AUXILIARY: IOR0, IWG, IXW, IYW, IZW, ICW2: ', &
                                                     IOR0, IWG, IXW, IYW, IZW, ICW2, G%AUX1(ICW2)
#endif
      LL = LL + 2
   ENDDO
ENDDO
...
! ------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_UNPACK_AUXILIARY (NM, NOM)
USE SCARC_POINTERS, ONLY: OL, OG, RECV_BUFFER_REAL
INTEGER, INTENT(IN) :: NM, NOM
...
#ifdef WITH_SCARC_DEBUG
IF (ICE1 > 0) WRITE(MSG%LU_DEBUG,'(A,2I6, E12.4)') 'UNPACK_AUXILIARY: IOR0, ICE1, AUX1-1:', IOR0, ICE1, G%AUX1(ICE1)
IF (ICE1 > 0) WRITE(MSG%LU_DEBUG,'(A,2I6, E12.4)') 'UNPACK_AUXILIARY: IOR0, ICE1, AUX1-1:', IOR0, ICE2, G%AUX1(ICE2)
#endif
      LL = LL + 2
   ENDDO
ENDDO
...
#ifdef WITH_SCARC_DEBUG
IF (ICW1 > 0) WRITE(MSG%LU_DEBUG,'(A,6I6, E12.4)') 'PACK_NULLSPACE: IOR0, IWG, IXW, IYW, IZW, ICW1: ', &
                                                    IOR0, IWG, IXW, IYW, IZW, ICW1, G%NULLSPACE(ICW1)
IF (ICW1 > 0) WRITE(MSG%LU_DEBUG,'(A,6I6, E12.4)') 'PACK_NULLSPACE: IOR0, IWG, IXW, IYW, IZW, ICW2: ', &
                                                    IOR0, IWG, IXW, IYW, IZW, ICW2, G%NULLSPACE(ICW2)
#endif
      LL = LL + 2
   ENDDO
ENDDO
...
#ifdef WITH_SCARC_DEBUG
IF (ICE1 > 0) WRITE(MSG%LU_DEBUG,'(A,2I6, E12.4)') 'UNPACK_NULLSPACE: IOR0, ICE1, NULLSPACE-1:', &
                                                   IOR0, ICE1, G%NULLSPACE(ICE1)
IF (ICE2 > 0) WRITE(MSG%LU_DEBUG,'(A,2I6, E12.4)') 'UNPACK_NULLSPACE: IOR0, ICE2, NULLSPACE-1:', &
                                                   IOR0, ICE2, G%NULLSPACE(ICE2)
#endif
      LL = LL + 2
   ENDDO
ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,3I6,E14.6)') 'UNPACK_VECTOR_PLAIN: IOR0, ICG, VC:', IOR0, ICG, ICE, RECV_BUFFER_REAL(LL)
#endif
      LL = LL + 1
   ENDDO
ENDDO
...
WRITE(MSG%LU_DEBUG,*) 'HUGE(LL)=',HUGE(LL)
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,6I8)') 'PACK_POISSON_COLS: SEND: IOR0, ICG, ICW, ICOL, LL, VAL:', &
                               IOR0, ICG, ICW, AC%ROW(ICW), LL, OS%SEND_BUFFER_INT(LL)
#endif
      LL = LL + 1                            
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,6I8)') 'PACK_POISSON_COLS: SEND: IOR0, ICG, ICW, ICOL, LL, VAL:', &
                            IOR0, ICG, ICW, ICOL, LL, OS%SEND_BUFFER_INT(LL)
#endif
         LL = LL + 1
      ENDDO
   ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,I6,A,4I12,I12)') 'NM=',NM,&
     ':UNPACK_POISSON_COLS:A:RECV:ICG,LL,RECV_BUFFER_INT,OAC%ROW(ICG),OAC%COL(LL), NEXT:', &
                                ICG, LL, RECV_BUFFER_INT(LL), OAC%COL(LL), RECV_BUFFER_INT(LL+1)
#endif
...
SELECT CASE (SCARC_MATRIX_LEVEL(NL))
   ...
   CASE (NSCARC_MATRIX_COMPACT)
      ...
      AC => SCARC_POINT_TO_CMATRIX(G, NMATRIX)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'PACK_POISSON_VALS: AC%N_ROW, AC%N_VAL=',AC%N_ROW, AC%N_VAL
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 5I8,E12.4)') 'PACK_POISSON_VALS:SG: SEND: IOR0, ICG, ICW, LL, ICOL, VAL:', &
                                                                  IOR0, ICG, ICW, LL, ICOL, AC%VAL(ICOL)
#endif
               LL = LL + 1
            ENDDO
         ENDDO
...
USE SCARC_POINTERS, ONLY: OL, OAB, OAC, RECV_BUFFER_REAL
...
WRITE(MSG%LU_DEBUG,'(8E12.4)') RECV_BUFFER_REAL
      ...
      DO IOR0 = -3, 3
         ...
         DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
            IF (OG%ICG_TO_ICE(ICG,1) < 0) CYCLE                     ! skip solid cells
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) '---------- UNPACK_POISSON_VALS: ICG, OAC%ROW(ICG)', ICG, OAC%ROW(ICG), OAC%ROW(ICG+1)-1
#endif
            DO ICOL = OAC%ROW(ICG), OAC%ROW(ICG+1)-1
               OAC%VAL(ICOL) = RECV_BUFFER_REAL(LL)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4I8,E12.4)') 'UNPACK_POISSON_VALS, RECV: IOR0, ICG, ICOL, LL, OAC%VAL', &
                                     IOR0, ICG, ICOL, LL, OAC%VAL(ICOL)
#endif
               LL = LL + 1
            ENDDO
         ENDDO
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,4I8,E12.4)') 'PACK_POISSON_DIAGS: IOR0, ICG, ICW, ICOL, VAL:', &
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, 3I8,E12.4)') 'UNPACK_POISSON_DIAGS: NOM, IOR0, ICG, ICE, DIAG:', &
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=', NM,': CELL_NEIGHBORS: RECV_BUFFER_INT: OG%NCG:', OG%NCG
   WRITE(MSG%LU_DEBUG,'(8I12)') RECV_BUFFER_INT
#endif
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      ...
      LL = LL + 2
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': UNPACK_CELL_NEIGHBORS: IOR0, ICG_TO_OCELL(',ICG,'):',IOR0,OG%ICG_TO_OCELL(ICG)
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': UNPACK_CELL_NEIGHBORS: IOR0, ICG_TO_GCELL(',ICG,'):',IOR0,OG%ICG_TO_GCELL(ICG)
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': UNPACK_CELL_NEIGHBORS: IOR0, ICG_TO_ICE(',ICG,') OLD:',IOR0,OG%ICG_TO_ICE(ICG, 1)
#endif
   ENDDO
   IOFF = OL%GHOST_LASTW(IOR0)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': IOFF=',IOFF
#endif
   DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      G%ICE2 = G%ICE2 + 1
      ...
      LL = LL + 2
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': UNPACK_CELL_NEIGHBORS: IOR0, ICG_TO_OCELL(',ICG+IOFF,'):',IOR0,OG%ICG_TO_OCELL(ICG+IOFF)
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': UNPACK_CELL_NEIGHBORS: IOR0, ICG_TO_GCELL(',ICG+IOFF,'):',IOR0,OG%ICG_TO_GCELL(ICG+IOFF)
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': UNPACK_CELL_NEIGHBORS: IOR0, ICG_TO_ICE2(',ICG+IOFF,') :',IOR0,OG%ICG_TO_ICE(ICG,2)
#endif
   ENDDO
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': CELL_NEIGHBORS: IOR0=', IOR0
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      ...
      LL = LL + 2
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A,I6,A,6I6)') 'NM=',NM,':UNPACK_ZONE_NEIGHBORS: IOR0, ICG_TO_OZONE(.):',IOR0,OG%ICG_TO_OZONE(KK)
WRITE(MSG%LU_DEBUG,'(A,I6,A,6I6)') 'NM=',NM,':UNPACK_ZONE_NEIGHBORS: IOR0, ICG_TO_GZONE(.):',IOR0,OG%ICG_TO_GZONE(KK)
#endif
   ENDDO
   DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      OG%ICG_TO_OZONE(KK) = RECV_BUFFER_INT(LL)
...
WRITE(MSG%LU_DEBUG,'(A,I6,A,6I6)') 'NM=',NM,':UNPACK_ZONE_NEIGHBORS: IOR0, ICG_TO_GZONE(.):',IOR0,OG%ICG_TO_GZONE(KK)
   ...
   ENDDO
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,'ZONE_NEIGHBORS: IOR0=', IOR0
...
RECV_BUFFER_INT => SCARC_POINT_TO_BUFFER_INT (NM, NOM, 1)
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':ZONE_TYPES: RECV_BUFFER_INT:'
   WRITE(MSG%LU_DEBUG,'(8I12)') RECV_BUFFER_INT
#endif
...
DO IOR0 = -3, 3
   ...
   DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      ...
      IF (G%ZONES_LOCAL(ICE) == 0) G%ZONES_LOCAL(ICE) = RECV_BUFFER_INT(LL+1)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,I6,A,5I8)') 'NM=',NM,':UNPACK_ZONE_TYPES: IOR0, ICG, ICW, LL, RECV(LL):', &
                                                                 IOR0, ICG, ICW, LL, RECV_BUFFER_INT(LL)
#endif
      LL = LL + 2
   ENDDO
ENDDO
...
SELECT CASE (NTYPE)
   ...
   CASE (1)
      IF (RNODE/=SNODE) THEN
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'POINT_TO_BUFFER_INT : NM, NOM, NTYPE:A', NM, NOM, NTYPE
#endif
         SCARC_POINT_TO_BUFFER_INT => SCARC(NM)%OSCARC(NOM)%RECV_BUFFER_INT
      ELSE
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'POINT_TO_BUFFER_INT : NM, NOM, NTYPE:B', NM, NOM, NTYPE
#endif
...
MESHES_ALLOCATION_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   CALL SCARC_ALLOCATE_INT1 (G%CPOINTS,      1, G%NCE,  NSCARC_INIT_ZERO, 'G%CPOINTS', CROUTINE)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':MESH_INT=',MESH_INT
#endif

   ! ---- Staggered aggregation procedure for SAMG
 
   CASE (NSCARC_COARSENING_AGGREGATEDS)

      CYCLES_LOOP1: DO ICYCLE = 1, SUB%N_CYCLES
   
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':================================= ICYCLE =', ICYCLE, SUB%N_CYCLES
...
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':MESH_INT=',MESH_INT
   ...
   CASE (NSCARC_COARSENING_CUBIC)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         CALL SCARC_POINT_TO_GRID(NM, NL)
         CALL SCARC_SETUP_CUBIC_ZONES(L, G)
         MESH_INT (NM) = G%N_ZONES
      ENDDO
...
#ifdef WITH_SCARC_DEBUG
   IF (NL == NLEVEL_MIN) CALL SCARC_BLENDER_ZONES(NM, NL)
#endif
...
! ------------------------------------------------------------------------------------------------------
SUBROUTINE SCARC_SETUP_CUBIC_ZONES(L, G)
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*)  'STARTING SETUP_CUBIC_ZONES'
#endif
...
CALL SCARC_ALLOCATE_INT1 (OFFY, 1, NXD, NSCARC_INIT_ZERO, 'OFFY', CROUTINE)
CALL SCARC_ALLOCATE_INT1 (OFFZ, 1, NZD, NSCARC_INIT_ZERO, 'OFFZ', CROUTINE)

#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'SETUP_CUBIC_ZONES:'
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   CALL SCARC_POINT_TO_GRID (NM, NL)
   CALL SCARC_DEALLOCATE_INT1 (G%CPOINTS, 'G%CPOINTS', CROUTINE)
...
#ifdef WITH_SCARC_DEBUG2
   WRITE(MSG%LU_DEBUG,'(A, 3I8,3E12.4)') 'NULLSPACE: IC; IDIAG, JC, AUX1=', &
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,'(A,I8,A, I8,3E12.4)') 'NM=',NM,': DIAG: IC =', IC, MG%OMEGA, G%DIAG(IC), G%AUX2(IC)
#endif
   ENDDO
   
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': RELAX_NULLSPACE:B: AUX2:'
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,I6,A,3I8)') 'NM=',NM,': =================>  ICCL, ICCG: ', ICCL, ICCG
#endif
      IS_INCLUDED = .FALSE.
      DO IC = 1, GF%NCE2
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A, I6,A,4I8)') 'NM=',NM,':      bingo, IC, ICC, ICCL, ICCG:', IC, ICC, ICCL, ICCG
#endif
         IP = IP + 1
      ENDDO
      IF (IS_INCLUDED) THEN
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   DO ICC = 1, Z%N_ROW-1
      ...
      DO ICOL = Z%ROW(ICC), Z%ROW(ICC+1)-1
         ...
         DSUM = DSUM + G%QQ(IQ)**2
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,'(A,I6,A,3I6,2E12.4)') 'NM=',NM,':QQ: ICC, IC, IQ, QQ(IQ), DSUM:', ICC, IC, IQ, G%QQ(IQ), DSUM
#endif
         IQ = IQ + 1
      ENDDO
      DO ICOL = Z%ROW(ICC), Z%ROW(ICC+1)-1
...
IF (NMESHES > 1) CALL SCARC_EXCHANGE (NSCARC_EXCHANGE_AUXILIARY, NSCARC_NONE, NL)
   ...
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':G%QQ:'
   WRITE(MSG%LU_DEBUG,*) G%QQ(1:G%NCE2)
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':G%AUX1:'
   WRITE(MSG%LU_DEBUG,*) G%AUX1(1:G%NCE2)
#endif
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':G%AUX1:'
   DO ICC=1, Z%N_ROW-1
      WRITE(MSG%LU_DEBUG,'(2I8,E12.4)') MYID+1, ICC, G%AUX1(ICC)
   ENDDO
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':G%RR(',ICC,')=', G%RR(ICC)
#endif
   ENDDO
   ...
   DO ICC = 1, Z%N_ROW-1
      DO ICOL = Z%ROW(ICC), Z%ROW(ICC+1)-1
         IC = Z%COL(ICOL)
IF (G%RR(ICC) <  1.E-14_EB) THEN
WRITE(*,*) G%RR(ICC-10:ICC)
WRITE(*,*) 'ZERO RR!!!!! NM=', NM,': ICC=', ICC, ': ICOL =', ICOL, ': IC=',IC,': Z_NROW=',Z%N_ROW,': RR=',G%RR(ICC)
ENDIF
         G%QQ(IQ) = G%QQ(IQ)/G%RR(ICC)
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':G%QQ(',IQ,')=', G%QQ(IQ)
#endif
         IQ = IQ + 1
      ENDDO
   ENDDO
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':================================ PART 3 :'
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,':G%QQ:'
...
#ifdef WITH_SCARC_DEBUG
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': RESTRICTION:  N_VAL, N_ROW=', RF%N_VAL, RF%N_ROW 
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': PROLONGATION: N_VAL, N_ROW=', PF%N_VAL, PF%N_ROW 
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': GC%LOCAL_TO_GLOBAL:', GC%LOCAL_TO_GLOBAL
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': GF%NC:', GF%NC
   WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': GF%NCE:', GF%NCE
#endif
...
#ifdef WITH_SCARC_VERBOSE
   WRITE(MSG%LU_VERBOSE,*) '--------------> NM=',NM,': BEFORE EXCHANGE POISSON_PROL'
#endif
...
#ifdef WITH_SCARC_VERBOSE
   WRITE(MSG%LU_VERBOSE,*) '--------------> NM=',NM,': AFTER EXCHANGE POISSON_PROL'
#endif
...
! ------------------------------------------------------------------------------------------------------
!> \brief Map global to local cell number 
! ------------------------------------------------------------------------------------------------------
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   IF (.NOT.ALLOCATED (AC%VAL)) THEN
      ...
      CALL SCARC_ALLOCATE_CMATRIX(AC, NL+1, NSCARC_PRECISION_DOUBLE, NSCARC_MATRIX_FULL, 'GC%POISSONC', CROUTINE)
   ENDIF

#ifdef WITH_SCARC_VERBOSE
   WRITE(MSG%LU_VERBOSE,*) '--------------> NM=',NM,': AFTER ALLOCATE GALERKIN '
#endif
   DO INBR = 1, SCARC(NM)%N_NEIGHBORS
      ...
      CALL SCARC_ALLOCATE_CMATRIX(OAC, NL, NSCARC_PRECISION_DOUBLE, NSCARC_MATRIX_FULL, 'OAC%POISSONC', CROUTINE)
...
#ifdef WITH_SCARC_VERBOSE
   WRITE(MSG%LU_VERBOSE,*) '--------------> NM=',NM,': AFTER RESORT GALERKIN '
#endif
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   AC => SCARC_POINT_TO_CMATRIX (GC, NSCARC_MATRIX_POISSON)         
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': SETUP_GALERKIN: AC%N_STENCIL_MAX =', AC%N_STENCIL_MAX
#endif
...
#ifdef WITH_SCARC_DEBUG
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': N_STENCIL_MAX  OVERALL:', RANK_INT
WRITE(MSG%LU_DEBUG,*) 'NM=',NM,': TYPE_MKL(0:2):', TYPE_MKL(0:2)
#endif

#ifdef WITH_SCARC_VERBOSE
   WRITE(MSG%LU_VERBOSE,*) '--------------> NM=',NM,': BEFORE GALERKIN MKL'
#endif
#ifdef WITH_MKL
...
#ifdef WITH_SCARC_VERBOSE
   WRITE(MSG%LU_VERBOSE,*) '--------------> NM=',NM,': AFTER GALERKIN MKL'
#endif
...
#ifdef WITH_SCARC_DEBUG2
WRITE(MSG%LU_DEBUG,'(A,5I8,3E12.4)') 'VALUE_AP: IC, JC, ICC, GLOB(ICC), JCC, AF%VAL(IA), PF%VAL(IP), DSUM:',&
...
INTEGER :: NWORK, NLEN(3), NSUM, I, IP
...
WRITE(MSG%LU_DEBUG,'(16I8)') G%CPOINTS
...
! ------------------------------------------------------------------------------------------------------
!> Temporarily preset B14 case with specified aggregation pattern
! ------------------------------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------------------------------
!> Temporarily preset B14_BIG case with specified aggregation pattern
! ------------------------------------------------------------------------------------------------------
...
! ================================================================================================
! ------------------------------------------------------------------------------------------------
!> \brief Standalone version only: Dump matrix and vectors belonging to pressure system 
! ------------------------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------------------------
!> \brief Standalone version only: Dump several arrays and structures needed for the standalone version of ScaRC 
! ------------------------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------------------------
!> \brief Standalone version only: Dump several arrays and structures needed for ScaRC - standalone version
! ------------------------------------------------------------------------------------------------
...
! ----------------------------------------------------------------------------------------------------
!> \brief Standalone version only: Allocate and initialize vectors pressure diagnostics - only for developping purposes
! ----------------------------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------------------------
!> \brief Standalone version only: Compute Differences between old and new pressure solutions - only for developping purposes
! ------------------------------------------------------------------------------------------------
...
! -----------------------------------------------------------------------------------------------
!> \brief Standalone version only: Store new pressure vector for comparison in next time step
! -----------------------------------------------------------------------------------------------
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
