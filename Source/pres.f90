...
INTEGER, INTENT(IN) :: NM
INTEGER :: IW,IOR,II,JJ,KK,IIO,JJO,KKO,N_INT_CELLS,IIO1,IIO2,JJO1,JJO2,KKO1,KKO2, III, KKK
...
IF (NM == 1) THEN
   ...
   WRITE(*,'(5I5)') (WALL(IW)%BOUNDARY_TYPE, IW = 1, N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS)
   WRITE(*,'(6E16.8)') ((MESHES(1)%FVX(III,1,KKK), III=0, MESHES(1)%IBAR), KKK=MESHES(1)%KBAR,0,-1)
   WRITE(*,*) 'PRES-INFO:H'
   WRITE(*,'(7E16.8)') ((MESHES(1)%H(III,1,KKK), III=0, MESHES(1)%IBAR+1), KKK=MESHES(1)%KBAR+1,0,-1)
   WRITE(*,*) 'PRES-INFO:U'
   WRITE(*,'(7E16.8)') ((MESHES(1)%U(III,1,KKK), III=0, MESHES(1)%IBAR), KKK=MESHES(1)%KBAR,0,-1)
!   WRITE(*,*) 'PRES-INFO:US'
!   WRITE(*,'(7E16.8)') ((MESHES(1)%US(III,1,KKK), III=0, MESHES(1)%IBAR), KKK=MESHES(1)%KBAR,0,-1)
ENDIF
...
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'P-1: ',IW,II,JJ,KK,U(II-1,JJ,KK), FVX(II-1,JJ,KK), H(II,JJ,KK), H(II-1,JJ,KK), DHFCT, UN_NEW
         CASE( 2)
            UN_NEW = V(II,JJ,KK)   - DT*(FVY(II,JJ,KK)   + RDYN(JJ)  *(H(II,JJ+1,KK)-H(II,JJ,KK))*DHFCT)
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'P 2: ',IW,II,JJ,KK,V(II,JJ,KK), FVY(II,JJ,KK), H(II,JJ+1,KK), H(II,JJ,KK), DHFCT, UN_NEW
         CASE(-2)
            UN_NEW = V(II,JJ-1,KK) - DT*(FVY(II,JJ-1,KK) + RDYN(JJ-1)*(H(II,JJ,KK)-H(II,JJ-1,KK))*DHFCT)
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'P-2: ',IW,II,JJ,KK,V(II,JJ-1,KK), FVY(II,JJ-1,KK), H(II,JJ,KK), H(II,JJ-1,KK), DHFCT, UN_NEW
         CASE( 3)
            UN_NEW = W(II,JJ,KK)   - DT*(FVZ(II,JJ,KK)   + RDZN(KK)  *(H(II,JJ,KK+1)-H(II,JJ,KK))*DHFCT)
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'P 3: ',IW,II,JJ,KK,W(II,JJ,KK), FVZ(II,JJ,KK), H(II,JJ,KK+1), H(II,JJ,KK), DHFCT, UN_NEW
         CASE(-3)
            UN_NEW = W(II,JJ,KK-1) - DT*(FVZ(II,JJ,KK-1) + RDZN(KK-1)*(H(II,JJ,KK)-H(II,JJ,KK-1))*DHFCT)
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'P-3: ',IW,II,JJ,KK,W(II,JJ,KK-1), FVZ(II,JJ,KK-1), H(II,JJ,KK), H(II,JJ,KK-1), DHFCT, UN_NEW
      END SELECT
   ELSE
      SELECT CASE(IOR)
         CASE( 1)
            UN_NEW = 0.5_EB*(U(II,JJ,KK)+US(II,JJ,KK)     - DT*(FVX(II,JJ,KK)   + RDXN(II)  *(HS(II+1,JJ,KK)-HS(II,JJ,KK))*DHFCT))
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'C 1: ',IW,II,JJ,KK,U(II,JJ,KK), FVX(II,JJ,KK), HS(II+1,JJ,KK), HS(II,JJ,KK),DHFCT,UN_NEW
         CASE(-1)
            UN_NEW = 0.5_EB*(U(II-1,JJ,KK)+US(II-1,JJ,KK) - DT*(FVX(II-1,JJ,KK) + RDXN(II-1)*(HS(II,JJ,KK)-HS(II-1,JJ,KK))*DHFCT))
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'C-1: ',IW,II,JJ,KK,U(II-1,JJ,KK), FVX(II-1,JJ,KK),HS(II,JJ,KK),HS(II-1,JJ,KK),DHFCT,UN_NEW
         CASE( 2)
            UN_NEW = 0.5_EB*(V(II,JJ,KK)+VS(II,JJ,KK)     - DT*(FVY(II,JJ,KK)   + RDYN(JJ)  *(HS(II,JJ+1,KK)-HS(II,JJ,KK))*DHFCT))
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'C 2: ',IW,II,JJ,KK,V(II,JJ,KK), FVY(II,JJ,KK), HS(II,JJ+1,KK), HS(II,JJ,KK),DHFCT,UN_NEW
         CASE(-2)
            UN_NEW = 0.5_EB*(V(II,JJ-1,KK)+VS(II,JJ-1,KK) - DT*(FVY(II,JJ-1,KK) + RDYN(JJ-1)*(HS(II,JJ,KK)-HS(II,JJ-1,KK))*DHFCT))
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'C-2: ',IW,II,JJ,KK,V(II,JJ-1,KK), FVY(II,JJ-1,KK),HS(II,JJ,KK),HS(II,JJ-1,KK),DHFCT,UN_NEW
         CASE( 3)
            UN_NEW = 0.5_EB*(W(II,JJ,KK)+WS(II,JJ,KK)     - DT*(FVZ(II,JJ,KK)   + RDZN(KK)  *(HS(II,JJ,KK+1)-HS(II,JJ,KK))*DHFCT))
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'C 3: ',IW,II,JJ,KK,W(II,JJ,KK), FVZ(II,JJ,KK), HS(II,JJ,KK+1), HS(II,JJ,KK),DHFCT,UN_NEW
         CASE(-3)
            UN_NEW = 0.5_EB*(W(II,JJ,KK-1)+WS(II,JJ,KK-1) - DT*(FVZ(II,JJ,KK-1) + RDZN(KK-1)*(HS(II,JJ,KK)-HS(II,JJ,KK-1))*DHFCT))
IF (NM == 1) WRITE(*,'(A,4i4,6E16.8)') 'C-3: ',IW,II,JJ,KK,W(II,JJ,KK-1), FVZ(II,JJ,KK-1),HS(II,JJ,KK),HS(II,JJ,KK-1),DHFCT,UN_NEW
      END SELECT
   ENDIF
!IF (NM == 1) WRITE(*,*) '---------------------------------------------------------------------', DHFCT
...
!IF (NM == 1) &
!   WRITE(*,'(A,I3,A,6E14.6)') 'WALL_WORK1(',IW,')=', ITERATIVE_FACTOR, UN_NEW, UN_NEW_OTHER, VELOCITY_ERROR, WC%ONE_D%RDN, DT
 ...
 WRITE(*,'(A,4I4, 3E16.8)') '--------------> FINAL : ',  IW, II, JJ, KK, VELOCITY_ERROR, VELOCITY_ERROR_MAX(NM)
