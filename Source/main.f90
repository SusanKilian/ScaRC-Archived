...
USE OPENMP
USE MPI
USE SCRC, ONLY: SCARC_SETUP, SCARC_SOLVER, SCARC_MASTER
...
INTEGER N_SLAVE_PROCESSES, MPI_SLAVE_WORLD
...
IF (MYID == NMESHES ) THEN

   CALL MPI_COMM_SPLIT(MPI_COMM_WORLD, MPI_UNDEFINED, MYID, MPI_SLAVE_WORLD, IERR)
WRITE(*,*) 'Master: Start'
   CALL SCARC_MASTER

ELSE

   CALL MPI_COMM_SPLIT(MPI_COMM_WORLD, 0, MYID, MPI_SLAVE_WORLD, IERR)
   CALL MPI_COMM_RANK(MPI_SLAVE_WORLD, MYID, IERR)
   CALL MPI_COMM_SIZE(MPI_SLAVE_WORLD, N_SLAVE_PROCESSES, IERR)
   N_MPI_PROCESSES = N_SLAVE_PROCESSES
...
CALL MPI_ALLREDUCE(MPI_IN_PLACE,SOLID_HT3D,INTEGER_ONE,MPI_LOGICAL,MPI_LOR,MPI_SLAVE_WORLD,IERR)
...
DO N=0,N_SLAVE_PROCESSES-1
   IF (MYID==N) CALL WRITE_SMOKEVIEW_FILE
   IF (N==N_SLAVE_PROCESSES-1) EXIT
   IF (SHARED_FILE_SYSTEM) CALL MPI_BARRIER(MPI_SLAVE_WORLD, IERR)
ENDDO
CALL MPI_BARRIER(MPI_SLAVE_WORLD, IERR)
...
DO WHILE (ANY(SETUP_PRESSURE_ZONES_INDEX==0))
   ...
   CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_INTEGER,SETUP_PRESSURE_ZONES_INDEX,COUNTS,DISPLS,&
                       MPI_INTEGER,MPI_SLAVE_WORLD,IERR)
ENDDO
...
IF (N_SLAVE_PROCESSES>1) THEN
   CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_INTEGER,CELL_COUNT,COUNTS,DISPLS,MPI_INTEGER,MPI_SLAVE_WORLD,IERR)
ENDIF
...
IF (ANY(EVACUATION_ONLY)) THEN
   ...
   IF (N_SLAVE_PROCESSES==1 .OR. (N_SLAVE_PROCESSES>1 .AND. MYID==EVAC_PROCESS)) CALL INIT_EVAC_GROUPS
...
IF (HVAC_SOLVE) THEN
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      CALL HVAC_BC_IN(NM)
   ENDDO
   IF (N_SLAVE_PROCESSES>1) CALL EXCHANGE_HVAC_BC
   IF (MYID==0) THEN
...
MAIN_LOOP: DO
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      IF (HVAC_SOLVE) THEN
         IF (FIRST_PASS .AND. N_SLAVE_PROCESSES>1) CALL EXCHANGE_HVAC_BC
         IF (MYID==0) CALL HVAC_CALC(T,DT,FIRST_PASS)
         IF (N_SLAVE_PROCESSES>1) CALL EXCHANGE_HVAC_SOLUTION
      ENDIF
      ...
      IF (N_SLAVE_PROCESSES>1) THEN
         ...
         IF (ANY(CHANGE_TIME_STEP_INDEX==-1) .OR. ALL(CHANGE_TIME_STEP_INDEX==1)) &
            CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_DOUBLE_PRECISION,DT_NEW,COUNTS,DISPLS, &
                                MPI_DOUBLE_PRECISION,MPI_SLAVE_WORLD,IERR)
   ...
   IF (DIAGNOSTICS) THEN
      CALL WRITE_STRINGS
      IF (.NOT.SUPPRESS_DIAGNOSTICS .AND. N_SLAVE_PROCESSES>1) CALL EXCHANGE_DIAGNOSTICS
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(1)
      ...
      ALLOCATE(COUNTS(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(COUNTS_HVAC(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(COUNTS_MASS(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(COUNTS_QM_DOT(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(COUNTS_TEN(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(COUNTS_TWENTY(0:N_SLAVE_PROCESSES-1))

      ALLOCATE(DISPLS(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(DISPLS_MASS(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(DISPLS_HVAC(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(DISPLS_QM_DOT(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(DISPLS_TEN(0:N_SLAVE_PROCESSES-1))
      ALLOCATE(DISPLS_TWENTY(0:N_SLAVE_PROCESSES-1))
   ...
   CASE(4)
         ...
         DO NM=1,NMESHES
            ...
            DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
               ...
               IF (NM/=NOM .AND. PROCESS(NM)/=MYID .AND. MESHES(NOM)%CONNECTED_MESH(NM)) THEN
                  ...
                  N_REQ0 = N_REQ0 + 1
                  CALL MPI_IRECV(M2%INTEGER_RECV_BUFFER(1),7,MPI_INTEGER,PROCESS(NM),NM,MPI_SLAVE_WORLD,REQ0(N_REQ0),IERR)
               ENDIF
            ENDDO
         ENDDO
      ...
      DO NM=1,NMESHES
         ...
         DO NOM=1,NMESHES
            ...
            IF (N_SLAVE_PROCESSES>1 .AND. NM/=NOM .AND. PROCESS(NOM)/=MYID .AND. MESHES(NM)%CONNECTED_MESH(NOM)) THEN
               ...
               N_REQ0 = N_REQ0 + 1
               CALL MPI_ISEND(M3%INTEGER_SEND_BUFFER(1),7,MPI_INTEGER,PROCESS(NOM),NM,MPI_SLAVE_WORLD,REQ0(N_REQ0),IERR)
            ELSE
               M2 => MESHES(NOM)%OMESH(NM)
      ...
      DO NM=1,NMESHES
         ...
         DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
            IF (EVACUATION_ONLY(NOM)) CYCLE
            M2 => MESHES(NOM)%OMESH(NM)
            IF (N_SLAVE_PROCESSES>1 .AND. NM/=NOM .AND. PROCESS(NM)/=MYID .AND. M2%NIC_S>0) THEN
               CALL MPI_IRECV(M2%IIO_S(1),M2%NIC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+1),IERR)
               CALL MPI_IRECV(M2%JJO_S(1),M2%NIC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+2),IERR)
               CALL MPI_IRECV(M2%KKO_S(1),M2%NIC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+3),IERR)
               CALL MPI_IRECV(M2%IOR_S(1),M2%NIC_S,MPI_INTEGER,PROCESS(NM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+4),IERR)
               N_REQ = N_REQ + 4
            ENDIF
         ENDDO
      ENDDO
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO NOM=1,NMESHES
            ...
            IF (PROCESS(NOM)/=MYID) THEN
               CALL MPI_ISEND(M3%IIO_R(1),M3%NIC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+1),IERR)
               CALL MPI_ISEND(M3%JJO_R(1),M3%NIC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+2),IERR)
               CALL MPI_ISEND(M3%KKO_R(1),M3%NIC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+3),IERR)
               CALL MPI_ISEND(M3%IOR_R(1),M3%NIC_R,MPI_INTEGER,PROCESS(NOM),NM,MPI_SLAVE_WORLD,REQ(N_REQ+4),IERR)
               N_REQ = N_REQ + 4
            ELSE
               M2 => MESHES(NOM)%OMESH(NM)
               M2%IIO_S = M3%IIO_R
      ...
      IF (N_REQ>0 .AND. N_SLAVE_PROCESSES>1) CALL MPI_WAITALL(N_REQ,REQ(1:N_REQ),MPI_STATUSES_IGNORE,IERR)
...
PRESSURE_ITERATION_LOOP: DO
   ...
   IF (N_SLAVE_PROCESSES>1) THEN
      ...
      CALL MPI_ALLGATHERV(REAL_BUFFER_2(1,DISPLS(MYID)+1),COUNTS_TEN(MYID),MPI_DOUBLE_PRECISION, &
                          REAL_BUFFER_3,COUNTS_TEN,DISPLS_TEN,MPI_DOUBLE_PRECISION,MPI_SLAVE_WORLD,IERR)
...
IF (N_SLAVE_PROCESSES>1) THEN
   CALL MPI_ALLREDUCE(RAD_Q_SUM,RAD_Q_SUM_ALL,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   CALL MPI_ALLREDUCE(KFST4_SUM,KFST4_SUM_ALL,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
ELSE
...
IF (MEAN_FORCING(1)) THEN
   IF (N_SLAVE_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,MEAN_FORCING_SUM_U_VOL,N_MEAN_FORCING_BINS,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,MEAN_FORCING_SUM_VOL_X,N_MEAN_FORCING_BINS,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   ENDIF
...
IF (MEAN_FORCING(2)) THEN
   IF (N_SLAVE_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,MEAN_FORCING_SUM_V_VOL,N_MEAN_FORCING_BINS,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,MEAN_FORCING_SUM_VOL_Y,N_MEAN_FORCING_BINS,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   ENDIF
...
IF (MEAN_FORCING(3)) THEN
   IF (N_SLAVE_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,MEAN_FORCING_SUM_W_VOL,N_MEAN_FORCING_BINS,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
      CALL MPI_ALLREDUCE(MPI_IN_PLACE,MEAN_FORCING_SUM_VOL_Z,N_MEAN_FORCING_BINS,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   ENDIF
...
CALL MPI_GATHERV(REAL_BUFFER_13(1,DISPLS(MYID)+1),COUNTS_TWENTY(MYID),MPI_DOUBLE_PRECISION, &
                 REAL_BUFFER_14,COUNTS_TWENTY,DISPLS_TWENTY,MPI_DOUBLE_PRECISION,0,MPI_SLAVE_WORLD,IERR)
...
IF (N_SLAVE_PROCESSES>1) THEN
   TNOW = CURRENT_TIME()
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,STOP_STATUS,1,MPI_INTEGER,MPI_MAX,MPI_SLAVE_WORLD,IERR)
   T_USED(11)=T_USED(11) + CURRENT_TIME() - TNOW
ENDIF
...
IF (N_SLAVE_PROCESSES>1) THEN
   CALL MPI_ALLREDUCE(DSUM_ALL_LOCAL(1),DSUM_ALL(1),N_ZONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   CALL MPI_ALLREDUCE(PSUM_ALL_LOCAL(1),PSUM_ALL(1),N_ZONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   CALL MPI_ALLREDUCE(USUM_ALL_LOCAL(1),USUM_ALL(1),N_ZONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_SLAVE_WORLD,IERR)
   CALL MPI_ALLREDUCE(CONNECTED_ZONES_LOCAL(0,0),CONNECTED_ZONES_GLOBAL(0,0),(N_ZONE+1)**2,MPI_LOGICAL,MPI_LOR,MPI_SLAVE_WORLD,IERR)
ELSE
...
MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   OTHER_MESH_LOOP: DO NOM=1,NMESHES
      ...
      INITIALIZATION_IF: IF (CODE==0) THEN
         ...
         CALL MPI_IRECV(M4%CELL_INDEX(0,0,0),SIZE(M4%CELL_INDEX),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         N_REQ = MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M4%SOLID(0),SIZE(M4%SOLID),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         N_REQ = MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M4%WALL_INDEX(0,-3),SIZE(M4%WALL_INDEX),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         N_REQ = MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M4%OBST_INDEX_C(0),SIZE(M4%OBST_INDEX_C),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         ...
         IF (M3%NIC_R>0) THEN
            ...
            N_REQ1 = N_REQ1 + 1
            CALL MPI_RECV_INIT(M3%REAL_RECV_PKG1(1),SIZE(M3%REAL_RECV_PKG1),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                               REQ1(N_REQ1),IERR)

            IF (OMESH_PARTICLES) THEN
               N_REQ2 = N_REQ2 + 1
               CALL MPI_RECV_INIT(M3%N_PART_ADOPT,SIZE(M3%N_PART_ADOPT),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,&
                                  REQ2(N_REQ2),IERR)
            ENDIF

            N_REQ3 = N_REQ3 + 1
            CALL MPI_RECV_INIT(M3%REAL_RECV_PKG3(1),SIZE(M3%REAL_RECV_PKG3),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                               REQ3(N_REQ3),IERR)

            IF (SOLID_HT3D) THEN
               N_REQ4 = N_REQ4 + 1
               CALL MPI_RECV_INIT(M3%REAL_RECV_PKG4(1),SIZE(M3%REAL_RECV_PKG4),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                                  REQ4(N_REQ4),IERR)
            ENDIF

            N_REQ7 = N_REQ7 + 1
            CALL MPI_RECV_INIT(M3%REAL_RECV_PKG7(1),SIZE(M3%REAL_RECV_PKG7),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                               REQ7(N_REQ7),IERR)

            N_REQ8 = N_REQ8 + 1
            CALL MPI_RECV_INIT(M3%BOUNDARY_TYPE(0),SIZE(M3%BOUNDARY_TYPE),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,&
                               REQ8(N_REQ8),IERR)

            IF (RADIATION) THEN
               N_REQ5 = N_REQ5 + 1
               CALL MPI_RECV_INIT(M3%REAL_RECV_PKG5(1),SIZE(M3%REAL_RECV_PKG5),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                                  REQ5(N_REQ5),IERR)
            ENDIF

            IF (VEG_LEVEL_SET .OR. TERRAIN_CASE) THEN
               N_REQ14 = N_REQ14 + 1
               CALL MPI_RECV_INIT(M3%REAL_RECV_PKG14(1),SIZE(M3%REAL_RECV_PKG14),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                                  REQ14(N_REQ14),IERR)
            ENDIF
      ...
      IF (CODE==8) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M3%N_WALL_CELLS_SEND,1,MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==9 .AND. M3%N_WALL_CELLS_SEND>0) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_IRECV(M3%WALL_CELL_INDICES_SEND,SIZE(M3%WALL_CELL_INDICES_SEND),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,&
                           REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==10 .AND. M3%N_EXPOSED_WALL_CELLS>0) THEN
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%REAL_RECV_PKG6(1),SIZE(M3%REAL_RECV_PKG6),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_SLAVE_WORLD,&
                            REQ6(N_REQ6),IERR)
      ENDIF
      ...
      IF (CODE==11) THEN
         ...
         N_REQ = N_REQ + 1
         CALL MPI_IRECV(M4%PRESSURE_ZONE(0,0,0),SIZE(M4%PRESSURE_ZONE),MPI_INTEGER,SNODE,NOM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      ENDIF
...
DO NOM=1,NMESHES
   ...
   IF (CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID==MAX(0,EVAC_PROCESS) .AND. .NOT.EVACUATION_ONLY(NOM)) THEN
      ...
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%ZZ(0,0,0,1),IWW*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%RHO(0,0,0),IWW,MPI_DOUBLE_PRECISION,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%RSUM(0,0,0),IWW,MPI_DOUBLE_PRECISION,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%TMP(0,0,0),IWW,MPI_DOUBLE_PRECISION,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%UII(0,0,0),IWW,MPI_DOUBLE_PRECISION,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%CELL_INDEX(0,0,0),IWW,MPI_INTEGER,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      IWW = MAXVAL(M4%CELL_INDEX)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M4%SOLID(0),IWW,MPI_LOGICAL,SNODE,TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
   ENDIF
ENDDO
...
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (RNODE/=SNODE) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M%CELL_INDEX(0,0,0),SIZE(M%CELL_INDEX),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M%SOLID(0),SIZE(M%SOLID),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M%WALL_INDEX(0,-3),SIZE(M%WALL_INDEX),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M%OBST_INDEX_C(0),SIZE(M%OBST_INDEX_C),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         ENDIF
         ...
         IF (M3%NIC_S>0 .AND. RNODE/=SNODE) THEN
            ...
            N_REQ1 = N_REQ1 + 1
            CALL MPI_SEND_INIT(M3%REAL_SEND_PKG1(1),SIZE(M3%REAL_SEND_PKG1),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                               REQ1(N_REQ1),IERR)

            IF (OMESH_PARTICLES) THEN
               N_REQ2 = N_REQ2 + 1
               CALL MPI_SEND_INIT(M3%N_PART_ORPHANS,SIZE(M3%N_PART_ORPHANS),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,&
                                  REQ2(N_REQ2),IERR)
            ENDIF

            N_REQ3 = N_REQ3 + 1
            CALL MPI_SEND_INIT(M3%REAL_SEND_PKG3(1),SIZE(M3%REAL_SEND_PKG3),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                               REQ3(N_REQ3),IERR)

            IF (SOLID_HT3D) THEN
               N_REQ4 = N_REQ4 + 1
               CALL MPI_SEND_INIT(M3%REAL_SEND_PKG4(1),SIZE(M3%REAL_SEND_PKG4),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                                  REQ4(N_REQ4),IERR)
            ENDIF

            N_REQ7 = N_REQ7 + 1
            CALL MPI_SEND_INIT(M3%REAL_SEND_PKG7(1),SIZE(M3%REAL_SEND_PKG7),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                               REQ7(N_REQ7),IERR)

            N_REQ8 = N_REQ8 + 1
            CALL MPI_SEND_INIT(M5%BOUNDARY_TYPE(0),SIZE(M5%BOUNDARY_TYPE),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,&
                               REQ8(N_REQ8),IERR)

            IF (RADIATION) THEN
               N_REQ5 = N_REQ5 + 1
               CALL MPI_SEND_INIT(M3%REAL_SEND_PKG5(1),SIZE(M3%REAL_SEND_PKG5),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                                  REQ5(N_REQ5),IERR)
            ENDIF

            IF (VEG_LEVEL_SET .OR. TERRAIN_CASE) THEN
               N_REQ14 = N_REQ14 + 1
               CALL MPI_SEND_INIT(M3%REAL_SEND_PKG14(1),SIZE(M3%REAL_SEND_PKG14),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                                  REQ14(N_REQ14),IERR)
            ENDIF
      ...
      IF (CODE==8) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%N_EXPOSED_WALL_CELLS,1,MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      IF (CODE==9 .AND. M3%N_EXPOSED_WALL_CELLS>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            CALL MPI_ISEND(M3%EXPOSED_WALL_CELL_BACK_INDICES,SIZE(M3%EXPOSED_WALL_CELL_BACK_INDICES),MPI_INTEGER,RNODE,NM,&
                           MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      IF (CODE==10 .AND. M3%N_WALL_CELLS_SEND>0) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%REAL_SEND_PKG6(1),SIZE(M3%REAL_SEND_PKG6),MPI_DOUBLE_PRECISION,RNODE,NM,MPI_SLAVE_WORLD,&
                                REQ6(N_REQ6),IERR)
         ENDIF
      ENDIF
         ...
         NODE_CHECK_PARTICLE: IF (SNODE/=RNODE) THEN
            ...
            DO IPC=1,N_LAGRANGIAN_CLASSES
               ...
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%REALS(1,1),LPC%N_STORAGE_REALS*N_STORAGE_SLOTS,MPI_DOUBLE_PRECISION, &
                              RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
               N_REQ=MIN(N_REQ+1,SIZE(REQ))
      ...
      IF (CODE==11) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ = N_REQ + 1
            CALL MPI_ISEND(M%PRESSURE_ZONE(0,0,0),SIZE(M%PRESSURE_ZONE),MPI_INTEGER,RNODE,NM,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
         ENDIF
      ENDIF
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   IF (CODE==6 .AND. EXCHANGE_EVACUATION.AND.MYID/=MAX(0,EVAC_PROCESS) .AND..NOT.EVACUATION_ONLY(NM)) THEN
      ...
      CALL MPI_ISEND(M%ZZ(0,0,0,1),IWW*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,MAX(0,EVAC_PROCESS),&
           TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_ISEND(M%RHO(0,0,0),IWW,MPI_DOUBLE_PRECISION,MAX(0,EVAC_PROCESS),TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_ISEND(M%RSUM(0,0,0),IWW,MPI_DOUBLE_PRECISION,MAX(0,EVAC_PROCESS),TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_ISEND(M%TMP(0,0,0),IWW,MPI_DOUBLE_PRECISION,MAX(0,EVAC_PROCESS),TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_ISEND(M%UII(0,0,0),IWW,MPI_DOUBLE_PRECISION,MAX(0,EVAC_PROCESS),TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_ISEND(M%CELL_INDEX(0,0,0),IWW,MPI_INTEGER,MAX(0,EVAC_PROCESS),TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
      IWW = MAXVAL(M%CELL_INDEX)
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_ISEND(M%SOLID(0),IWW,MPI_LOGICAL,MAX(0,EVAC_PROCESS),TAG_EVAC,MPI_SLAVE_WORLD,REQ(N_REQ),IERR)
   ENDIF
ENDDO
...
CHARACTER(LEN=LINE_LENGTH), DIMENSION(0:N_SLAVE_PROCESSES-1) :: LINE_ARRAY
...
IF (MYID>0) THEN
   CALL MPI_SEND(LINE,LINE_LENGTH,MPI_CHARACTER,0,MYID,MPI_SLAVE_WORLD,IERR)
ELSE
   LINE_ARRAY(0) = LINE
   DO N=1,N_SLAVE_PROCESSES-1
      CALL MPI_RECV(LINE_ARRAY(N),LINE_LENGTH,MPI_CHARACTER,N,N,MPI_SLAVE_WORLD,STATUS,IERR)
   ENDDO
...
IF (MYID==0) THEN
   ...
   OTHER_MESH_LOOP: DO NOM=2,NMESHES
      IF (PROCESS(NOM)>0) THEN
         CALL MPI_RECV(N_STRINGS_DUM,1,MPI_INTEGER,PROCESS(NOM),1,MPI_SLAVE_WORLD,STATUS,IERR)
         IF (N_STRINGS_DUM>0) THEN
            ALLOCATE(STRING_DUM(N_STRINGS_DUM))
            CALL MPI_RECV(STRING_DUM(1),N_STRINGS_DUM*MESH_STRING_LENGTH, &
            MPI_CHARACTER,PROCESS(NOM),NOM,MPI_SLAVE_WORLD,STATUS,IERR)
         ENDIF
      ELSE
...
CALL MPI_GATHERV(REAL_BUFFER_11(1,DISP),COUNTS_QM_DOT(MYID),MPI_DOUBLE_PRECISION,&
                 REAL_BUFFER_12,COUNTS_QM_DOT,DISPLS_QM_DOT,MPI_DOUBLE_PRECISION,0,MPI_SLAVE_WORLD,IERR)
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   CALL MPI_SSEND(MESH_SEND, 1, MESH_STRUCT_TYPE, 0, 1, MPI_SLAVE_WORLD, &
       IERR)
ENDDO
...
IF (T>=HRR_CLOCK .AND. N_SLAVE_PROCESSES>1) THEN
...
IF (T>=MINT_CLOCK .AND. N_SLAVE_PROCESSES>1) THEN
   REAL_BUFFER_5 = MINT_SUM
   CALL MPI_GATHERV(REAL_BUFFER_5(0,DISP),COUNTS_MASS(MYID),MPI_DOUBLE_PRECISION, &
                    MINT_SUM,COUNTS_MASS,DISPLS_MASS,MPI_DOUBLE_PRECISION,0,MPI_SLAVE_WORLD,IERR)
ENDIF
...
EXCHANGE_DEVICE: IF (N_DEVC>0) THEN
   ...
   ENDDO
   IF (N_SLAVE_PROCESSES>1) THEN
      CALL MPI_ALLREDUCE(STATE_LOC(1),STATE_GLB(1),2*N_DEVC,MPI_LOGICAL,MPI_LXOR,MPI_SLAVE_WORLD,IERR)
   ELSE
      STATE_GLB = STATE_LOC
   ENDIF
   DO N=1,N_DEVC
   ...
   OPERATION_LOOP: DO OP_INDEX=1,3
      ...
      IF (N_SLAVE_PROCESSES>1) THEN
         SELECT CASE(OP_INDEX)
            CASE(1) ; CALL MPI_ALLREDUCE(TC_LOC(1),TC_GLB(1),DIM_FAC*N_DEVC,MPI_DOUBLE_PRECISION,MPI_OP_INDEX,MPI_SLAVE_WORLD,IERR)
            CASE(2:3) ; CALL MPI_ALLREDUCE(TC2_LOC,TC2_GLB,N_DEVC,MPI_2DOUBLE_PRECISION,MPI_OP_INDEX,MPI_SLAVE_WORLD,IERR)
         END SELECT
      ELSE
         SELECT CASE(OP_INDEX)
      ...
      DEVICE_LOOP_2: DO N=1,N_DEVC
         ...
         IF (OP_INDEX>1 .AND. (DV%SPATIAL_STATISTIC(1:6)=='MINLOC'.OR.DV%SPATIAL_STATISTIC(1:6)=='MAXLOC')) THEN
            ...
            DO NN=1,DV%N_SUBDEVICES
               ...
               IF (PROCESS(SDV%MESH)==MYID) THEN
                  IF (SDV%MESH==NINT(TC2_GLB(2,N))) THEN
                     ...
                     IF (MYID>0) THEN
                        CALL MPI_SEND(DV%VALUE_1,1,MPI_DOUBLE_PRECISION,0,999,MPI_SLAVE_WORLD,IERR)
                     ELSE
            ...
            IF (MYID==0 .AND. .NOT.NO_NEED_TO_RECV) &
               CALL MPI_RECV(DV%VALUE_1,1,MPI_DOUBLE_PRECISION,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_SLAVE_WORLD,STATUS,IERR)
            CALL MPI_BARRIER(MPI_SLAVE_WORLD,IERR)
            CALL MPI_BCAST(DV%VALUE_1,1,MPI_DOUBLE_PRECISION,0,MPI_SLAVE_WORLD,IERR)
         ENDIF
      ENDDO DEVICE_LOOP_2
...
IF (T>=DEVC_CLOCK .AND. N_DEVC>0) THEN
   ...
   DO N=1,N_DEVC
      ...
      IF (PROCESS(DV%MESH)==MYID .AND. MYID>0) CALL MPI_SEND(DV%HISTOGRAM_COUNTS(1), &
          PROPERTY(DV%PROP_INDEX)%HISTOGRAM_NBINS,MPI_DOUBLE_PRECISION,0,DV%MESH,MPI_SLAVE_WORLD,IERR)
      IF (PROCESS(DV%MESH)>0 .AND. MYID==0) CALL MPI_RECV(DV%HISTOGRAM_COUNTS(1), &
          PROPERTY(DV%PROP_INDEX)%HISTOGRAM_NBINS,MPI_DOUBLE_PRECISION,PROCESS(DV%MESH),DV%MESH,MPI_SLAVE_WORLD,STATUS,IERR)
   ENDDO
...
DO NM=1,NMESHES
   IF (N_SLAVE_PROCESSES>1 .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NM)) THEN
      M=>MESHES(NM)
...
IF (N_SLAVE_PROCESSES==1 .OR. (N_SLAVE_PROCESSES>1 .AND. MYID==EVAC_PROCESS))  CALL INITIALIZE_EVAC_DUMPS(T_TMP,T_EVAC_SAVE)
...
IF (N_SLAVE_PROCESSES>1 .AND. MYID /= EVAC_PROCESS) CALL EVAC_MESH_EXCHANGE(T_EVAC,T_EVAC_SAVE,I_EVAC,ICYC,EXCHANGE_EVACUATION,1)
IF (N_SLAVE_PROCESSES==1 .OR. (N_SLAVE_PROCESSES>1 .AND. MYID==EVAC_PROCESS)) &
...
EVAC_TIME_STEP_LOOP: DO WHILE (T_EVAC < T_FIRE)
   ...
   IF (N_SLAVE_PROCESSES==1 .OR. (N_SLAVE_PROCESSES>1 .AND. MYID==EVAC_PROCESS)) CALL PREPARE_TO_EVACUATE(ICYC)
   DO NM = 1, NMESHES
      IF (EVACUATION_ONLY(NM)) THEN
   ...
   IF (N_SLAVE_PROCESSES==1 .OR. (N_SLAVE_PROCESSES>1 .AND. MYID==EVAC_PROCESS)) CALL CLEAN_AFTER_EVACUATE(ICYC, I_EVAC)
...
CALL MPI_GATHERV(REAL_BUFFER_6(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION, &
                 REAL_BUFFER_8,COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_SLAVE_WORLD,IERR)
...
CALL MPI_BCAST(REAL_BUFFER_1,(2+N_TRACKED_SPECIES)*N_DUCTNODES+N_DUCTS,MPI_DOUBLE_PRECISION,0,MPI_SLAVE_WORLD,IERR)
