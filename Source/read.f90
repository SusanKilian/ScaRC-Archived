...
NAMELIST /MESH/ CHECK_MESH_ALIGNMENT,COLOR,CYLINDRICAL,EVACUATION,EVAC_HUMANS,EVAC_Z_OFFSET, FYI,ID,IJK,LEVEL,MPI_PROCESS,MULT_ID,&
                N_THREADS,RGB,TRNX_ID,TRNY_ID,TRNZ_ID,XB, SCARC_MASTER_SLAVE
...
SCARC_MASTER_SLAVE_IF: IF (SCARC_MASTER_SLAVE) THEN
   NMESHES_SCARC = NMESHES +1
ENDIF SCARC_MASTER_SLAVE_IF
...
IF (SCARC_MASTER_SLAVE) THEN
   IF (NMESHES_SCARC>N_MPI_PROCESSES) THEN
      WRITE(MESSAGE,'(A,I0,A,I0)') 'ERROR: Not enough MPI processes, ',N_MPI_PROCESSES,&
                                   ', for ScaRC Master-Slave with meshes, ',NMESHES
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
ELSE IF (NO_EVACUATION) THEN
...
IF (PROCESS(NMESHES) < N_MPI_PROCESSES-1 .AND. .NOT. SCARC_MASTER_SLAVE) THEN
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,&
                ...
                CNF_CUTOFF,COMPUTE_CUTCELLS_ONLY,CONSTANT_H_SOLID,CONSTANT_SPECIFIC_HEAT_RATIO,&
                ...
                MAXIMUM_GEOMETRY_FACES,MAXIMUM_GEOMETRY_ZVALS,MAXIMUM_VISIBILITY,MAX_LEAK_PATHS,MPI_TIMEOUT,&
                NEAR_WALL_TURBULENCE_MODEL,&
                NOISE,NOISE_VELOCITY,NO_EVACUATION,NUCLEATION_SITES,&
                OVERWRITE,PARTICLE_CFL,PARTICLE_CFL_MAX,PARTICLE_CFL_MIN,PERIODIC_TEST,POSITIVE_ERROR_TEST,&
                POROUS_FLOOR,PR,PROCESS_ALL_MESHES,PROFILING,&
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                N_STRATA,ORIENTATION,PERIODIC_X,PERIODIC_Y,PERIODIC_Z,PERMEABILITY,POROUS_VOLUME_FRACTION,&
...
REAL(EB), DIMENSION(MAX_REACTIONS) :: A,E,HEATING_RATE,PYROLYSIS_RANGE,HEAT_OF_REACTION, &
          ...
          THRESHOLD_SIGN,GAS_DIFFUSION_DEPTH,NU_O2_CHAR,ALPHA_CHAR,BETA_CHAR
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ALLOW_SHRINKING,ALLOW_SWELLING,ALPHA_CHAR,BETA_CHAR,BOILING_TEMPERATURE,&
...
NAMELIST /SURF/ ADIABATIC,BACKING,BURN_AWAY,CELL_SIZE_FACTOR,C_FORCED_CONSTANT,C_FORCED_PR_EXP,C_FORCED_RE,C_FORCED_RE_EXP,&
                ...
                MATL_MASS_FRACTION,MINIMUM_BURNOUT_TIME,MINIMUM_LAYER_THICKNESS,MLRPUA,MOISTURE_FRACTION,&
                N_LAYER_CELLS_MAX,NET_HEAT_FLUX,&
                NO_SLIP,NPPC,PACKING_RATIO,PARTICLE_MASS_FLUX,PARTICLE_SURFACE_DENSITY,PART_ID,PLE,PROFILE,RADIUS,RAMP_EF,RAMP_MF,&
                ...
                VEG_LSET_BETA,VEG_LSET_COUPLED,VEG_LSET_ELLIPSE,VEG_LSET_ELLIPSE_HEAD,VEG_LSET_HT,VEG_LSET_IGNITE_TIME,&
                VEG_LSET_QCON,VEG_LSET_ROS_BACK,VEG_LSET_ROS_FLANK,VEG_LSET_ROS_HEAD,VEG_LSET_SPREAD,VEG_LSET_SIGMA,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   SELECT CASE(GEOMETRY)
      ...
      CASE('BLOWING PLATE')
         SF%GEOMETRY       = SURF_BLOWING_PLATE
         IF (SF%WIDTH>0._EB)                 SF%BACKING = INSULATED
      CASE('CYLINDRICAL')
   ...
   SELECT CASE(HEAT_TRANSFER_MODEL)
      ...
      CASE('BLOWING SPHERE')
         SF%HEAT_TRANSFER_MODEL = H_BLOWING_SPHERE
      CASE('BLOWING PLATE')
         SF%HEAT_TRANSFER_MODEL = H_BLOWING_PLATE
   END SELECT
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%INTERNAL_RADIATION .AND. .NOT.(SF%GEOMETRY==SURF_CARTESIAN .OR. SF%GEOMETRY==SURF_BLOWING_PLATE)) THEN
...
REAL(EB) :: DELX, DELY, DELZ
REAL(FB) :: RLE_MIN, RLE_MAX
TYPE (SLICE_TYPE), POINTER :: SL
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,DB,EVACUATION,FACE_CENTERED,FYI,ID,IOR,MAXIMUM_VALUE,MATL_ID,MESH_NUMBER,&
