...
fds_key = {'FDS {\it h/\deltax}=2' 'FDS {\it h/\deltax}=4' 'FDS {\it h/\deltax}=8' };
...
for i=1:lnx
    if ~exist([datadir,'ribbed_channel_',num2str(nx(i)),'_4mesh_uscarc_line.csv'])
        display(['Error: File ' [datadir,'ribbed_channel_',num2str(nx(i)),'_4mesh_uscarc_line.csv'] ' does not exist. Skipping case.'])
        return
    end
end
...
for i=1:lnx
    M{i} = importdata([datadir,'ribbed_channel_',num2str(nx(i)),'_4mesh_uscarc_line.csv'],',',2);
end
...
Git_Filename = [datadir,'ribbed_channel_',num2str(nx(1)),'_4mesh_uscarc_git.txt'];
